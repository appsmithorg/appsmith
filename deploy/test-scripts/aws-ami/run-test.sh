...
# Create account
status_code_register=$(curl -X POST --write-out %{http_code} --silent --output /dev/null http://$public_ip/api/v1/users \
    -H "Content-Type: application/x-www-form-urlencoded" \
    -H "Referer: http://$public_ip/user/signup" \
    --data-raw "email=$email&password=$password")

# Login
status_code_login=$(curl -X POST --write-out %{http_code} --silent --output /dev/null "http://$public_ip/api/v1/login" \
    -H "Referer: http://$public_ip/user/login" \
