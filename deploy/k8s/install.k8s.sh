...
download_template_file() {
    ...
    (
        ...
        if [[ "$ssl_enable" == "true" ]]; then
            curl --remote-name-all --silent --show-error -o issuer-template.yaml.sh "$template_endpoint/deploy/k8s/scripts/issuer-template.yaml.sh"
        fi
    )
    (
        cd "$install_dir"
        ...
        curl --remote-name-all --silent --show-error -o imago-template.yaml "$template_endpoint/deploy/k8s/templates/imago-template.yaml"
    )
