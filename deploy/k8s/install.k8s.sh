...
applicationEnv="$install_dir/config-template/appsmith-configmap.yaml"
if test -f "$applicationEnv"; then
...
# In the new image, we will need to wait for the initializing of the MongoDB & Redis. Therefore, to ensure all services in the image
# are up & ready to serve, we need to wait around 180 seconds
wait_for_application_start 180
