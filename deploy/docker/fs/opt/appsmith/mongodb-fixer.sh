...
for _ in {1..60}; do
  # On each attempt: read the current FCV, raise it to 6.0 if it is below 6.0,
  # and print the resulting FCV as the last line of output so the shell can
  # capture it for the marker file.
  #
  # We deliberately do NOT raise FCV to 7.0 — keeping it at 6.0 preserves the
  # ability to roll back to a 6.x Appsmith release if something goes wrong.
  # The raise-to-6.0 branch is not expected to fire post-upgrade (MongoDB 7.0
  # won't start on data with FCV below 6.0 — the entrypoint pre-flight check
  # catches that). It's kept as a safety net for edge cases.
  #
  # `setFeatureCompatibilityVersion` requires `confirm: true` as of MongoDB 7.0.
  if fcv="$(mongosh --quiet "$APPSMITH_DB_URL" --eval '
    const current = db.adminCommand({getParameter: 1, featureCompatibilityVersion: 1}).featureCompatibilityVersion.version;
    if (parseFloat(current) < 6) {
