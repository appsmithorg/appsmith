# Configuration - AWS base image
variable "base_ami" {
  type    = string
  default = "ami-0851b76e8b1bce90b"
}

# Configuration - AWS provisioning instance type
variable "instance_type" {
  type    = string
  default = "t2.micro"
}

# Configuration - AWS subnet
variable "subnet_id" {
  type    = string
  default = ""
}

# Configuration - AWS VPC
variable "vpc_id" {
  type    = string
  default = ""
}

# Configuration - DO token
variable "token" {
  type    = string
  default = "${env("DIGITALOCEAN_TOKEN")}"
}


# "timestamp" template function replacement
locals { timestamp = regex_replace(timestamp(), "[- TZ:]", "") }

# Variable - AMI naming
locals {
  ami_name   = "appsmith-ami-${local.timestamp}"
  image_name = "appsmith-marketplace-snapshot-${local.timestamp}"
}

# Builder - Provision AWS instance
source "amazon-ebs" "autogenerated_1" {
  ami_name      = "${local.ami_name}"
  instance_type = "${var.instance_type}"
  launch_block_device_mappings {
    delete_on_termination = true
    device_name           = "/dev/sda1"
    volume_size           = 15
    volume_type           = "gp2"
  }
  region       = "ap-south-1"
  source_ami   = "${var.base_ami}"
  ssh_username = "appsmith"
  subnet_id    = "${var.subnet_id}"
  vpc_id       = "${var.vpc_id}"
  skip_create_ami = false
  user_data_file = "./defaults.cfg"
}

# Builder - Provision DO droplet
source "digitalocean" "autogenerated_2" {
  api_token     = "${var.token}"
  image         = "ubuntu-20-04-x64"
  region        = "blr1"
  size          = "s-1vcpu-1gb"
  snapshot_name = "${local.image_name}"
  ssh_username  = "appsmith"
  user_data_file = "./defaults.cfg"
}

# Provisioning - Setup Appsmith
build {
  sources = ["source.amazon-ebs.autogenerated_1", "source.digitalocean.autogenerated_2"]

  # Wait for cloud-init to be ready
  provisioner "shell" {
    inline = ["cloud-init status --wait"]
  }

  provisioner "file" {
    destination = "/tmp/defaults.cfg"
    source 			= "./defaults.cfg"
  }

  # Setup new user 'appsmith'
  provisioner "shell" {
    inline = [
      "sudo mv /tmp/defaults.cfg /etc/cloud/cloud.cfg.d/defaults.cfg"
    ]
  }

  provisioner "shell" {
    scripts = ["./setup.sh"]
  }

  # Add provisioning scripts
  provisioner "file" {
    destination = "/tmp/"
    source      = "appsmith"
  }

  provisioner "shell" {
    inline = [
      "sudo cp -r /tmp/appsmith /home/appsmith/",
      "sudo chown -R appsmith:appsmith /home/appsmith"
    ]
  }

  # Add script to create default user for AMI only
  provisioner "file" {
    only = ["amazon-ebs.autogenerated_1"]
    destination = "/tmp/user-data.sh"
    source      = "var/user-data.sh"
  }

  provisioner "shell" {
    only = ["amazon-ebs.autogenerated_1"]
    inline = ["sudo cp /tmp/user-data.sh /var/lib/cloud/scripts/per-instance"]
  }

  # Add script to create default user for AMI only
  provisioner "file" {
    destination = "/tmp/init-ssh.sh"
    source      = "var/init-ssh.sh"
  }

  provisioner "shell" {
    inline = ["sudo cp /tmp/init-ssh.sh /var/lib/cloud/scripts/per-instance"]
  }

  # Install Docker & Docker Compose
  provisioner "shell" {
    scripts = ["install-docker.sh"]
  }

  # Add cron job to run provision script when server reboot
  provisioner "file" {
    destination = "/tmp/appsmith-cron"
    source      = "startup/appsmith"
  }

  provisioner "shell" {
    inline = ["sudo cp /tmp/appsmith-cron /etc/cron.d"]
  }

  # Reload cron service to load new job
  provisioner "shell" {
    inline = ["/etc/init.d/cron reload"]
  }

  # Clean up image to ensure secure manner
  provisioner "shell" {
    execute_command = "echo 'vagrant' | {{ .Vars }} sudo -S -E sh -eux '{{ .Path }}'"
    scripts         = ["clean-up.sh"]
  }

}
