...
function main() {
  ...
  prompts([
    ...
    if (dependencies && dependencies.length > 0) {
      console.log(
        `Installing all dependencies of ${values.scope} to verify shared dependencies`
      );
      exec(
        `
      ...
      `,
        (err) => {
          if (err) {
            console.log(
              "\x1b[31m",
              `Unable to install packages for ${values.scope}`
            );
            return;
          }
          ...
          if (absentDep.length > 0) {
            console.log(
              "\x1b[31m",
              `Some shared dependencies are absent for ${values.scope} :`
            );
            absentDep.forEach((v) => {
            ...
            console.log(
              "\x1b[32m",
              "All Shared Dependencies are verified successfully ✔"
            );
            console.log(
              "\x1b[32m",
              "*******************************************"
      ...
      console.log(
        "\x1b[34m",
        `No Shared Dependency to verify installation in ${values.scope}`
      );
      console.log("\x1b[34m", "*******************************************");
    }
