...
`;
  ...
  it("5. Verify Generate CRUD for the new table & Verify Deploy mode for table - Productlines", () => {
    ...
    _.table.ReadTableRowColumnData(1, 0, "v2", 200).then(($cellData) => {
      expect($cellData).to.eq("Motorcycles");
    });
    _.table.ReadTableRowColumnData(2, 0, "v2", 200).then(($cellData) => {
      expect($cellData).to.eq("Planes");
    });
    _.table.ReadTableRowColumnData(3, 0, "v2", 200).then(($cellData) => {
      expect($cellData).to.eq("Ships");
    });
    _.table.ReadTableRowColumnData(4, 0, "v2", 200).then(($cellData) => {
      expect($cellData).to.eq("Trains");
    });
    _.table.ReadTableRowColumnData(5, 0, "v2", 200).then(($cellData) => {
      expect($cellData).to.eq("Trucks and Buses");
    });
    _.table.ReadTableRowColumnData(6, 0, "v2", 200).then(($cellData) => {
  ...
  it.skip("6. Verify Update/Delete row/Delete field data from Deploy page - on Productlines - existing record + Bug 14063", () => {
    ...
    _.table.ReadTableRowColumnData(3, 2, "v2", 200).then(($cellData) => {
  ...
  function GenerateCRUDNValidateDeployPage(
    ...
    _.table.ReadTableRowColumnData(0, 1, "v2", 200).then(($cellData) => {
      expect($cellData).to.eq(col2Text);
    });
    _.table.ReadTableRowColumnData(0, 2, "v2", 200).then(($cellData) => {
