...
describe("Validate Postgres Generate CRUD with JSON Form", () => {
  ...
  function GenerateCRUDNValidateDeployPage(
    ...
    _.table.ReadTableRowColumnData(0, 1, "v2", 4000).then(($cellData) => {
      expect($cellData).to.eq(col1Text);
    });
    _.table.ReadTableRowColumnData(0, 3, "v2", 200).then(($cellData) => {
      expect($cellData).to.eq(col2Text);
    });
    _.table.ReadTableRowColumnData(0, 4, "v2", 200).then(($cellData) => {
