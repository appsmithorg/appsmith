...
describe("Static URL - Page Slug Persistence", { tags: ["@tag.Settings"] }, () => {
  ...
  before(() => {
    ...
    cy.get("@fetchAppSlugSuggestion").then((interception: any) => {
      ...
      _.agHelper.AssertText(AdminsSettings.generalSettingsStaticAppSlug, "val", applicationSlug);
  ...
  it("1. Should auto-generate page slug from page name and show in URL preview", () => {
    ...
    _.agHelper.GetText(AdminsSettings.pageSettingsName, "val").then((pageName) => {
      ...
      _.agHelper.GetText(AdminsSettings.pageSettingsStaticPageSlug, "val").then((pageSlug) => {
        ...
        _.agHelper.AssertContains(`/app/${applicationSlug}/${pageSlugStr}`, "be.visible");
      });
    });
  });
  ...
  it("2. Should automatically persist page slug when available and reflect on page reload", () => {
    ...
    _.agHelper.GetText(AdminsSettings.pageSettingsStaticPageSlug, "val").then((currentPageSlug) => {
      ...
      _.agHelper.ClearNType(AdminsSettings.pageSettingsStaticPageSlug, modifiedPageSlug);
      ...
      _.agHelper.AssertContains(`/app/${applicationSlug}/${modifiedPageSlug}`, "be.visible");
      ...
      _.agHelper.AssertContains("Deploy app to apply this slug", "be.visible");
      ...
      _.agHelper.AssertElementVisibility(AdminsSettings.pageSettingsStaticPageSlug);
      _.agHelper.AssertText(AdminsSettings.pageSettingsStaticPageSlug, "val", modifiedPageSlug);
  ...
  it("3. Should show unavailable status for duplicate page slug and auto-persist when unique", () => {
    ...
    _.agHelper.GetText(AdminsSettings.pageSettingsStaticPageSlug, "val").then((persistedPageSlug) => {
      ...
      _.agHelper.ClearNType(AdminsSettings.pageSettingsStaticPageSlug, persistedPageSlugStr);

      // Verify the "Unavailable" message appears
      _.agHelper.AssertContains("There is already a page with this slug.", "be.visible");

      // Change to another unique value
      const uniquePageSlug = persistedPageSlugStr + "2";
      _.agHelper.ClearNType(AdminsSettings.pageSettingsStaticPageSlug, uniquePageSlug);
      ...
      // Verify the page slug persists after reload
      _.agHelper.AssertElementVisibility(AdminsSettings.pageSettingsStaticPageSlug);
      _.agHelper.AssertText(AdminsSettings.pageSettingsStaticPageSlug, "val", uniquePageSlug);
  ...
  it("6. Should validate only old routes work after disabling static URL", () => {
    ...
    cy.visit(`/app/${applicationSlug}/${page1Slug}`, { timeout: 10000, failOnStatusCode: false });
    ...
    cy.visit(`/app/${applicationSlug}/${page2Slug}`, { timeout: 10000, failOnStatusCode: false });
    // The static URL route should not work - page should not show the expected content
