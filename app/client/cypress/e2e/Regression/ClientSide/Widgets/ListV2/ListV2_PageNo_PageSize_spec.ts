...
const listData = [
  ...
  describe("List widget V2 page number and page size", () => {
    ...
    it("1. List widget V2 with client side pagination", () => {
      ...
      propPane.TypeTextIntoField("items", JSON.stringify(listData));
      assertHelper.WaitForNetworkCall("@updateLayout");
      entityExplorer.SelectEntityByName("Text3");
      propPane.TypeTextIntoField("text", "PageSize {{List1.pageSize}}");
      assertHelper.WaitForNetworkCall("@updateLayout");
  
      agHelper.GetNAssertElementText(locators._textWidgetStyleInDeployed, "PageSize 4");
  
      entityExplorer.SelectEntityByName("Text3");
      agHelper.Sleep(2000);
      propPane.TypeTextIntoField("text", "Page Number {{List1.pageNo}}");
      agHelper.Sleep(2000);
      assertHelper.WaitForNetworkCall("@updateLayout");
      agHelper.GetNAssertElementText(locators._textWidgetStyleInDeployed, "Page Number 1");
  
      table.NavigateToNextPage_List("v2", 0);
      agHelper.GetNAssertElementText(locators._textWidgetStyleInDeployed, "Page Number 2");
    ...
    it("2. List widget V2 with server side pagination", () => {
      ...
      agHelper.GetNAssertElementText(locators._textWidgetStyleInDeployed, "PageSize 3");
  
      // toggle serversidepagination -> true
      entityExplorer.SelectEntityByName("List1");
      agHelper.GetNClick(locators._propertyControlInput("serversidepagination"));
  
      agHelper.GetNAssertElementText(locators._textWidgetStyleInDeployed, "PageSize 2");
    });
  
    it(
      "excludeForAirgap 3. should reset page no if higher than max when switched from server side to client side",
      () => {
        ...
        agHelper.AssertElementLength(locators._widgetInDeployed(draggableWidgets.CONTAINER), 3);
      });
  
    it(
      "airgap 3. should reset page no if higher than max when switched from server side to client side - airgap",
      () => {
        agHelper.AddDsl("Listv2/listWithServerSideData");
        agHelper.Sleep(2000);
