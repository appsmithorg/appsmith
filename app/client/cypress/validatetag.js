...
function checkTagsValidity(filePath) {
  try {
    ...
    console.error(error.message);const fs = require('fs');
    const path = require('path');
    const { Tag } = require('./tags');
    ...
    function checkTagsValidity(filePath) {
      try {
        ...
        if (invalidTags.length > 0) {
          const errorMessage = `Invalid tag(s) found in file '${filePath}': ${invalidTags.join(", ")}. Allowed tags are: ${Tag.join(", ")}`;
          process.stdout.write(`::error file=${filePath},line=1::${errorMessage}\n`);
          process.exitCode = 1;
        } else {
          process.stdout.write(`Tags in file '${filePath}' are valid.\n`);
        }
      } catch (error) {
