...
cat ./docker/templates/nginx-app.conf.template |sed -e "s|APPSMITH_NGINX_SSL_CMNT|""|g"|sed -e "s|APPSMITH_PORT|80|g" | sed -e "s|APPSMITH_DOMAIN|http://dev.appsmith.com|g" | sed -e "s|__APPSMITH_CLIENT_PROXY_PASS__|http://localhost:3000|g" | sed -e "s|__APPSMITH_SERVER_PROXY_PASS__|http://localhost:8080|g" | envsubst ${vars_to_substitute} | sed -e 's|\${\(APPSMITH_[A-Z0-9_]*\)}||g' > ./docker/nginx.conf
