...
describe("4. Check frozen common and/or custom columns retain position on query change", () => {
  before(() => {
    cy.dragAndDropToCanvas(WIDGET.TABLE, { x: 600, y: 200 });
    cy.dragAndDropToCanvas(WIDGET.TEXT, { x: 600, y: 50 });

    cy.openPropertyPane(WIDGET.TABLE);
    cy.updateCodeInput(PROPERTY_SELECTOR.tableData, TABLE_DATA_DYNAMIC_STATIC);

    cy.openPropertyPane(WIDGET.TEXT);
    cy.updateCodeInput(PROPERTY_SELECTOR.text, `{{Table1.columnOrder}}`);

    /**
     * Create two table data with common columns
     */
  });
  it("4.1 Check if common frozen columns retain position", () => {
  ...
  it("4.2 Check if custom frozen columns retain position", () => {
    // Add a custom column
  ...
  it("4.3 Check if the custom + common columns retain their positon", () => {
    cy.wait(500);
  ...
  it("4.4 Check if frozen column retains its position and unfrozen goes at the first position", () => {
    // Freeze the common column:
    cy.freezeColumnFromDropdown("id", "right");
  ...
  it("4.5 Check if the order of the forzen columns remains the same", () => {
    cy.freezeColumnFromDropdown("id", "right");
