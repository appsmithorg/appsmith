...
describe("Verify various Table_Filter combinations", function () {
  ...
  it("30. Import TableFilter application & verify all filters for same FirstName (one word column) + Bug 13334", () => {
    ...
    agHelper.GetText(table._showPageItemsCount).then($count => expect($count).contain('4'))
    table.CloseFilter()
    agHelper.GetText(table._filtersCount).then($count => expect($count).contain('2'))
  ...
  it("31. Verify all filters for same FullName (two word column) + Bug 13334", () => {
    ...
    agHelper.GetText(table._showPageItemsCount).then($count => expect($count).contain('3'))

    table.OpenNFilterTable("LastName", "ends with", "son", 'OR', 2)
    agHelper.GetText(table._showPageItemsCount).then($count => expect($count).contain('10'))
    table.CloseFilter()
    agHelper.GetText(table._filtersCount).then($count => expect($count).contain('3'))

    table.OpenFilter()
    table.RemoveFilterNVerify('1', true, false)
  });

  function filterOnlyCondition(condition: string, expectedCount: string, input: string | '' = '') {
    agHelper.GetNClick(table._filterConditionDropdown)
    cy.get(table._dropdownText).contains(condition).click()
    if (input)
      agHelper.GetNClick(table._filterInputValue, 0).type(input).wait(500)
    agHelper.ClickButton("APPLY")
    agHelper.GetText(table._showPageItemsCount).then($count => expect($count).contain(expectedCount))
  }
