...
if [ "$target" == "ci" ]; then
    ...
    $(npm bin)/DEBUG=cypress:* npx cypress run --headless \
    ...
    $(npm bin)/DEBUG=cypress:* npx cypress run --headless --browser chromium --spec "cypress/integration/Regression_TestSuite/**/*.js"
fi
