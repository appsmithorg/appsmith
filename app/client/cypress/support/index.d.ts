...
declare namespace Cypress {
    interface Chainable<Subject> {
      ...
      AddFilterWithOperator(operator:string, option:string, condition:string, value:string)
      stubPostHeaderReq()
      addOAuth2AuthorizationCodeDetails(accessTokenUrl:string, clientId:string, clientSecret:string, authURL:string)
      addOAuth2ClientCredentialsDetails(accessTokenUrl:string, clientId:string, clientSecret:string, scope:string)
      ...
      EvaluateCurrentValue(currentValue:string, isValueToBeEvaluatedDynamic?:boolean)
      ...
      dragAndDropToWidget(widgetType:string, destinationWidget:string, { x:number, y:number })
      dragAndDropToWidgetBySelector(widgetType:string, destinationSelector:string, { x:number, y:number })
      changeButtonColor(buttonColor:string)
      closePropertyPane()
      onClickActions(forSuccess:string, forFailure:string, actionType:string, actionValue:string, idx?: number)
      ...
      typeValueNValidate(valueToType:string, fieldName?:string, isDynamic?:boolean)
      checkCodeInputValue(selector:string)
      clickButton(btnVisibleText:string, toForceClick?:boolean)
      actionContextMenuByEntityName(entityNameinLeftSidebar:string, action?:string, subActions:string)
      ...
      StartNewContainer(url:string, path:string, version:string, containerName:string)
      ...
      RenameWidgetFromPropertyPane(widgetType:string, oldName:string, newName:string)
