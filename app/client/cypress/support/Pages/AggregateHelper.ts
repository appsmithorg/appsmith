...
export class AggregateHelper {
  ...
  public NavigateToDSAdd() {
    cy.get(locator._addNewDataSource)
      .last()
      .scrollIntoView()
      .should("be.visible")
      .click({ force: true });
  ...
  public RenameWithInPane(renameVal: string, query = true) {
    const name = query ? locator._queryName : locator._dsName;
    const text = query ? locator._queryNameTxt : locator._dsNameTxt;
  ...
  public DragDropWidgetNVerify(widgetType: string, x: number, y: number) {
    ...
    this.Sleep();
    cy.get(locator._widgetPageIcon(widgetType))
      .first()
  ...
  public ActionContextMenuByEntityName(
    ...
    cy.xpath(locator._contextMenu(entityNameinLeftSidebar))
      .last()
      .click({ force: true });
    cy.xpath(locator._contextMenuItem(action)).click({ force: true });
    this.Sleep(500);
    if (subAction) {
      cy.xpath(locator._contextMenuItem(subAction)).click({ force: true });
      this.Sleep(500);
    }
  }
