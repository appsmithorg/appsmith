  ...
  for await (const collectionName of sortedCollectionNames) {
    ...
    for await (const doc of mongoDb
      ...
      if (outFile == null) {
        ...
        outFile = fs.openSync(
          EXPORT_ROOT + "/" + collectionName + ".jsonl",
          "w",
        );
      }
...
function extractValueFromArg(arg: string): string {
  return arg.replace(/^.*?=/, "");
}

function isArchivedObject(doc: Document): boolean {
  return doc.deleted === true || doc.deletedAt != null;
}

function toJsonSortedKeys(obj: Document) {
 ...
 */
function transformFields(obj: Document) {
 ...
 */
function mapClassToType(_class: string): "primary key" | "foreign key" | null {
  switch (_class) {
 ...
 * @returns - A promise that resolves to true if the data has been migrated to a stable version, false otherwise.
 */
async function isMongoDataMigratedToStableVersion(mongoDb: Db): Promise<boolean> {
