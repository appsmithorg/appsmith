...
import { TypographyVariant, TypographyType } from "@design-system/theming";
...
<Canvas>
  <Story name="Semantic type">
    <Flex
      ...
      direction="row-reverse"
    >
      {Object.keys(TypographyType).map((type) => (
        <Text variant="heading" type={type}>
          aA
        </Text>
      ))}
    </Flex>
