...
case "${unameOut}" in
    ...
    "
                envsubst \$SENTRY_DSN,\$HOTJAR_HJID,\$GOOGLE_CLIENT_ID,\$GITHUB_CLIENT_ID < ./docker/templates/nginx-linux.conf.template > ./docker/nginx.conf &&
    ...
    "
                envsubst \$SENTRY_DSN,\$HOTJAR_HJID,\$GOOGLE_CLIENT_ID,\$GITHUB_CLIENT_ID < ./docker/templates/nginx-mac.conf.template > ./docker/nginx.conf &&
