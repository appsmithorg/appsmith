...
describe("createCanvasWidget functionality", () => {
  ...
  it("populates __evaluation__ errors with default error message if error message format is unrecognizable", () => {
    const canvasWidget = {} as unknown as FlattenedWidgetProps;
    ...
    const dataTree = {
      __evaluation__: {
        errors: {
          propertyname: [
            {
              "random error format": "Error Message Stack",
            },
          ],
        },
      },
    ...
    expect(response.errors.length).toEqual(1);
    expect(response.errors[0].name).toStrictEqual("Unknown Error");
    expect(response.errors[0].message).toStrictEqual("Unavailable");
    expect(response.errors[0].stack).toStrictEqual("Unavailable");
  });

  it("populates __evaluation__ errors inside widget error property even if the error is not property formatted", () => {
    const canvasWidget = {} as unknown as FlattenedWidgetProps;
  ...
  it("populates __evaluation__ errors inside widget error property even if the error name/message is not present", () => {
    const canvasWidget = {} as unknown as FlattenedWidgetProps;
    ...
    const dataTree = {
      __evaluation__: {
        errors: {
          text: [
            {
              errorMessage: { unknownfield1: 123, unknownfield2: {} },
              unknownfield3: () => {
                return "error";
              },
            },
