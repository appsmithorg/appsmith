...
import { SetFocusMethod } from "navigation/FocusableElement";
...
export type EditorProps = EditorStyleProps &
  EditorConfig & {
    ...
    focusedEditorState?: FocusState;
    setEditorFocus?: SetFocusMethod;
...
class CodeEditor extends Component<Props, State> {
  ...
  handleEditorFocus = (cm: CodeMirror.Editor) => {
    ...
    const entityInformation: FieldEntityInformation = this.props
      .entityInformation;
    ...
    if (
      ...
      line === 0 &&
      sticky === null &&
      this.props.focusedEditorState?.moreInfo?.cursorState
    ) {
      cm.setCursor(this.props.focusedEditorState.moreInfo.cursorState);
    }
    if (this.props.setEditorFocus) {
      this.props.setEditorFocus(entityInformation.propertyPath, {
        cursorState: { line, ch },
      });
    }
  };
  ...
  render() {
    ...
    const entityInformation = this.props.entityInformation;
    const propertyPath = entityInformation?.propertyPath
      ? btoa(entityInformation.propertyPath).replaceAll("=", "")
