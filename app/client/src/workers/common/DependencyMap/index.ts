    ...
    if (entityType !== "noop") {
      switch (event) {
              ...
              if (!isEmpty(entityDependencyMap)) {
                ...
                Object.entries(entityDependencyMap).forEach(
                  ([entityDependent, entityDependencies]) => {
                    ...
                    const updatedAsyncJSFunctions = asyncJsFunctionInDataFields.update(
                      entityDependent,
                      validReferences,
                      unEvalDataTree,
                    );
        ...
        case DataTreeDiffEvent.DELETE: {
          ...
          const updatedAsyncJSFunctions = asyncJsFunctionInDataFields.handlePathDeletion(
            fullPropertyPath,
            unEvalDataTree,
          );
        ...
        case DataTreeDiffEvent.EDIT: {
          ...
          if (isWidgetActionOrJsObject(entity) && typeof value === "string") {
            ...
            if (isADynamicBindingPath) {
              ...
              const updatedAsyncJSFunctions = asyncJsFunctionInDataFields.handlePathEdit(
                fullPropertyPath,
                validReferences,
                unEvalDataTree,
                inverseDependencyMap,
