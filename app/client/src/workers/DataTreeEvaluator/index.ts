...
export default class DataTreeEvaluator {
  ...
  createFirstTree(
    ...
    } = createDependencyMap(this, clonedUnEvalTree, entityConfigCollection);
    ...
    this.setEvalTree(getValidatedTree(evaluatedTree, entityConfigCollection));
    ...
    Object.keys(this.currentJSCollectionState).forEach((update) => {
      ...
      if (!!unEvalTree[update]) {
        Object.keys(updates).forEach((key) => {
          const data = _.get(unEvalTree, `${update}.${key}.data`, undefined);
          if (isJSObjectFunction(unEvalTree, update, key)) {
            _.set(unEvalTree, `${update}.${key}`, new String(updates[key]));
            _.set(unEvalTree, `${update}.${key}.data`, data);
          } else {
            _.set(unEvalTree, `${update}.${key}`, updates[key]);
          }
        });
      }
    });
  ...
  updateDataTree(
    ...
    const oldUnEvalTreeJSCollections = getJSEntities(
      this.oldUnEvalTree,
      entityConfigCollection,
    );
    const localUnEvalTreeJSCollection = getJSEntities(
      localUnEvalTree,
      entityConfigCollection,
    );
    ...
    const jsTranslatedDiffs = _.flatten(
      jsDifferences.map((diff) =>
        translateDiffEventToDataTreeDiffEvent(diff, entityConfigCollection),
      ),
    );
    ...
    try {
      const evaluatedTree = sortedDependencies.reduce(
        (currentTree: EvalTree, fullPropertyPath: string) => {
          ...
          const entityConfig = this.entityConfigCollection[entityName];
          ...
          if (requiresEval) {
            const evaluationSubstitutionType =
              (entityConfig as EntityConfigWithBindings).reactivePaths[
                propertyPath
          ...
          if (isWidget(entity) && !isATriggerPath) {
            const widgetEntity = entity as WidgetEvalTree;
            if (propertyPath) {
              const parsedValue = validateAndParseWidgetProperty({
                fullPropertyPath,
                widget: widgetEntity,
                widgetConfig: entityConfig as WidgetEntityConfig,
                currentTree,
                evalPropertyValue,
                unEvalPropertyValue,
                entityConfigCollection: this.entityConfigCollection,
              });
            ...
            const actionConfig = entity as ActionEntityConfig;
            if (this.allActionValidationConfig) {
              ...
              const validationConfig =
                !!this.allActionValidationConfig[actionConfig.actionId] &&
                this.allActionValidationConfig[actionConfig.actionId][
                  configProperty
                ];
  ...
  async evaluateTriggers({
    ...
    return evaluateAsync({
      userScript: jsSnippets[0] || userScript,
      dataTree,
      requestId,
      resolvedFunctions,
      context,
  ...
  evaluateDynamicBoundValue(
    ...
    try {
      return evaluateSync({
        ...
        dataTree: data,
        resolvedFunctions,
        isJSCollection: createGlobalData,
        context: contextData,
        evalArguments: callbackData,
    ...
    if (this.inverseValidationDependencyMap[fullPropertyPath]) {
      ...
      pathsToRevalidate.forEach((fullPath) => {
        ...
        validateAndParseWidgetProperty({
          ...
          widget,
          currentTree,
          widgetConfig: this.entityConfigCollection[en],
