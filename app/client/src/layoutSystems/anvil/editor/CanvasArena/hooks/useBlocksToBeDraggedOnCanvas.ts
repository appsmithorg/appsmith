...
import type { AutoCanvasDraggingArenaProps } from "../CanvasDraggingArena";
...
export const useBlocksToBeDraggedOnCanvas = ({
  ...
  const updateChildrenPositions = (
    ...
    if (isNewWidget) addNewWidget(dropPayload, drawingBlocks);
    // else
    //   dispatch({
  ...
  const addNewWidget = (
    ...
    dispatch({
      type: ReduxActionTypes.ANVIL_ADD_NEW_WIDGETS,
      payload: {
        dropPayload,
