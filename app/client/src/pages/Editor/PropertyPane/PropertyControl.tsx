...
const PropertyControl = memo(
  (props: Props) => {
    ...
    const onDeleteProperties = useCallback(
      (propertyPaths: string[]) => {
        dispatch(
          deleteWidgetProperty(widgetProperties.widgetId, propertyPaths),
        );
      },
      [widgetProperties.widgetId],
    );
    ...
    if (widgetProperties) {
      ...
      const getCustomJSControl = (): string | undefined => {
        if (childWidgetCustomJSControlEnhancementFn) {
          return childWidgetCustomJSControlEnhancementFn() as
            | string
            | undefined;
        }
  ...
  (currProps: Props, prevProps: Props) => {
