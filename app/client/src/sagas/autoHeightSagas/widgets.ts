...
import { ReflowedSpace } from "reflow/reflowTypes";
import { reflowMoveAction } from "actions/reflowActions";
...
export function* computeWidgetsToUpdate(
  ...
  let dynamicHeightLayoutTree: Record<string, TreeNode> = yield select(
    getAutoHeightLayoutTree,
  );
  if (action === undefined) {
    const tree: {
      tree: AutoHeightLayoutTreeReduxState;
      canvasLevelMap: CanvasLevelsReduxState;
    } = yield getLayoutTree(false);
    dynamicHeightLayoutTree = tree.tree;
  }
  ...
  const reflowMoveActionPayload: Record<string, ReflowedSpace> = {};
  ...
  for (const widgetId in updates) {
    ...
    if (widget && !widget.detachFromLayout) {
      ...
      console.log("Dynamic Height: Updates to consider:", { expectedUpdates });
    } else if (widget) {
  ...
  if (expectedUpdates.length > 0) {
            ...
            if (isAutoHeightEnabledForWidget(parentContainerLikeWidget)) {
              ...
              if (layoutData === undefined) {
                layoutData = {
                  ...
                  originalBottomRow:
                    parentContainerLikeWidget.originalBottomRow ||
                    parentContainerLikeWidget.bottomRow,
                  originalTopRow:
                    parentContainerLikeWidget.originalTopRow ||
    ...
    for (const changedWidgetId in changesSoFar) {
      ...
      reflowMoveActionPayload[changedWidgetId] = {
        height:
          (changesSoFar[changedWidgetId].bottomRow -
            changesSoFar[changedWidgetId].topRow) *
          GridDefaults.DEFAULT_GRID_ROW_HEIGHT,
        Y: changesSoFar[changedWidgetId].topRow,
      };
  ...
  log.debug(
    "Dynamic Height: All computations:",
    performance.now() - start,
    "ms",
  );

  log.debug(
    "Dynamic Height: Including Redux update:",
    performance.now() - start,
    "ms",
  );

  yield put(reflowMoveAction(reflowMoveActionPayload));

  return { widgetsToUpdate, shouldRecomputeContainers };
}
