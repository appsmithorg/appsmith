...
function* startAppEngine(action: ReduxAction<AppEnginePayload>) {
  try {
    const engine = AppEngineFactory(action.payload.mode);
    engine.startPerformanceTracking();
    yield call(engine.bootstrapEditor.bind(engine), action.payload);
    const { applicationId, toLoadPageId } = yield call(
      engine.loadAppData.bind(engine),
      action.payload,
    );
    yield call(
      engine.initiateURLUpdate.bind(engine),
      toLoadPageId,
      action.payload.pageId,
    );
    yield call(
      engine.loadAppEntities.bind(engine),
      toLoadPageId,
      applicationId,
    );
    yield call(engine.initiateGit.bind(engine), applicationId);
    yield call(engine.completeChore.bind(engine));
