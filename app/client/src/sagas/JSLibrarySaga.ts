...
export function* installLibrarySaga(lib: Partial<IJSLibrary>) {
  const { url } = lib;
  ...
  const installedLibraries: IJSLibrary[] = yield select(
...
function* uninstallLibrarySaga(action: ReduxAction<IJSLibrary>) {
...
function* fetchJSLibraries(action: ReduxAction<string>) {
  ...
  try {
    ...
    const libraries = response.data as Array<IJSLibrary & { defs: string }>;
...
function* startInstallationRequestChannel() {
  ...
  while (true) {
    const action: ReduxAction<Partial<IJSLibrary>> =
