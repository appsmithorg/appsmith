...
import { getSelectedAppThemeStylesheet } from "selectors/appThemeSelectors";
...
function* getThemeDefaultConfig(type: string) {
  const selectedTheme = yield select(getSelectedAppThemeStylesheet);

  const config = selectedTheme[type];
