...
import type { ImportCustomBuildingBlockToApplicationResponse } from "@appsmith/api/ApplicationApi";
import ApplicationApi, {
  type ImportBuildingBlockToApplicationResponse,
  type ImportCustomBuildingBlockToApplicationRequest,
...
} from "selectors/buildingBlocksSelectors";
import {
...
function* addBuildingBlockActionsToApplication(dragDetails: DragDetails) {
  ...
  const buildingBlocks: any[] = yield select(customBuildingBlocks);
  const currentPageId: string = yield select(getCurrentPageId);
  const workspaceId: string = yield select(getCurrentWorkspaceId);
  const selectedBuildingBlock = buildingBlocks.find(
    (buildingBlock) => buildingBlock.name === buildingblockName,
  );

  const body: ImportCustomBuildingBlockToApplicationRequest = {
    pageId: currentPageId,
    applicationId,
    workspaceId,
    templateId: selectedBuildingBlock.id,
    bbid: selectedBuildingBlock.id,
  };
  try {
    // api call adds DS, queries and JS to page and returns new page dsl with building block
    const response: ApiResponse<ImportCustomBuildingBlockToApplicationResponse> =
...
export function* addCustomBuildingBlockToCanvasSaga(
