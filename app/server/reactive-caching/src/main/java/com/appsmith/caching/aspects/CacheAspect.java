...
public class CacheAspect {
            ...
            return ((Mono<?>) joinPoint.proceed())
                    .zipWhen(
                            value ->
                                    cacheManager.put(
                                            cacheName, key,
                                            value)) // Call CacheManager.put() to cache the object
            ...
            return ((Flux<?>) joinPoint.proceed())
                    ...
                    .zipWhen(
                            value ->
                                    cacheManager.put(
                                            cacheName, key,
                                            value)) // Call CacheManager.put() to cache the list
    ...
    private String deriveKeyWithArguments(Object[] args) {
        ...
        SimpleKey simpleKey =
                new SimpleKey(
                        args); // Create SimpleKey from arguments and return its toString() value
                            ...
                            Mono.defer(
                                    () ->
                                            callMonoMethodAndCache(
                                                    joinPoint, cacheName,
                                                    key))); // defer the creation of Mono until
    ...
    public Object cacheEvict(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        if (!returnType.isAssignableFrom(Mono.class)) {
            throw new RuntimeException(
                    "Invalid usage of @CacheEvict for "
                            + method.getName()
