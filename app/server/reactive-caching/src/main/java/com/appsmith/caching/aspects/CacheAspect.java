...
public class CacheAspect {
  ...
  /**
   * This method is used to call original Mono<T> returning method and return the the result after
   * caching it with CacheManager
   *
  ...
  private Mono<Object> callMonoMethodAndCache(
      ProceedingJoinPoint joinPoint, String cacheName, String key) {
    try {
      return ((Mono<?>) joinPoint.proceed())
          .zipWhen(
              value ->
                  cacheManager.put(
                      cacheName, key, value)) // Call CacheManager.put() to cache the object
  ...
  /**
   * This method is used to call original Flux<T> returning method and return the the result after
   * caching it with CacheManager
   *
   ...
   */
  private Flux<?> callFluxMethodAndCache(
      ProceedingJoinPoint joinPoint, String cacheName, String key) {
    try {
      return ((Flux<?>) joinPoint.proceed())
          ...
          .zipWhen(
              value ->
                  cacheManager.put(
                      cacheName, key, value)) // Call CacheManager.put() to cache the list
  ...
  /**
   * This method is used to derive the key name for caching the result of a method call based on
   * method arguments. This uses original strategy used by Spring's Cacheable annotation.
   *
  ...
  private String deriveKeyWithArguments(Object[] args) {
    ...
    SimpleKey simpleKey =
        new SimpleKey(args); // Create SimpleKey from arguments and return its toString() value
    return simpleKey.toString();
  }

  /**
   * This method is used to derive the key name for caching the result of a method call based on
   * method arguments and expression provided.
   *
   ...
   * @return Key name for caching the result of the method call
   */
  private String deriveKeyWithExpression(
  ...
  public Object cacheable(ProceedingJoinPoint joinPoint) throws Throwable {
    ...
    if (returnType.isAssignableFrom(Mono.class)) { // If method returns Mono<T>
      return cacheManager
          ...
          .switchIfEmpty(
              Mono.defer(
                  () ->
                      callMonoMethodAndCache(
                          joinPoint, cacheName, key))); // defer the creation of Mono until
      // subscription as it will call original
      // function
    }
    ...
    if (returnType.isAssignableFrom(Flux.class)) { // If method returns Flux<T>
      return cacheManager
          ...
          .switchIfEmpty(
              Mono.defer(
                  () ->
                      callFluxMethodAndCache(joinPoint, cacheName, key)
                          .collectList())) // defer the creation of Flux
          // until subscription as it
    ...
    throw new IllegalAccessException(
        "Invalid usage of @Cache annotation. Only reactive objects Mono and Flux are"
            + " supported for caching.");
  }

  /**
   * This method defines a Aspect to handle method calls annotated with ReactiveEvict. Original
   * method should return Mono<?>
   *
   * @param joinPoint ProceedingJoinPoint of the method call
