...
public Object cacheable(ProceedingJoinPoint joinPoint) throws Throwable {
			...
			Mono.defer(
				() ->
					callMonoMethodAndCache(
						joinPoint, cacheName,
						key))); // defer the creation of Mono until subscription as it will call
	// original function
	}
			...
			Mono.defer(
				() ->
					callFluxMethodAndCache(joinPoint, cacheName, key)
						.collectList())) // defer the creation of Flux until subscription as it
	...
	throw new IllegalAccessException(
		"Invalid usage of @Cache annotation. Only reactive objects Mono and Flux are supported for"
			+ " caching.");
}
