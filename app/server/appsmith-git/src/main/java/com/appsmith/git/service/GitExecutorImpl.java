...
public class GitExecutorImpl implements GitExecutor {
    ...
    private static final String SUCCESS_MERGE_STATUS =
            "This branch has no conflicts with the base branch.";

    /**
     * This method will handle the git-commit functionality. Under the hood it checks if the repo
     * has already been initialised and will be initialised if git repo is not present
     *
    ...
    public Mono<String> commitApplication(
        ...
        final String finalAuthorName =
                StringUtils.isEmptyOrNull(authorName)
                        ? AppsmithBotAsset.APPSMITH_BOT_USERNAME
                        : authorName;
        final String finalAuthorEmail =
                StringUtils.isEmptyOrNull(authorEmail)
                        ? AppsmithBotAsset.APPSMITH_BOT_EMAIL
                        : authorEmail;
        return Mono.fromCallable(
                        () -> {
                            ...
                            Stopwatch processStopwatch =
                                    StopwatchHelpers.startStopwatch(
     ...
     *
     * @param repoSuffix Path used to generate the repo url specific to the application for which
     *     the commit history is requested
        ...
        return Mono.fromCallable(
                        () -> {
                            ...
                            Stopwatch processStopwatch =
                                    StopwatchHelpers.startStopwatch(
                                            repoPath,
                                            AnalyticsEvents.GIT_COMMIT_HISTORY.getEventName());
                            try (Git git = Git.open(repoPath.toFile())) {
                                Iterable<RevCommit> gitLogs =
                                        git.log().setMaxCount(Constraint.MAX_COMMIT_LOGS).call();
                                gitLogs.forEach(
                                        revCommit -> {
     ...
     *
     * @param repoSuffix Path used to generate the repo url specific to the application which needs
     *     to be pushed to remote
    ...
    public Mono<String> pushApplication(
            ...
            String privateKey,
            String branchName) {
        // We can safely assume that repo has been already initialised either in commit or clone
        // flow and can directly
        // open the repo
        return Mono.fromCallable(
                        () -> {
                            log.debug(
     ...
     */
    @Override
    public Mono<String> cloneApplication(
            Path repoSuffix, String remoteUrl, String privateKey, String publicKey) {

        Stopwatch processStopwatch =
                StopwatchHelpers.startStopwatch(
                        repoSuffix, AnalyticsEvents.GIT_CLONE.getEventName());
        return Mono.fromCallable(
                        () -> {
                            log.debug(
                                    Thread.currentThread().getName()
                                            + ": Cloning the repo from the remote "
                                            + remoteUrl);
                            final TransportConfigCallback transportConfigCallback =
                                    new SshTransportConfigCallback(privateKey, publicKey);
                            File file =
                                    Paths.get(gitServiceConfig.getGitRootPath())
                                            .resolve(repoSuffix)
                                            .toFile();
                            while (file.exists()) {
    ...
    public Mono<String> createAndCheckoutToBranch(Path repoSuffix, String branchName) {
        ...
        Stopwatch processStopwatch =
                StopwatchHelpers.startStopwatch(
                        repoSuffix, AnalyticsEvents.GIT_CREATE_BRANCH.getEventName());
        return Mono.fromCallable(
                        () -> {
                            log.debug(
                                    Thread.currentThread().getName()
    ...
    public Mono<Boolean> deleteBranch(Path repoSuffix, String branchName) {
        ...
        Stopwatch processStopwatch =
                StopwatchHelpers.startStopwatch(
                        repoSuffix, AnalyticsEvents.GIT_DELETE_BRANCH.getEventName());
        return Mono.fromCallable(
                        () -> {
                            log.debug(
                                    Thread.currentThread().getName()
    ...
    public Mono<Boolean> checkoutToBranch(Path repoSuffix, String branchName) {
        ...
        Stopwatch processStopwatch =
                StopwatchHelpers.startStopwatch(
                        repoSuffix, AnalyticsEvents.GIT_CHECKOUT.getEventName());
        return Mono.fromCallable(
                        () -> {
                            log.debug(
                                    Thread.currentThread().getName()
                                            ...
                                            + branchName);
                            // We can safely assume that repo has been already initialised either in
                            // commit or clone flow and can directly
                            // open the repo
                            Path baseRepoPath = createRepoPath(repoSuffix);
                            try (Git git = Git.open(baseRepoPath.toFile())) {
                                ...
                                return StringUtils.equalsIgnoreCase(
                                        checkedOutBranch, "refs/heads/" + branchName);
                            } catch (Exception e) {
    ...
    public Mono<MergeStatusDTO> pullApplication(
        ...
        Stopwatch processStopwatch =
                StopwatchHelpers.startStopwatch(
                        repoSuffix, AnalyticsEvents.GIT_PULL.getEventName());
        TransportConfigCallback transportConfigCallback =
                new SshTransportConfigCallback(privateKey, publicKey);
                            ...
                            () -> {
                                ...
                                if (mergeResult.getMergeStatus().isSuccessful()) {
                                    ...
                                    mergeStatus.setStatus(
                                            count + " commits merged from origin/" + branchName);
                                } else {
                                    // If there are conflicts add the conflicting file names to the
                                    // response structure
    ...
    public Mono<List<GitBranchDTO>> listBranches(
        ...
        String gitAction =
                Boolean.TRUE.equals(refreshBranches)
        ...
        return Mono.fromCallable(
                        () -> {
                            log.debug(
                                    Thread.currentThread().getName()
                                            + ": Get branches for the application "
                                            + repoSuffix);
                            TransportConfigCallback transportConfigCallback =
                                    new SshTransportConfigCallback(privateKey, publicKey);
    ...
    public Mono<GitStatusDTO> getStatus(Path repoPath, String branchName) {
        Stopwatch processStopwatch =
                StopwatchHelpers.startStopwatch(
                        repoPath, AnalyticsEvents.GIT_STATUS.getEventName());
        return Mono.fromCallable(
                        () -> {
                            try (Git git = Git.open(repoPath.toFile())) {
                                log.debug(
                                ...
                                for (String x : modifiedAssets) {
                                    if (x.contains(CommonConstants.CANVAS)) {
                                        ...
                                        String queryName =
                                                x.split(GitDirectories.ACTION_DIRECTORY + "/")[1];
                                        ...
                                        if (position != -1) {
                                            ...
                                            if (!queriesModified.contains(pageName + queryName)) {
                                                ...
                                                modifiedQueries++;
                                                    ...
                                                    GitDirectories.ACTION_COLLECTION_DIRECTORY
                                                            + "/")
                                            && !x.endsWith(".json")) {
                                        ...
                                        if (!jsObjectsModified.contains(pageName + queryName)) {
                                            ...
                                            GitDirectories.DATASOURCE_DIRECTORY + "/")) {
                                        modifiedDatasources++;
                                ...
                                if (trackingStatus != null) {
                                    ...
                                    response.setRemoteBranch(
                                            trackingStatus.getRemoteTrackingBranch());
                                } else {
                                    log.debug(
                                            "Remote tracking details not present for branch: {},"
                                                    + " repo: {}",
                                            branchName,
                                            repoPath);
                                ...
                                // Remove modified changes from current branch so that checkout to
                                // other branches will be possible
                                if (!status.isClean()) {
                                    return resetToLastCommit(git)
                                            .map(
                                                    ref -> {
    ...
    private int getModifiedQueryCount(
            Set<String> jsObjectsModified, int modifiedCount, String filePath) {
    ...
    public Mono<String> mergeBranch(
            Path repoSuffix, String sourceBranch, String destinationBranch) {
        return Mono.fromCallable(
                        () -> {
                            Stopwatch processStopwatch =
                                    StopwatchHelpers.startStopwatch(
    ...
    public Mono<String> fetchRemote(
        ...
        Stopwatch processStopwatch =
                StopwatchHelpers.startStopwatch(
                        repoSuffix, AnalyticsEvents.GIT_FETCH.getEventName());
    ...
    public Mono<MergeStatusDTO> isMergeBranch(
        ...
        Stopwatch processStopwatch =
                StopwatchHelpers.startStopwatch(
                        repoSuffix, AnalyticsEvents.GIT_MERGE_CHECK.getEventName());
        return Mono.fromCallable(
                        () -> {
                            log.debug(
                                    Thread.currentThread().getName()
                            ...
                            try (Git git = Git.open(createRepoPath(repoSuffix).toFile())) {
                                ...
                                if (mergeResult.getMergeStatus().isSuccessful()) {
                                    ...
                                    // If there aer conflicts add the conflicting file names to the
                                    // response structure
                                    mergeStatus.setMergeAble(false);
    ...
    public Mono<String> checkoutRemoteBranch(Path repoSuffix, String branchName) {
        ...
        return Mono.fromCallable(
                        () -> {
                            log.debug(
                                    Thread.currentThread().getName()
                                            + ": Checking out remote branch origin/"
                                            + branchName
                                            + " for the repo "
                                            + repoSuffix);
                            // open the repo
    ...
    private Mono<Ref> resetToLastCommit(Git git) throws GitAPIException {
        Stopwatch processStopwatch =
                StopwatchHelpers.startStopwatch(
                        git.getRepository().getDirectory().toPath().getParent(),
                        AnalyticsEvents.GIT_RESET.getEventName());
        return Mono.fromCallable(
                        () -> {
