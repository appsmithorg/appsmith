...
public class GitExecutorImpl implements GitExecutor {
  ...
  /**
   * This method will handle the git-commit functionality. Under the hood it checks if the repo has
   * already been initialised and will be initialised if git repo is not present
   *
   ...
   *
   * @param repoSuffix Path used to generate the repo url specific to the application for which the
   *     commit history is requested
  ...
  public Mono<List<GitLogDTO>> getCommitHistory(Path repoSuffix) {
    return Mono.fromCallable(
            () -> {
              log.debug(
                  Thread.currentThread().getName() + ": get commit history for  " + repoSuffix);
   ...
   *
   * @param repoSuffix Path used to generate the repo url specific to the application which needs to
   *     be pushed to remote
  ...
  public Mono<String> pushApplication(
    ...
    return Mono.fromCallable(
            () -> {
              log.debug(
                  Thread.currentThread().getName() + ": pushing changes to remote " + remoteUrl);
              // open the repo
            ...
            () -> {
              ...
              try (Git git = Git.open(baseRepoPath.toFile())) {
                ...
                List<String> deleteBranchList =
                    git.branchDelete().setBranchNames(branchName).setForce(Boolean.TRUE).call();
                processStopwatch.stopAndLogTimeInMillis();
  ...
  public Mono<Boolean> checkoutToBranch(Path repoSuffix, String branchName) {
    ...
    return Mono.fromCallable(
            () -> {
              log.debug(
                  Thread.currentThread().getName() + ": Switching to the branch " + branchName);
              ...
              try (Git git = Git.open(repoPath.toFile())) {
                ...
                for (String x : modifiedAssets) {
                  if (x.contains(CommonConstants.CANVAS)) {
                    ...
                    if (x.contains(GitDirectories.ACTION_DIRECTORY + "/")) {
                      ...
                      if (x.contains(GitDirectories.ACTION_COLLECTION_DIRECTORY + "/")
                          && !x.endsWith(".json")) {
                        ...
                        if (x.contains(GitDirectories.DATASOURCE_DIRECTORY + "/")) {
                          modifiedDatasources++;
                        } else {
                          if (x.contains(GitDirectories.JS_LIB_DIRECTORY + "/")) {
                            modifiedJSLibs++;
                          }
                        }
                      }
