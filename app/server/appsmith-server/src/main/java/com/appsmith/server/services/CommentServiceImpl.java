...
public class CommentServiceImpl extends BaseService<CommentRepository, Comment, String> implements CommentService {
        ...
        return userMono.flatMap(user ->
                ...
                .flatMapMany(thread -> {
                    ...
                    if (TRUE.equals(thread.getIsPrivate())) {
                        ...
                        commentSaverMonos.add(createBotComment(thread, user, CommentState.COMMENTED));
                    }
    ...
    private Mono<Boolean> triggerBotThreadResolved(CommentThread resolvedThread, User user) {
        return userDataRepository.findByUserId(user.getId())
                .defaultIfEmpty(new UserData(user.getId()))
                .flatMap(userData -> {
                    if (userData.getCommentState() == CommentState.COMMENTED) {
                        // update the user data
                        userData.setCommentState(CommentState.RESOLVED);
                                ...
                                .flatMap(application -> {
                                    ...
                                    return saveCommentThread(commentThread, application, user)
                                            .flatMap(savedCommentThread ->
                                                    createBotComment(savedCommentThread, user, CommentState.RESOLVED)
    ...
    private Mono<Comment> createBotComment(CommentThread commentThread, User user, CommentState commentState) {
        ...
        try {
            if (commentState == CommentState.COMMENTED) {
