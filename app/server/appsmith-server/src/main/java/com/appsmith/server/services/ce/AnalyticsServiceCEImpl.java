...
public Mono<Void> sendEvent(
	...
	if (userId != null
		...
		// But send the email intact for the subscribe event, which is sent only if the user has
...
public <T extends BaseDomain> Mono<T> sendObjectEvent(
	...
	// In case the event needs to be sent during sign in, then `sessionUserService.getCurrentUser()`
	// returns Mono.empty()
	...
	return userMono
		...
		.flatMap(
			...
			// In case the user is anonymous, don't raise an event, unless it's a signup, logout,
			// page view or action execution event.
...
private <T extends BaseDomain> String getEventTag(AnalyticsEvents event, T object) {
	// In case of action execution or instance setting update, event.getEventName() only is used to
	// support backward compatibility of event name
