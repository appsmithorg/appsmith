...
public Flux<T> get(MultiValueMap<String, String> params) {
	// In the base service we aren't handling the query parameters. In order to filter records using
	// the query params,
	// each service must implement it for their usecase. Need to come up with a better strategy for
	// doing this.
...
public Mono<T> getById(ID id) {
	...
	return repository
		.findById(id)
		.switchIfEmpty(
			Mono.error(new AppsmithException(AppsmithError.NO_RESOURCE_FOUND, "resource", id)));
}
	...
	return policies.stream()
		.collect(
			Collectors.groupingBy(
				Policy::getPermission, Collectors.mapping(Function.identity(), toSet())));
}
