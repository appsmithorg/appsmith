...
public class CustomNewPageRepositoryCEImpl extends BaseAppsmithRepositoryImpl<NewPage>
    ...
    public Flux<NewPage> findByApplicationId(
            String applicationId, Optional<AclPermission> permission) {
        Criteria applicationIdCriteria =
                where(fieldName(QNewPage.newPage.applicationId)).is(applicationId);
    ...
    @Override
    public Flux<NewPage> findByApplicationIdAndNonDeletedEditMode(
            String applicationId, AclPermission aclPermission) {
        Criteria applicationIdCriteria =
                where(fieldName(QNewPage.newPage.applicationId)).is(applicationId);
        // In case a page has been deleted in edit mode, but still exists in deployed mode, NewPage
        // object would exist. To handle this, only fetch non-deleted pages
    ...
    public Mono<NewPage> findByIdAndLayoutsIdAndViewMode(
        ...
        if (Boolean.TRUE.equals(viewMode)) {
            ...
            // In case a page has been deleted in edit mode, but still exists in deployed mode,
            // NewPage object would exist. To handle this, only fetch non-deleted pages
    ...
    @Override
    public Mono<NewPage> findByNameAndViewMode(
            String name, AclPermission aclPermission, Boolean viewMode) {
    ...
    public Flux<NewPage> findAllPageDTOsByIds(List<String> ids, AclPermission aclPermission) {
        ArrayList<String> includedFields =
                new ArrayList<>(
                        List.of(
                                ...
                                (fieldName(QNewPage.newPage.unpublishedPage)
                                        + "."
                                        + fieldName(QNewPage.newPage.unpublishedPage.icon)),
                                (fieldName(QNewPage.newPage.unpublishedPage)
                                        + "."
        ...
        return this.queryAll(
                new ArrayList<>(List.of(idsCriterion)), includedFields, aclPermission, null);
    }
                ...
                .map(
                        p -> {
                            PageDTO page =
                                    (isPublishedName
                                            ? p.getPublishedPage()
                                            : p.getUnpublishedPage());
                            if (page != null) {
                                return page.getName();
                            }
                            // If the page hasn't been published, just send the unpublished page
                            // name
    ...
    @Override
    public Flux<NewPage> findSlugsByApplicationIds(
            List<String> applicationIds, AclPermission aclPermission) {
        Criteria applicationIdCriteria =
                where(fieldName(QNewPage.newPage.applicationId)).in(applicationIds);
        String unpublishedSlugFieldPath =
                String.format(
                        "%s.%s",
                        fieldName(QNewPage.newPage.unpublishedPage),
                        fieldName(QNewPage.newPage.unpublishedPage.slug));
        String unpublishedCustomSlugFieldPath =
                String.format(
                        "%s.%s",
                        fieldName(QNewPage.newPage.unpublishedPage),
                        fieldName(QNewPage.newPage.unpublishedPage.customSlug));
        ...
        String publishedCustomSlugFieldPath =
                String.format(
                        "%s.%s",
                        fieldName(QNewPage.newPage.publishedPage),
                        fieldName(QNewPage.newPage.publishedPage.customSlug));
    ...
    public Mono<NewPage> findByGitSyncIdAndDefaultApplicationId(
        ...
        return findByGitSyncIdAndDefaultApplicationId(
                defaultApplicationId, gitSyncId, Optional.ofNullable(permission));
    }
    ...
    public Mono<NewPage> findByGitSyncIdAndDefaultApplicationId(
        ...
        final String defaultResources =
                fieldName(QBranchAwareDomain.branchAwareDomain.defaultResources);
