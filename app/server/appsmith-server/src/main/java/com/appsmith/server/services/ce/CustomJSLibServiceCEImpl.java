...
public class CustomJSLibServiceCEImpl
        extends BaseService<CustomJSLibRepository, CustomJSLib, String>
    ...
    public Mono<Boolean> addJSLibToApplication(
        ...
        return getAllJSLibApplicationDTOFromApplication(applicationId, branchName, false)
                ...
                .map(
                        tuple -> {
                            /**
                             * TODO: try to convert it into a single update op where reading of list
                             * is not required Tracked here:
                ...
                .flatMap(
                        updatedJSLibDTOList -> {
                            ...
                            return applicationService.update(
                                    applicationId, fieldNameValueMap, branchName);
                        })
        ...
        return repository
                ...
                .flatMap(
                        foundJSLib -> {
                            ...
                            */
                            if ((jsLib.getDefs().length() > foundJSLib.getDefs().length())
                                    || isForceInstall) {
        ...
        return getAllJSLibApplicationDTOFromApplication(applicationId, branchName, false)
                .map(
                        jsLibDTOSet -> {
                             ...
                             * https://github.com/appsmithorg/appsmith/issues/18226
                             */
                            CustomJSLibApplicationDTO currentJSLibDTO =
    ...
    private Mono<List<CustomJSLib>> getAllCustomJSLibsFromApplication(
        ...
        return getAllJSLibApplicationDTOFromApplication(applicationId, branchName, isViewMode)
                ...
                .map(
                        jsLibList -> {
                            Collections.sort(
                                    jsLibList, Comparator.comparing(CustomJSLib::getUidString));
                            return jsLibList;
                        });
    }
        ...
        return applicationService
                .findByIdAndBranchName(
                        ...
                        List.of(
                                isViewMode
                                        ? FieldName
                                                .PUBLISHED_JS_LIBS_IDENTIFIER_IN_APPLICATION_CLASS
                                        : FieldName
                                                .UNPUBLISHED_JS_LIBS_IDENTIFIER_IN_APPLICATION_CLASS),
                        branchName)
                .map(
