...
public class RedisConfig {
  ...
  /**
   * This is the topic to which we will publish & subscribe to. We can have multiple topics based on
   * the messages that we wish to broadcast. Starting with a single one for now.
   *
   * @return
   */
  @Bean
  ...
  private static class JSONSessionRedisSerializer implements RedisSerializer<Object> {
    ...
    public Object deserialize(byte[] bytes) {
      if (ByteUtils.startsWith(bytes, SESSION_DATA_PREFIX)) {
        ...
        if (ByteUtils.startsWith(bytes, OAUTH_CLIENT_PREFIX)) {
