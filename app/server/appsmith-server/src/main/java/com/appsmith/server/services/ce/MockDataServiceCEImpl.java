...
public Mono<MockDataDTO> getMockDataSet() {
	...
	return WebClientUtils.create(baseUrl + "/api/v1/mocks")
		...
		.flatMap(
			...
			if (response.statusCode().is2xxSuccessful()) {
				return response.bodyToMono(
					new ParameterizedTypeReference<ResponseDTO<MockDataDTO>>() {});
			} else {
				return Mono.error(
		...
		.doOnError(
			error -> log.error("Error fetching mock data sets config from cloud services", error));
}
	...
	return mockDataSet.flatMap(
		...
		if (mockDataSource.getPackageName().equals("mongo-plugin")) {
			datasourceConfiguration =
				getMongoDataSourceConfiguration(mockDataSource.getName(), mockDataDTO);
		} else {
			datasourceConfiguration =
				getPostgresDataSourceConfiguration(mockDataSource.getName(), mockDataDTO);
		}
...
private Mono<Datasource> createSuffixedDatasource(
	Datasource datasource, String name, int suffix) {
	...
	password =
		((DBAuth) datasource.getDatasourceConfiguration().getAuthentication()).getPassword();
	}
