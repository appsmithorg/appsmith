			...
			contextView ->
				Mono.justOrEmpty(
					contextView
						...
						.getHeaders()
						.getFirst(headerName)))
		// An error is thrown when the context is not available. We don't want to fail the request
		// in this case.
		.onErrorResume(error -> Mono.empty());
}

/**
* Returns the value of `X-Anonymous-User-Id` header, from the _current_ request. Since this gets
* the header from the current request, it has to be called from a request context. It won't work
* in new background contexts, like when calling `.subscribe()` on a Mono.
*
