...
/**
 * This class is invoked for SSO logins like <strong>Google</strong> which implement the {@link
 * OAuth2User} interface for the user. We transform the {@link OAuth2User} object to {@link User}
 * object via the {@link #loadUser(OidcUserRequest)} We also create the user if it doesn't exist we
 * create it via {@link #checkAndCreateUser(OidcUser, OidcUserRequest)}
 */
@Slf4j
...
public Mono<User> checkAndCreateUser(OidcUser oidcUser, OidcUserRequest userRequest) {
	...
	String username =
		(!StringUtils.isEmpty(oidcUser.getEmail())) ? oidcUser.getEmail() : oidcUser.getName();
