...
/**
 * This code has been copied from {@link WebSessionServerOAuth2AuthorizedClientRepository} which
 * also implements ServerOAuth2AuthorizedClientRepository. This was done to make changes to
 * saveAuthorizedClient to also handle adding users to UserRepository.
 *
 * <p>This was done because on authorization, the user needs to be stored in appsmith domain. To
 * achieve this, saveAuthorizedClient function has been edited in the following manner. In the
 * reactive flow, post doOnSuccess transformation, another Mono.then has been added. In this,
 * Authentication object is passed to checkAndCreateUser function. This object is used to get
 * OidcUser from which user attributes like name, email, etc are extracted. If the user doesnt exist
 * in User Repository, a new user is created and saved.
 *
 * <p>The ClientUserRepository is created during SecurityWebFilterChain Bean creation. By
 * configuring to use Oauth2Login, this ServerOAuth2AuthorizedClientRepository implementation is
 * injected. This hack is used to ensure that on successful authentication, we are able to record
 * the user in our database. Since ServerOAuth2AuthorizedClientRepository's saveAuthorizedClient is
 * called on every successful OAuth2 authentication, this solves the problem of plugging a handler
...
public class ClientUserRepository implements ServerOAuth2AuthorizedClientRepository {
    ...
    public static final String DEFAULT_AUTHORIZED_CLIENTS_ATTR_NAME =
            WebSessionServerOAuth2AuthorizedClientRepository.class.getName()
                    + ".AUTHORIZED_CLIENTS";
    ...
    public Mono<Void> saveAuthorizedClient(
        ...
        // This is to provide more control over which accounts can be used to access the
        // application.
        // TODO: This is not a good way to do this. Ideally, we should pass "hd=example.com" to
        // OAuth2 provider to list relevant accounts only
        if (!commonConfig.getOauthAllowedDomains().isEmpty()) {
            ...
            if (principal.getPrincipal() instanceof DefaultOidcUser) {
                ...
                if (!commonConfig.getOauthAllowedDomains().contains(domain)) {
                    return Mono.error(
                            new AppsmithException(AppsmithError.UNAUTHORIZED_DOMAIN, domain));
                }
            }
        }
                ...
                .doOnSuccess(
                        session -> {
                            ...
                            session.getAttributes()
                                    .put(this.sessionAttributeName, authorizedClients);
                        })
                /*
                 * TODO
