...
@Component
@Slf4j
public class CustomThemeRepositoryCEImpl extends BaseAppsmithRepositoryImpl<Theme>
        implements CustomThemeRepositoryCE {
    ...
    public Mono<Theme> getSystemThemeByName(String themeName) {
        ...
        Criteria criteria =
                where(fieldName(QTheme.theme.name))
                ...
                .flatMap(
                        permissionGroups -> {
                            Criteria permissionCriteria =
                                    userAcl(permissionGroups, AclPermission.MANAGE_THEMES);
    ...
    public Mono<Boolean> archiveDraftThemesById(
            String editModeThemeId, String publishedModeThemeId) {
        Criteria criteria =
                where(fieldName(QTheme.theme.id))
