...
public class CustomApplicationRepositoryCEImpl extends BaseAppsmithRepositoryImpl<Application>
  ...
  @Override
  public Flux<Application> findByMultipleWorkspaceIds(
  ...
  public Flux<Application> findAllUserApps(AclPermission permission) {
    Mono<User> currentUserWithTenantMono =
  ...
  public Flux<Application> findByClonedFromApplicationId(
    ...
    Criteria clonedFromCriteria =
        where(fieldName(QApplication.application.clonedFromApplicationId)).is(applicationId);
  ...
  public Mono<UpdateResult> setDefaultPage(String applicationId, String pageId) {
    // Since this can only happen during edit, the page in question is unpublished page. Hence
    // the update should
    // be to pages and not publishedPages

    final Mono<UpdateResult> setAllAsNonDefaultMono =
    ...
    final Mono<UpdateResult> setDefaultMono =
        mongoOperations.updateFirst(
            Query.query(getIdCriteria(applicationId))
  ...
  @Override
  public Mono<UpdateResult> setGitAuth(
  ...
  public Mono<Application> getApplicationByGitBranchAndDefaultApplicationId(
    ...
    return getApplicationByGitBranchAndDefaultApplicationId(
        defaultApplicationId, null, branchName, aclPermission);
  }
  ...
  public Mono<Application> getApplicationByGitBranchAndDefaultApplicationId(
    ...
    Criteria branchNameCriteria =
        where(
            ...
            .is(branchName);
    return queryOne(
        List.of(defaultAppCriteria, branchNameCriteria), projectionFieldNames, aclPermission);
  }
  ...
  public Mono<List<String>> getAllApplicationId(String workspaceId) {
    ...
    return mongoOperations.find(query, Application.class).map(BaseDomain::getId).collectList();
  }
  ...
  public Flux<Application> getGitConnectedApplicationByWorkspaceId(String workspaceId) {
    String gitApplicationMetadata = fieldName(QApplication.application.gitApplicationMetadata);
    // isRepoPrivate and gitAuth will be stored only with default application which ensures we
    ...
    Criteria workspaceIdCriteria =
        where(fieldName(QApplication.application.workspaceId)).is(workspaceId);
    return queryAll(
        List.of(workspaceIdCriteria, repoCriteria, gitAuthCriteria),
        ...
        where(
            ...
            .equals(
                "this."
                    ...
                    + "."
                    + fieldName(
                        QApplication.application.gitApplicationMetadata.defaultBranchName)));
  ...
  public Mono<UpdateResult> setAppTheme(
    ...
    if (StringUtils.hasLength(editModeThemeId)) {
      updateObj =
          updateObj.set(fieldName(QApplication.application.editModeThemeId), editModeThemeId);
    }
    if (StringUtils.hasLength(publishedModeThemeId)) {
      updateObj =
          updateObj.set(
              fieldName(QApplication.application.publishedModeThemeId), publishedModeThemeId);
    }
