    ...
    public static Comparator<MemberInfoDTO> workspaceMembersComparator() {
        return new Comparator<>() {
            ...
            public int compare(MemberInfoDTO o1, MemberInfoDTO o2) {
                ...
                boolean workspaceRolePresent1 =
                        o1.getRoles().stream()
                                .anyMatch(
                                        role ->
                                                Workspace.class
                                                        .getSimpleName()
                                                        .equals(role.getEntityType()));
                boolean workspaceRolePresent2 =
                        o1.getRoles().stream()
                                .anyMatch(
                                        role ->
                                                Workspace.class
                ...
                // Order based on the workspace default role (Administrator > Developer > App
                // Viewer)
            ...
            private int getOrderBasedOnWorkspaceRole(MemberInfoDTO member) {
                PermissionGroupInfoDTO role =
                        member.getRoles().stream()
                                .filter(
                                        role1 ->
                                                Workspace.class
                                                        .getSimpleName()
                                                        .equals(role1.getEntityType()))
                                .findFirst()
                                .get();
                if (Objects.nonNull(role.getName())
                        && role.getName().startsWith(FieldName.ADMINISTRATOR)) {
                    return 0;
                } else if (Objects.nonNull(role.getName())
                        && role.getName().startsWith(FieldName.DEVELOPER)) {
                    return 1;
                } else if (Objects.nonNull(role.getName())
                        && role.getName().startsWith(FieldName.VIEWER)) {
                    return 2;
                }
                return 3;
            }
