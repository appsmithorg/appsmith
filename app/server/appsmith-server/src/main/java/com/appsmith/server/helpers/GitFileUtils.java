...
public class GitFileUtils {
    ...
    /**
     * This method will save the complete application in the local repo directory. Path to repo will
     * be :
     ...
     * @param applicationJson application reference object from which entire application can be
     *     rehydrated
    ...
    public Mono<Path> saveApplicationToLocalRepo(
        ...
        */
        Stopwatch stopwatch =
                new Stopwatch(
                        AnalyticsEvents.GIT_SERIALIZE_APP_RESOURCES_TO_LOCAL_FILE.getEventName());
        ...
        try {
            Mono<Path> repoPathMono =
                    fileUtils
                            .saveApplicationToGitRepo(
                                    baseRepoSuffix, applicationReference, branchName)
                            .cache();
            return Mono.zip(repoPathMono, sessionUserService.getCurrentUser())
                    .flatMap(
                            tuple -> {
                                ...
                                // Path to repo will be :
                                // ./container-volumes/git-repo/workspaceId/defaultApplicationId/repoName/
    ...
    /**
     * Method to convert application resources to the structure which can be serialised by
     * appsmith-git module for serialisation
     *
    ...
    public ApplicationGitReference createApplicationReference(ApplicationJson applicationJson) {
        ...
        // No need to commit publish mode theme as it leads to conflict resolution at both the
        // places if any
        applicationJson.setPublishedTheme(null);

        // Pass metadata
        Iterable<String> keys =
                Arrays.stream(applicationJson.getClass().getDeclaredFields())
                        .map(Field::getName)
        ...
        applicationJson.getPageList().stream()
                // As we are expecting the commit will happen only after the application is
                // published, so we can safely
                // assume if the unpublished version is deleted entity should not be committed to
                // git
                .filter(
                        newPage ->
        ...
        // Insert active actions and also assign the keys which later will be used for saving the
        // resource in actual filepath
        ...
        applicationJson.getActionList().stream()
                ...
                .forEach(
                        newAction -> {
                            ...
                            // This is a special case where we are handling REMOTE type plugins
                            // based actions such as Twilio
                            // The user configured values are stored in a attribute called formData
                            // which is a map unlike the body
                            ...
                            // This is a special case where we are handling JS actions as we don't
                            // want to commit the body of JS actions
        ...
        // Insert JSOObjects and also assign the keys which later will be used for saving the
        // resource in actual filepath
        ...
        applicationJson.getActionCollectionList().stream()
                ...
                .filter(
                        collection ->
                                collection.getUnpublishedCollection() != null
                                        && collection.getUnpublishedCollection().getDeletedAt()
                                                == null)
                .forEach(
                        actionCollection -> {
                            String prefix =
                            ...
                            String body =
                                    actionCollection.getUnpublishedCollection().getBody() != null
        ...
        applicationReference.setActionCollectionBody(
                new HashMap<>(resourceMapActionCollectionBody));
    ...
    public Mono<ApplicationJson> reconstructApplicationJsonFromGitRepo(
        ...
        Mono<ApplicationGitReference> appReferenceMono =
                fileUtils.reconstructApplicationReferenceFromGitRepo(
                        ...
                        tuple -> {
                            ...
                            ApplicationJson metadata =
                                    getApplicationResource(
                                            applicationReference.getMetadata(),
                                            ApplicationJson.class);
                            ApplicationJson applicationJson =
                                    getApplicationJsonFromGitReference(applicationReference);
    ...
    /**
     * Once the user connects the existing application to a remote repo, we will initialize the repo
     * with Readme.md - Url to the deployed app(view and edit mode) Link to discord channel for
     * support Link to appsmith documentation for Git related operations Welcome message
     *
     * @param baseRepoSuffix path suffix used to create a branch repo path as per worktree
     *     implementation
     ...
     */
    public Mono<Path> initializeReadme(Path baseRepoSuffix, String viewModeUrl, String editModeUrl)
            throws IOException {
        return fileUtils
                        ...
                        e ->
                                Mono.error(
                                        new AppsmithException(
                                                AppsmithError.GIT_FILE_SYSTEM_ERROR, e)));
    }
    ...
    private void removeUnwantedFieldsFromPage(NewPage page) {
        // As we are publishing the app and then committing to git we expect the published and
        // unpublished PageDTO will
    ...
    private void removeUnwantedFieldsFromApplication(Application application) {
        // Don't commit application name as while importing we are using the repoName as application
        // name
    ...
    private ApplicationJson getApplicationJsonFromGitReference(
            ApplicationGitReference applicationReference) {
        ApplicationJson applicationJson = new ApplicationJson();
        // Extract application data from the json
        Application application =
                getApplicationResource(applicationReference.getApplication(), Application.class);
        applicationJson.setExportedApplication(application);
        applicationJson.setEditModeTheme(
                getApplicationResource(applicationReference.getTheme(), Theme.class));
        // Clone the edit mode theme to published theme as both should be same for git connected
        ...
        if (application != null && !CollectionUtils.isNullOrEmpty(application.getPages())) {
            ...
            application
                    ...
                    .forEach(
                            applicationPage ->
                                    applicationPages.add(
                                            gson.fromJson(
                                                    gson.toJson(applicationPage),
        ...
        // Extract pages
        List<NewPage> pages =
                getApplicationResource(applicationReference.getPages(), NewPage.class);
        // Remove null values
        org.apache.commons.collections.CollectionUtils.filter(
                pages, PredicateUtils.notNullPredicate());
        pages.forEach(
                newPage -> {
                    // As we are publishing the app and then committing to git we expect the
                    // published and unpublished PageDTO
                    // will be same, so we create a deep copy for the published version for page
                    // from the unpublishedPageDTO
                    newPage.setPublishedPage(
                            gson.fromJson(
                                    gson.toJson(newPage.getUnpublishedPage()), PageDTO.class));
                });
        ...
        if (CollectionUtils.isNullOrEmpty(applicationReference.getActions())) {
            ...
            List<NewAction> actions =
                    getApplicationResource(applicationReference.getActions(), NewAction.class);
            // Remove null values if present
            org.apache.commons.collections.CollectionUtils.filter(
                    actions, PredicateUtils.notNullPredicate());
            actions.forEach(
                    newAction -> {
                        // With the file version v4 we have split the actions and metadata
                        // separately into two files
                        ...
                        if (actionBody != null
                            ...
                            // For REMOTE plugin like Twilio the user actions are stored in key
                            // value pairs and hence they need to be
                            // deserialized separately unlike the body which is stored as string in
                            // the db.
                            if (newAction.getPluginType().toString().equals("REMOTE")) {
                                Map<String, Object> formData =
                                        new Gson().fromJson(actionBody.get(keyName), Map.class);
                                newAction
                                        .getUnpublishedAction()
                                        .getActionConfiguration()
                        ...
                        // actionDTO will be same, so we create a deep copy for the published
                        // version for action from
                        // unpublishedActionDTO
                        newAction.setPublishedAction(
                                gson.fromJson(
                                        gson.toJson(newAction.getUnpublishedAction()),
                                        ActionDTO.class));
                    });
        ...
        if (CollectionUtils.isNullOrEmpty(applicationReference.getActionCollections())) {
            ...
            Map<String, String> actionCollectionBody =
                    applicationReference.getActionCollectionBody();
            List<ActionCollection> actionCollections =
                    getApplicationResource(
                            applicationReference.getActionCollections(), ActionCollection.class);
            // Remove null values if present
            org.apache.commons.collections.CollectionUtils.filter(
                    actionCollections, PredicateUtils.notNullPredicate());
            actionCollections.forEach(
                    actionCollection -> {
                        // Set the js object body to the unpublished collection
                        // Since file version v3 we are splitting the js object code and metadata
                        // separately
                        String keyName =
                                actionCollection.getUnpublishedCollection().getName()
                                        + actionCollection.getUnpublishedCollection().getPageId();
                        if (actionCollectionBody != null
                                && actionCollectionBody.containsKey(keyName)) {
                            actionCollection
                                    .getUnpublishedCollection()
                                    .setBody(actionCollectionBody.get(keyName));
                        }
                        // As we are publishing the app and then committing to git we expect the
                        // published and unpublished
                        // actionCollectionDTO will be same, so we create a deep copy for the
                        // published version for
                        // actionCollection from unpublishedActionCollectionDTO
                        actionCollection.setPublishedCollection(
                                gson.fromJson(
                                        gson.toJson(actionCollection.getUnpublishedCollection()),
                                        ActionCollectionDTO.class));
                    });
        ...
        applicationJson.setDatasourceList(
                getApplicationResource(
                        applicationReference.getDatasources(), DatasourceStorage.class));
