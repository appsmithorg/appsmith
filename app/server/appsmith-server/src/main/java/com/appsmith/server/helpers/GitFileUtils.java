...
public ApplicationGitReference createApplicationReference(ApplicationJson applicationJson) {
	...
	// No need to commit publish mode theme as it leads to conflict resolution at both the places if
	// any
	...
	applicationJson.getPageList().stream()
		// As we are expecting the commit will happen only after the application is published, so we
		// can safely
	...
	applicationJson.getActionList().stream()
		...
		.forEach(
			...
			// This is a special case where we are handling REMOTE type plugins based actions such
			// as Twilio
			// The user configured values are stored in a attribute called formData which is a map
			// unlike the body
			...
			// This is a special case where we are handling JS actions as we don't want to commit
			// the body of JS actions
	...
	// Insert JSOObjects and also assign the keys which later will be used for saving the resource
	// in actual filepath
...
private ApplicationJson getApplicationJsonFromGitReference(
	...
	pages.forEach(
		...
		// As we are publishing the app and then committing to git we expect the published and
		// unpublished PageDTO
		// will be same, so we create a deep copy for the published version for page from the
		// unpublishedPageDTO
	...
	actions.forEach(
		newAction -> {
			// With the file version v4 we have split the actions and metadata separately into two
			// files
			...
			// For REMOTE plugin like Twilio the user actions are stored in key value pairs and
			...
			// As we are publishing the app and then committing to git we expect the published and
			// unpublished
			// actionDTO will be same, so we create a deep copy for the published version for action
			// from
	...
	actionCollections.forEach(
		actionCollection -> {
			...
			// As we are publishing the app and then committing to git we expect the published and
			// unpublished
			// actionCollectionDTO will be same, so we create a deep copy for the published version
			// for
