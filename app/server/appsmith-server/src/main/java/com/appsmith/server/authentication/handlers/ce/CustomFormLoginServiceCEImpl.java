...
public Mono<UserDetails> findByUsername(String username) {
	return repository
		...
		.map(
			user -> {
			// As this will be used for form login only, if the password field is null we can
			// assume this login
			// request will fail because of invalid credentials but actual reason might be user
			// has signed up
			// using form and then used OAuth for login which removes the password field from user
			// object for
			// more details refer AuthenticationSuccessHandler
			if (user.getPassword() == null && !LoginSource.FORM.equals(user.getSource())) {
				// We can have a implementation to give which login method user should use but this
				// will
