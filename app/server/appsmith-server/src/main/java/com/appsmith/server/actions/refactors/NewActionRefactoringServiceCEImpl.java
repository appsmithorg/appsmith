...
public class NewActionRefactoringServiceCEImpl implements EntityRefactoringServiceCE<Action> {
    ...
    public Mono<Void> refactorReferencesInExistingEntities(
        ...
        return getActionsByContextId(contextId, contextType)
                 ...
                 */
                .flatMap(tuple -> {
                    final Action newAction = tuple.getT1();
                    ...
                    ActionDTO action = actionService.generateActionByViewMode(newAction, false);
                    ...
                    return this.refactorNameInAction(action, oldName, newName, evalVersion, oldNamePattern)
                            .flatMap(updates -> {
                                ...
                                return actionService
                                        ...
                                        .then(actionService.save(newAction));
                            });
                })
    ...
    protected Flux<Action> getActionsByContextId(String contextId, CreatorContextType contextType) {
        return actionService.findAllActionsByContextIdAndContextTypeAndViewMode(
    ...
    public Mono<Void> updateRefactoredEntity(RefactorEntityNameDTO refactorEntityNameDTO, String branchName) {
        return actionService
                ...
                .map(branchedAction -> actionService.generateActionByViewMode(branchedAction, false))
                .flatMap(action -> {
                    ...
                    return actionService.updateUnpublishedAction(action.getId(), action);
                })
                .then();
    }
    ...
    protected Flux<ActionDTO> getExistingEntities(
        ...
        return actionService.getUnpublishedActionsByPageId(contextId, null).flatMap(actionDTO -> {
            /*
