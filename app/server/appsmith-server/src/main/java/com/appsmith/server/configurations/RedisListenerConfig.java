...
public class RedisListenerConfig {
  ...
  public RedisListenerConfig(
      ObjectMapper objectMapper, PluginService pluginService, ChannelTopic topic) {
  ...
  /**
   * This is the listener that will receive all the messages from the Redis channel topic configured
   * in topic(). Currently the only topic we are listening to is for install plugin requests.
   *
   * @param factory
   * @return
   */
  ...
  public ReactiveRedisMessageListenerContainer container(ReactiveRedisConnectionFactory factory) {
    ...
    container
        // The receive function can subscribe to multiple topics as well. Can also subscribe
        // via regex pattern
        // to multiple channels
        .receive(topic)
        // Extract the message from the incoming object. By default it's String
        // serialization. The receive() fxn
        ...
        .map(
            redisPluginObj ->
                pluginService.redisInstallPlugin((InstallPluginRedisDTO) redisPluginObj))
        // Handle this error because it prevents the Redis connection from shutting down
        // when the server is shut down
        // TODO: Verify if this is invoked in normal redis pubsub execution as well
        .doOnError(
            throwable -> {
              if (!(throwable instanceof CancellationException)) {
                // The Reactive RedisListener doesn't shut down properly. Hence,
                // only printing errors for
