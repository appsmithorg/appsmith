...
// Static metadata to indicate if the plugin is suitable for generating CRUD page from DB table if
// yes then page
...
// Field to distinguish if the plugin is supported in air-gap instance, by default all the plugins
// will be supported.
// One can opt out by adding this field in DB object. Generally SaaS plugins and DB which can't be
// self-hosted can
