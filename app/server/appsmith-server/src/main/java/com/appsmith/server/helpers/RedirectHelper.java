...
public class RedirectHelper {
  ...
  /**
   * If redirectUrl is empty, it'll be set to DEFAULT_REDIRECT_URL. If the redirectUrl does not have
   * the base url, it'll prepend that from header origin.
   *
   * @param redirectUrl
   * @param httpHeaders
   * @return
  ...
  public Mono<String> getRedirectUrl(ServerHttpRequest request) {
    ...
    if (queryParams.getFirst(REDIRECT_URL_QUERY_PARAM) != null) {
      return Mono.just(
          fulfillRedirectUrl(queryParams.getFirst(REDIRECT_URL_QUERY_PARAM), httpHeaders));

    } else {
      if (queryParams.getFirst(FORK_APP_ID_QUERY_PARAM) != null) {
        ...
        return applicationService
            ...
            .map(
                application -> {
                  // Get the default page in the application, or if there's no
                  // default page, get the first page
  ...
  public String getRedirectDomain(HttpHeaders httpHeaders) {
    ...
    if (!StringUtils.isEmpty(httpHeaders.getOrigin())) {
      ...
      if (!StringUtils.isEmpty(httpHeaders.getFirst(Security.REFERER_HEADER))) {
        ...
        if (!StringUtils.isEmpty(httpHeaders.getHost())) {
          // For HTTP v1 requests
          String port =
              httpHeaders.getHost().getPort() != 80 ? ":" + httpHeaders.getHost().getPort() : "";
  ...
  public String buildApplicationUrl(Application application, HttpHeaders httpHeaders) {
    ...
    if (application != null
      ...
      redirectUrl =
          String.format(
              RedirectHelper.APPLICATION_PAGE_URL, application.getId(), applicationPage.getId());
    }
