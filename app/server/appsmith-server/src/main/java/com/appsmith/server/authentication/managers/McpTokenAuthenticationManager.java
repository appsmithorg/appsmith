...
public class McpTokenAuthenticationManager implements ReactiveAuthenticationManager {
    ...
    // Bucket-key component for a syntactically invalid token (no "mcp_<uuid>_" / "mcp_<uuid>." shape). Such tokens
    // can never authenticate, so collapsing them into one shared bucket only throttles obvious garbage and never
    // affects a real token's bucket.
     ...
     * {@code mcp_<mcpKeyId>_<secret>}, the same value returned by list() as {@code id}, safe to key on and never the
     * secret half after the separator) so each key is throttled in its own bucket. The client address is retained as a
     * prefix so the keying stays correct if a future ingress path ever supplies a real client IP.
     */
    ...
    /**
     * Extracts the public mcp key id for use as a bucket key. Never returns the secret portion of the user key.
     * Falls back to {@link #INVALID_TOKEN_ID} for any token that does not match the expected shape.
     */
    private static String tokenIdComponent(String token) {
        String mcpKeyId = McpUserKeyCodec.expandKeyId(token);
        return mcpKeyId == null ? INVALID_TOKEN_ID : mcpKeyId;
    }
