...
public class TenantServiceImpl extends TenantServiceCEImpl implements TenantService {
     ...
     */
    public Mono<Void> addTenantLicenseKey(String licenseKey, ServerWebExchange exchange) {
        return saveTenantLicenseKey(licenseKey)
                .flatMap(tuple -> {
                    ...
                    Mono<Application> returnMono = Mono.just(new Application());
                    if (isActivateInstance) {
                    ...
                    return returnMono
                            .flatMap(application ->  redirectHelper.handleRedirect(webFilterExchange, application, false));
                });
    }
