...
public class ApplicationTemplateServiceCEImpl implements ApplicationTemplateServiceCE {
                ...
                if (clientResponse.statusCode().isError()) {
                  return Flux.error(
                      new AppsmithException(
                          AppsmithError.CLOUD_SERVICES_ERROR, clientResponse.statusCode()));
                } else {
  ...
  private Mono<ApplicationJson> getApplicationJsonFromTemplate(String templateId) {
    ...
    return webClient
        ...
        .switchIfEmpty(
            Mono.error(
                new AppsmithException(AppsmithError.NO_RESOURCE_FOUND, "template", templateId)));
  }
   ...
   * CloudService and/or serialise and de-serialise the application. This process takes time and the
   * client may cancel the request. This leads to the flow getting stopped midway producing
   * corrupted states. We use the synchronous sink to ensure that even though the client may have
   * cancelled the flow, git operations should proceed uninterrupted and whenever the user refreshes
   * the page, we will have the sane state. Synchronous sink does not take subscription
