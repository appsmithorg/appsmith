...
public class ApplicationTemplateServiceCEImpl implements ApplicationTemplateServiceCE {
    ...
    @Override
    public Flux<ApplicationTemplate> getSimilarTemplates(
            String templateId, MultiValueMap<String, String> params) {
        UriComponents uriComponents =
                UriComponentsBuilder.fromUriString(cloudServicesConfig.getBaseUrl())
        ...
        return WebClientUtils.create(apiUrl)
                .get()
                .exchangeToFlux(
                        clientResponse -> {
        ...
        return WebClientUtils.create(baseUrl + "/api/v1/app-templates?" + uriComponents.getQuery())
                ...
                .map(
                        objects -> {
                            ...
                            List<String> recentlyUsedTemplateIds =
                                    userData.getRecentlyUsedTemplateIds();
                            if (!CollectionUtils.isEmpty(recentlyUsedTemplateIds)) {
                                applicationTemplateList.sort(
                                        Comparator.comparingInt(
                                                o -> {
                                                    int index =
                                                            recentlyUsedTemplateIds.indexOf(
                                                                    o.getId());
                                                    if (index < 0) {
                                                        // template not in recent list, return a
                                                        // large value so that it's sorted out to
                                                        // the end
    ...
    private Mono<ApplicationJson> getApplicationJsonFromTemplate(String templateId) {
        ...
        return webClient
                ...
                .switchIfEmpty(
                        Mono.error(
                                new AppsmithException(
                                        AppsmithError.NO_RESOURCE_FOUND, "template", templateId)));
    }

    @Override
    public Mono<ApplicationImportDTO> importApplicationFromTemplate(
            String templateId, String workspaceId) {
                ...
                .flatMap(
                        applicationImportDTO -> {
                            ...
                            return analyticsService
                                    .sendObjectEvent(
                                            AnalyticsEvents.FORK, applicationTemplate, data)
    ...
    /**
     * Merge Template API is slow today because it needs to communicate with ImportExport Service,
     * CloudService and/or serialise and de-serialise the application. This process takes time and
     * the client may cancel the request. This leads to the flow getting stopped midway producing
     * corrupted states. We use the synchronous sink to ensure that even though the client may have
     * cancelled the flow, git operations should proceed uninterrupted and whenever the user
     * refreshes the page, we will have the sane state. Synchronous sink does not take subscription
     * cancellations into account. This means that even if the subscriber has cancelled its
    ...
    public Mono<ApplicationImportDTO> mergeTemplateWithApplication(
        ...
        Mono<ApplicationImportDTO> importedApplicationMono =
                getApplicationJsonFromTemplate(templateId)
                        .flatMap(
                                applicationJson -> {
                                    if (branchName != null) {
