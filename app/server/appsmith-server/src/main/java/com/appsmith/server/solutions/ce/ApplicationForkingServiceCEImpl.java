...
public Mono<ApplicationImportDTO> forkApplicationToWorkspace(
	...
	Mono<ApplicationImportDTO> forkApplicationMono =
		Mono.zip(sourceApplicationMono, targetWorkspaceMono, userMono)
			.flatMap(
				...
				// If the forking application is connected to git, do not copy those data to the
				// new forked application
	...
	// Fork application is currently a slow API because it needs to create application, clone all
	// the pages, and then
	// copy all the actions and collections. This process may take time and the client may cancel
	// the request.
	// This leads to the flow getting stopped midway producing corrupted DB objects. The following
	// ensures that even
	// though the client may have cancelled the flow, the forking of the application should proceed
	// uninterrupted
