...
public class ApplicationForkingServiceCEImpl implements ApplicationForkingServiceCE {
  ...
  public Mono<ApplicationImportDTO> forkApplicationToWorkspace(
    ...
    final Mono<Application> sourceApplicationMono =
        applicationService
    ...
    final Mono<Workspace> targetWorkspaceMono =
        workspaceService
    ...
    Mono<ApplicationImportDTO> forkApplicationMono =
        Mono.zip(sourceApplicationMono, targetWorkspaceMono, userMono)
            .flatMap(
                tuple -> {
                  ...
                  // If the forking application is connected to git, do not copy
                  // those data to the new forked application
                  application.setGitApplicationMetadata(null);

                  boolean allowFork =
                      (
                          // Is this a non-anonymous user that has access
                          // to this application?
                          !user.isAnonymous()
                              && application
                                  .getUserPermissions()
                                  .contains(applicationPermission.getEditPermission().getValue()))
                          || Boolean.TRUE.equals(application.getForkingEnabled());

                  if (!allowFork) {
                    return Mono.error(
                        new AppsmithException(AppsmithError.APPLICATION_FORKING_NOT_ALLOWED));
                  }
                      ...
                      .flatMap(
                          application ->
                              sendForkApplicationAnalyticsEvent(
                                  srcApplicationId, targetWorkspaceId,
    ...
    // Fork application is currently a slow API because it needs to create application, clone
    // all the pages, and then
    // copy all the actions and collections. This process may take time and the client may
    // cancel the request.
    // This leads to the flow getting stopped midway producing corrupted DB objects. The
    // following ensures that even
    // though the client may have cancelled the flow, the forking of the application should
    ...
    return Mono.create(
        sink ->
            forkApplicationMono.subscribe(sink::success, sink::error, null, sink.currentContext()));
  }
      ...
      return applicationService
          ...
          .flatMap(
              application -> {
                ...
                // In such cases we should fork the application from the new default
                // branch
    ...
    return applicationService
        ...
        .flatMap(
            branchedApplicationId ->
                forkApplicationToWorkspace(branchedApplicationId, targetWorkspaceId))
        .map(
