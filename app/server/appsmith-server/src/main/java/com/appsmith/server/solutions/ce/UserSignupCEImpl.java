...
/**
* This function does the sign-up flow of the given user object as a new user, and then logs that
* user. After the login is successful, the authentication success handlers will be called
	...
	Mono<UserSignupDTO> createUserAndSendEmailMono =
		userService
			...
			.map(
				pair -> {
				log.debug(
					"UserSignupCEImpl::Time taken for create user and send email: {} ms",
					pair.getT1());
				return pair.getT2();
				});
			...
			Mono<Integer> authenticationSuccessMono =
				authenticationSuccessHandler
					...
					.flatMap(
						pair -> {
							log.debug(
								"UserSignupCEImpl::Time taken for authentication success: {} ms",
								pair.getT1());
...
public Mono<User> signupAndLoginSuper(
			...
			.flatMap(
				...
				return userMono1
					...
					.map(
						pair -> {
							log.debug(
								"UserSignupCEImpl::Time taken to complete signupAndLogin: {} ms",
								pair.getT1());
							return pair.getT2();
						});
				})
				...
				Mono<UserData> userDataMono =
					userDataService
						...
						.map(
							pair -> {
								log.debug(
									"UserSignupCEImpl::Time taken to update user data for user: {}"
										+ " ms",
									pair.getT1());
								return pair.getT2();
							});
				...
				Mono<EnvChangesResponseDTO> applyEnvManagerChangesMono =
					envManager
						...
						.map(
							pair -> {
								log.debug(
									"UserSignupCEImpl::Time taken to apply env changes: {} ms",
									pair.getT1());
								return pair.getT2();
							});
				...
				Mono<Long> allSecondaryFunctions =
					Mono.when(
						...
						.map(
							pair -> {
								log.debug(
									"UserSignupCEImpl::Time taken to complete all secondary"
										+ " functions: {} ms",
									pair.getT1());
								return pair.getT2();
							});
	...
	return userMono
		...
		.map(
			pair -> {
			log.debug(
				"UserSignupCEImpl::Time taken for the user mono to complete: {} ms",
				pair.getT1());
			return pair.getT2();
			});
}
...
private Mono<Boolean> sendInstallationSetupAnalyticsOnSeparateThreadMono(
	...
	Mono<String> getExternalAddressMono =
		NetworkUtils.getExternalAddress()
			...
			.map(
				pair -> {
				log.debug(
					"UserSignupCEImpl::Time taken to get external address: {} ms", pair.getT1());
				return pair.getT2();
				});
	...
	Mono.zip(getInstanceIdMono, getExternalAddressMono)
		.flatMap(
			...
			analyticsProps.put(
				DISABLE_TELEMETRY, !userFromRequest.isAllowCollectingAnonymousData());
			...
			return analyticsService
				...
				.map(
					pair -> {
						log.debug(
							"UserSignupCEImpl::Time taken to send installation setup complete"
								+ " analytics event: {} ms",
							pair.getT1());
						return pair.getT2();
					});
...
private Mono<User> sendCreateSuperUserEventOnSeparateThreadMono(User user) {
	analyticsService
		...
		.map(
			pair -> {
			log.debug(
				"UserSignupCEImpl::Time taken to send create super user event: {} ms",
				pair.getT1());
			return pair.getT2();
			})
		.subscribeOn(Schedulers.boundedElastic())
