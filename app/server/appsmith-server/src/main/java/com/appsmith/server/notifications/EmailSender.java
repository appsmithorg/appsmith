...
public Mono<Boolean> sendMail(
	 ...
	 * immediately. We do not wait for the blocking call of `sendMailSync` to finish. BoundedElastic
	 * scheduler would ensure that when the number of tasks go beyond the number of available
	 * threads, the tasks would be deferred till a thread becomes available without overloading the
	 * server.
	 */
	Mono.fromCallable(
			() -> {
...
private void sendMailSync(String to, String subject, String text, String replyTo) {
	...
	// Check if the email address is valid. It's possible for certain OAuth2 providers to not return
	// the email ID
