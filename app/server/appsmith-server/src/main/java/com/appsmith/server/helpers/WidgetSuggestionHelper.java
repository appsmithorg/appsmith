    ...
    if (node.isObject()) {
      ...
      if (JsonNodeType.STRING.equals(nodeType)) {
        ...
        if (JsonNodeType.ARRAY.equals(nodeType)) {
          widgetTypeList =
              getWidgetsForTypeArray(dataFields.getFields(), dataFields.getNumericFields());
        } else {
      ...
      if (node.isArray() || node.isNumber() || node.isTextual()) {
        DataFields dataFields = collectFieldsFromData(node.fields());
  ...
  private static List<WidgetSuggestionDTO> getWidgetsForTypeNestedObject(
    ...
    if (!fields.isEmpty()) {
      if (fields.size() < 2) {
        widgetTypeList.add(
            getWidgetNestedData(
                WidgetType.SELECT_WIDGET, nestedFieldName, fields.get(0), fields.get(0)));
      } else {
        widgetTypeList.add(
            getWidgetNestedData(
                WidgetType.SELECT_WIDGET, nestedFieldName, fields.get(0), fields.get(1)));
      }
      if (!numericFields.isEmpty()) {
        widgetTypeList.add(
            getWidgetNestedData(
