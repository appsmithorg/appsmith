...
public abstract class BaseController<S extends CrudService<T, ID>, T extends BaseDomain, ID> {
  ...
  /**
   * TODO : Remove this function completely if this is not being used. If not, atleast remove it for
   * : 1. Page 2. Datasources
   *
   * @param params
   * @return
   */
  ...
  public Mono<ResponseDTO<T>> getByIdAndBranchName(
    ...
    return service
        .findByIdAndBranchName(id, branchName)
        .map(resources -> new ResponseDTO<>(HttpStatus.OK.value(), resources, null));
  }
  ...
  public Mono<ResponseDTO<T>> delete(
    ...
    return service
        .archiveByIdAndBranchName(id, branchName)
