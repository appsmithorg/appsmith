...
public abstract class BaseController<S extends CrudService<T, ID>, T extends BaseDomain, ID> {
    ...
    public Mono<ResponseDTO<T>> create(
        ...
        log.debug(
                "Going to create resource from base controller {}", resource.getClass().getName());
        return service.create(resource)
                .map(created -> new ResponseDTO<>(HttpStatus.CREATED.value(), created, null));
    }

    /**
     * TODO : Remove this function completely if this is not being used. If not, atleast remove it
     * for : 1. Page 2. Datasources
     *
     * @param params
     * @return
     */
    ...
    public Mono<ResponseDTO<List<T>>> getAll(
        ...
        if (!StringUtils.isEmpty(branchName)) {
            modifiableParams.add(
                    FieldName.DEFAULT_RESOURCES + "." + FieldName.BRANCH_NAME, branchName);
        }
