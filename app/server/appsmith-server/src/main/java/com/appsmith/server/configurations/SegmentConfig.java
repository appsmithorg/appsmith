...
public Analytics analyticsRunner() {
	...
	// We don't have the Segment Key, returning `null` here will disable analytics calls from
	// AnalyticsService.
	return null;
	}
	...
	// We use a different analytics instance for sending events about the analytics system itself so
	// we don't end up
			...
			TrackMessage.builder("segment_error")
				...
				.properties(
					Map.of(
						...
						"args",
							ObjectUtils.defaultIfNull(logData.getArgs(), Collections.emptyList()),
