...
public class MigrationHelperMethods {
    ...
    // Method to embed application pages in imported application object as per modified
    // serialization format where we
                            ...
                            .filter(
                                    newPage ->
                                            newPage.getUnpublishedPage() != null
                                                    && newPage.getUnpublishedPage().getDeletedAt()
                                                            == null)
    ...
    // Method to embed mongo escaped widgets in imported layouts as per modified serialization
    // format where we are
                        ...
                        newPage -> {
                                        ...
                                        .forEach(
                                                layout -> {
                                                    layout.setMongoEscapedWidgetNames(
                                                            unpublishedMongoEscapedWidget.get(
                                                                    layout.getId()));
                                                });
                            }
                            ...
                            if (newPage.getPublishedPage() != null
                                    && publishedMongoEscapedWidget.containsKey(
                                            newPage.getPublishedPage().getName())
                                    && !CollectionUtils.isNullOrEmpty(
                                            newPage.getPublishedPage().getLayouts())) {
                                ...
                                newPage.getPublishedPage()
                                        ...
                                        .forEach(
                                                layout -> {
                                                    layout.setMongoEscapedWidgetNames(
                                                            publishedMongoEscapedWidget.get(
                                                                    layout.getId()));
                                                });
                            }
                        });
    ...
    // Method to embed userSetOnLoad in imported actions as per modified serialization format where
    // we are serialising
    ...
    public static void evictPermissionCacheForUsers(
        ...
        userIds.forEach(
                userId -> {
                    ...
                    if (user != null) {
                        // blocking call for cache eviction to ensure its subscribed immediately
