...
public abstract class BaseAppsmithRepositoryCEImpl<T extends BaseDomain> {
  ...
  public BaseAppsmithRepositoryCEImpl(
    ...
    this.genericDomain =
        (Class<T>)
            GenericTypeResolver.resolveTypeArgument(getClass(), BaseAppsmithRepositoryCEImpl.class);
  }
        ...
        .map(
            userPermissionGroupIds -> {
              ...
              return userPermissionGroupIds.stream()
                  .filter(
                      userPermissionGroupId -> permissionGroupsIds.contains(userPermissionGroupId))
                  .findFirst()
    ...
    return getCurrentUserPermissionGroupsIfRequired(permission)
        .flatMap(
            permissionGroups -> {
              query.addCriteria(
                  new Criteria()
                      .andOperator(notDeleted(), userAcl(permissionGroups, permission.get())));
              return mongoOperations.updateFirst(query, updateObj, this.genericDomain);
            });
  }
          ...
          return mongoOperations
              ...
              .matching(
                  createQueryWithPermission(
                      criterias, projectionFieldNames, permissionGroups, permission))
              .one()
              .flatMap(obj -> setUserPermissionsInObject(obj, permissionGroups));
        });
  }
  ...
  protected Query createQueryWithPermission(
    ...
    if (aclPermission == null) {
      ...
      query.addCriteria(
          new Criteria().andOperator(notDeleted(), userAcl(permissionGroups, aclPermission)));
    }
    ...
    return permissionGroupsMono.flatMapMany(
        permissionGroups ->
            queryAllWithPermissionGroups(
                criterias, includeFields, permission, sort, permissionGroups, limit));
  }
  ...
  /**
   * 1. Get all the user groups associated with the user 2. Get all the permission groups associated
