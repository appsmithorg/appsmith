 ...
 * BaseRepositoryImpl.java and BaseAppsmithRepositoryCEImpl.java, Arpit's comments on this might be
 * helpful: ``` BaseRepository is required for running any JPA queries. This doesn’t invoke any ACL
 * permissions. This is used when we wish to fetch data from the DB without ACL. For eg, Fetching a
 * user by username during login Usage example: ActionCollectionRepositoryCE extends BaseRepository
 * to power JPA queries using the ReactiveMongoRepository. AppsmithRepository is the one that we
 * should use by default (unless the use case demands that we don’t need ACL). It is implemented by
 * BaseAppsmithRepositoryCEImpl and BaseAppsmithRepositoryImpl. This interface allows us to define
 * custom Mongo queries by including the delete functionality & ACL permissions. Usage example:
 * CustomActionCollectionRepositoryCE extends AppsmithRepository and then implements the functions
 * defined there. I agree that the naming is a little confusing. Open to hearing better naming
 * suggestions so that we can improve the understanding of these interfaces. ``` Ref:
...
public abstract class BaseAppsmithRepositoryCEImpl<T extends BaseDomain> {
        ...
        this.genericDomain =
                (Class<T>)
                        GenericTypeResolver.resolveTypeArgument(
                                getClass(), BaseAppsmithRepositoryCEImpl.class);
    }

    public Mono<Boolean> isPermissionPresentForUser(
        ...
        return mongoOperations
                ...
                .map(
                        userPermissionGroupIds -> {
                            Optional<Policy> interestingPolicyOptional =
                                    policies.stream()
                                            .filter(
                                                    policy ->
                            ...
                            Set<String> permissionGroupsIds =
                                    interestingPolicy.getPermissionGroups();
    ...
    public static final Optional<Criteria> userAcl(
            Set<String> permissionGroups, Optional<AclPermission> permission) {
        if (permission.isEmpty()) {
            return Optional.empty();
        }
        // Check if the permission is being provided by any of the permission groups
        Criteria permissionGroupCriteria =
                Criteria.where(fieldName(QBaseDomain.baseDomain.policies))
                        .elemMatch(
    ...
    public Mono<T> findById(
            String id, List<String> projectionFieldNames, AclPermission permission) {
        if (id == null) {
    ...
    public Mono<T> findById(
            String id, List<String> projectionFieldNames, Optional<AclPermission> permission) {
        if (id == null) {
            return Mono.error(new AppsmithException(AppsmithError.INVALID_PARAMETER, FieldName.ID));
        }
                ...
                .flatMap(
                        user -> {
                            ...
                            return (permission.isPresent()
                                    ...
                                    .flatMap(
                                            permissionGroups -> {
                                                Optional<Criteria> userAcl =
                                                        userAcl(permissionGroups, permission);
                                                query.addCriteria(notDeleted());
                                                if (userAcl.isPresent()) {
                                                    query.addCriteria(userAcl.get());
                                                }
                                                return mongoOperations
                                                        ...
                                                        .flatMap(
                                                                obj -> {
                                                                    return setUserPermissionsInObject(
                                                                            obj, permissionGroups);
                                                                });
                                            });
                        });
    ...
    public Mono<UpdateResult> updateById(
            String id, Update updateObj, Optional<AclPermission> permission) {
        Query query = new Query(Criteria.where("id").is(id));
                        ...
                        permissionGroups -> {
                            query.addCriteria(
                                    new Criteria()
                                            .andOperator(
                                                    notDeleted(),
                                                    userAcl(permissionGroups, permission.get())));
                            return mongoOperations.updateFirst(
                                    query, updateObj, this.genericDomain);
                        });
    }
    ...
    public Mono<UpdateResult> updateByCriteria(List<Criteria> criteriaList, Update updateObj) {
        if (criteriaList == null) {
            return Mono.error(
                    new AppsmithException(AppsmithError.INVALID_PARAMETER, "criteriaList"));
        }
        if (updateObj == null) {
    ...
    protected Mono<T> queryOne(
            List<Criteria> criterias, List<String> projectionFieldNames, AclPermission permission) {
        Mono<Set<String>> permissionGroupsMono =
                getCurrentUserPermissionGroupsIfRequired(Optional.ofNullable(permission));

        return permissionGroupsMono.flatMap(
                permissionGroups -> {
                    return mongoOperations
                            .query(this.genericDomain)
                            .matching(
    ...
    protected Mono<T> queryOne(
            ...
            Optional<AclPermission> permission) {
        Mono<Set<String>> permissionGroupsMono =
                getCurrentUserPermissionGroupsIfRequired(permission);
        ...
        return permissionGroupsMono.flatMap(
                permissionGroups -> {
                    return mongoOperations
                            ...
                            .matching(
                                    createQueryWithPermission(
                                            criterias, null, permissionGroups, permission))
                            .first()
                            .flatMap(obj -> setUserPermissionsInObject(obj, permissionGroups));
                });
    }
    ...
    protected Query createQueryWithPermission(
            ...
            Optional<AclPermission> permission) {
        return createQueryWithPermission(
                criterias, null, permissionGroups, permission.orElse(null));
    }
    ...
    protected Query createQueryWithPermission(
        ...
        return createQueryWithPermission(
                criterias, projectionFieldNames, permissionGroups, permission.orElse(null));
    }
        ...
        return permissionGroupsMono.flatMap(
                permissionGroups ->
                        mongoOperations.count(
                                createQueryWithPermission(criterias, permissionGroups, permission),
                                this.genericDomain));
    }
    ...
    public Flux<T> queryAll(
            List<Criteria> criterias, Optional<AclPermission> permission, Optional<Sort> sort) {
    ...
    public Flux<T> queryAll(
        ...
        Mono<Set<String>> permissionGroupsMono =
                getCurrentUserPermissionGroupsIfRequired(permission);
        return permissionGroupsMono.flatMapMany(
                permissionGroups ->
                        queryAllWithPermissionGroups(
                                criterias,
                                includeFields,
                                permission,
                                sort,
    ...
    /**
     * 1. Get all the user groups associated with the user 2. Get all the permission groups
     * associated with anonymous user 3. Return the set of all the permission groups.
     *
     * @param user
     * @return
     */
    protected Mono<Set<String>> getAllPermissionGroupsForUser(User user) {
        ...
        return userMono.flatMap(
                        userWithTenant ->
                                Mono.zip(
                                        cacheableRepositoryHelper.getPermissionGroupsOfUser(
                                                userWithTenant),
        ...
        return ReactiveSecurityContextHolder.getContext()
                ...
                .flatMap(
                        principal -> {
                            ...
                            return mongoOperations.query(this.genericDomain).matching(query).one();
                        });
    }
