...
public class RedisUtils {
    ...
    public Mono<Boolean> addFileLock(String key) {
        return this.addFileLock(key, FILE_LOCK_TIME_LIMIT, AppsmithError.GIT_FILE_IN_USE);
    }

    public Mono<Boolean> addFileLock(String key, Duration expirationPeriod, AppsmithError error) {
        return redisOperations.hasKey(key).flatMap(isKeyPresent -> {
            if (Boolean.TRUE.equals(isKeyPresent)) {
                return Mono.error(new AppsmithException(error));
            }
