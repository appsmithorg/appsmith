...
@Slf4j
public class GroupServiceCEImpl extends BaseService<GroupRepository, Group, String>
        implements GroupServiceCE {
        ...
        if (params != null && !params.isEmpty()) {
            params.entrySet().stream()
                    .forEach(
                            entry ->
                                    query.addCriteria(
                                            Criteria.where(entry.getKey()).in(entry.getValue())));
        }
    ...
    /**
     * This function fetches the default groups belonging to the workspace {@link
     * AclConstants#DEFAULT_ORG_ID} and then copies them over to the current workspace. This is to
     * ensure that each workspace has groups & permissions specific to them.
     *
    ...
    public Flux<Group> createDefaultGroupsForWorkspace(String workspaceId) {
        ...
        return this.repository
                .getAllByWorkspaceId(AclConstants.DEFAULT_ORG_ID)
                .flatMap(
                        group -> {
                            ...
                            log.debug(
                                    "Creating group {} for org: {}", group.getName(), workspaceId);
