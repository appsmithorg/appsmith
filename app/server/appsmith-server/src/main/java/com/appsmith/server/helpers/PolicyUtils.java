...
public <T extends BaseDomain> T addPoliciesToExistingObject(
	...
	if (policyMap1.containsKey(permission)) {
		...
		// Remove this permission from the policyMap as this has been accounted for in the above
		// code
...
public Flux<Datasource> updateWithNewPoliciesToDatasourcesByWorkspaceId(
	...
	return datasourceRepository
		// fetch datasources with execute permissions so that app viewers can invite other app
		// viewers
		.findAllByWorkspaceId(workspaceId, datasourcePermission.getExecutePermission())
		// In case we have come across a datasource for this workspace that the current user is not
		// allowed to manage, move on.
...
public Flux<Datasource> updateWithNewPoliciesToDatasourcesByDatasourceIds(
	...
	return datasourceRepository
		...
		// In case we have come across a datasource the current user is not allowed to manage, move
		// on.
		.switchIfEmpty(Mono.empty())
		.flatMap(
			datasource -> {
...
public Flux<Application> updateWithNewPoliciesToApplicationsByWorkspaceId(
	...
	return applicationRepository
		...
		// In case we have come across an application for this workspace that the current user is
		// not allowed to manage, move on.
		.switchIfEmpty(Mono.empty())
		.map(
			application -> {
			if (addPolicyToObject) {
...
public Flux<NewPage> updateWithApplicationPermissionsToAllItsPages(
	...
	// AND the unpublished pages are updated with the new policy change [This covers the edge cases
	// where a page may exist
	// in published app but has been deleted in the edit mode]. This means that we don't have to do
