...
public class PolicyUtils {
        ...
        .map(
            perm -> {
              ...
              Policy policyWithCurrentPermission =
                  Policy.builder().permission(perm.getValue()).users(Set.of(username)).build();
              // Generate any and all lateral policies that might come with the
              // current permission
              Set<Policy> policiesForUser =
                  policyGenerator.getLateralPolicies(perm, Set.of(username), null);
  ...
  public Map<String, Policy> generatePolicyFromPermissionGroupForObject(
    ...
    return permissions.stream()
        ...
        .collect(
            Collectors.toMap(
                Policy::getPermission, Function.identity(), (policy1, policy2) -> policy1));
  }
    ...
    return permissions.stream()
        .map(
            perm -> {
              ...
              // role
              Policy policyWithCurrentPermission =
                  Policy.builder().permission(perm.getValue()).users(usernames).build();
              // Generate any and all lateral policies that might come with the
              // current permission
              Set<Policy> policiesForUser =
        ...
        .flatMap(
            datasource -> {
              ...
              if (addPolicyToObject) {
                ...
                updatedDatasource =
                    removePoliciesFromExistingObject(datasourcePolicyMap, datasource);
              }
   ...
   * application. 2. If the action is associated with different pages (in published/unpublished page
   * due to movement of action), fetching actions by applicationId ensures that we update ALL the
   * actions and don't have to do special handling for the same.
   *
   * @param applicationId
