...
public class OrganizationServiceCEImpl extends BaseService<OrganizationRepository, Organization, String>
    ...
    /**
     * Whether this instance hosts more than one organization. Single-org instances (CE, and EE without the multi-org
     * licence) treat the MCP toggle as an instance-wide setting; multi-org EE treats it per organization.
     */
    ...
    /**
     * {@code APPSMITH_MCP_INTERNAL_SECRET} is normally generated once by the container entrypoint at first boot, so
     * that the backend and the MCP service both read the same value from the env file when they start and enabling
     * MCP needs no restart. This is the fallback for a deployment whose env file predates that (or does not go through
     * the entrypoint): on an explicit {@code mcpConfig.enabled = true} toggle, generate and persist a value when none is
     * configured. The value is always generated by {@link McpTokenUtils}; the client never supplies it.
     *
     * <p>Disabling MCP never clears the secret. The toggle is enforced per request from the organization
     * configuration, so an mcp_ token is rejected while MCP is off regardless of the marker, and keeping the value
     * means re-enabling later (or on another replica) works without a restart.
     */
    private Mono<Void> syncMcpInternalSecret(boolean isMultiOrg, McpConfig incoming) {
        if (incoming == null || !McpOrganizationConfigurationHelper.isExplicitEnable(incoming)) {
            return Mono.empty();
        }
    ...
    private Mono<Void> addMcpInternalSecretIfUnset() {
        ...
        // The already-running MCP service started without this value and only reads it at its own start, so an
        // instance that reaches this fallback still needs an MCP restart before mcp_ tokens authenticate.
        log.warn("APPSMITH_MCP_INTERNAL_SECRET was not set at startup; generating one now. The MCP service must be "
                + "restarted to pick it up (on the container image the entrypoint generates it at boot, so this "
                + "normally means docker.env is not managed by the entrypoint)");
