...
public class AppSmithErrorWebExceptionHandler extends DefaultErrorWebExceptionHandler {
  ...
  public static final String DESERIALIZATION_ERROR_MESSAGE =
      "Failed to deserialize payload. Is the byte array a result of corresponding"
          + " serialization for DefaultDeserializer";
  ...
  private Mono<ServerResponse> render(ServerRequest request) {
    ...
    if (errorCode == 500
      ...
      // If the error is regarding a deserialization error in the session data, then the user
      // is essentially locked out.
      // They have to use a different browser, or Incognito, or clear their cookies to get
      // back in. So, we'll delete
      // the SESSION cookie here, so that the user gets sent back to the Login page, and they
      // can unblock themselves.
      responseBuilder =
          responseBuilder.cookie(
              ResponseCookie.from("SESSION", "").httpOnly(true).path("/").maxAge(0).build());
    }
    ...
    return responseBuilder.body(
        BodyInserters.fromValue(
            new ResponseDTO<>(
                errorCode,
                new ErrorDTO(String.valueOf(errorCode), String.valueOf(error.get("error"))))));
  }
}
