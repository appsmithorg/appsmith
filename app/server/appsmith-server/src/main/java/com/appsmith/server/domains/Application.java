...
* possible to write a migration. User need to update the application manually. In such cases, we
* can use this field to determine whether we need to notify user about that breaking change so
* that they can update their application. Once updated, we should set applicationVersion to
...
/**
* Changing name, change in pages, widgets and datasources will set lastEditedAt. Other activities
* e.g. changing policy will not change this property. We're adding JsonIgnore here because it'll
* be exposed as modifiedAt to keep it backward compatible
*/
@JsonView(Views.Internal.class)
...
/**
* Earlier this was returning value of the updatedAt property in the base domain. As this property
* is modified by the framework when there is any change in domain, a new property lastEditedAt
* has been added to track the edit actions from users. This method exposes that property.
*
...
public static class AppLayout implements Serializable {
	 ...
	 *     migration. After the migration has been run, it may be removed (along with the migration
	 *     or there'll be compile errors there).
	 */
	@JsonView(Views.Internal.class)
	@Deprecated(forRemoval = true)
	Integer width = null;
