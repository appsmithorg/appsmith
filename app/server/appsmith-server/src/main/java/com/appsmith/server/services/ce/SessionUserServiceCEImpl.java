...
public class SessionUserServiceCEImpl implements SessionUserServiceCE {
                ...
                .flatMap(
                        tuple -> {
                            ...
                            if (currentToken instanceof UsernamePasswordAuthenticationToken) {
                                ...
                                log.error(
                                        "Unrecognized session token type when updating user in"
                                                + " session: {}.",
                                        currentToken.getClass());
                                return Mono.error(
                                        new AppsmithException(
                                                AppsmithError.FAIL_UPDATE_USER_IN_SESSION));
                            }
    ...
    public Mono<List<String>> getSessionKeysByUserEmail(String email) {
        // This pattern string comes from calling
        // `ReactiveRedisSessionRepository.getSessionKey("*")` private method.
        return redisOperations
                ...
                .flatMap(
                        key ->
                                Mono.zip(
                                        Mono.just(key),
                                        // The values are maps, containing various pieces of session
                                        // related information.
                                        // One of them, holds the serialized User object. We want
                                        // just that.
                                        redisOperations
                                                .opsForHash()
                                                .entries(key)
