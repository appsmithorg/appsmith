...
public class CollectionServiceCEImpl extends BaseService<CollectionRepository, Collection, String>
    ...
    public Mono<ActionDTO> addSingleActionToCollection(String collectionId, ActionDTO action) {
        ...
        if (action.getId() == null) {
            return Mono.error(
                    new AppsmithException(AppsmithError.INVALID_PARAMETER, FieldName.ACTION));
        }
                ...
                .switchIfEmpty(
                        Mono.error(
                                new AppsmithException(
                                        AppsmithError.INVALID_PARAMETER, FieldName.COLLECTION_ID)))
                .flatMap(
                        collection1 -> {
                            ...
                            /**
                             * TODO Use MonoTemplate instead of Mongo repository to do this for
                             * better performance. Refer to the following link for more details :
    ...
    @Override
    public Mono<NewAction> removeSingleActionFromCollection(
            String collectionId, Mono<NewAction> actionMono) {
        if (collectionId == null) {
            return Mono.error(new AppsmithException(AppsmithError.INVALID_PARAMETER, FieldName.ID));
        }
                ...
                .flatMap(
                        tuple -> {
                            ...
                            if (action.getId() == null) {
                                return Mono.error(
                                        new AppsmithException(
                                                AppsmithError.INVALID_PARAMETER, FieldName.ACTION));
                            }
