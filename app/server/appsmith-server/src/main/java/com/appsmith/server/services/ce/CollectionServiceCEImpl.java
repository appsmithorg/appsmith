...
public Mono<ActionDTO> addSingleActionToCollection(String collectionId, ActionDTO action) {
	...
	return repository
		...
		.switchIfEmpty(
			Mono.error(
				new AppsmithException(AppsmithError.INVALID_PARAMETER, FieldName.COLLECTION_ID)))
		.flatMap(
			collection1 -> {
			...
			/**
			* TODO Use MonoTemplate instead of Mongo repository to do this for better
			* performance. Refer to the following link for more details :
