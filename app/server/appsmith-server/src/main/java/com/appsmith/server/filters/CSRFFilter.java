...
public class CSRFFilter implements WebFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
        ...
        if (!isGetOrHead && !EXEMPT.contains(request.getPath().value())) {
            // For POST requests, either a `X-Requested-By: Appsmith` header or a `Content-Type:
            // application/json`
            // is required. If neither is present, reject the request. This is to prevent CSRF
            // attacks.
            if (MediaType.APPLICATION_JSON.equals(request.getHeaders().getContentType())
                    || X_REQUESTED_BY_VALUE.equals(
                            request.getHeaders().getFirst(X_REQUESTED_BY_NAME))) {
                return chain.filter(exchange);
            }

            log.error(
                    "CSRF header requirements not satisfied to {}. Rejecting request.",
                    request.getPath());
            return Mono.error(
