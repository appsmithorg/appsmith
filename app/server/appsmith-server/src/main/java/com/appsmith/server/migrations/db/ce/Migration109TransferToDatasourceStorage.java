...
/**
 * In this migration, we are moving configurations from each valid datasource to a new collection.
 * The datasource will cease to have configurations after this point
 */
@Slf4j
@ChangeUnit(order = "109", id = "migrate-configurations-to-data-storage", author = " ")
// Although this migration is common to CE and BE, the ordering of this change-unit needs to be
// specifically after 107-ee,
// as this migration has EE overrides for default environmentId, and for getting the environments on
// existing workspaces, 107-ee needs to run
public class Migration109TransferToDatasourceStorage {
    ...
    private final DatasourceStorageMigrationSolution solution =
    ...
    public void executeMigration() {
        ...
        final Query performanceOptimizedUpdateQuery =
        ...
        mongoTemplate.stream(performanceOptimizedUpdateQuery, Datasource.class)
                .forEach(
                        datasource -> {
                            // For each of these datasource, we want to build a new datasource
                            // storage

                            // Fetch the correct environment id to use with this datasource storage
                            String environmentId =
                                    solution.getEnvironmentIdForDatasource(
                                            environmentsMap, datasource.getWorkspaceId());

                            // If none exists, this is an error scenario, log the error and skip the
                            // datasource
                            if (environmentId == null) {
                                log.error(
                                        "Could not find default environment id for workspace id:"
                                                + " {}, skipping datasource id: {}",
                            ...
                            log.debug(
                                    "Creating datasource storage for datasource id: {} with"
                                            + " environment id: {}",
                                    datasource.getId(),
                                    environmentId);
                            ...
                            try {
                                ...
                                log.warn(
                                        "Looks like the datasource storage already exists for"
                                                + " datasource id: {}",
                                        datasource.getId());
                                log.warn("We will attempt to reset the datasource again.");
                            }

                            // Once the datasource storage exists, delete the older config inside
                            // datasource
                            // And set `hasDatasourceStorage` value to true
                            mongoTemplate.updateFirst(
                                    new Query()
                                            .addCriteria(
                                                    where(fieldName(QDatasource.datasource.id))
                                                            .is(datasource.getId())),
                                    new Update()
                                            .set(migrationFlag, true)
                                            .unset(
                                                    fieldName(
                                                            QDatasource.datasource
    ...
    private Criteria findDatasourceIdsToUpdate() {
        return new Criteria()
                .andOperator(
                        ...
                        // these checks are placed because we are getting values which are empty and
                        // while decrypting
                        // those values, we are getting ArrayOutOfBoundException because password is
                        // set to ""
                        where(fieldName(QDatasource.datasource.workspaceId)).exists(true),
                        where(fieldName(QDatasource.datasource.workspaceId)).ne(null),
                        where(
                                        datasourceConfigurationFieldName
                                                + delimiter
