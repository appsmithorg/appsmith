...
public class DatasourceServiceCEImpl extends BaseService<DatasourceRepository, Datasource, String> implements DatasourceServiceCE {
    ...
    public Mono<Datasource> archiveById(String id) {
        return repository
                ...
                .flatMap(toDelete -> {
                    return Mono.zip(datasourceContextService.deleteDatasourceContext(datasourceContextService.createDsContextIdentifier(toDelete)),
                                    repository.archive(toDelete),
                                    datasourceConfigurationStorageService.archiveByDatasourceId(toDelete.getId()).collectList())
                            .thenReturn(toDelete);
                })
                .flatMap(datasource -> {
