...
public class DatasourceServiceCEImpl extends BaseService<DatasourceRepository, Datasource, String>
    ...
    if (!StringUtils.hasLength(datasource.getName())) {
      datasourceMono =
          sequenceService
              ...
              .zipWith(
                  datasourceMono,
                  (sequenceNumber, datasource1) -> {
                    datasource1.setName(Datasource.DEFAULT_NAME_PREFIX + sequenceNumber);
                    return datasource1;
                  });
    }

    Mono<Datasource> datasourceWithPoliciesMono =
        datasourceMono.flatMap(
            datasource1 -> {
        ...
        .flatMap(
            savedDatasource ->
                analyticsService.sendCreateEvent(
                    savedDatasource, getAnalyticsProperties(savedDatasource)))
              ...
              workspace -> {
                Set<Policy> documentPolicies =
                    policyGenerator.getAllChildPolicies(
                        workspace.getPolicies(), Workspace.class, Datasource.class);
  ...
  public Mono<Datasource> populateHintMessages(Datasource datasource) {
    ...
    return pluginExecutorMono
        .flatMap(
            pluginExecutor ->
                pluginExecutor.getHintMessages(null, datasource.getDatasourceConfiguration()))
        .flatMap(
            tuple -> {
  ...
  @Override
  public Mono<Datasource> getValidDatasourceFromActionMono(
      ActionDTO actionDTO, AclPermission aclPermission) {
  ...
  public Mono<Datasource> update(String id, Datasource datasource) {
    ...
    // adding a default false value here, the value is true only when the user calls the update
    // event from datasource controller, else it's false.
            ...
            savedDatasource -> {
              ...
              if (isUserRefreshedUpdate.equals(Boolean.TRUE)) {
                analyticsProperties.put(
                    FieldName.IS_DATASOURCE_UPDATE_USER_INVOKED_KEY, Boolean.TRUE);
              } else {
                analyticsProperties.put(
                    FieldName.IS_DATASOURCE_UPDATE_USER_INVOKED_KEY, Boolean.FALSE);
              }
        ...
        workspaceService
            ...
            .switchIfEmpty(
                Mono.defer(
                    () -> {
                      invalids.add(
                          AppsmithError.PLUGIN_NOT_INSTALLED.getMessage(datasource.getPluginId()));
        ...
        .flatMap(
            pluginExecutor -> {
              ...
              if (datasourceConfiguration != null
                  && !pluginExecutor.isDatasourceValid(datasourceConfiguration)) {
   ...
   * inside the datasource object. We only fetch the password from the db if its a saved datasource
   * before testing.
   */
  @Override
  public Mono<DatasourceTestResult> testDatasource(Datasource datasource, String environmentName) {
    ...
    // This scenario would happen whenever an existing datasource is being tested and no changes
    // are present in the
    // encrypted field (because encrypted fields are not sent over the network after encryption
    // back to the client
    if (datasource.getId() != null
        && datasource.getDatasourceConfiguration() != null
        && datasource.getDatasourceConfiguration().getAuthentication() != null) {
      datasourceMono =
          getById(datasource.getId())
              .map(
                  datasource1 -> {
    ...
    return analyticsService
        .sendObjectEvent(
            AnalyticsEvents.DS_TEST_EVENT, datasource, getAnalyticsProperties(datasource))
        .then(verifyDatasourceAndTest(datasourceMono));
  }
            ...
            datasource1 -> {
              ...
              if (CollectionUtils.isEmpty(datasource1.getInvalids())) {
                ...
                datasourceTestResultMono =
                    Mono.just(new DatasourceTestResult(datasource1.getInvalids()));
              }
                          ...
                          return analyticsService
                              .sendObjectEvent(
                                  ...
                                  getAnalyticsPropertiesForTestEventStatus(
                                      datasource1, datasourceTestResult))
  ...
  protected Mono<DatasourceTestResult> testDatasourceViaPlugin(Datasource datasource) {
    ...
    return pluginExecutorMono.flatMap(
        pluginExecutor -> pluginExecutor.testDatasource(datasource.getDatasourceConfiguration()));
  }

  @Override
  public Mono<Datasource> findByNameAndWorkspaceId(
      String name, String workspaceId, AclPermission permission) {
  ...
  public Flux<Datasource> get(MultiValueMap<String, String> params) {
    ...
    if (params.getFirst(fieldName(QDatasource.datasource.workspaceId)) != null) {
      return findAllByWorkspaceId(
              params.getFirst(fieldName(QDatasource.datasource.workspaceId)),
              datasourcePermission.getReadPermission())
          .collectList()
          .map(
              datasourceList -> {
  ...
  @Override
  public Flux<Datasource> findAllByWorkspaceId(
  ...
  public Mono<Datasource> archiveById(String id) {
    return repository
        ...
        .switchIfEmpty(
            Mono.error(
                new AppsmithException(AppsmithError.NO_RESOURCE_FOUND, FieldName.DATASOURCE, id)))
        ...
        .flatMap(
            objects -> {
              ...
              if (actionsCount > 0) {
                return Mono.error(
                    new AppsmithException(AppsmithError.DATASOURCE_HAS_ACTIONS, actionsCount));
              }
        ...
        .flatMap(
            toDelete -> {
              return datasourceContextService
                  .deleteDatasourceContext(
                      datasourceContextService.createDsContextIdentifier(toDelete))
  ...
  /**
   * This is a composite method for retrieving datasource, datasourceContextIdentifier and
   * environmentMap See EE override for complete usage
   *
   * @param datasource
  ...
  public Mono<Tuple3<Datasource, DatasourceContextIdentifier, Map<String, BaseDomain>>>
    ...
    Mono<DatasourceContextIdentifier> datasourceContextIdentifierMono =
        Mono.just(datasourceContextService.createDsContextIdentifier(datasource));
