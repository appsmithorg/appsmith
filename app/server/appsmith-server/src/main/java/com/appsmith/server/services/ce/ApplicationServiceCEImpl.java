...
public class ApplicationServiceCEImpl extends BaseService<ApplicationRepository, Application, String>
    ...
    @Override
    public Flux<Application> findByWorkspaceIdAndRecentlyUsedOrder(String workspaceId) {
        ...
        return userDataMono.flatMapMany(userData -> this.findByWorkspaceId(
                        workspaceId, applicationPermission.getReadPermission())
                // sort transformation
                .transform(
                        domainFlux -> sortDomainsBasedOnOrderedDomainIds(domainFlux, userData.getRecentlyUsedAppIds()))
                // Git connected apps will have gitApplicationMetadata
                .filter(application -> application.getGitApplicationMetadata() == null
                        ...
                        // 2. When the DefaultBranchName is missing due to branch creation flow failures or
                        // corrupted scenarios
                        || (StringUtils.hasLength(
                                        application.getGitApplicationMetadata().getBranchName())
                                && application
