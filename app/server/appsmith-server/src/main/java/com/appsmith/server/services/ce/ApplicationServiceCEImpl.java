...
private Mono<? extends Application> generateAndSetPoliciesForPublicView(
	...
	// Use the same policy map as actions for action collections since action collections have the
	...
	return updatedActionsFlux
		...
		.flatMap(
			...
			// Update the datasource policies without permission since the applications and
			// datasources are at
			// the same level in the hierarchy. A user may have permission to change view on
			// application, but may
...
public Mono<GitAuth> createOrUpdateSshKeyPair(String applicationId, String keyType) {
	...
	return repository
		...
		.flatMap(
			...
			// Fetch root application and then make updates. We are storing the git metadata only
			// in root application
			if (StringUtils.isEmpty(gitData.getDefaultApplicationId())) {
				throw new AppsmithException(
					AppsmithError.INVALID_GIT_CONFIGURATION,
					"Unable to find root application, please connect your application to remote"
						+ " repo to resolve this issue.");
			}
			gitAuth.setRegeneratedKey(true);
