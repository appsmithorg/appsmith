...
public enum AppsmithError {
    ...
    APPLICATION_NOT_FORKED_MISSING_PERMISSIONS(403, AppsmithErrorCode.APPLICATION_NOT_FORKED_MISSING_PERMISSIONS.getCode(), "Unable to fork application. You don''t have required permissions for {0}: {1}",
            AppsmithErrorAction.DEFAULT, "Forking application failed", ErrorType.INTERNAL_ERROR, null),

    ASSIGN_UNASSIGN_MISSING_PERMISSION(403, AppsmithErrorCode.ASSIGN_UNASSIGN_MISSING_PERMISSION.getCode(), "Unable to perform assign or unassign operation. You don''t have permissions for {0}: {1}",
            AppsmithErrorAction.DEFAULT, "Assign or unassign operation failed", ErrorType.INTERNAL_ERROR, null),

    SAML_ALREADY_CONFIGURED(500, AppsmithErrorCode.SAML_ALREADY_CONFIGURED.getCode(), "SAML configuration already exists. Please disconnect SAML before trying to configure it again", AppsmithErrorAction.DEFAULT, "SAML configuration already exists", ErrorType.INTERNAL_ERROR, null),

    SAML_CONFIGURATION_FAILURE(400, AppsmithErrorCode.SAML_CONFIGURATION_FAILURE.getCode(), "SAML configuration failed. {0}", AppsmithErrorAction.DEFAULT, "SAML configuration failed", ErrorType.ARGUMENT_ERROR, null),

    UNABLE_TO_DEPLOY_MISSING_PERMISSION(403, AppsmithErrorCode.UNABLE_TO_DEPLOY_MISSING_PERMISSION.getCode(), "Unable to deploy the Application. You don''t have required permissions for {0}: {1}",
