...
public class ForkExamplesWorkspaceServiceCEImpl implements ForkExamplesWorkspaceCE {
    ...
    /**
     * Clones the template workspace (as specified in config collection) for the given user. The
     * given user will be the owner of the cloned workspace. This method also assumes that the given
     * user is the same as the user in the current Spring session.
     *
    ...
    private Mono<Workspace> forkExamplesWorkspace(User user) {
        if (!CollectionUtils.isEmpty(user.getWorkspaceIds())) {
            // Don't create an examples workspace if the user already has some workspaces, perhaps
            // because they
    ...
    /**
     * Given a workspace ID and a user, clone the workspace and make the given user the owner of the
     * cloned workspace. This recursively clones all objects inside the workspace. This method also
     * assumes that the given user is the same as the user in the current Spring session.
     *
        ...
        return workspaceRepository
                ...
                .doOnSuccess(
                        workspace -> {
                            if (workspace == null) {
                                log.error(
                                        "Template examples workspace not found. Not creating a"
                                                + " clone for user {}.",
                                        user.getEmail());
                            }
                        })
                ...
                .zipWhen(
                        newWorkspace ->
                                workspaceService.getDefaultEnvironmentId(newWorkspace.getId()))
                .flatMap(
                        tuple2 -> {
    ...
    /**
     * Clone all applications (except deleted ones), including its pages and actions from one
     * workspace into another. Also clones all datasources (not just the ones used by any
     * applications) provided in the parameter list. This allows us to have more fine-grained
     * control over the clone operation.
     *
    ...
    public Mono<List<String>> forkApplications(
        ...
        // A map of datasourceId => { a cached Mono that clones this datasource and yields the
        // cloned datasource }
        ...
        return datasourceFlux
                .flatMap(
                        datasource -> {
                            // Here, we are creating a cache entry for every datasource provided in
                            // the list
                            // This might be overridden later by datasources actually used in the
                            // applications
                            final String datasourceId = datasource.getId();
                            // forkWithConfiguration is dependent on application, here we are
                            // calling cloneDatasource
                            // for the workspace hence Boolean.TRUE is passed as the third parameter
                            // because by default
                            // user should have access to the datasource credentials already present
                            // in the workspace

                            // The use case for this is: In the example workspace, we need a welcome
                            // tour which is based on
                            // one of the datasource, to get the credentials of that datasource, we
                            // have set this value as True
                ...
                .flatMap(
                        application -> {
                            ...
                            // Extracting forkWithConfiguration to use below before resetting it for
                            // newly forked app
                            // forkWithConfig by default remains FALSE for datasources used in an
                            // application
                            ...
                            // Setting the forkWithConfiguration, exportWithConfiguration and
                            // forkingEnabled fields to null for
                            // newly forked app
                ...
                .flatMap(
                        tuple -> {
                            ...
                            return applicationPageService
                                    ...
                                    .flatMap(
                                            savedPage ->
                                                    newPageRepository.findById(savedPage.getId()))
                                    .flatMap(
                                            savedPage -> {
                                                ...
                                                return newActionService
                                                        ...
                                                        .flatMap(
                                                                newAction -> {
                                                                    ...
                                                                    Mono<ActionDTO> actionMono =
                                                                            Mono.just(action);
                                                                    final Datasource
                                                                            datasourceInsideAction =
                                                                                    action
                                                                                            .getDatasource();
                                                                    if (datasourceInsideAction
                                                                            != null) {
                                                                        if (datasourceInsideAction
                                                                                        .getId()
                                                                                != null) {
                                                        ...
                                                        // This call to `collectMap` will wait for
                                                        // all actions in all pages to have been
                                                        // processed, and so the
                                                        // `clonedPages` list will also contain all
                                                        // pages cloned.
                                                        .collect(
                                                        ...
                                                        .flatMap(
                                                                actionIdsMap -> {
                                                                    // Map of <originalCollectionId,
                                                                    // clonedActionCollectionIds>
                ...
                .flatMap(
                        tuple ->
                                updateActionAndCollectionsIdsInForkedPages(
                                        clonedPages, tuple.getT1(), tuple.getT2()))
                // Now publish all the example applications which have been cloned to ensure that
                // there is a
        ...
        for (final NewPage page : clonedPages) {
            ...
            for (final Layout layout : page.getUnpublishedPage().getLayouts()) {
                if (layout.getLayoutOnLoadActions() != null) {
                    shouldSave =
                            updateOnLoadActionsWithNewActionAndCollectionIds(
                                    actionIdsMap,
                                    actionCollectionIdsMap,
                                    page.getId(),
                                    shouldSave,
                                    layout);
                }
                ...
                if (actionIdsMap.containsKey(actionDTO.getId())) {
                    ...
                    if (StringUtils.hasLength(srcCollectionId)) {
                        actionDTO.setDefaultCollectionId(
                                collectionIdsMap.get(actionDTO.getCollectionId()));
                    ...
                    log.error(
                            "Couldn't find cloned action ID for publishedLayoutOnLoadAction {} in"
                                    + " page {}",
                            actionDTO.getId(),
                            pageId);
                }
    ...
    /**
     * This function simply creates a clone of the Application object without cloning its children
     * (page and actions) Once the new application object is created, it adds the new application's
     * id into the list applicationIds
     *
    ...
    private Mono<UpdateResult> forkThemes(Application srcApplication, Application destApplication) {
        return Mono.zip(
                        themeService.cloneThemeToApplication(
                                srcApplication.getEditModeThemeId(), destApplication),
                        themeService.cloneThemeToApplication(
                                srcApplication.getPublishedModeThemeId(), destApplication))
                .flatMap(
                        themes -> {
    ...
    private Mono<Application> forkApplicationDocument(Application application) {
        ...
        // Clean the object so that it will be saved as a new application for the currently signed
        // in user.
    ...
    // forkWithConfiguration parameter if TRUE, returns the datasource with credentials else returns
    // datasources without credentials
    public Mono<Datasource> forkDatasource(
        ...
        final Mono<String> destinationEnvironmentIdMono =
                workspaceService.getDefaultEnvironmentId(toWorkspaceId);

        final Mono<List<Datasource>> existingDatasourcesInNewWorkspaceMono =
                datasourceService
        ...
        return Mono.zip(datasourceToForkMono, existingDatasourcesInNewWorkspaceMono)
                .flatMap(
                        tuple -> {
                            ...
                            final List<Datasource> existingDatasourcesWithoutStorages =
                                    tuple.getT2();
                            ...
                            final AuthenticationDTO authentication =
                                    storageDTOToFork.getDatasourceConfiguration() == null
                                            ? null
                                            : storageDTOToFork
                            ...
                            return destinationEnvironmentIdMono.flatMap(
                                    destinationEnvironmentId ->
                                            Flux.fromIterable(existingDatasourcesWithoutStorages)
                                                    ...
                                                    .next() // Get the first matching datasource, we
                                                    // don't need more than one here.
                                                    .switchIfEmpty(
                                                            Mono.defer(
                                                                    () -> {
                                                                                ...
                                                                                storageDTO =
                                                                                        datasourceToFork
                                                                                                ...
                                                                                                .fork(
                                                                                                        forkWithConfiguration,
                                                                                                        toWorkspaceId);
                                                                        storageDTO.setEnvironmentId(
                                                                                destinationEnvironmentId);
                                                                        newDs.getDatasourceStorages()
                                                                                .put(
                                                                                        destinationEnvironmentId,
                                                                                        storageDTO);
                ...
                .onErrorResume(
                        ...
                        error -> {
                            if (error.getMessage() != null
                                    && error.getMessage()
                                            .contains(
                                                    "workspace_datasource_deleted_compound_index")) {
