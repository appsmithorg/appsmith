...
public class Migration112EEAddCustomPermissionGroupsToExistingEnvironments {
    ...
    public void executeMigration() {
        ...
        Criteria notDeletedCriteria = new Criteria().andOperator(
        ...
        fetchWorkspaceDefaultGroupsQuery.fields()
                .include(fieldName(QWorkspace.workspace.defaultPermissionGroups),
                        fieldName(QWorkspace.workspace.policies));

        final Query performanceOptimizedWorkspaceQuery = CompatibilityUtils
                .optimizeQueryForNoCursorTimeout(mongoTemplate, fetchWorkspaceDefaultGroupsQuery, Workspace.class);

        Set<Workspace> workspaces = mongoTemplate
                .stream(performanceOptimizedWorkspaceQuery, Workspace.class)
                .collect(Collectors.toSet());

        Map<String, Set<String>> workspaceDefaultGroupsMap = workspaces.stream()
                .collect(Collectors.toMap(
                        workspace -> workspace.getId(),
                        workspace -> Objects.requireNonNullElseGet(workspace.getDefaultPermissionGroups(), HashSet::new)));
        ...
        // but are not a default permission group
        Query fetchDatasourcesQuery = new Query().addCriteria(notDeletedCriteria.and(fieldName(QDatasource.datasource.workspaceId)).exists(true));
        fetchDatasourcesQuery.fields()
                .include(fieldName(QDatasource.datasource.workspaceId),
                        fieldName(QDatasource.datasource.policies));

        final Query performanceOptimizedDatasourceQuery = CompatibilityUtils
                .optimizeQueryForNoCursorTimeout(mongoTemplate, fetchDatasourcesQuery, Datasource.class);
        ...
        workspaceIdToApplicablePermissionGroupIdsMap.forEach((workspaceId, customPermissionGroupIds) -> {
            Criteria applicableEnvironmentsCriteria = new Criteria().andOperator(
                    olderCheckForDeletedCriteria(),
                    newerCheckForDeletedCriteria(),
                    Criteria.where(fieldName(QEnvironment.environment.workspaceId)).is(workspaceId),
                    Criteria.where(fieldName(QEnvironment.environment.policies))
                            ...
                            .and("policies.permissionGroups").exists(true)
            );
            Query applicableEnvironmentsQuery = new Query().addCriteria(applicableEnvironmentsCriteria);

            Update environmentUpdateQuery = new Update().addToSet("policies.$.permissionGroups").each(customPermissionGroupIds);
    ...
    private void getWorkspacesToUpdateFromDatasourcePermissions(String publicPermissionGroupId,
        ...
        mongoTemplate
                .stream(performanceOptimizedDatasourceQuery, Datasource.class)
                .filter(datasource -> {
                    ...
                    return datasource.getPolicies()
                            ...
                            .anyMatch(policy -> {
                                ...
                                boolean isExecutePermission = AclPermission.EXECUTE_DATASOURCES.getValue().equals(policy.getPermission());
                                ...
                                if (isExecutePermission) {
                                    ...
                                    if (workspaceDefaultGroupsMap.containsKey(datasource.getWorkspaceId())) {
                                        policy.getPermissionGroups().removeAll(workspaceDefaultGroupsMap.get(datasource.getWorkspaceId()));
                                    }
                ...
                .forEach(datasource -> {
                    Set<String> customPermissionGroupIds = workspaceIdToApplicablePermissionGroupIdsMap.get(datasource.getWorkspaceId());
                    ...
                    datasource.getPolicies()
                            .stream()
                            .filter(policy -> AclPermission.EXECUTE_DATASOURCES.getValue().equals(policy.getPermission()))
                            .forEach(policy -> finalCustomPermissionGroupIds.addAll(policy.getPermissionGroups()));

                    if (isNewSet) {
                        workspaceIdToApplicablePermissionGroupIdsMap
                                .merge(datasource.getWorkspaceId(), finalCustomPermissionGroupIds, Sets::union);
                    }
                });
    }
    ...
    private static void getWorkspacesToUpdateFromWorkspacePermissions(String publicPermissionGroupId,
        ...
        workspaces.stream()
                .filter(workspace -> {
                    ...
                    return workspace.getPolicies()
                            .stream()
                            .anyMatch(policy -> {
                                // First check if this policy is related to execute workspace datasources
                                boolean isExecutePermission = AclPermission.WORKSPACE_EXECUTE_DATASOURCES.getValue().equals(policy.getPermission());

                                if (isExecutePermission) {
                                    // If it is, remove defaults and anon permissions from allocated permission groups
                                    if (workspaceDefaultGroupsMap.containsKey(workspace.getId())) {
                                        policy.getPermissionGroups().removeAll(workspaceDefaultGroupsMap.get(workspace.getId()));
                                    }
                                    policy.getPermissionGroups().remove(publicPermissionGroupId);
                ...
                .forEach(workspace -> {
                    Set<String> customPermissionGroupIds = workspaceIdToApplicablePermissionGroupIdsMap.get(workspace.getId());
                    boolean isNewSet = false;
                    if (customPermissionGroupIds == null) {
                        customPermissionGroupIds = ConcurrentHashMap.newKeySet();
                        isNewSet = true;
                    ...
                    workspace.getPolicies()
                            .stream()
                            .filter(policy -> AclPermission.WORKSPACE_EXECUTE_DATASOURCES.getValue().equals(policy.getPermission()))
                            .forEach(policy -> finalCustomPermissionGroupIds.addAll(policy.getPermissionGroups()));

                    if (isNewSet) {
                        workspaceIdToApplicablePermissionGroupIdsMap
                                .merge(workspace.getId(), finalCustomPermissionGroupIds, Sets::union);
                    }
                });
    }
}