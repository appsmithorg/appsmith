...
public interface BaseRepository<T, ID extends Serializable> extends ReactiveMongoRepository<T, ID> {
  ...
  /**
   * This function directly updates the document by setting the deleted flag to true for the entity
   * with the given id
   *
  ...
  /**
   * This function directly updates the DB by setting the deleted flag to true for all the documents
   * in the collection with the given list of ids.
   *
  ...
  /**
   * This method is supposed to update the given list of fields in an object as opposed to replacing
   * the entire object.
   */
