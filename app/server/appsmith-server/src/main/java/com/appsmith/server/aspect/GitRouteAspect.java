...
public class GitRouteAspect {
    ...
    public Object handleGitRoute(ProceedingJoinPoint joinPoint, GitRoute gitRoute) {
        ...
        Context ctx =
                new Context().setJoinPoint(joinPoint).setGitRoute(gitRoute).setFieldValue(fieldValue);
    ...
    private Mono<?> artifact(Context ctx) {
        ...
        return switch (artifactType) {
            case APPLICATION -> findApplication(artifactId);
            // case PACKAGE -> findPackage(artifactId);
    ...
    private Mono<?> parent(Context ctx) {
        ...
        return switch (artifactType) {
            case APPLICATION -> findApplication(parentArtifactId);
            case PACKAGE -> findPackage(parentArtifactId);
            default -> Mono.error(() -> new AppsmithException(AppsmithError.GIT_ROUTE_HANDLER_NOT_FOUND, artifactType));
        };
    }
    ...
    // Finds application artifact
    private Mono<Artifact> findApplication(String artifactId) {
    ...
    // // Finds package artifact
    // private Mono<Artifact> findPackage(String artifactId) {
    //     return packageRepository
    //             .findById(artifactId)
    //             .switchIfEmpty(Mono.error(new AppsmithException(AppsmithError.NO_RESOURCE_FOUND)))
    //             .map(pkg -> (Artifact) pkg);
    // }
}
