...
public class PermissionGroupServiceCEImpl
        extends BaseService<PermissionGroupRepository, PermissionGroup, String>
                ...
                .map(
                        pg -> {
                            Set<Permission> permissions =
                                    new HashSet<>(
                                            Optional.ofNullable(pg.getPermissions())
                                                    .orElse(Set.of()));
                ...
                .flatMap(
                        permissionGroup -> {
                            ...
                            if (assignedToUserIds == null || assignedToUserIds.isEmpty()) {
                                ...
                                returnMono =
                                        bulkUnassignFromUserIds(
                                                        permissionGroup,
                                                        List.copyOf(assignedToUserIds))
    ...
    public Mono<PermissionGroup> bulkAssignToUsers(PermissionGroup pg, List<User> users) {
        ...
        Mono<PermissionGroup> permissionGroupUpdateMono =
                repository
    ...
    public Mono<PermissionGroup> bulkUnassignFromUsers(PermissionGroup pg, List<User> users) {
        ...
        return Mono.zip(
                        repository.updateById(
                                pg.getId(), pg, permissionGroupPermission.getUnAssignPermission()),
                        cleanPermissionGroupCacheForUsers(userIds).thenReturn(TRUE))
                .map(tuple -> tuple.getT1());
    }
    ...
    @Override
    public Flux<PermissionGroup> getByDefaultWorkspace(
            Workspace workspace, AclPermission permission) {
        return repository.findByDefaultWorkspaceId(workspace.getId(), permission);
    }

    @Override
    public Flux<PermissionGroup> getByDefaultWorkspaces(
                            ...
                            return Flux.fromIterable(userIds)
                                    .flatMap(
                                            userId -> {
                                                ...
                                                return repository
                                                        .evictAllPermissionGroupCachesForUser(
                                                                email, defaultTenantId)
                                                        .thenReturn(TRUE);
                                            });
                        })
                .then();
    ...
    public boolean isEntityAccessible(
            BaseDomain object, String permission, String permissionGroupId) {
        return object.getPolicies().stream()
                .filter(
                        policy ->
                                policy.getPermission().equals(permission)
    ...
    protected Mono<PermissionGroup> sendEventUsersAssociatedToRole(
        ...
        if (CollectionUtils.isNotEmpty(usernames)) {
            ...
            sendAssignedUsersToPermissionGroupEvent =
                    analyticsService.sendObjectEvent(
                            AnalyticsEvents.ASSIGNED_USERS_TO_PERMISSION_GROUP,
                            permissionGroup,
                            analyticsProperties);
        }
    ...
    protected Mono<PermissionGroup> sendEventUserRemovedFromRole(
            PermissionGroup permissionGroup, List<String> usernames) {
        Mono<PermissionGroup> sendUnAssignedUsersToPermissionGroupEvent =
                Mono.just(permissionGroup);
        if (CollectionUtils.isNotEmpty(usernames)) {
            Map<String, Object> eventData =
            ...
            Map<String, Object> analyticsProperties =
                    Map.of(
                            ...
                            FieldName.CLOUD_HOSTED_EXTRA_PROPS,
                            extraPropsForCloudHostedInstance);
            sendUnAssignedUsersToPermissionGroupEvent =
                    analyticsService.sendObjectEvent(
                            AnalyticsEvents.UNASSIGNED_USERS_FROM_PERMISSION_GROUP,
                            permissionGroup,
                            analyticsProperties);
        }
    ...
    @Override
    public Mono<PermissionGroup> assignToUserAndSendEvent(
            PermissionGroup permissionGroup, User user) {
        return bulkAssignToUserAndSendEvent(permissionGroup, List.of(user));
    }

    @Override
    public Mono<PermissionGroup> bulkAssignToUserAndSendEvent(
    ...
    @Override
    public Mono<PermissionGroup> unAssignFromUserAndSendEvent(
            PermissionGroup permissionGroup, User user) {
        return bulkUnAssignFromUserAndSendEvent(permissionGroup, List.of(user));
    }

    @Override
    public Mono<PermissionGroup> bulkUnAssignFromUserAndSendEvent(
            PermissionGroup permissionGroup, List<User> users) {
        List<String> usernames = users.stream().map(User::getUsername).toList();
        return bulkUnassignFromUsers(permissionGroup, users)
                .flatMap(
                        permissionGroup1 ->
                                sendEventUserRemovedFromRole(permissionGroup, usernames));
    }
}
