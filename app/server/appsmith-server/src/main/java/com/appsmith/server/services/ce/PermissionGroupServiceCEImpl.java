...
public class PermissionGroupServiceCEImpl
        ...
        .map(
            pg -> {
              Set<Permission> permissions =
                  new HashSet<>(Optional.ofNullable(pg.getPermissions()).orElse(Set.of()));
              ...
              Map<String, Policy> policyMap =
                  policyUtils.generatePolicyFromPermissionGroupForObject(pg, pg.getId());
        ...
        .flatMap(
            permissionGroup -> {
              ...
              if (assignedToUserIds == null || assignedToUserIds.isEmpty()) {
                ...
                returnMono =
                    bulkUnassignFromUserIds(permissionGroup, List.copyOf(assignedToUserIds))
  ...
  public Mono<PermissionGroup> bulkAssignToUsers(PermissionGroup pg, List<User> users) {
    ...
    return Mono.zip(
            permissionGroupUpdateMono, cleanPermissionGroupCacheForUsers(userIds).thenReturn(TRUE))
