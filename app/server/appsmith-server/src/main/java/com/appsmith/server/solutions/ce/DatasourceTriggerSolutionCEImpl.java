...
public class DatasourceTriggerSolutionCEImpl implements DatasourceTriggerSolutionCE {
  ...
  public Mono<TriggerResultDTO> trigger(
      String datasourceId, TriggerRequestDTO triggerRequestDTO, String environmentName) {

    Mono<Datasource> datasourceMono =
        datasourceService
            .findById(datasourceId, datasourcePermission.getReadPermission())
            .switchIfEmpty(
                Mono.error(new AppsmithException(AppsmithError.INVALID_PARAMETER, "datasourceId")))
            .cache();
    ...
    Mono<TriggerResultDTO> resultFromPluginMono =
        datasourceAndPluginEssentialsMono.flatMap(
            tuple -> {
              ...
              DatasourceContextIdentifier datasourceContextIdentifier =
                  (DatasourceContextIdentifier) tuple.getT4();
              Map<String, BaseDomain> environmentMap = (Map<String, BaseDomain>) tuple.getT5();

              final Mono<Datasource> validDatasourceMono =
                  authenticationValidator.validateAuthentication(
                      datasource, datasourceContextIdentifier.getEnvironmentId());

              return validDatasourceMono
                  .flatMap(
                      datasource1 -> {
                        if (plugin.isRemotePlugin()) {
                          return datasourceContextService.getRemoteDatasourceContext(
                              plugin, datasource1);
                        } else {
                          return datasourceContextService.getDatasourceContext(
                              datasource, datasourceContextIdentifier, environmentMap);
                        }
                      })
                  // Now that we have the context (connection details), execute
                  // the action.
                  // datasource remains unevaluated for datasource of DBAuth Type
                  // Authentication,
                  // However the context comes from evaluated datasource.
                  .flatMap(
                      resourceContext -> {
                        return (Mono<TriggerResultDTO>)
                            pluginExecutor.trigger(
    ...
    Mono<TriggerResultDTO> defaultResultMono =
        datasourceMono
            .flatMap(
                datasource ->
                    entitySelectorTriggerSolution(datasourceId, triggerRequestDTO, environmentName))
            .map(
                entityNames -> {
  ...
  private Mono<Set<String>> entitySelectorTriggerSolution(
      String datasourceId, TriggerRequestDTO request, String environmentName) {
    ...
    Mono<DatasourceStructure> structureMono =
        datasourceStructureSolution.getStructure(datasourceId, false, environmentName);

    return structureMono.map(
        structure -> {
          ...
          if (tables != null && !tables.isEmpty()) {
            ...
            if (parameters == null || parameters.isEmpty()) {
              ...
              if (parameters.size() == 1) {
                ...
                Optional<DatasourceStructure.Table> tableNamePresent =
                    tables.stream().filter(table -> table.getName().equals(tableName)).findFirst();
