...
public class DatasourceTriggerSolutionCEImpl implements DatasourceTriggerSolutionCE {
    ...
    public Mono<TriggerResultDTO> trigger(
        ...
        Mono<Datasource> datasourceMonoCached =
                datasourceService
                        .findById(datasourceId, datasourcePermission.getExecutePermission())
                        .cache();

        Mono<DatasourceStorage> datasourceStorageMonoCached =
                datasourceMonoCached
                        .flatMap(
                                datasource1 ->
                                        datasourceService
        ...
        Mono<PluginExecutor> pluginExecutorMono =
                pluginMono
                        .flatMap(
                                plugin -> pluginExecutorHelper.getPluginExecutor(Mono.just(plugin)))
                        .cache();
        ...
        Mono<DatasourceStorage> validatedDatasourceStorageMono =
                datasourceStorageMonoCached.flatMap(
        ...
        Mono<TriggerResultDTO> resultFromPluginMono =
                Mono.zip(validatedDatasourceStorageMono, pluginMono, pluginExecutorMono)
                        .flatMap(
                                tuple -> {
                                    ...
                                    return datasourceContextService
                                            ...
                                            // Now that we have the context (connection details),
                                            // execute the action.
                                            // datasource remains unevaluated for datasource of
                                            // DBAuth Type Authentication,
