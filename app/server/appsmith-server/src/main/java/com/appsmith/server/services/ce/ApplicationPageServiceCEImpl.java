...
public Mono<PageDTO> createPage(PageDTO page) {
	...
	return pageMono
		...
		// After the page has been saved, update the application (save the page id inside the
		// application)
	...
	return applicationRepository
		...
		.doOnSuccess(
			result -> {
				...
				log.error(
					"Add page to application didn't update anything, probably because application"
						+ " wasn't found.");
				}
			});
	} else {
...
private Mono<PageDTO> clonePageGivenApplicationId(
	...
	Flux<NewAction> sourceActionFlux =
		newActionService
			...
			// Set collection reference in actions to null to reset to the new application's
			// collections later
			.map(
				newAction -> {
	...
	return sourcePageMono
					...
					actionIdsMap -> {
						...
						return sourceActionCollectionsFlux
							.flatMap(
								...
								//
								// 1. If the app is connected with git keep the actionDefaultId as
								// it is and
								// update branchedActionId only
								//
								// 2. If app is not connected then both default and
								...
								// Set id as null, otherwise create (which is using under the hood
								// save)
								// will try to overwrite same resource instead of creating a new
								// resource
								actionCollection.setId(null);
								// Set published version to null as the published version of the
								// page does
		...
		// Calculate the on load actions for this page now that the page and actions have been
		// created
		.flatMap(
			savedPage -> {
...
public Mono<Application> cloneApplication(String applicationId, String branchName) {
	...
	// We don't have to sanitise the response to update the Ids with the default ones as client want
	// child application only
	Mono<Application> clonedResultMono =
		Mono.zip(applicationMono, newAppNameMono)
			.flatMap(
				...
				// Create a new clone application object without the pages using the parameterized
				...
				return setApplicationPolicies(
					...
					// Create the cloned application with the new name and policies before
					// proceeding further.
					.zipWith(userMono)
					.flatMap(
					...
					// Now fetch the pages of the source application, clone and add them to this
					// new application
					.flatMap(
						savedApplication ->
					...
					// duplicate the source application's themes if required i.e. if they were
					// customized
					.flatMap(
						application ->
							themeService
	...
	// Clone Application is currently a slow API because it needs to create application, clone all
	// the pages, and then
	// clone all the actions. This process may take time and the client may cancel the request. This
	// leads to the flow
	// getting stopped midway producing corrupted clones. The following ensures that even though the
	// client may have
	// cancelled the flow, the cloning of the application should proceed uninterrupted and whenever
	// the user refreshes
...
public Mono<Application> publish(String applicationId, boolean isPublishedManually) {
	...
	Mono<List<NewPage>> publishApplicationAndPages =
		applicationMono
			...
			.flatMap(
				...
				// This is the time to delete any page which was deleted in edit mode but still
				// exists in the published mode
				...
				*/
				...
				// Archive the deleted pages and save the application changes and then return the
				// pages so that
			...
			.flatMap(
				applicationPage ->
					newPageService
						...
						// For a git connected app if the user does not have permission to edit few
						// pages in master branch
						// They don't get access to the same resources in feature branch. When they
						// do operations like commit and push we publish the changes automatically
						// and this will fail due to permission issue. Hence removing the throwing
						// error part and handling it gracefully
			...
			.collectList()
			.cache(); // caching as we'll need this to send analytics attributes after publishing
	// the app
	...
	Mono<Map<PluginType, Collection<NewAction>>> publishedActionsListMono =
		newActionService
			...
			.flatMap(
				newAction -> {
				// If the action was deleted in edit mode, now this document can be safely
				// archived
	...
	Mono<List<ActionCollection>> publishedActionCollectionsListMono =
		actionCollectionService
			...
			.flatMap(
				collection -> {
				// If the collection was deleted in edit mode, now this can be safely deleted from
				// the repository
				...
				// Publish the collection by copying the unpublished collectionDTO to published
				// collectionDTO
...
public Mono<ApplicationPagesDTO> reorderPage(
	...
	return newPageService
		...
		.flatMap(
			...
			Application application = tuple.getT2();
			// Update the order in unpublished pages here, since this should only ever happen in
			// edit mode.
			List<ApplicationPage> pages = application.getPages();
...
public Mono<Application> createOrUpdateSuffixedApplication(
	...
	// We are taking pessimistic approach as this flow is used in import application where we are
	// using transactions
						...
						.flatMap(
							...
							application1.setModifiedBy(
								tuple
									.getT2()
									.getUsername()); // setting modified by to current user
							// We can't use create or createApplication method here as we are
							// expecting update operation if the
