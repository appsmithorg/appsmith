...
public class ApplicationPageServiceCEImpl implements ApplicationPageServiceCE {
    ...
    public Mono<PageDTO> createPage(PageDTO page) {
        if (page.getId() != null) {
            ...
            return Mono.error(
                    new AppsmithException(AppsmithError.INVALID_PARAMETER, FieldName.NAME));
        ...
        return pageMono.flatMap(newPageService::createDefault)
                // After the page has been saved, update the application (save the page id inside
                // the application)
                ...
                .flatMap(
                        tuple -> {
                            ...
                            return addPageToApplication(application, savedPage, false)
                                    .then(
                                            applicationService.saveLastEditInformation(
                                                    application.getId()))
    ...
    public Mono<PageDTO> createPageWithBranchName(PageDTO page, String branchName) {
        ...
        DefaultResources defaultResources =
                page.getDefaultResources() == null
                        ? new DefaultResources()
                        : page.getDefaultResources();
    ...
    /**
     * This function is called during page create in Page Service. It adds the given page to its
     * ApplicationPages list. Note: It is assumed here that `application` is already checked for the
     * MANAGE_APPLICATIONS policy.
     *
        ...
        if (isDuplicatePage(application, page.getId())) {
            return applicationRepository
                    .addPageToApplication(
                            application.getId(), page.getId(), isDefault, defaultPageId)
                    .doOnSuccess(
                            result -> {
                                if (result.getModifiedCount() != 1) {
                                    log.error(
                                            "Add page to application didn't update anything,"
                                                + " probably because application wasn't found.");
                                }
                            });
        } else {
            ...
            int count =
                    (int)
                            application.getPages().stream()
                                    .filter(
                                            applicationPage ->
                                                    applicationPage.getId().equals(pageId))
                                    .count();
            if (count > 0) {
    ...
    @Override
    public Mono<PageDTO> getPageByBranchAndDefaultPageId(
        ...
        return applicationService
                ...
                .switchIfEmpty(
                        Mono.error(
                                new AppsmithException(
                                        AppsmithError.ACL_NO_RESOURCE_FOUND,
                                        FieldName.PAGE + " by application name",
                                        applicationName)))
                .flatMap(
                        application ->
                                newPageService.findByNameAndApplicationIdAndViewMode(
                                        pageName, application.getId(), pagePermission1, viewMode))
                .switchIfEmpty(
                        Mono.error(
                                new AppsmithException(
                                        AppsmithError.ACL_NO_RESOURCE_FOUND,
                                        FieldName.PAGE + " by page name",
                                        pageName)));
    }
    ...
    public Mono<Application> makePageDefault(String applicationId, String pageId) {
        // Since this can only happen during edit, the page in question is unpublished page. Set the
        ...
        return newPageService
                ...
                .then(
                        applicationService.findById(
                                applicationId, applicationPermission.getEditPermission()))
                .switchIfEmpty(
                        Mono.error(
                                new AppsmithException(
                                        AppsmithError.NO_RESOURCE_FOUND,
    ...
    @Override
    public Mono<Application> makePageDefault(
            String defaultApplicationId, String defaultPageId, String branchName) {
        // TODO remove the dependency of applicationId as pageId and branch can get the exact
        // resource
        return newPageService
                .findByBranchNameAndDefaultPageId(
                        branchName, defaultPageId, pagePermission.getEditPermission())
                .flatMap(
                        branchedPage ->
                                makePageDefault(
                                        branchedPage.getApplicationId(), branchedPage.getId()))
                ...
                .flatMap(
                        tuple -> {
                            ...
                            application1.setModifiedBy(
                                    tuple.getT2()
                                            .getUsername()); // setting modified by to current user
                            ...
                            return themeService
                                    ...
                                    .then(
                                            applicationService.createDefaultApplication(
                                                    application1));
                        })
                .flatMap(
                        savedApplication -> {
                            ...
                            return newPageService
                                    ...
                                    .flatMap(
                                            savedPage ->
                                                    addPageToApplication(
                                                            savedApplication, savedPage, true))
                                    // Now publish this newly created app with default states so
                                    // that
                                    // launching of newly created application is possible.
                                    .flatMap(
                                            updatedApplication ->
                                                    publish(savedApplication.getId(), false)
                                                            .then(
                                                                    applicationService.findById(
                                                                            savedApplication
                                                                                    .getId(),
    ...
    @Override
    public Mono<Application> setApplicationPolicies(
    ...
    public void generateAndSetPagePolicies(Application application, PageDTO page) {
        Set<Policy> documentPolicies =
                policyGenerator.getAllChildPolicies(
                        application.getPolicies(), Application.class, Page.class);
        page.setPolicies(documentPolicies);
    }

    /**
     * This function performs a soft delete for the application along with it's associated pages and
     * actions.
     *
                        ...
                        application -> {
                            ...
                            if (gitData != null
                                ...
                                return applicationService.findAllApplicationsByDefaultApplicationId(
                                        ...
                                        applicationPermission.getDeletePermission());
                            }
    ...
    public Mono<Application> deleteApplicationByResource(Application application) {
        log.debug(
                "Archiving actionCollections, actions, pages and themes for applicationId: {}",
                application.getId());
        return actionCollectionService
                .archiveActionCollectionByApplicationId(
                        application.getId(), actionPermission.getDeletePermission())
                .then(
                        newActionService.archiveActionsByApplicationId(
                                application.getId(), actionPermission.getDeletePermission()))
                .then(
                        newPageService.archivePagesByApplicationId(
                ...
                .flatMap(
                        deletedApplication -> {
                            ...
                            final Map<String, Object> data =
                                    Map.of(FieldName.EVENT_DATA, eventData);
    ...
    public Mono<PageDTO> clonePage(String pageId) {
        ...
        return newPageService
                ...
                .switchIfEmpty(
                        Mono.error(
                                new AppsmithException(
                                        AppsmithError.ACTION_IS_NOT_AUTHORIZED, "Clone Page")))
                .flatMap(
                        page ->
                                applicationService
                                        .saveLastEditInformation(page.getApplicationId())
                                        .then(
                                                clonePageGivenApplicationId(
                                                        pageId, page.getApplicationId(), " Copy")));
    }

    @Override
    public Mono<PageDTO> clonePageByDefaultPageIdAndBranch(
    ...
    private Mono<PageDTO> clonePageGivenApplicationId(
        ...
        // Find the source page and then prune the page layout fields to only contain the required
        // fields that should be
        // copied.
        Mono<PageDTO> sourcePageMono =
                newPageService
                        ...
                        .flatMap(
                                page ->
                                        Flux.fromIterable(page.getLayouts())
                                                .map(
                                                        layout -> {
                                                            Layout newLayout = new Layout();
                                                            String id = new ObjectId().toString();
                                                            newLayout.setId(id);
                                                            newLayout.setMongoEscapedWidgetNames(
                                                                    layout
                                                                            .getMongoEscapedWidgetNames());
        ...
        final Flux<ActionCollection> sourceActionCollectionsFlux =
                actionCollectionService.findByPageId(pageId);

        Flux<NewAction> sourceActionFlux =
                newActionService
                        .findByPageId(pageId, actionPermission.getEditPermission())
                        // Set collection reference in actions to null to reset to the new
                        // application's collections later
                        .map(
                                newAction -> {
        ...
        return sourcePageMono
                .flatMap(
                        page -> {
                            ...
                            Mono<Application> destinationApplicationMono =
                                    applicationService
                                            .findById(
                                                    applicationId,
                            ...
                            return Mono.zip(pageNamesMono, destinationApplicationMono)
                                    ...
                                    // set a unique name for the cloned page and then create the
                                    // page.
                                    .flatMap(
                                            tuple -> {
                                                ...
                                                if (!Strings.isNullOrEmpty(newPageNameSuffix)) {
                                                    String newPageName =
                                                            page.getName() + newPageNameSuffix;

                                                    Set<String> names =
                                                            pageNames.getPages().stream()
                ...
                .flatMap(
                        clonedPage -> {
                            ...
                            final DefaultResources clonedPageDefaultResources =
                                    clonedPage.getDefaultResources();
                            return sourceActionFlux
                                    ...
                                    .collect(
                                            ...
                                            (map, tuple2) ->
                                                    map.put(tuple2.getT2(), tuple2.getT1()))
                                    .flatMap(
                                            actionIdsMap -> {
                                                ...
                                                return sourceActionCollectionsFlux
                                                        .flatMap(
                                                                actionCollection -> {
                                                                    ...
                                                                    // Replace all action Ids from
                                                                    // map
                                                                    Map<String, String>
                                                                            updatedDefaultToBranchedActionId =
                                                                                    new HashMap<>();
                                                                    // Check if the application is
                                                                    // connected with git and update
                                                                    // defaultActionIds accordingly
                                                                    //
                                                                    // 1. If the app is connected
                                                                    // with git keep the
                                                                    // actionDefaultId as it is and
                                                                    // update branchedActionId only
                                                                    //
                                                                    // 2. If app is not connected
                                                                    // then both default and
                                                                    // branchedActionId will be
                                                                    // same as newly created action
                                                                    // Id
                                                                    ...
                                                                    // Set id as null, otherwise
                                                                    // create (which is using under
                                                                    // the hood save)
                                                                    // will try to overwrite same
                                                                    // resource instead of creating
                                                                    // a new resource
                                                                    actionCollection.setId(null);
                                                                    // Set published version to null
                                                                    // as the published version of
                                                                    // the page does
                                                                    // not exists when we clone the
                                                                    // page.
                                                                    actionCollection
                                                                    ...
                                                                    actionCollection
                                                                            .getDefaultResources()
                                                                            .setPageId(null);
                                                                    // Assign new gitSyncId for
                                                                    // cloned actionCollection
                ...
                // Calculate the on load actions for this page now that the page and actions have
                // been created
                .flatMap(
                        savedPage -> {
                            ...
                            return Flux.fromIterable(layouts)
                                    .flatMap(
                                            layout -> {
                                                layout.setDsl(
                                                        layoutActionService
                                                                .unescapeMongoSpecialCharacters(
                                                                        layout));
    ...
    public Mono<Application> cloneApplication(String applicationId, String branchName) {
        ...
        Mono<Application> applicationMono =
                applicationService
                        ...
                        .flatMap(
                                application -> {
                                    // For git connected application user can update the default
                                    // branch
                                    // In such cases we should fork the application from the new
                                    // default branch
        ...
        // 2. Find the name for the cloned application which wouldn't lead to duplicate key
        // exception
        Mono<String> newAppNameMono =
                applicationMono.flatMap(
                        application ->
                                applicationService
                                        // TODO: Convert this into a query that projects only
                                        // application names
                                        .findAllApplicationsByWorkspaceId(
                                                application.getWorkspaceId())
        ...
        // We don't have to sanitise the response to update the Ids with the default ones as client
        // want child application only
        Mono<Application> clonedResultMono =
                Mono.zip(applicationMono, newAppNameMono)
                        .flatMap(
                                tuple -> {
                                    ...
                                    // Create a new clone application object without the pages using
                                    // the parameterized Application constructor
                                    ...
                                    newApplication.setEvaluationVersion(
                                            sourceApplication.getEvaluationVersion());

                                    if (sourceApplication.getApplicationVersion() != null) {
                                        newApplication.setApplicationVersion(
                                                sourceApplication.getApplicationVersion());
                                    } else {
                                        newApplication.setApplicationVersion(
                                    ...
                                    return setApplicationPolicies(
                                                    userMono,
                                                    sourceApplication.getWorkspaceId(),
                                                    newApplication)
                                            // Create the cloned application with the new name and
                                            // policies before proceeding further.
                                            .zipWith(userMono)
                                            .flatMap(
                                                    applicationUserTuple2 -> {
                                                        Application application1 =
                                                                applicationUserTuple2.getT1();
                                            ...
                                            // 4. Now fetch the pages of the source application,
                                            // clone and add them to this new application
                                            .flatMap(
                                                    savedApplication ->
                                                            Flux.fromIterable(
                                                                            sourceApplication
                                                                    ...
                                                                    .collectList()
                                                                    // Set the cloned pages into the
                                                                    // cloned application and save.
                                                                    .flatMap(
                                                                            clonedPages -> {
                                                                                savedApplication
                                                                                        .setPages(
                                            ...
                                            // 5. Duplicate the source application's themes if
                                            // required i.e. if they were customized
                                            .flatMap(
                                                    application ->
                                                            themeService
                                                                    .cloneThemeToApplication(
                                                                            sourceApplication
                                                                                    .getEditModeThemeId(),
                                                                            application)
                                                                    .zipWith(
                                            ...
                                            .flatMap(
                                                    application ->
                                                            publish(application.getId(), false))
                                            .flatMap(
                                                    application ->
                                                            sendCloneApplicationAnalyticsEvent(
                                                                    sourceApplication,
                                                                    application));
                                });

        // Clone Application is currently a slow API because it needs to create application, clone
        // all the pages, and then
        // clone all the actions. This process may take time and the client may cancel the request.
        // This leads to the flow
        // getting stopped midway producing corrupted clones. The following ensures that even though
        // the client may have
        // cancelled the flow, the cloning of the application should proceed uninterrupted and
        ...
        return Mono.create(
                sink ->
                        clonedResultMono.subscribe(
                                sink::success, sink::error, null, sink.currentContext()));
    }

    /**
     * This function archives the unpublished page. This also archives the unpublished action. The
     * reason that the entire action is not deleted at this point is to handle the following edge
     * case : An application is published with 1 page and 1 action. Post publish, create a new page
     * and move the action from the existing page to the new page. Now delete this newly created
     * page. In this scenario, if we were to delete all actions associated with the page, we would
     * end up deleting an action which is currently in published state and is being used.
     *
    ...
    private Mono<PageDTO> deleteUnpublishedPageEx(String id, Optional<AclPermission> permission) {
        ...
        return newPageService
                ...
                .switchIfEmpty(
                        Mono.error(
                                new AppsmithException(
                                        AppsmithError.NO_RESOURCE_FOUND, FieldName.PAGE, id)))
                .flatMap(
                        page -> {
                            log.debug(
                                    "Going to archive pageId: {} for applicationId: {}",
                                    page.getId(),
                                    page.getApplicationId());
                            Mono<Application> applicationMono =
                                    applicationService
                                    ...
                                    newPageMono
                                            .flatMap(
                                                    newPage -> {
                                                        ...
                                                        return analyticsService.sendDeleteEvent(
                                                                newPage, data);
                                                    })
                                            .flatMap(
                                                    newPage ->
                                                            newPageService.getPageByViewMode(
                                                                    newPage, false));

                            /**
                             * Only delete unpublished action and not the entire action. Also filter
                             * actions embedded in actionCollection which will be deleted while
                             * deleting the collection, this will avoid the race condition for
                             * delete action
                             */
                            Mono<List<ActionDTO>> archivedActionsMono =
                                    newActionService
                                            .findByPageId(
                                                    page.getId(),
                            ...
                            /**
                             * Only delete unpublished action collection and not the entire action
                             * collection.
                             */
                            Mono<List<ActionCollectionDTO>> archivedActionCollectionsMono =
                            ...
                            return Mono.zip(
                                            archivedActionsMono,
                                            archivedActionCollectionsMono,
                                            applicationMono)
                                    .map(
                                            tuple -> {
                                                List<ActionDTO> actions = tuple.getT1();
                                                final List<ActionCollectionDTO> actionCollections =
                                                        tuple.getT2();
                                                Application application = tuple.getT3();
                                                log.debug(
                                                        "Archived {} actions and {} action"
                                                            + " collections for applicationId: {}",
                                                        actions.size(),
                                    ...
                                    .flatMap(
                                            pageDTO ->
                                                    // save the last edit information as page is
                                                    // deleted from application
                                                    applicationService
                                                            .saveLastEditInformation(
                                                                    pageDTO.getApplicationId())
                                                            .thenReturn(pageDTO));
                        });
    }

    public Mono<PageDTO> deleteUnpublishedPageByBranchAndDefaultPageId(
            String defaultPageId, String branchName) {
        return newPageService
                .findByBranchNameAndDefaultPageId(
                        branchName, defaultPageId, pagePermission.getDeletePermission())
                .flatMap(newPage -> deleteUnpublishedPage(newPage.getId()))
                .map(responseUtils::updatePageDTOWithDefaultResources);
    }

    /**
     * This function walks through all the pages in the application. In each page, it walks through
     * all the layouts. In a layout, dsl and publishedDsl JSONObjects exist. Publish function is
     * responsible for copying the dsl into the publishedDsl.
     *
    ...
    public Mono<Application> publish(String applicationId, boolean isPublishedManually) {
        /**
         * Please note that it is a cached Mono, hence please be careful with using this Mono to
         * update / read data when latest updated application object is desired.
         */
        Mono<Application> applicationMono =
                applicationService
                ...
                applicationMono
                        ...
                        .flatMap(
                                application -> {
                                    ...
                                    // This is the time to delete any page which was deleted in edit
                                    // mode but still exists in the published mode
                                    ...
                                    /**
                                     * Now add the published page ids and edited page ids into a
                                     * single set and then remove the edited page ids to get a set
                                     * of page ids which have been deleted in the edit mode. For
                                     * example : Published page ids : [ A, B, C ] Edited Page ids :
                                     * [ B, C, D ] aka A has been deleted and D has been added Step
                                     * 1. Add both the ids into a single set : [ A, B, C, D] Step 2.
                                     * Remove Edited Page Ids : [ A ] Result : Page A which has been
                                     * deleted in the edit mode
                                     */
                                    ...
                                    if (!publishedPageIds.isEmpty()) {
                                        archivePageListMono =
                                                Flux.fromStream(publishedPageIds.stream())
                                    ...
                                    application.setPublishedAppLayout(
                                            application.getUnpublishedAppLayout());
                                    application.setPublishedApplicationDetail(
                                            application.getUnpublishedApplicationDetail());

                                    if (isPublishedManually) {
                                        application.setLastDeployedAt(Instant.now());
                                    }
                                    // Archive the deleted pages and save the application changes
                                    // and then return the pages so that
                        ...
                        .flatMap(
                                applicationPage ->
                                        newPageService
                                                ...
                                                // For a git connected app if the user does not have
                                                // permission to edit few pages in master branch
                                                // They don't get access to the same resources in
                                                // feature branch. When they do operations like
                                                // commit and push we publish the changes
                                                // automatically
                                                // and this will fail due to permission issue. Hence
                                                // removing the throwing error part and handling it
                                                // gracefully
                                                // Only the pages which the user pocesses permission
                                                // will be published
                                                .map(
                                                        page -> {
                                                            page.setPublishedPage(
                                                                    page.getUnpublishedPage());
                                                            return page;
                                                        }))
                        .flatMap(newPageService::save)
                        .collectList()
                        .cache(); // caching as we'll need this to send analytics attributes after
        // publishing the app

        Mono<Map<PluginType, Collection<NewAction>>> publishedActionsListMono =
                newActionService
                        .findAllByApplicationIdAndViewMode(
                                applicationId, false, actionPermission.getEditPermission(), null)
                        .flatMap(
                                newAction -> {
                                    // If the action was deleted in edit mode, now this document can
                                    // be safely archived
                                    ...
                                    // Publish the action by copying the unpublished actionDTO to
                                    // published actionDTO
        ...
        Mono<List<ActionCollection>> publishedActionCollectionsListMono =
                actionCollectionService
                        ...
                        .flatMap(
                                collection -> {
                                    // If the collection was deleted in edit mode, now this can be
                                    // safely deleted from the repository
                                    if (collection.getUnpublishedCollection().getDeletedAt()
                                            != null) {
                                        return actionCollectionService
                                                .archiveById(collection.getId())
                                                .then(Mono.empty());
                                    }
                                    // Publish the collection by copying the unpublished
                                    // collectionDTO to published collectionDTO
                                    collection.setPublishedCollection(
                                            collection.getUnpublishedCollection());
    ...
    private int getActionCount(
            Map<PluginType, Collection<NewAction>> pluginTypeCollectionMap, PluginType pluginType) {
    ...
    private Mono<Application> sendApplicationPublishedEvent(
        ...
        return Mono.zip(
                        ...
                        // not using existing applicationMono because we need the latest Application
                        // after published
                        applicationService.findById(
                                applicationId, applicationPermission.getEditPermission()),
                ...
                .flatMap(
                        objects -> {
                            ...
                            Map<PluginType, Collection<NewAction>> pluginTypeCollectionMap =
                                    objects.getT2();
                            Integer dbQueryCount =
                                    getActionCount(pluginTypeCollectionMap, PluginType.DB);
                            Integer apiCount =
                                    getActionCount(pluginTypeCollectionMap, PluginType.API);
                            Integer jsFuncCount =
                                    getActionCount(pluginTypeCollectionMap, PluginType.JS);
                            Integer saasQueryCount =
                                    getActionCount(pluginTypeCollectionMap, PluginType.SAAS);
                            Integer remoteQueryCount =
                                    getActionCount(pluginTypeCollectionMap, PluginType.REMOTE);
                            ...
                            extraProperties.put(
                                    "publishedAt",
                                    defaultIfNull(application.getLastDeployedAt(), ""));
                            ...
                            return analyticsService.sendObjectEvent(
                                    AnalyticsEvents.PUBLISH_APPLICATION,
                                    application,
                                    extraProperties);
                        });
    }
    ...
    @Override
    public Mono<Application> publish(
    ...
    /**
     * This function walks through all the pages and reorders them and updates the order as per the
     * user preference. A page can be moved up or down from the current position and accordingly the
     * order of the remaining page changes.
     *
        ...
        return newPageService
                ...
                .flatMap(
                        tuple -> {
                            ...
                            Application application = tuple.getT2();
                            // Update the order in unpublished pages here, since this should only
                            // ever happen in edit mode.
                            List<ApplicationPage> pages = application.getPages();
                            ...
                            return applicationRepository
                                    ...
                                    .then(
                                            newPageService
                                                    .findApplicationPagesByApplicationIdViewMode(
                                                            application.getId(),
                                                            Boolean.FALSE,
                                                            false));
                        })
    ...
    /**
     * This method will create a new suffixed application or update the existing application if
    ...
    public Mono<Application> createOrUpdateSuffixedApplication(
        ...
        Mono<Application> applicationMono =
                applicationService.findByNameAndWorkspaceId(
                        actualName, application.getWorkspaceId(), MANAGE_APPLICATIONS);

        // We are taking pessimistic approach as this flow is used in import application where we
        // are using transactions
        // which creates problem if we hit duplicate key exception
        return applicationMono
                .flatMap(
                        application1 ->
                                this.createOrUpdateSuffixedApplication(
                                                ...
                                                .flatMap(
                                                        tuple -> {
                                                            ...
                                                            application1.setModifiedBy(
                                                                    tuple.getT2()
                                                                            .getUsername()); // setting modified by to current user
                                                            // We can't use create or
                                                            // createApplication method here as we
                                                            // are expecting update operation if the
                                                            // _id is available with application
                                                            // object
        ...
        return workspaceService
                ...
                .flatMap(
                        workspace -> {
                            ...
                            final Map<String, Object> data =
                                    Map.of(
                                            FieldName.SOURCE_APPLICATION_ID,
                            ...
                            return analyticsService.sendObjectEvent(
                                    AnalyticsEvents.CLONE, application, data);
                        });
    }
    ...
    private Mono<NewPage> sendPageViewAnalyticsEvent(NewPage newPage, boolean viewMode) {
        String view =
                viewMode ? ApplicationMode.PUBLISHED.toString() : ApplicationMode.EDIT.toString();
        final Map<String, Object> eventData =
                Map.of(
