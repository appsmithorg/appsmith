...
public class ApplicationPageServiceCEImpl implements ApplicationPageServiceCE {
   ...
   * @return UpdateResult object with details on how many documents have been updated, which should
   *     be 0 or 1.
   */
  @Override
        ...
        .flatMap(
            newPage -> {
              return sendPageViewAnalyticsEvent(newPage, viewMode)
                  .then(getPage(newPage.getId(), viewMode));
            })
        .map(responseUtils::updatePageDTOWithDefaultResources);
  }
  ...
  public Mono<Application> makePageDefault(
    ...
    return newPageService
        ...
        .flatMap(
            branchedPage -> makePageDefault(branchedPage.getApplicationId(), branchedPage.getId()))
              ...
              org -> {
                ...
                Set<Policy> documentPolicies =
                    policyGenerator.getAllChildPolicies(
                        org.getPolicies(), Workspace.class, Application.class);
    ...
    Mono<Application> applicationMono =
        applicationRepository
            ...
            .switchIfEmpty(
                Mono.error(
                    new AppsmithException(
                        AppsmithError.NO_RESOURCE_FOUND, FieldName.APPLICATION, id)))
            .cache();
        ...
        .flatMap(
            application -> {
              ...
              if (gitData != null
                ...
                Path repoPath =
                    Paths.get(
                        application.getWorkspaceId(), gitData.getDefaultApplicationId(), repoName);
  ...
  public Mono<PageDTO> clonePage(String pageId) {
    ...
    return newPageService
        ...
        .switchIfEmpty(
            Mono.error(new AppsmithException(AppsmithError.ACTION_IS_NOT_AUTHORIZED, "Clone Page")))
        .flatMap(
            page ->
                applicationService
                  ...
                  .flatMap(
                      action -> {
                        ...
                        action
                            ...
                            .setDefaultResources(clonedPageDefaultResources);
                        /*
                        ...
                        return sourceActionCollectionsFlux
                            .flatMap(
                                actionCollection -> {
                                  ...
                                  // map
                                  Map<String, String> updatedDefaultToBranchedActionId =
                                      new HashMap<>();
                                  ...
                                  if (StringUtils.isEmpty(
                                      clonedPageDefaultResources.getBranchName())) {
                                    unpublishedCollection
                                        .getDefaultToBranchedActionIdsMap()
                                        .forEach(
                                            (defaultId, oldActionId) -> {
                                              // Filter out the actionIds for which the reference is
                                              // not
                                              // present in cloned actions, this happens when we
                                              // have
  ...
  public Mono<Application> cloneApplication(String applicationId, String branchName) {
            ...
            .flatMap(
                tuple -> {
                  ...
                  return setApplicationPolicies(
                          userMono, sourceApplication.getWorkspaceId(), newApplication)
                      ...
                      .flatMap(
                          applicationUserTuple2 -> {
                            ...
                            application1.setModifiedBy(
                                applicationUserTuple2.getT2().getUsername()); // setting
                            // modified
                            // by to
                            // current
                            // user
                      ...
                      // Now fetch the pages of the source application, clone
                      // and add them to this new application
                      .flatMap(
                          savedApplication ->
                              Flux.fromIterable(sourceApplication.getPages())
                                  .flatMap(
                                      applicationPage -> {
                                        String pageId = applicationPage.getId();
                                        Boolean isDefault = applicationPage.getIsDefault();
                                        return this.clonePageGivenApplicationId(
                                                pageId, savedApplication.getId())
                                            .map(
                                                clonedPage -> {
                                                  ApplicationPage newApplicationPage =
                                                      new ApplicationPage();
                                                  ...
                                                  newApplicationPage.setDefaultPageId(
                                                      clonedPage.getId());
                      ...
                      // duplicate the source application's themes if required
                      // i.e. if they were customized
                      .flatMap(
                          application ->
                              themeService
                                  .cloneThemeToApplication(
                                      sourceApplication.getEditModeThemeId(), application)
                                  .zipWith(
                                      themeService.cloneThemeToApplication(
                      ...
                      .flatMap(
                          application ->
                              sendCloneApplicationAnalyticsEvent(sourceApplication, application));
                });
    ...
    return Mono.create(
        sink ->
            clonedResultMono.subscribe(sink::success, sink::error, null, sink.currentContext()));
  }

  /**
   * This function archives the unpublished page. This also archives the unpublished action. The
   * reason that the entire action is not deleted at this point is to handle the following edge case
   * : An application is published with 1 page and 1 action. Post publish, create a new page and
   * move the action from the existing page to the new page. Now delete this newly created page. In
   * this scenario, if we were to delete all actions associated with the page, we would end up
   * deleting an action which is currently in published state and is being used.
   *
  ...
  private Mono<PageDTO> deleteUnpublishedPageEx(String id, Optional<AclPermission> permission) {
    ...
    return newPageService
        ...
        .switchIfEmpty(
            Mono.error(new AppsmithException(AppsmithError.NO_RESOURCE_FOUND, FieldName.PAGE, id)))
        .flatMap(
            page -> {
              log.debug(
              ...
              /**
               * Only delete unpublished action and not the entire action. Also filter actions
               * embedded in actionCollection which will be deleted while deleting the collection,
               * this will avoid the race condition for delete action
               */
              Mono<List<ActionDTO>> archivedActionsMono =
                  newActionService
                      ...
                      .flatMap(
                          action -> {
                            log.debug(
                                "Going to archive actionId: {} for" + " applicationId: {}",
                                action.getId(),
                                id);
  ...
  public Mono<Application> publish(String applicationId, boolean isPublishedManually) {
    /**
     * Please note that it is a cached Mono, hence please be careful with using this Mono to update
     * / read data when latest updated application object is desired.
     */
    Mono<Application> applicationMono =
        applicationService
            .findById(applicationId, applicationPermission.getEditPermission())
        ...
        applicationMono
            ...
            .flatMap(
                application -> {
                  ...
                  /**
                   * Now add the published page ids and edited page ids into a single set and then
                   * remove the edited page ids to get a set of page ids which have been deleted in
                   * the edit mode. For example : Published page ids : [ A, B, C ] Edited Page ids :
                   * [ B, C, D ] aka A has been deleted and D has been added Step 1. Add both the
                   * ids into a single set : [ A, B, C, D] Step 2. Remove Edited Page Ids : [ A ]
    ...
    return publishApplicationAndPages
        .flatMap(
            newPages ->
                Mono.zip(
                    publishedActionsListMono, publishedActionCollectionsListMono, publishThemeMono))
        .then(
    ...
    return Mono.zip(
        ...
        .flatMap(
            objects -> {
              ...
              extraProperties.put(
                  "queryCount",
                  (dbQueryCount + apiCount + jsFuncCount + saasQueryCount + remoteQueryCount));
  ...
  public Mono<ApplicationPagesDTO> reorderPage(
    ...
    return newPageService
        ...
        .switchIfEmpty(
            Mono.error(
                new AppsmithException(
                    AppsmithError.NO_RESOURCE_FOUND, FieldName.PAGE, defaultPageId)))
        .zipWhen(
            branchedPage ->
                applicationService
                    .findById(
                        branchedPage.getApplicationId(), applicationPermission.getEditPermission())
                    .switchIfEmpty(
                        Mono.error(
                            new AppsmithException(
                                AppsmithError.NO_RESOURCE_FOUND,
  ...
  /**
   * This method will create a new suffixed application or update the existing application if there
   * is name conflict
   *
  ...
  public Mono<Application> createOrUpdateSuffixedApplication(
    ...
    return applicationMono
        .flatMap(
            application1 -> this.createOrUpdateSuffixedApplication(application, name, 1 + suffix))
        .switchIfEmpty(
            Mono.defer(
                () ->
