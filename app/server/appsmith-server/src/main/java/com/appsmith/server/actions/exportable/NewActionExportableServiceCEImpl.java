...
public class NewActionExportableServiceCEImpl implements ExportableServiceCE<Action> {

    private final ActionPermission actionPermission;
    protected final ArtifactBasedExportableService<Action, Application> applicationExportableService;

    @Override
    public ArtifactBasedExportableService<Action, ?> getArtifactBasedExportableService(
    ...
    public Mono<Void> getExportableEntities(
        ...
        ArtifactBasedExportableService<Action, ?> artifactBasedExportableService =
        ...
        Flux<Action> actionFlux = artifactBasedExportableService.findByContextIdsForExport(
        ...
        return actionFlux
                ...
                .flatMap(newActionList -> {
                    ...
                    List<Action> exportableActions = getExportableNewActions(newActionList);
                    return Mono.zip(Mono.just(exportableActions), Mono.just(dbNamesUsedInActions));
                })
                .map(tuple -> {
                    List<Action> actionList = tuple.getT1();
    ...
    protected List<Action> getExportableNewActions(List<Action> actionList) {
        return actionList;
    }
    ...
    protected boolean hasExportableDatasource(Action action) {
        return true;
    }
}
