...
public class UserServiceCEImpl extends BaseService<UserRepository, User, String>
  ...
  /**
   * This function switches the user's currentWorkspace in the User collection in the DB. This means
   * that on subsequent logins, the user will see applications for their last used workspace.
   *
   * @param workspaceId
   * @return
   */
              ...
              if (workspaceIds == null || workspaceIds.isEmpty()) {
                return Mono.error(
                    new AppsmithException(
                        AppsmithError.USER_DOESNT_BELONG_ANY_WORKSPACE, user.getId()));
              }
  ...
  /**
   * This function creates a one-time token for resetting the user's password. This token is stored
   * in the `passwordResetToken` collection with an expiry time of 48 hours. The user must provide
   * this one-time token when updating with the new password.
   *
  ...
  public Mono<Boolean> forgotPasswordTokenGenerate(ResetUserPasswordDTO resetUserPasswordDTO) {
    ...
    return repository
        ...
        .flatMap(
            passwordResetToken -> {
              log.debug(
                  "Password reset Token: {} for email: {}", token, passwordResetToken.getEmail());
              ...
              log.debug(
                  "Password reset url for email: {}: {}", passwordResetToken.getEmail(), resetUrl);
   ...
   *
   * @param encryptedToken The one-time token provided to the user for resetting the password
   * @param user The user object that contains the email & password fields in order to save the new
            ...
            passwordResetToken -> {
              boolean matches =
                  this.passwordEncoder.matches(
                      emailTokenDTO.getToken(), passwordResetToken.getTokenHash());
              if (!matches) {
        ...
        .flatMap(
            savedPermissionGroup -> {
              ...
              User updatedWithPolicies =
                  policyUtils.addPoliciesToExistingObject(crudUserPolicies, savedUser);
  ...
  /**
   * This function creates a new user in the system. Primarily used by new users signing up for the
   * first time on the platform. This flow also ensures that a default workspace name is created for
   * the user. The new user is then given admin permissions to the default workspace.
   *
    ...
    return repository
        ...
        .flatMap(
            savedUser -> {
              ...
              return Mono.error(
                  new AppsmithException(
                      AppsmithError.USER_ALREADY_EXISTS_SIGNUP, savedUser.getUsername()));
            })
        .switchIfEmpty(
            Mono.defer(
                () -> {
                  return signupIfAllowed(user)
                      ...
                      .flatMap(
                          userSignupDTO ->
                              findByEmail(userSignupDTO.getUser().getEmail())
                                  .map(
                                      user1 -> {
                      ...
                      .map(
                          pair -> {
                            log.debug(
                                "UserServiceCEImpl::Time taken to" + " find created user: {} ms",
                                pair.getT1());
                            return pair.getT2();
                          });
                }))
  ...
  public Mono<User> update(String id, User userUpdate) {
    Mono<User> userFromRepository =
        repository
            ...
            .switchIfEmpty(
                Mono.error(
                    new AppsmithException(AppsmithError.NO_RESOURCE_FOUND, FieldName.USER, id)));
  ...
  public Mono<User> updateCurrentUser(final UserUpdateDTO allUpdates, ServerWebExchange exchange) {
    ...
    if (allUpdates.hasUserUpdates()) {
      ...
      updatedUserMono =
          sessionUserService.getCurrentUser().flatMap(user -> findByEmail(user.getEmail()));
    }
