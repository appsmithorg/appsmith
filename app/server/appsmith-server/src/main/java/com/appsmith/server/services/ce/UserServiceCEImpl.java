...
@Slf4j
public class UserServiceCEImpl extends BaseService<UserRepository, User, String>
    ...
    private static final String FORGOT_PASSWORD_EMAIL_TEMPLATE =
            "email/forgotPasswordTemplate.html";
    private static final String FORGOT_PASSWORD_CLIENT_URL_FORMAT =
            "%s/user/resetPassword?token=%s";
    ...
    private static final Pattern ALLOWED_ACCENTED_CHARACTERS_PATTERN =
    ...
    /**
     * This function switches the user's currentWorkspace in the User collection in the DB. This
     * means that on subsequent logins, the user will see applications for their last used
     * workspace.
     *
     * @param workspaceId
     * @return
    ...
    public Mono<User> switchCurrentWorkspace(String workspaceId) {
        if (workspaceId == null || workspaceId.isEmpty()) {
            return Mono.error(
                    new AppsmithException(AppsmithError.INVALID_PARAMETER, "workspaceId"));
        }
        return sessionUserService
                ...
                .flatMap(
                        user -> {
                            log.debug(
                                    "Going to set workspaceId: {} for user: {}",
                                    workspaceId,
                                    user.getId());
                            ...
                            if (workspaceIds == null || workspaceIds.isEmpty()) {
                                return Mono.error(
                                        new AppsmithException(
                                                AppsmithError.USER_DOESNT_BELONG_ANY_WORKSPACE,
                                                user.getId()));
                            }

                            Optional<String> maybeWorkspaceId =
                                    workspaceIds.stream()
                                            .filter(
                                                    workspaceId1 ->
                                                            workspaceId1.equals(workspaceId))
                                            .findFirst();
                            ...
                            // Throw an exception if the workspaceId is not part of the user's
                            // workspaces
                            return Mono.error(
                                    new AppsmithException(
                                            AppsmithError.USER_DOESNT_BELONG_TO_WORKSPACE,
                                            user.getId(),
                                            workspaceId));
                        });
    }
    ...
    /**
     * This function creates a one-time token for resetting the user's password. This token is
     * stored in the `passwordResetToken` collection with an expiry time of 48 hours. The user must
     * provide this one-time token when updating with the new password.
     *
    ...
    public Mono<Boolean> forgotPasswordTokenGenerate(ResetUserPasswordDTO resetUserPasswordDTO) {
        if (resetUserPasswordDTO.getEmail() == null || resetUserPasswordDTO.getEmail().isBlank()) {
            return Mono.error(
                    new AppsmithException(AppsmithError.INVALID_PARAMETER, FieldName.EMAIL));
        }
        ...
        // Check if the user exists in our DB. If not, we will not send a password reset link to the
        // user
        return repository
                .findByEmail(email)
                ...
                .switchIfEmpty(
                        Mono.error(
                                new AppsmithException(
                                        AppsmithError.NO_RESOURCE_FOUND, FieldName.USER, email)))
                .flatMap(
                        user -> {
                                    ...
                                    .switchIfEmpty(
                                            Mono.defer(
                                                    () -> {
                                                        ...
                                                        passwordResetToken.setEmail(
                                                                user.getEmail());
                                                        passwordResetToken.setRequestCount(0);
                                                        passwordResetToken.setFirstRequestTime(
                                                                Instant.now());
                ...
                .flatMap(
                        passwordResetToken -> {
                            ...
                            nameValuePairs.add(
                                    new BasicNameValuePair("email", passwordResetToken.getEmail()));
                            ...
                            return updateTenantLogoInParams(
                                            params, resetUserPasswordDTO.getBaseUrl())
                                    .flatMap(
                                            updatedParams ->
    ...
    /**
     * This method checks whether the reset request limit has been exceeded. If the limit has been
     * exceeded, it raises an Exception. Otherwise, it'll update the counter and date in the
     * resetToken object
     *
    ...
    /**
     * This function verifies if the password reset token and email match each other. Should be
     * initiated after the user has already initiated a password reset request via the 'Forgot
     * Password' link. The tokens are stored in the DB using BCrypt hash.
     *
     * @param encryptedToken The one-time token provided to the user for resetting the password
     * @return Publishes a boolean indicating whether the given token is valid for the given email
     *     address
     */
    @Override
    public Mono<Boolean> verifyPasswordResetToken(String encryptedToken) {
        ...
        try {
            ...
            return Mono.error(
                    new AppsmithException(AppsmithError.INVALID_PARAMETER, FieldName.TOKEN));
        }

        return passwordResetTokenRepository
                .findByEmail(emailTokenDTO.getEmail())
                .switchIfEmpty(
                        Mono.error(new AppsmithException(AppsmithError.INVALID_PASSWORD_RESET)))
                .map(
                        obj ->
                                this.passwordEncoder.matches(
                                        emailTokenDTO.getToken(), obj.getTokenHash()));
    }

    /**
     * This function resets the password using the one-time token & email of the user. This function
     * can only be called via the forgot password route.
     *
     * @param encryptedToken The one-time token provided to the user for resetting the password
     * @param user The user object that contains the email & password fields in order to save the
                ...
                .flatMap(
                        emailAddress ->
                                repository
                                        ...
                                        .flatMap(
                                                userFromDb -> {
                                                    if (!ValidationUtils.validateLoginPassword(
                                                            user.getPassword())) {
                                                        return Mono.error(
                                                                new AppsmithException(
                                                                        AppsmithError
                                                    ...
                                                    // User has verified via the forgot password
                                                    // token verfication route. Allow the user to
                                                    // set new password.
                                                    ...
                                                    // If the user has been invited but has not
                                                    // signed up yet, and is following the route of
                                                    // reset
                                                    // password flow to set up their password,
                                                    // enable the user's account as well
    ...
    public Mono<User> userCreate(User user, boolean isAdminUser) {
                ...
                Mono.just(user)
                        .flatMap(
                                userBeforeSave -> {
                                    ...
                                    // The tenant has been set already. No need to set the default
                                    // tenant id.
        ...
        return userWithTenantMono
                ...
                .map(
                        pair -> {
                            log.debug(
                                    "UserServiceCEImpl::Time taken to save user: {} ms",
                                    pair.getT1());
                            return pair.getT2();
                        })
                ...
                .map(
                        pair -> {
                            log.debug(
                                    "UserServiceCEImpl::Time taken to create user role: {} ms",
                                    pair.getT1());
                            return pair.getT2();
                        })
                .then(
                ...
                .map(
                        pair -> {
                            log.debug(
                                    "UserServiceCEImpl::Time taken to identify user: {} ms",
                                    pair.getT1());
                            return pair.getT2();
                        });
    }
    ...
    private Mono<User> addUserPolicies(User savedUser, Boolean isAdminUser) {
        ...
        userManagementPermissionGroup.setName(
                savedUser.getUsername() + FieldName.SUFFIX_USER_MANAGEMENT_ROLE);
        // Add CRUD permissions for user to the group
        userManagementPermissionGroup.setPermissions(
                Set.of(new Permission(savedUser.getId(), MANAGE_USERS)));
        ...
        return permissionGroupService
                .save(userManagementPermissionGroup)
                .flatMap(
                        savedPermissionGroup -> {
                            Map<String, Policy> crudUserPolicies =
                                    policyUtils.generatePolicyFromPermissionGroupForObject(
                                            savedPermissionGroup, savedUser.getId());

                            User updatedWithPolicies =
                                    policyUtils.addPoliciesToExistingObject(
                                            crudUserPolicies, savedUser);
                ...
                .flatMap(
                        crudUser -> {
                            if (isAdminUser) {
                                return userUtils
                                        .makeSuperUser(List.of(crudUser))
    ...
    /**
     * This function creates a new user in the system. Primarily used by new users signing up for
     * the first time on the platform. This flow also ensures that a default workspace name is
     * created for the user. The new user is then given admin permissions to the default workspace.
     *
     * <p>For new user invite flow, please {@link
     * com.appsmith.server.solutions.UserAndAccessManagementService#inviteUsers(InviteUsersDTO,
     * String)}
     *
    ...
    public Mono<UserSignupDTO> createUserAndSendEmail(User user, String originHeader) {
        ...
        // If the user doesn't exist, create the user. If the user exists, return a duplicate key
        // exception
        return repository
                ...
                .flatMap(
                        savedUser -> {
                            ...
                            return Mono.error(
                                    new AppsmithException(
                                            AppsmithError.USER_ALREADY_EXISTS_SIGNUP,
                                ...
                                () -> {
                                    return signupIfAllowed(user)
                                            .flatMap(
                                                    savedUser -> {
                                                        ...
                                                        return workspaceService
                                                                .createDefault(
                                                                        new Workspace(), savedUser)
                                                                .elapsed()
                                                                .map(
                                                                        pair -> {
                                                                            log.debug(
                                                                                    "UserServiceCEImpl::Time"
                                                                                        + " taken"
                                                                                        + " to create"
                                                                                        + " default"
                                                                                        + " workspace:"
                                            ...
                                            .map(
                                                    pair -> {
                                                        log.debug(
                                                                "UserServiceCEImpl::Time taken to"
                                                                    + " find created user: {} ms",
                                                                pair.getT1());
                                                        return pair.getT2();
                                                    });
                                }))
    ...
    private Mono<User> signupIfAllowed(User user) {
        ...
        if (!commonConfig.getAdminEmails().contains(user.getEmail())) {
            // If this is not an admin email address, only then do we check if signup should be
            // allowed or not. Being an
            // explicitly set admin email address trumps all everything and signup for this email
            // can never be disabled.

            if (commonConfig.isSignupDisabled()) {
                // Signing up has been globally disabled. Reject.
                return Mono.error(
                        new AppsmithException(AppsmithError.SIGNUP_DISABLED, user.getUsername()));
            }

            final List<String> allowedDomains =
                    user.getSource() == LoginSource.FORM
            ...
            if (!CollectionUtils.isEmpty(allowedDomains)
                ...
                // There is an explicit whitelist of email address domains that should be allowed.
                // If the new email is
        ...
        return userCreate(user, isAdminUser)
                ...
                .map(
                        pair -> {
                            log.debug(
                                    "UserServiceCEImpl::Time taken for create user: {} ms",
                                    pair.getT1());
                            return pair.getT2();
                        });
    }
    ...
    public Mono<User> sendWelcomeEmail(User user, String originHeader) {
        ...
        return updateTenantLogoInParams(params, originHeader)
                ...
                .map(
                        pair -> {
                            log.debug(
                                    "UserServiceCEImpl::Time taken to send email: {} ms",
                                    pair.getT1());
                            return pair.getT2();
                        })
                .onErrorResume(
                        error -> {
                            // Swallowing this exception because we don't want this to affect the
                            // rest of the flow.
                            log.error(
                                    "Ignoring error: Unable to send welcome email to the user {}."
                                            + " Cause: ",
                                    user.getEmail(),
    ...
    public Mono<? extends User> createNewUserAndSendInviteEmail(
        ...
        // The invite token is not used today and doesn't need to be verified. We still save the
        // invite token with the
        ...
        // Call user service's userCreate function so that the default workspace, etc are also
        // created along with assigning basic permissions.
        return userCreate(newUser, isAdminUser)
                .flatMap(
                        createdUser -> {
                            log.debug(
                                    "Going to send email for invite user to {}",
                            ...
                            Map<String, String> params =
                                    getEmailParams(workspace, inviter, inviteUrl, true);
    ...
    public Mono<User> updateCurrentUser(
            final UserUpdateDTO allUpdates, ServerWebExchange exchange) {
    ...
    public Map<String, String> getEmailParams(
            Workspace workspace, User inviter, String inviteUrl, boolean isNewUser) {
        Map<String, String> params = new HashMap<>();
        ...
        if (inviter != null) {
            params.put(
                    ...
                    org.apache.commons.lang3.StringUtils.defaultIfEmpty(
                            inviter.getName(), inviter.getEmail()));
        }
        if (workspace != null) {
                        ...
                        tuple -> {
                            ...
                            profile.setIntercomConsentGiven(
                                    commonConfig.isCloudHosting()
                                            ? true
                                            : userData.isIntercomConsentGiven());
                            profile.setSuperUser(isSuperUser);
                            profile.setConfigurable(
                                    !StringUtils.isEmpty(commonConfig.getEnvFilePath()));
    ...
    private EmailTokenDTO parseValueFromEncryptedToken(String encryptedToken) {
        ...
        List<NameValuePair> nameValuePairs =
                URLEncodedUtils.parse(decryptString, StandardCharsets.UTF_8);
        Map<String, String> params = new HashMap<>();
    ...
    @Override
    public Mono<Map<String, String>> updateTenantLogoInParams(
            Map<String, String> params, String origin) {
