			...
			emailAddress ->
				repository
					...
					.flatMap(
						...
						// User has verified via the forgot password token verfication route.
						// Allow the user to set new password.
						userFromDb.setPasswordResetInitiated(false);
						userFromDb.setPassword(passwordEncoder.encode(user.getPassword()));

						// If the user has been invited but has not signed up yet, and is
						// following the route of reset
						// password flow to set up their password, enable the user's account as
						// well
						...
						return passwordResetTokenRepository
							...
							.doOnSuccess(
								result ->
									// In a separate thread, we delete all other sessions of this
									// user.
									sessionUserService
						...
						savedUser -> {
							...
							return workspaceService
								...
								.map(
									pair -> {
									log.debug(
										"UserServiceCEImpl::Time taken to create default"
											+ " workspace: {} ms",
										pair.getT1());
									return pair.getT2();
									})
...
private Mono<User> signupIfAllowed(User user) {
	...
	// If this is not an admin email address, only then do we check if signup should be allowed or
	// not. Being an
	// explicitly set admin email address trumps all everything and signup for this email can
	// never be disabled.
	...
	if (!CollectionUtils.isEmpty(allowedDomains)
		...
		// There is an explicit whitelist of email address domains that should be allowed. If the
		// new email is
...
public Mono<User> sendWelcomeEmail(User user, String originHeader) {
	...
	return updateTenantLogoInParams(params, originHeader)
		...
		.onErrorResume(
			error -> {
			// Swallowing this exception because we don't want this to affect the rest of the
			// flow.
			log.error(
...
public Mono<? extends User> createNewUserAndSendInviteEmail(
	...
	// The invite token is not used today and doesn't need to be verified. We still save the invite
	// token with the
	...
	// Call user service's userCreate function so that the default workspace, etc are also created
	// along with assigning basic permissions.
