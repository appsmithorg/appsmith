...
protected void createPermissionGroupPolicyGraph() {
	...
	lateralGraph.addEdge(
		AclPermission.MANAGE_PERMISSION_GROUPS, AclPermission.READ_PERMISSION_GROUP_MEMBERS);
	lateralGraph.addEdge(
		AclPermission.ASSIGN_PERMISSION_GROUPS, AclPermission.READ_PERMISSION_GROUP_MEMBERS);
}
...
* aclPermission and user Should be used in places where we are creating a document to ensure that
* the correct permissions are assigned to the new document.
*
* @param policy
* @param aclPermission
...
public Set<Policy> getChildPolicies(
	...
	// In case the calling function could not translate the string value to AclPermission, return an
	// empty set to handle
	// erroneous cases
	...
	childPolicySet.addAll(
		getLateralPolicies(childPermission, policy.getPermissionGroups(), destinationEntity));
	}
