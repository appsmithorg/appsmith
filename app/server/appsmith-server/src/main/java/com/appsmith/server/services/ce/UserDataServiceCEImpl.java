...
public Mono<UserData> getForUser(String userId) {
	// If an entry for this userId doesn't exist in the database, we return an empty object, since
	// data in this
	// collection is treated to be sparse. That is, missing objects in the database are the same as
	// empty objects.
		...
		.flatMap(
			user ->
				tenantService
					...
					.flatMap(
						tenantId ->
							userRepository.findByEmailAndTenantId(user.getEmail(), tenantId)))
...
public Mono<UserData> update(String userId, UserData resource) {
	...
	return Mono.error(
		new AppsmithException(
			AppsmithError.INVALID_PARAMETER, fieldName(QUserData.userData.userId)));
	}
	...
	return Mono.justOrEmpty(user.getId())
		.switchIfEmpty(
			tenantService
				.getDefaultTenantId()
				.flatMap(
					tenantId -> userRepository.findByEmailAndTenantId(user.getEmail(), tenantId))
...
public Mono<UserData> saveProfilePhoto(Part filePart) {
	...
	final Mono<Asset> uploaderMono =
		assetService.upload(List.of(filePart), MAX_PROFILE_PHOTO_SIZE_KB, true);
...
/**
* The application.workspaceId is prepended to the list {@link
* UserData#getRecentlyUsedWorkspaceIds}. The application.id is prepended to the list {@link
...
/**
* Removes provided workspace id and all other application id under that workspace from the user
* data
*
...
public Mono<UpdateResult> removeRecentWorkspaceAndApps(String userId, String workspaceId) {
	return applicationRepository
		...
		.flatMap(
			appIdsList -> repository.removeIdFromRecentlyUsedList(userId, workspaceId, appIdsList));
}
}
