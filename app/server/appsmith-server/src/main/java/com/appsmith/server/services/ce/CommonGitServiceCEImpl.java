...
public class CommonGitServiceCEImpl implements CommonGitServiceCE {
    ...
    private final static String REMOTE_NAME_REPLACEMENT = "";
        ...
        */
        Mono<GitStatusDTO> statusMono = Mono.zip(getGitArtifactMetadata(defaultArtifactId, artifactJsonType), branchedAppMono)
            .flatMap(tuple2 -> {
                ...
                return Mono.zip(Mono.just(gitArtifactMetadata), Mono.just(branchedArtifact), exportedArtifactJsonMono);
            })
            ...
            .flatMap(tuple -> {
                ...
                Path repoSuffix = Paths.get(exportableArtifact.getWorkspaceId(), gitData.getDefaultApplicationId(), gitData.getRepoName());

                try {
                    // Create a Mono to fetch the status from remote
                    Path repoSuffixForFetchRemote = Paths.get(
                        exportableArtifact.getWorkspaceId(), gitData.getDefaultApplicationId(), gitData.getRepoName());
                    ...
                    return Mono.zip(
                        gitFileUtils.saveApplicationToLocalRepoWithAnalytics(
