...
public Mono<Application> updateGitMetadata(
	...
	// For default application we expect a GitAuth to be a part of gitMetadata. We are using save
	// method to leverage
...
public Mono<Map<String, GitProfile>> updateOrCreateGitProfileForCurrentUser(
	...
	// 1. Updating or creating global git profile (defaultApplicationId = "default") and update is
	// made with empty
	//    authorName or authorEmail
	// 2. Updating or creating repo specific profile and user want to use repo specific profile but
	// provided empty
			...
			user ->
				userDataService
					...
					.switchIfEmpty(
						Mono.defer(
							() -> {
							// If profiles are empty use Appsmith's user profile as git default
							// profile
	...
	Mono<String> commitMono =
		this.getApplicationById(defaultApplicationId)
			...
			.flatMap(
				...
				// Check if the repo is public for current application and if the user have
				// changed the access after
				// the connection
				...
				return GitUtils.isRepoPrivate(defaultGitMetadata.getBrowserSupportedRemoteUrl())
					.flatMap(
						isPrivate -> {
							// Check the repo limit if the visibility status is updated, or it is
							// private
							...
							return applicationService
								...
								// Check if the private repo count is less than the allowed repo
								// count
			...
			.flatMap(
				...
				String status = tuple.getT1();
				Application childApplication = tuple.getT2();
				// Update json schema versions so that we can detect if the next update was made
				// by DB migration or by the user
				Application update = new Application();
				// Reset migration related fields before commit to detect the updates correctly
				// between the commits
								...
								return fileUtils
									...
									.flatMap(
										app -> {
											...
											// To catch TransportHttp cast error in case HTTP URL
											// is passed instead of SSH URL
					...
					return Mono.zip(
							fileUtils
								...
								.onErrorMap(
									throwable -> {
									log.error(
										"Error while initialising git repo, {0}", throwable);
						...
						.flatMap(
							ignore -> {
							// Commit and push application to check if the SSH key has the write
							// access
							...
							return this.commitApplication(
								...
								.onErrorResume(
									error ->
										// If the push fails remove all the cloned files from
										// local repo
		...
		publishAndOrGetApplication(applicationId, doPublish)
			...
			.flatMap(
				...
				if (pushResult.contains("REJECTED_NONFASTFORWARD")) {
					...
					// Mostly happens when the remote branch is protected or any specific rules in
					// place on the branch
					// Since the users will be in a bad state where the changes are committed
					// locally but they are not able to
						...
						.then(
							Mono.error(
								new AppsmithException(
									AppsmithError.GIT_ACTION_FAILED,
									"push",
									"Unable to push changes as pre-receive hook declined. Please"
										+ " make sure that you don't have any rules enabled on the"
										+ " branch "
										+ application
											.getGitApplicationMetadata()
											.getBranchName())));
...
public Mono<Application> detachRemote(String defaultApplicationId) {
	...
	Mono<Application> disconnectMono =
		getApplicationById(defaultApplicationId)
			...
			.flatMap(
				application ->
					// Update all the resources to replace defaultResource Ids with the resource Ids
					// as branchName
					// will be deleted
		...
		applicationService
			...
			.flatMap(
				...
				if (srcBranchGitData == null
					...
					return Mono.error(
						new AppsmithException(
							AppsmithError.INVALID_GIT_CONFIGURATION,
							"Unable to find the parent branch. Please create a branch from other"
								+ " available branches"));
				}
				Path repoSuffix =
					Paths.get(
				...
				return addFileLock(srcBranchGitData.getDefaultApplicationId())
					...
					.flatMap(
						branchName -> {
							...
							// Save a new application in DB and update from the parent branch
							// application
			...
			.flatMap(
				...
				return importExportApplicationService
					...
					.flatMap(
						application -> {
							// Commit and push for new branch created this is to avoid issues when
...
public Mono<Application> checkoutBranch(String defaultApplicationId, String branchName) {
	...
	// If the user is trying to check out remote branch, create a new branch if the branch does not
	// exist already
			...
			.flatMap(
				...
				* */
				...
				return applicationService
					...
					// if user switches default branch and tries to delete the default branch we
					// do not delete resource from db
					// This is an exception only for the above case and in such case if the user
					// tries to check out the branch again
					// It results in an error as the resources are already present in db
					// So we just rehydrate from the file system to the existing resource on the
					// db
					.onErrorResume(
						throwable -> {
							...
							log.error(
								" Git checkout remote branch failed {}", throwable.getMessage());
							return Mono.error(
								new AppsmithException(
									AppsmithError.GIT_ACTION_FAILED,
									" --checkout",
				...
				if (!CollectionUtils.isNullOrEmpty(status.getModified())) {
					return Mono.error(
						new AppsmithException(
							AppsmithError.GIT_ACTION_FAILED,
							"pull",
							"There are uncommitted changes present in your local. Please commit"
								+ " them first and then try git pull"));
				}
		...
		getApplicationById(defaultApplicationId)
			.flatMap(
				...
				if (Boolean.TRUE.equals(pruneBranches)) {
					...
					// Fetch default branch from DB if the pruneBranches is false else fetch from
					// remote
					gitBranchDTOMono =
						gitExecutor.listBranches(
							repoPath,
				...
				if (Boolean.TRUE.equals(pruneBranches)) {
					...
					return applicationService
						.findByBranchNameAndDefaultApplicationId(
							...
							defaultApplicationId,
							applicationPermission.getEditPermission())
						// Check if the branch is already present, If not follow checkout remote
						// flow
						.onErrorResume(
							throwable ->
	...
	Mono<GitStatusDTO> statusMono =
		getGitApplicationMetadata(defaultApplicationId)
			.flatMap(
				...
				if (Boolean.TRUE.equals(isFileLock)) {
					// Add file lock for the status API call to avoid sending wrong info on the
					// status
					return redisUtils
			...
			.flatMap(
				...
				return Mono.zip(Mono.just(defaultApplication), pathToFile)
					.onErrorResume(
						error -> {
							log.error(
								"Error in repo status check for application "
									+ defaultApplicationId,
								error);
			...
			.flatMap(
				...
				return redisUtils
					...
					.onErrorResume(
						error -> {
							log.debug(
								"Error in merge status check application " + defaultApplicationId,
								error);
							if (error instanceof AppsmithException) {
							return Mono.error(error);
							}
		...
		workspaceMono
			...
			.switchIfEmpty(
				Mono.error(
					new AppsmithException(
						AppsmithError.INVALID_GIT_CONFIGURATION,
						"Unable to find git configuration for logged-in user. Please contact"
							+ " Appsmith team for support")))
			// Check the limit for number of private repo
			.flatMap(
				tuple -> {
				// Check if the repo is public
				...
				Mono<String> defaultBranchMono =
					gitExecutor
						...
						.onErrorResume(
							error -> {
								log.error(
									"Error while cloning the remote repo, {}", error.getMessage());
								return addAnalyticsForGitOperation(
										AnalyticsEvents.GIT_IMPORT.getEventName(),
										application,
										error.getClass().getName(),
				...
				Mono<ApplicationJson> applicationJsonMono =
					fileUtils
						...
						.onErrorResume(
							error -> {
								log.error(
									"Error while constructing application from git repo", error);
				...
				return Mono.zip(applicationJsonMono, datasourceMono, pluginMono)
					.flatMap(
						data -> {
							...
							// If there is an existing datasource with the same name but a different
							// type from that in the repo, the import api should fail
...
private boolean checkIsDatasourceNameConflict(
	...
	// If we have an existing datasource with the same name but a different type from that in the
	// repo, the import api should fail
							...
							.flatMap(
								branchName ->
									applicationService
										...
										.flatMap(
											application1 -> {
											// Add the locally checked out branch to the
											// branchList
										...
										// Return empty mono when the branched application is not in
										// db
