...
public class GitServiceCEImpl implements GitServiceCE {
        ...
        .map(
            tuple -> {
              ...
              if (!CollectionUtils.isNullOrEmpty(userData.getGitProfiles())) {
                ...
                gitProfiles.put(
                    defaultApplicationId, userData.getGitProfileByKey(defaultApplicationId));
              }
              if (gitData == null) {
  ...
  public Mono<Map<String, GitProfile>> updateOrCreateGitProfileForCurrentUser(
    ...
    if (DEFAULT.equals(defaultApplicationId)) {
      ...
      if (!Boolean.TRUE.equals(gitProfile.getUseGlobalProfile())) {
                        ...
                        userData -> {
                          ...
                          if (savedProfile == null
                            ...
                            userData.setGitProfiles(
                                userData.setGitProfileByKey(defaultApplicationId, gitProfile));
                            ...
                            if (defaultGitProfile == null) {
                              ...
                              userData.setGitProfiles(
                                  userData.setGitProfileByKey(DEFAULT, userProfile));
                            }
  ...
  /**
   * This method will make a commit to local repo This is used directly from client, and we need to
   * acquire file lock before starting to keep the application in a sane state
   *
  ...
  /**
   * This method will make a commit to local repo and is used internally in flows like create, merge
   * branch Since the lock is already acquired by the other flows, we do not need to acquire file
   * lock again
   *
   * @param commitDTO information required for making a commit
   * @param defaultApplicationId application branch on which the commit needs to be done
                                  ...
                                  .flatMap(
                                      isRepoLimitReached -> {
                                        ...
                                        throw new AppsmithException(
                                            AppsmithError.GIT_APPLICATION_LIMIT_ERROR);
                                      });
                            } else {
                              return Mono.just(defaultApplication);
                            }
                ...
                branchedApplication -> {
                  ...
                  if (StringUtils.isEmptyOrNull(gitApplicationMetadata.getBranchName())) {
                    ...
                    if (StringUtils.isEmptyOrNull(
                      ...
                      if (StringUtils.isEmptyOrNull(gitApplicationMetadata.getRepoName())) {
            ...
            .onErrorResume(
                e -> {
                  ...
                  if (e instanceof RepositoryNotFoundException) {
                    return Mono.error(
                        new AppsmithException(
                            AppsmithError.REPOSITORY_NOT_FOUND, defaultApplicationId));
                  } else {
                    ...
                    return addAnalyticsForGitOperation(
                        ...
                        .flatMap(
                            user ->
                                Mono.error(
                                    new AppsmithException(
                                        AppsmithError.INVALID_GIT_CONFIGURATION, errorMessage)));
                  }
                  ...
                  return gitExecutor
                      ...
                      .onErrorResume(
                          e ->
                              Mono.error(
                                  new AppsmithException(
                                      AppsmithError.GIT_ACTION_FAILED, "log", e.getMessage())));
                });
   ...
   * perform all the git operation for an application We are implementing the deployKey approach and
   * since the deploy-keys are repo level these keys are store under application. Each application
   * is equal to a repo in the git(and each branch creates a new application with default
    ...
    Mono<UserData> currentUserMono =
        userDataService
            ...
            .switchIfEmpty(
                Mono.error(
                    new AppsmithException(
                        AppsmithError.INVALID_GIT_CONFIGURATION, GIT_PROFILE_ERROR)));
            ...
            .flatMap(
                application -> {
                  ...
                  if (isInvalidDefaultApplicationGitMetadata(
                    ...
                    Path repoSuffix =
                        Paths.get(application.getWorkspaceId(), defaultApplicationId, repoName);
                    Mono<String> defaultBranchMono =
                        gitExecutor
                            ...
                            .onErrorResume(
                                error -> {
                                  log.error(
                                      "Error while cloning" + " the remote" + " repo, ", error);
                                  return fileUtils
                                      ...
                                      .flatMap(
                                          app -> {
                                            ...
                                            if (error instanceof TimeoutException) {
                                              return Mono.error(
                                                  new AppsmithException(
                                                      AppsmithError.GIT_EXECUTION_TIMEOUT));
                                            }
                                            if (error instanceof ClassCastException) {
                                              // To
                                              // catch
                                              // TransportHttp cast error in case HTTP URL is passed
                                              // instead of SSH URL
                    ...
                    return Mono.zip(
                            fileUtils
                                ...
                                .onErrorMap(
                                    throwable -> {
                                      log.error(
                                          "Error while" + " initialising" + " git repo," + " {0}",
                                          throwable);
                            ...
                            tuple -> {
                              ...
                              return gitExecutor.commitApplication(
                                  ...
                                  DEFAULT_COMMIT_MESSAGE
                                      + GitDefaultCommitMessage.CONNECT_FLOW.getReason(),
  ...
  private Mono<String> pushApplication(String applicationId, boolean doPublish) {
                  ...
                  return applicationService
                      ...
                      .map(
                          defaultApp -> {
                            application
                                .getGitApplicationMetadata()
                                .setGitAuth(defaultApp.getGitApplicationMetadata().getGitAuth());
                            return application;
                          });
                })
            .flatMap(
                application -> {
                  ...
                  return gitExecutor
                      .checkoutToBranch(
                          baseRepoSuffix, application.getGitApplicationMetadata().getBranchName())
                      .then(
                          gitExecutor
                              .pushApplication(
                                  baseRepoSuffix,
                  ...
                  if (pushResult.contains("REJECTED_NONFASTFORWARD")) {
                    ...
                    return addAnalyticsForGitOperation(
                        ...
                        .flatMap(
                            application1 ->
                                Mono.error(
                                    new AppsmithException(AppsmithError.GIT_UPSTREAM_CHANGES)));
                  } else {
                    if (pushResult.contains("REJECTED_OTHERREASON")
                        || pushResult.contains("pre-receive hook declined")) {
                      // Mostly happens when the remote branch is protected or
                      // any specific rules in place on the branch
                      // Since the users will be in a bad state where the
                      // changes are committed locally but they are not able
                      // to
    ...
    return Mono.create(
        sink -> pushStatusMono.subscribe(sink::success, sink::error, null, sink.currentContext()));
  }
                                  ...
                                  .flatMap(
                                      branchList -> {
                                        boolean isDuplicateName =
                                            branchList.stream()
                                                // We are only supporting origin as the remote name
                                                // so this is safe
                                                //  but needs to be altered if we start supporting
                                                // user defined remote names
                                                .anyMatch(
                                                    branch ->
                                                        branch
                                                            .getBranchName()
              ...
              gitBranchDTOList -> {
                long branchMatchCount =
                    gitBranchDTOList.stream()
                        .filter(
                            gitBranchDTO -> gitBranchDTO.getBranchName().equals(finalBranchName))
                        .count();
        ...
        getApplicationById(defaultApplicationId)
                          ...
                          application1 ->
                              fileUtils
                                  .reconstructApplicationJsonFromGitRepo(
                                      srcApplication.getWorkspaceId(), defaultApplicationId,
                                      srcApplication.getGitApplicationMetadata().getRepoName(),
                                          branchName)
            ...
            .flatMap(
                tuple -> {
                  ...
                  return importExportApplicationService
                      .importApplicationInWorkspace(
                          application.getWorkspaceId(), applicationJson,
                          application.getId(), branchName)
                      .flatMap(
                          application1 ->
                              addAnalyticsForGitOperation(
  ...
  /**
   * Method to pull application json files from remote repo, make a commit with the changes present
   * in local DB and make a system commit to remote repo
   *
  ...
  public Mono<GitPullDTO> pullApplication(String defaultApplicationId, String branchName) {
    ...
    return Mono.create(
        sink -> pullMono.subscribe(sink::success, sink::error, null, sink.currentContext()));
  }
  ...
  public Mono<List<GitBranchDTO>> listBranchForApplication(
                      ...
                      .onErrorResume(
                          error -> {
                            if (error instanceof RepositoryNotFoundException) {
                              Mono<List<GitBranchDTO>> branchListMono =
                                  handleRepoNotFoundException(defaultApplicationId);
                              return Mono.zip(
                                  branchListMono, Mono.just(application), Mono.just(repoPath));
                            }
                            return Mono.error(
                                new AppsmithException(
                                    AppsmithError.GIT_ACTION_FAILED,
                        ...
                        .filter(
                            branchDTO ->
                                StringUtils.equalsIgnoreCase(
                                    branchDTO.getBranchName(), dbDefaultBranch))
                        .findFirst()
    ...
    return Mono.create(
        sink -> branchMono.subscribe(sink::success, sink::error, null, sink.currentContext()));
  }
   ...
   * @return Map of json file names which are added, modified, conflicting, removed and the working
   *     tree if this is clean
   */
        ...
        getGitApplicationMetadata(defaultApplicationId)
            .flatMap(
                gitApplicationMetadata -> {
                  Mono<Tuple2<Application, ApplicationJson>> applicationJsonTuple =
                      applicationService
                          .findByBranchNameAndDefaultApplicationId(
                              finalBranchName,
                              defaultApplicationId,
            ...
            .flatMap(
                tuple -> {
                  Mono<GitStatusDTO> branchedStatusMono =
                      gitExecutor.getStatus(tuple.getT1(), finalBranchName).cache();
                  try {
                    return gitExecutor
                        .fetchRemote(
                            tuple.getT1(),
                    ...
                    return Mono.error(
                        new AppsmithException(AppsmithError.GIT_GENERIC_ERROR, e.getMessage()));
                  }
                });
  ...
  public Mono<MergeStatusDTO> mergeBranch(String defaultApplicationId, GitMergeDTO gitMergeDTO) {
    ...
    if (StringUtils.isEmptyOrNull(sourceBranch) || StringUtils.isEmptyOrNull(destinationBranch)) {
      ...
      if (sourceBranch.startsWith("origin/")) {
        ...
        if (destinationBranch.startsWith("origin/")) {
          return Mono.error(
              new AppsmithException(
                  AppsmithError.UNSUPPORTED_OPERATION_FOR_REMOTE_BRANCH, destinationBranch));
        }
      }
    }
            ...
            .flatMap(
                defaultApplication -> {
                  ...
                  Mono<Path> pathToFile =
                      this.getStatus(defaultApplicationId, sourceBranch, false)
                          .flatMap(
                              status -> {
                                ...
                                return this.getStatus(
                                        defaultApplicationId, destinationBranch, false)
                                    .map(
                                        status1 -> {
                                          if (!Integer.valueOf(0).equals(status.getBehindCount())) {
                                            return Mono.error(
                          ...
                          application1 -> {
                            ...
                            commitDTO.setCommitMessage(
                                DEFAULT_COMMIT_MESSAGE
                                    ...
                                    + sourceBranch);
                            return this.commitApplication(
                                    commitDTO, defaultApplicationId, destinationBranch)
    ...
    return Mono.create(
        sink -> mergeMono.subscribe(sink::success, sink::error, null, sink.currentContext()));
  }
                      ...
                      .flatMap(
                          srcBranchStatus -> {
                            if (!Integer.valueOf(0).equals(srcBranchStatus.getBehindCount())) {
                              ...
                              if (!CollectionUtils.isNullOrEmpty(srcBranchStatus.getModified())) {
                                return Mono.error(
                                    Exceptions.propagate(
                                        new AppsmithException(
                                            AppsmithError.GIT_MERGE_FAILED_LOCAL_CHANGES,
                ...
                tuple -> {
                  ...
                  return gitExecutor
                      .createAndCheckoutToBranch(
                          repoSuffix, branchName + MERGE_CONFLICT_BRANCH_NAME)
                      .flatMap(
                          conflictedBranchName ->
                              commitAndPushWithDefaultCommit(
                                  ...
                                  .flatMap(
                                      successMessage ->
                                          gitExecutor.checkoutToBranch(repoSuffix, branchName))
                                  .flatMap(
                                      isCheckedOut ->
                                          gitExecutor.deleteBranch(
                                              repoSuffix, conflictedBranchName))
        ...
        workspaceMono
            ...
            .flatMap(
                tuple -> {
                  ...
                  Mono<Application> applicationMono =
                      applicationPageService.createOrUpdateSuffixedApplication(
            ...
            .flatMap(
                tuple -> {
                  ...
                  Application application = tuple.getT2();
                  Path repoSuffix =
                      Paths.get(application.getWorkspaceId(), application.getId(), repoName);
                  Mono<Map<String, GitProfile>> profileMono =
                      updateOrCreateGitProfileForCurrentUser(
                          gitConnectDTO.getGitProfile(), application.getId());
                                    ...
                                    .flatMap(
                                        user ->
                                            fileUtils
                                                .deleteLocalRepo(repoSuffix)
                                                .then(
                                                    applicationPageService.deleteApplication(
                                                        application.getId())))
                                    .flatMap(
                                        application1 -> {
                                          if (error instanceof TransportException) {
                  ...
                  Mono<List<Datasource>> datasourceMono =
                      datasourceService
                          .findAllByWorkspaceId(
                              workspaceId, datasourcePermission.getEditPermission())
                          .collectList();
                  ...
                  Mono<ApplicationJson> applicationJsonMono =
                      fileUtils
                          ...
                          .onErrorResume(
                              error -> {
                                log.error(
                                    "Error while constructing" + " application from" + " git repo",
                                    error);
                            ...
                            return applicationService
                                ...
                                .flatMap(
                                    application1 -> {
                                      ...
                                      return applicationPageService.deleteApplicationByResource(
                                          application1);
                                    })
                                .onErrorResume(
                                    throwable -> {
                                      log.warn(
                                          "Unable to find" + " branch" + " with" + " name ",
                                          throwable);
                                      return addAnalyticsForGitOperation(
                    ...
                    releaseFileLock(defaultApplicationId)
                        .then(
                            this.addAnalyticsForGitOperation(
                                AnalyticsEvents.GIT_DISCARD_CHANGES.getEventName(),
                                application,
                                null)))
            .map(responseUtils::updateApplicationWithDefaultResources);
   ...
   * call has to be made to ssh-keypair. import: cannot make a ssh-keypair call because application
   * Id doesn’t exist yet, so API fails.
   *
                  ...
                  .flatMap(
                      gitBranchDTOList -> {
                        List<String> branchList =
                            gitBranchDTOList.stream()
                                .filter(
                                    gitBranchDTO -> gitBranchDTO.getBranchName().contains("origin"))
                                .map(
                                    gitBranchDTO ->
                                    ...
                                    applicationService
                                        ...
                                        .flatMap(
                                            application1 -> {
                                              ...
                                              return gitExecutor.checkoutRemoteBranch(
                                                  repoPath, branchName);
                                            })
                                        // Return empty mono
                                        // when the branched
    ...
    return branchedApplicationMono
        .flatMap(
            branchedApplication -> {
              ...
              try {
                ...
                return Mono.zip(
                    pullStatusMono, Mono.just(branchedApplication), applicationJsonMono);
              } catch (IOException e) {
                return Mono.error(
                    new AppsmithException(AppsmithError.GIT_FILE_SYSTEM_ERROR, e.getMessage()));
              }
        ...
        .flatMap(
            tuple -> {
              ...
              return importExportApplicationService
                  .importApplicationInWorkspace(
                      branchedApplication.getWorkspaceId(), applicationJson,
                      branchedApplication.getId(), branchName)
                  .flatMap(
                      application ->
                          addAnalyticsForGitOperation(
