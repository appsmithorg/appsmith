...
/**
 * Git APIs are slow today because these needs to communicate with remote repo and/or serialise and
 * de-serialise the application. This process takes time and the client may cancel the request. This
 * leads to the flow getting stopped midway producing corrupted states. We use the synchronous sink
 * to ensure that even though the client may have cancelled the flow, git operations should proceed
 * uninterrupted and whenever the user refreshes the page, we will have the sane state. synchronous
 * sink does not take subscription cancellations into account. This means that even if the
 * subscriber has cancelled its subscription, the create method still generates its event.
 */
@Slf4j
...
public class GitServiceCEImpl implements GitServiceCE {
    ...
    @Override
    public Mono<Application> updateGitMetadata(
            String applicationId, GitApplicationMetadata gitApplicationMetadata) {

        if (Optional.ofNullable(gitApplicationMetadata).isEmpty()) {
            return Mono.error(
                    new AppsmithException(
                            AppsmithError.INVALID_PARAMETER, "Git metadata values cannot be null"));
        }

        // For default application we expect a GitAuth to be a part of gitMetadata. We are using
        // save method to leverage
        ...
        return Mono.zip(
                ...
                .map(
                        tuple -> {
                            ...
                            GitApplicationMetadata gitData =
                            ...
                            if (!CollectionUtils.isNullOrEmpty(userData.getGitProfiles())) {
                                ...
                                gitProfiles.put(
                                        ...
                                        userData.getGitProfileByKey(defaultApplicationId));
                            }
                            if (gitData == null) {
    ...
    public Mono<Map<String, GitProfile>> updateOrCreateGitProfileForCurrentUser(
        ...
        // 1. Updating or creating global git profile (defaultApplicationId = "default") and update
        // is made with empty
        //    authorName or authorEmail
        // 2. Updating or creating repo specific profile and user want to use repo specific profile
        // but provided empty
        //    values for authorName and email

        if ((DEFAULT.equals(defaultApplicationId)
                        || Boolean.FALSE.equals(gitProfile.getUseGlobalProfile()))
                && StringUtils.isEmptyOrNull(gitProfile.getAuthorName())) {
            return Mono.error(
                    new AppsmithException(AppsmithError.INVALID_PARAMETER, "Author Name"));
        } else if ((DEFAULT.equals(defaultApplicationId)
                        || Boolean.FALSE.equals(gitProfile.getUseGlobalProfile()))
                && StringUtils.isEmptyOrNull(gitProfile.getAuthorEmail())) {
            return Mono.error(
                    new AppsmithException(AppsmithError.INVALID_PARAMETER, "Author Email"));
                ...
                .flatMap(
                        user ->
                                userDataService
                                        ...
                                        .flatMap(
                                                userData -> {
                                                    // GitProfiles will be null if the user has not
                                                    // created any git profile.
                                                    GitProfile savedProfile =
                                                            userData.getGitProfileByKey(
                                                                    defaultApplicationId);
                                                    GitProfile defaultGitProfile =
                                                            userData.getGitProfileByKey(DEFAULT);

                                                    if (savedProfile == null
                                                            || !savedProfile.equals(gitProfile)
                                                            || defaultGitProfile == null) {
                                                        userData.setGitProfiles(
                                                                userData.setGitProfileByKey(
                                                                        defaultApplicationId,
                                                                        gitProfile));

                                                        // Assign appsmith user profile as a
                                                        // fallback git profile
                                                        if (defaultGitProfile == null) {
                                                            GitProfile userProfile =
                                                                    new GitProfile();
                                                            String authorName =
                                                        ...
                                                        requiredUpdates.setGitProfiles(
                                                                userData.getGitProfiles());
                                                        return userDataService
                                                                .updateForUser(
                                                                        user, requiredUpdates)
                                        ...
                                        .switchIfEmpty(
                                                Mono.defer(
                                                        () -> {
                                                            // If profiles are empty use Appsmith's
                                                            // user profile as git default profile
                                                            ...
                                                            UserData requiredUpdates =
                                                                    new UserData();
                                                            requiredUpdates.setGitProfiles(
                                                                    Map.of(DEFAULT, gitProfile));
                                                            return userDataService
                                                                    .updateForUser(
                                                                            user, requiredUpdates)
                                                                    .map(UserData::getGitProfiles);
                                        ...
                                        .filter(
                                                profiles ->
                                                        !CollectionUtils.isNullOrEmpty(profiles)));
    }
    ...
    public Mono<GitProfile> getDefaultGitProfileOrCreateIfEmpty() {
        // Get default git profile if the default is empty then use Appsmith profile as a fallback
        // value
                ...
                .map(
                        userData -> {
                            ...
                            if (gitProfile != null && gitProfile.getUseGlobalProfile() == null) {
                                ...
                                // If the profile is requested for repo specific using the
                                // applicationId
        ...
        return sessionUserService
                ...
                .flatMap(
                        currentUser -> {
                            ...
                            String authorName =
                                    StringUtils.isEmptyOrNull(currentUser.getName())
    ...
    /**
     * This method will make a commit to local repo This is used directly from client, and we need
     * to acquire file lock before starting to keep the application in a sane state
     *
    ...
    /**
     * This method will make a commit to local repo and is used internally in flows like create,
     * merge branch Since the lock is already acquired by the other flows, we do not need to acquire
     * file lock again
     *
     * @param commitDTO information required for making a commit
     * @param defaultApplicationId application branch on which the commit needs to be done
     ...
     */
    public Mono<String> commitApplication(
            GitCommitDTO commitDTO, String defaultApplicationId, String branchName) {
     ...
     * @param doAmend if we want to amend the commit with the earlier one, used in connect flow
     * @param isFileLock boolean value indicates whether the file lock is needed to complete the
     *     operation
     * @return success message
     */
    private Mono<String> commitApplication(
                                                            ...
                                                            gitProfile.setAuthorName(
                                                                    StringUtils.isEmptyOrNull(
                                                                            ...
                                                                            ? user.getUsername()
                                                                                    .split("@")[0]
                                                                            : user.getName());
                                                            gitProfile.setAuthorEmail(
                                                                    user.getEmail());
                                                            Map<String, GitProfile> updateProfiles =
                                                                    userData.getGitProfiles();
                                                            if (CollectionUtils.isNullOrEmpty(
                                                                    updateProfiles)) {
                                                                updateProfiles =
        ...
        Mono<String> commitMono =
                this.getApplicationById(defaultApplicationId)
                        .flatMap(
                                application -> {
                                    GitApplicationMetadata gitData =
                                            application.getGitApplicationMetadata();
                        ...
                        .flatMap(
                                defaultApplication -> {
                                    GitApplicationMetadata defaultGitMetadata =
                                    ...
                                    // Check if the repo is public for current application and if
                                    // the user have changed the access after
                                    // the connection
                                    final String workspaceId = defaultApplication.getWorkspaceId();
                                    return GitUtils.isRepoPrivate(
                                                    defaultGitMetadata
                                                            .getBrowserSupportedRemoteUrl())
                                            .flatMap(
                                                    isPrivate -> {
                                                        // Check the repo limit if the visibility
                                                        // status is updated, or it is private
                                                        if (!isPrivate.equals(
                                                                        defaultGitMetadata
                                                                                .getIsRepoPrivate())
                        ...
                        .flatMap(
                                branchedApplication -> {
                                    GitApplicationMetadata gitApplicationMetadata =
                                            branchedApplication.getGitApplicationMetadata();
                                    ...
                                    } else if (StringUtils.isEmptyOrNull(
                        ...
                        .flatMap(
                                tuple -> {
                                    ...
                                    GitApplicationMetadata gitData =
                                            childApplication.getGitApplicationMetadata();
                                    Path baseRepoSuffix =
                                            Paths.get(
                                    ...
                                    Mono<Application> branchedApplicationMono =
                                            updateGitMetadata(childApplication.getId(), gitData);
                                    return Mono.zip(
                                            repoPathMono,
                                            currentUserMono,
                        ...
                        .onErrorResume(
                                e -> {
                                    ...
                                    if (e instanceof RepositoryNotFoundException) {
                                        return Mono.error(
                                                new AppsmithException(
                                                        AppsmithError.REPOSITORY_NOT_FOUND,
                                                        defaultApplicationId));
                        ...
                        .flatMap(
                                tuple -> {
                                    ...
                                    GitApplicationMetadata gitApplicationData =
                                            childApplication.getGitApplicationMetadata();

                                    GitProfile authorProfile =
                                            currentUserData.getGitProfileByKey(
                                                    gitApplicationData.getDefaultApplicationId());

                                    if (authorProfile == null
                                            || StringUtils.isEmptyOrNull(
                                                    authorProfile.getAuthorName())
                                            || Boolean.TRUE.equals(
                                                    authorProfile.getUseGlobalProfile())) {

                                        // Use default author profile as the fallback value
                                        if (currentUserData.getGitProfileByKey(DEFAULT) != null) {
                                            authorProfile =
                                                    currentUserData.getGitProfileByKey(DEFAULT);
                                        }
                                    }
                                    ...
                                    if (authorProfile == null
                                            || StringUtils.isEmptyOrNull(
                                                    authorProfile.getAuthorName())) {
                                        String errorMessage =
                                                "Unable to find git author configuration for"
                                                        + " logged-in user. You can set up a git"
                                                        + " profile from the user profile section.";
                        ...
                        .flatMap(
                                tuple -> {
                                    String status = tuple.getT1();
                                    Application childApplication = tuple.getT2();
                                    // Update json schema versions so that we can detect if the next
                                    // update was made by DB migration or by the user
                                    Application update = new Application();
                                    // Reset migration related fields before commit to detect the
                                    // updates correctly between the commits
    ...
    public Mono<List<GitLogDTO>> getCommitHistory(String defaultApplicationId, String branchName) {
        ...
        Mono<List<GitLogDTO>> commitHistoryMono =
                applicationService
                        ...
                        .flatMap(
                                application -> {
                                    ...
                                    return Mono.zip(
                                            gitExecutor
                                                    .checkoutToBranch(
                                                            baseRepoSuffix, gitData.getBranchName())
                                                    .onErrorResume(
                                                            e ->
                                                                    Mono.error(
                                                                            new AppsmithException(
        ...
        return Mono.create(
                sink ->
                        commitHistoryMono.subscribe(
                                sink::success, sink::error, null, sink.currentContext()));
    }

    /**
     * Connect the application from Appsmith to a git repo This is the prerequisite step needed to
     * perform all the git operation for an application We are implementing the deployKey approach
     * and since the deploy-keys are repo level these keys are store under application. Each
     * application is equal to a repo in the git(and each branch creates a new application with
     * default application as parent)
     *
     * @param gitConnectDTO applicationId - this is used to link the local git repo to an
     *     application remoteUrl - used for connecting to remote repo etc
    ...
    public Mono<Application> connectApplicationToGit(
        ...
        if (StringUtils.isEmptyOrNull(originHeader)) {
            return Mono.error(
                    new AppsmithException(AppsmithError.INVALID_PARAMETER, FieldName.ORIGIN));
        }
        ...
        Mono<UserData> currentUserMono =
                userDataService
                        ...
                        .filter(
                                userData ->
                                        !CollectionUtils.isNullOrEmpty(userData.getGitProfiles()))
                        .switchIfEmpty(
                                Mono.error(
                                        new AppsmithException(
                                                AppsmithError.INVALID_GIT_CONFIGURATION,
        ...
        Mono<Application> connectApplicationMono =
                profileMono
                        ...
                        .flatMap(
                                application -> {
                                    GitApplicationMetadata gitApplicationMetadata =
                                            application.getGitApplicationMetadata();
                                    if (isInvalidDefaultApplicationGitMetadata(
                                            application.getGitApplicationMetadata())) {
                                        return Mono.error(
                                                new AppsmithException(
                                                        AppsmithError
                                        ...
                                        String repoName =
                                                GitUtils.getRepoName(gitConnectDTO.getRemoteUrl());
                                        Path repoSuffix =
                                                Paths.get(
                                                        application.getWorkspaceId(),
                                                        defaultApplicationId,
                                                                            ...
                                                                            .flatMap(
                                                                                    app -> {
                                                                                        ...
                                                                                        if (error
                                                                                            ...
                                                                                            // To
                                                                                            // catch
                                                                                            // TransportHttp cast error in case HTTP URL is passed instead of SSH URL
                                                                                            if (error.getMessage()
                                                                                                    .contains(
                                                                                                            "TransportHttp")) {
                                                                                                return Mono
                        ...
                        .flatMap(
                                tuple -> {
                                    ...
                                    try {
                                                            ...
                                                            if (!isEmpty) {
                                                                ...
                                                                gitApplicationMetadata
                                                                        .setDefaultBranchName(
                                                                                defaultBranch);
                                                                gitApplicationMetadata.setRemoteUrl(
                                                                        gitConnectDTO
                                                                                .getRemoteUrl());
                                                                gitApplicationMetadata.setRepoName(
                                                                        repoName);
                                                                gitApplicationMetadata
                                                                        .setBrowserSupportedRemoteUrl(
                                                                                browserSupportedUrl);
                                        ...
                                        log.error(
                                                "Error while cloning the remote repo, {}",
                                                e.getMessage());
                                        return Mono.error(
                                                new AppsmithException(
                                                        AppsmithError.GIT_FILE_SYSTEM_ERROR,
                                ...
                                application -> {
                                    ...
                                    try {
                                        return Mono.zip(
                                                ...
                                                .flatMap(
                                                        ignore -> {
                                                            // Commit and push application to check
                                                            // if the SSH key has the write access
                                                            GitCommitDTO commitDTO =
                                                                    new GitCommitDTO();
        ...
        return Mono.create(
                sink ->
                        connectApplicationMono.subscribe(
                                sink::success, sink::error, null, sink.currentContext()));
    }
    ...
    private Mono<String> pushApplication(String applicationId, boolean doPublish) {
        ...
        Mono<String> pushStatusMono =
                publishAndOrGetApplication(applicationId, doPublish)
                        .flatMap(
                                application -> {
                                ...
                                tuple -> {
                                    ...
                                    if (pushResult.contains("REJECTED_NONFASTFORWARD")) {
                                        ...
                                        return addAnalyticsForGitOperation(
                                                        ...
                                                        application,
                                                        AppsmithError.GIT_UPSTREAM_CHANGES
                                                                .getErrorType(),
                                                        AppsmithError.GIT_UPSTREAM_CHANGES
                                                                .getMessage(),
                                                        application
                                        ...
                                        // Mostly happens when the remote branch is protected or any
                                        // specific rules in place on the branch
                                        // Since the users will be in a bad state where the changes
                                        // are committed locally but they are not able to
        ...
        return Mono.create(
                sink ->
                        pushStatusMono.subscribe(
                                sink::success, sink::error, null, sink.currentContext()));
    }

    /**
     * Method to remove all the git metadata for the application and connected resources. This will
     * remove: - local repo - all the branched applications present in DB except for default
     * application
     *
    ...
    public Mono<Application> detachRemote(String defaultApplicationId) {
        ...
        Mono<Application> disconnectMono =
                getApplicationById(defaultApplicationId)
                        .flatMap(
                                defaultApplication -> {
                                    if (Optional.ofNullable(
                                                            defaultApplication
                                                                    .getGitApplicationMetadata())
                                                    .isEmpty()
                                            || isInvalidDefaultApplicationGitMetadata(
                                                    defaultApplication
                                                            .getGitApplicationMetadata())) {
                                        return Mono.error(
                                                new AppsmithException(
                                                        AppsmithError.INVALID_GIT_CONFIGURATION,
                                                        "Please reconfigure the application to"
                                                                + " connect to git repo"));
                                    }
                                    ...
                                    Path repoSuffix =
                                            Paths.get(
                                                    ...
                                                    gitApplicationMetadata
                                                            .getDefaultApplicationId(),
                                                    repoName);
                                    String defaultApplicationBranchName =
                                            gitApplicationMetadata.getBranchName();
                        ...
                        .flatMap(
                                tuple -> {
                                    ...
                                    defaultApplication
                                            .getPages()
                                            .forEach(page -> page.setDefaultPageId(page.getId()));
                                    if (!CollectionUtils.isNullOrEmpty(
                                            defaultApplication.getPublishedPages())) {
                                        defaultApplication
                                                .getPublishedPages()
                                                .forEach(
                                                        page ->
                                                                page.setDefaultPageId(
                                                                        page.getId()));
                                    }
                                    return fileUtils
                        ...
                        .flatMap(
                                application ->
                                        // Update all the resources to replace defaultResource Ids
                                        // with the resource Ids as branchName
                                        // will be deleted
                                        Flux.fromIterable(application.getPages())
                                                .flatMap(
                                                        page ->
                                                                newPageService.findById(
                                                                        page.getId(),
                                                                        Optional.empty()))
                                                .map(
                                                        newPage -> {
                                                            newPage.setDefaultResources(null);
                                                            return createDefaultIdsOrUpdateWithGivenResourceIds(
                                                                    newPage, null);
                                                        })
                                                .collectList()
                                                ...
                                                .then(
                                                        addAnalyticsForGitOperation(
                                                                AnalyticsEvents.GIT_DISCONNECT
                                                                        .getEventName(),
                                                                application,
                                                                false))
                                                .map(
        ...
        return Mono.create(
                sink ->
                        disconnectMono.subscribe(
                                sink::success, sink::error, null, sink.currentContext()));
    }

    public Mono<Application> createBranch(
            String defaultApplicationId, GitBranchDTO branchDTO, String srcBranch) {
        ...
        if (StringUtils.isEmptyOrNull(srcBranch)
            ...
            return Mono.error(
                    new AppsmithException(AppsmithError.INVALID_PARAMETER, FieldName.BRANCH_NAME));
        }

        Mono<Application> createBranchMono =
                applicationService
                        .findByBranchNameAndDefaultApplicationId(
                                srcBranch,
                                defaultApplicationId,
                        ...
                        .zipWhen(
                                srcApplication -> {
                                    GitApplicationMetadata gitData =
                                            srcApplication.getGitApplicationMetadata();
                                    if (gitData.getDefaultApplicationId()
                                            .equals(srcApplication.getId())) {
                                        return Mono.just(
                                                srcApplication
                                                        .getGitApplicationMetadata()
                                                        .getGitAuth());
                                    ...
                                    return applicationService
                                            ...
                                            .map(
                                                    gitAuthDTO -> {
                                                        ...
                                                        gitAuth.setPrivateKey(
                                                                gitAuthDTO.getPrivateKey());
                                                        gitAuth.setPublicKey(
                                                                gitAuthDTO.getPublicKey());
                        ...
                        .flatMap(
                                tuple -> {
                                    ...
                                    GitApplicationMetadata srcBranchGitData =
                                            srcApplication.getGitApplicationMetadata();
                                    if (srcBranchGitData == null
                                        ...
                                        return Mono.error(
                                                new AppsmithException(
                                                        AppsmithError.INVALID_GIT_CONFIGURATION,
                                                        "Unable to find the parent branch. Please"
                                                                + " create a branch from other"
                                                                + " available branches"));
                                    }
                                    Path repoSuffix =
                                            Paths.get(
                                    ...
                                    return addFileLock(srcBranchGitData.getDefaultApplicationId())
                                            .flatMap(
                                                    status ->
                                                            gitExecutor.checkoutToBranch(
                                                                    repoSuffix, srcBranch))
                                            .onErrorResume(
                                                    error ->
                                                            Mono.error(
                                                                    new AppsmithException(
                                            ...
                                            .flatMap(
                                                    branchName -> {
                                                        final String srcApplicationId =
                                                                srcApplication.getId();
                                                        srcBranchGitData.setBranchName(branchName);
                                                        srcBranchGitData.setIsRepoPrivate(null);
                                                        // Save a new application in DB and update
                                                        // from the parent branch application
                                                        srcBranchGitData.setGitAuth(null);
                                                        srcBranchGitData.setLastCommittedAt(
                                                                Instant.now());
                                                        ...
                                                        srcApplication.setPublishedModeThemeId(
                                                                null);
                                                        srcApplication.setGitApplicationMetadata(
                                                                srcBranchGitData);
                                                        return Mono.zip(
                                                                applicationService.save(
                                                                        srcApplication),
                        ...
                        .flatMap(
                                tuple -> {
                                    ...
                                    return importExportApplicationService
                                            ...
                                            .flatMap(
                                                    application -> {
                                                        // Commit and push for new branch created
                                                        // this is to avoid issues when user tries
                                                        // to create a
    ...
    public Mono<Application> checkoutBranch(String defaultApplicationId, String branchName) {
        ...
        // If the user is trying to check out remote branch, create a new branch if the branch does
        // not exist already
                    ...
                    .flatMap(
                            gitBranchDTOList -> {
                                long branchMatchCount =
                                        gitBranchDTOList.stream()
                                                .filter(
                                                        gitBranchDTO ->
                                                                gitBranchDTO
                                                                        .getBranchName()
                                                                        .equals(finalBranchName))
                                                .count();
                                if (branchMatchCount == 0) {
                                    return checkoutRemoteBranch(
                                            defaultApplicationId, finalBranchName);
                                } else {
                                    return Mono.error(
                                            new AppsmithException(
    ...
    private Mono<Application> checkoutRemoteBranch(String defaultApplicationId, String branchName) {
        Mono<Application> checkoutRemoteBranchMono =
                getApplicationById(defaultApplicationId)
                        .flatMap(
                                application -> {
                                    GitApplicationMetadata gitApplicationMetadata =
                        ...
                        .flatMap(
                                tuple -> {
                                    ...
                                    // Save a new application in DB and update from the parent
                                    // branch application
                                    srcBranchGitData.setGitAuth(null);
                                    srcBranchGitData.setIsRepoPrivate(null);
                                                    ...
                                                    application1 ->
                                                            fileUtils
                                                                    ...
                                                                    .zipWith(
                                                                            Mono.just(
                                                                                    application1)))
                                            // We need to handle the case specifically for default
                                            // branch of Appsmith
                                            // if user switches default branch and tries to delete
                                            // the default branch we do not delete resource from db
                                            // This is an exception only for the above case and in
                                            // such case if the user tries to check out the branch
                                            // again
                                            // It results in an error as the resources are already
                                            // present in db
                                            // So we just rehydrate from the file system to the
                                            // existing resource on the db
                                            .onErrorResume(
                                                    throwable -> {
                                                        if (throwable
        ...
        return Mono.create(
                sink ->
                        checkoutRemoteBranchMono.subscribe(
                                sink::success, sink::error, null, sink.currentContext()));
    }
    ...
    /**
     * Method to pull application json files from remote repo, make a commit with the changes
     * present in local DB and make a system commit to remote repo
     *
                                    ...
                                    if (!CollectionUtils.isNullOrEmpty(status.getModified())) {
                                        return Mono.error(
                                                new AppsmithException(
                                                        AppsmithError.GIT_ACTION_FAILED,
                                                        "pull",
                                                        "There are uncommitted changes present in"
                                                                + " your local. Please commit them"
                                                                + " first and then try git pull"));
                                    }
                                    return pullAndRehydrateApplication(
                                                    defaultApplication, branchName)
    ...
    public Mono<List<GitBranchDTO>> listBranchForApplication(
        ...
        Mono<List<GitBranchDTO>> branchMono =
                getApplicationById(defaultApplicationId)
                        .flatMap(
                                application -> {
                                    GitApplicationMetadata gitApplicationMetadata =
                                    ...
                                    if (Boolean.TRUE.equals(pruneBranches)) {
                                        ...
                                        // Fetch default branch from DB if the pruneBranches is
                                        // false else fetch from remote
                                        gitBranchDTOMono =
                                                gitExecutor.listBranches(
                                                        repoPath,
                                            ...
                                            .onErrorResume(
                                                    error -> {
                                                        if (error
                                                            ...
                                                            Mono<List<GitBranchDTO>>
                                                                    branchListMono =
                                                                            handleRepoNotFoundException(
                                                                                    defaultApplicationId);
                                                            return Mono.zip(
                                                                    branchListMono,
                                                                    Mono.just(application),
                                        ...
                                        if (defaultBranchRemote.equals(dbDefaultBranch)) {
                                            ...
                                            return applicationService
                                                    .findByBranchNameAndDefaultApplicationId(
                                                            ...
                                                            applicationPermission
                                                                    .getEditPermission())
                                                    // Check if the branch is already present, If
                                                    // not follow checkout remote flow
                                                    .onErrorResume(
                                                            throwable ->
                                                                    checkoutRemoteBranch(
                                                                            defaultApplicationId,
                                                                            defaultBranchRemote))
                                                    // Update the default branch name in all the
                                                    // child applications
                                                    .flatMapMany(
                                                            application1 ->
                                                                    applicationService
     ...
     * @param isFileLock if the locking is required, since the status API is used in the other flows
     *     of git Only for the direct hits from the client the locking will be added
     * @return Map of json file names which are added, modified, conflicting, removed and the
     *     working tree if this is clean
     */
        ...
        */
        ...
        Mono<GitStatusDTO> statusMono =
                getGitApplicationMetadata(defaultApplicationId)
                        .flatMap(
                                gitApplicationMetadata -> {
                                    Mono<Tuple2<Application, ApplicationJson>>
                                            applicationJsonTuple =
                                                    applicationService
                                                            .findByBranchNameAndDefaultApplicationId(
                                                                    finalBranchName,
                                                                    defaultApplicationId,
                                    ...
                                    if (Boolean.TRUE.equals(isFileLock)) {
                                        // Add file lock for the status API call to avoid sending
                                        // wrong info on the status
                                        return redisUtils
                                                .addFileLock(
                                                        gitApplicationMetadata
                                                                .getDefaultApplicationId())
                                                .retryWhen(
                                                        Retry.fixedDelay(MAX_RETRIES, RETRY_DELAY)
                                                                .jitter(0.75)
                        ...
                        .flatMap(
                                tuple -> {
                                    GitApplicationMetadata defaultApplicationMetadata =
                                            tuple.getT1();
                        ...
                        .flatMap(
                                tuple -> {
                                    Mono<GitStatusDTO> branchedStatusMono =
                                            gitExecutor
                                    ...
                                    try {
                                        return gitExecutor
                                                ...
                                                // Remove any files which are copied by hard
                                                // resetting the repo
                                                .then(
                                                        gitExecutor.resetToLastCommit(
                                                                tuple.getT3(), branchName))
                                                .flatMap(
                                                        gitStatusDTO -> {
                                                            if (Boolean.TRUE.equals(isFileLock)) {
                                                                return releaseFileLock(
                                        ...
                                        return Mono.error(
                                                new AppsmithException(
                                                        AppsmithError.GIT_GENERIC_ERROR,
                                                        e.getMessage()));
                                    }
                                });
    ...
    public Mono<MergeStatusDTO> mergeBranch(String defaultApplicationId, GitMergeDTO gitMergeDTO) {
        ...
        if (StringUtils.isEmptyOrNull(sourceBranch)
                || StringUtils.isEmptyOrNull(destinationBranch)) {
            return Mono.error(
                    new AppsmithException(AppsmithError.INVALID_PARAMETER, FieldName.BRANCH_NAME));
        } else if (sourceBranch.startsWith("origin/")) {
            return Mono.error(
                    new AppsmithException(
                            AppsmithError.UNSUPPORTED_OPERATION_FOR_REMOTE_BRANCH, sourceBranch));
        } else if (destinationBranch.startsWith("origin/")) {
            return Mono.error(
                    new AppsmithException(
                            AppsmithError.UNSUPPORTED_OPERATION_FOR_REMOTE_BRANCH,
                            destinationBranch));
        }

        Mono<MergeStatusDTO> mergeMono =
                getApplicationById(defaultApplicationId)
                        .flatMap(
                                application -> {
                                    GitApplicationMetadata gitData =
                        ...
                        .flatMap(
                                defaultApplication -> {
                                    ...
                                    // 1. Hydrate from db to file system for both branch
                                    // Applications
                                    Mono<Path> pathToFile =
                                            this.getStatus(
                                                            defaultApplicationId,
                                                            sourceBranch,
                                                            false)
                                    ...
                                    return Mono.zip(Mono.just(defaultApplication), pathToFile)
                                            .onErrorResume(
                                                    error -> {
                                                        log.error(
                                                                "Error in repo status check for"
                                                                        + " application "
                                                                        + defaultApplicationId,
                                                                error);
    ...
    public Mono<MergeStatusDTO> isBranchMergeable(
            String defaultApplicationId, GitMergeDTO gitMergeDTO) {
        ...
        Mono<MergeStatusDTO> mergeableStatusMono =
                getApplicationById(defaultApplicationId)
                        .flatMap(
                                application -> {
                                    GitApplicationMetadata gitApplicationMetadata =
                                                                    ...
                                                                    Exceptions.propagate(
                                                                            new AppsmithException(
                                                                                    ...
                                                                                    srcBranchStatus
                                                                                            .getBehindCount(),
                                                                                    sourceBranch)));
                                                        } else if (!CollectionUtils.isNullOrEmpty(
                                                                srcBranchStatus.getModified())) {
                                                            return Mono.error(
                                                                    Exceptions.propagate(
                                                                            new AppsmithException(
                                                                                    AppsmithError
    ...
    public Mono<String> createConflictedBranch(String defaultApplicationId, String branchName) {
        ...
        Mono<String> conflictedBranchMono =
                Mono.zip(
                                getGitApplicationMetadata(defaultApplicationId),
                                applicationService
                                        .findByBranchNameAndDefaultApplicationId(
                        ...
                        .flatMap(
                                tuple -> {
                                    ...
                                    return gitExecutor
                                            .createAndCheckoutToBranch(
                                                    repoSuffix,
                                                    branchName + MERGE_CONFLICT_BRANCH_NAME)
                                            .flatMap(
                                                    conflictedBranchName ->
                                                            commitAndPushWithDefaultCommit(
                                                                            repoSuffix,
                                                                            gitData.getGitAuth(),
                                                                            gitData,
                                                                            GitDefaultCommitMessage
                                                                    ...
                                                                    .thenReturn(
                                                                            conflictedBranchName
                                                                                    + CONFLICTED_SUCCESS_MESSAGE));
                                });

        return Mono.create(
                sink ->
                        conflictedBranchMono.subscribe(
                                sink::success, sink::error, null, sink.currentContext()));
    }

    @Override
    public Mono<ApplicationImportDTO> importApplicationFromGit(
        ...
        if (StringUtils.isEmptyOrNull(workspaceId)) {
            return Mono.error(
                    new AppsmithException(AppsmithError.INVALID_PARAMETER, "Invalid workspace id"));
        }
                ...
                workspaceMono
                        ...
                        .switchIfEmpty(
                                Mono.error(
                                        new AppsmithException(
                                                AppsmithError.INVALID_GIT_CONFIGURATION,
                                                "Unable to find git configuration for logged-in"
                                                        + " user. Please contact Appsmith team for"
                                                        + " support")))
                        // Check the limit for number of private repo
                        .flatMap(
                                tuple -> {
                                            ...
                                            .flatMap(
                                                    tuple2 -> {
                                                        ...
                                                        gitApplicationMetadata
                                                                .setDefaultApplicationId(
                                                                        application.getId());
                                                        gitApplicationMetadata.setBranchName(
                                                                defaultBranch);
                                                        gitApplicationMetadata.setDefaultBranchName(
                                                                defaultBranch);
                                                        gitApplicationMetadata.setRemoteUrl(
                                                                gitConnectDTO.getRemoteUrl());
                                                        gitApplicationMetadata.setRepoName(
                                                        ...
                                                        gitApplicationMetadata
                                                                .setBrowserSupportedRemoteUrl(
                                                                        GitUtils
                                                                                .convertSshUrlToBrowserSupportedUrl(
                                                                                        gitConnectDTO
                                                                                                .getRemoteUrl()));
                                                        gitApplicationMetadata.setIsRepoPrivate(
                                                                isRepoPrivate);
                                                        gitApplicationMetadata.setLastCommittedAt(
                                                                Instant.now());
                        ...
                        .flatMap(
                                objects -> {
                                    ...
                                    Mono<List<Datasource>> datasourceMono =
                                            datasourceService
                                    ...
                                    return Mono.zip(applicationJsonMono, datasourceMono, pluginMono)
                                            .flatMap(
                                                    data -> {
                                                        ...
                                                        // If there is an existing datasource with
                                                        // the same name but a different type from
                                                        // that in the repo, the import api should
                                                        // fail
                                                        if (checkIsDatasourceNameConflict(
                                                                datasourceList,
                                                                applicationJson.getDatasourceList(),
                                                                pluginList)) {
                                                            return deleteApplicationCreatedFromGitImport(
                                                                            application.getId(),
                                                                            application
                                                        ...
                                                        return importExportApplicationService
                                                                ...
                                                                .onErrorResume(
                                                                        throwable ->
                                                                                deleteApplicationCreatedFromGitImport(
                                                                                                application
                                                                                                        .getId(),
                                                                                                application
                                                                                                        .getWorkspaceId(),
                        ...
                        .flatMap(
                                application ->
                                        importExportApplicationService.getApplicationImportDTO(
                                                application.getId(),
                                                application.getWorkspaceId(),
                                                application))
                        // Add analytics event
                            ...
                            return gitDeployKeysRepository
                                    ...
                                    .flatMap(
                                            gitDeployKeys1 -> {
                                                ...
                                                gitDeployKeys1.setGitAuth(
                                                        gitDeployKeys.getGitAuth());
                        ...
                        application -> {
                            ...
                            return gitExecutor
                                    ...
                                    .onErrorResume(
                                            error -> {
                                                log.error(
                                                        "Error while testing the connection to th"
                                                                + " remote repo "
                                                                + gitApplicationMetadata
                                                                        .getRemoteUrl()
                                                                + " ",
    ...
    public Mono<Application> deleteBranch(String defaultApplicationId, String branchName) {
        Mono<Application> deleteBranchMono =
                getApplicationById(defaultApplicationId)
                        .flatMap(
                                application -> {
                                    GitApplicationMetadata gitApplicationMetadata =
    ...
    public Mono<Application> discardChanges(String defaultApplicationId, String branchName) {
        ...
        discardChangeMono =
                branchedApplicationMono
                        ...
                        .flatMap(
                                application ->
                                        addFileLock(defaultApplicationId).thenReturn(application))
                        .flatMap(
                                branchedApplication -> {
                                    GitApplicationMetadata gitData =
                                            branchedApplication.getGitApplicationMetadata();
                        ...
                        .flatMap(
                                application ->
                                        releaseFileLock(defaultApplicationId)
                                                .then(
                                                        this.addAnalyticsForGitOperation(
                                                                AnalyticsEvents.GIT_DISCARD_CHANGES
                                                                        .getEventName(),
                                                                application,
                                                                null)))
                        .map(responseUtils::updateApplicationWithDefaultResources);
        ...
        return Mono.create(
                sink ->
                        discardChangeMono.subscribe(
                                sink::success, sink::error, null, sink.currentContext()));
    }

    /**
     * In some scenarios: connect: after loading the modal, keyTypes is not available, so a network
     * call has to be made to ssh-keypair. import: cannot make a ssh-keypair call because
     * application Id doesn’t exist yet, so API fails.
     *
    ...
    private boolean checkIsDatasourceNameConflict(
        ...
        // If we have an existing datasource with the same name but a different type from that in
        // the repo, the import api should fail
        for (DatasourceStorage datasourceStorage : importedDatasources) {
            // Collect the datasource(existing in workspace) which has same as of imported
            // datasource
            // As names are unique we will need filter first element to check if the plugin id is
            // matched
                    ...
                    existingDatasources.stream()
                            .filter(
                                    datasource1 ->
                                            datasource1
                                                    .getName()
                                                    .equals(datasourceStorage.getName()))
                            .findFirst()
                            .orElse(null);
    ...
    private boolean isInvalidDefaultApplicationGitMetadata(
            GitApplicationMetadata gitApplicationMetadata) {
    ...
    public Mono<List<GitBranchDTO>> handleRepoNotFoundException(String defaultApplicationId) {
        ...
        // clone application to the local filesystem again and update the defaultBranch for the
        // application
                                                                        ...
                                                                        applicationService
                                                                                ...
                                                                                .flatMap(
                                                                                        application1 -> {
                                                                                            // Add
                                                                                            // the
                                                                                            // locally checked out branch to the branchList
                                                                                            GitBranchDTO
                                                                                                    gitBranchDTO =
                                                                                                            new GitBranchDTO();
                                                                                            gitBranchDTO
    ...
    public Mono<Long> getApplicationCountWithPrivateRepo(String workspaceId) {
        return applicationService.getGitConnectedApplicationsCountWithPrivateRepoByWorkspaceId(
                workspaceId);
    }
     ...
     */
    private Mono<GitPullDTO> pullAndRehydrateApplication(
            Application defaultApplication, String branchName) {
        ...
        */
        ...
        if (isInvalidDefaultApplicationGitMetadata(gitData)) {
            return Mono.error(
                    new AppsmithException(
                            AppsmithError.INVALID_GIT_CONFIGURATION, GIT_CONFIG_ERROR));
        }
        Path repoSuffix =
                Paths.get(
                        defaultApplication.getWorkspaceId(),
        ...
        return branchedApplicationMono
                .flatMap(
                        branchedApplication -> {
                            ...
                            try {
                                Mono<MergeStatusDTO> pullStatusMono =
                                        gitExecutor
                                                .checkoutToBranch(repoSuffix, branchName)
                                                .then(
                            ...
                            return importExportApplicationService
                                    ...
                                    .flatMap(
                                            application -> {
                                                ...
                                                // Make commit and push after pull is successful to
                                                // have a clean repo
                                                return this.commitApplication(
                                                                commitDTO,
                                                                application
    ...
    private Mono<Application> addAnalyticsForGitOperation(
        ...
        return addAnalyticsForGitOperation(
                eventName, application, errorType, errorMessage, isRepoPrivate, false);
    }
                            ...
                            return this.getApplicationCountWithPrivateRepo(workspaceId)
                                    .map(
                                            privateRepoCount -> {
                                                ...
                                                // isClearCache is true for the connect & import
                                                // flow
                                                if (!isClearCache) {
