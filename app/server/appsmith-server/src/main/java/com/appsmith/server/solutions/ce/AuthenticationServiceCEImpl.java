		...
		.flatMap(
			...
			UriComponentsBuilder uriComponentsBuilder =
				UriComponentsBuilder.fromUriString(oAuth2.getAuthorizationUrl())
					...
					// The state is used internally to calculate the redirect url when returning
...
public Mono<String> getAccessTokenForGenericOAuth2(AuthorizationCodeCallbackDTO callbackDTO) {
	...
	return Mono.justOrEmpty(state)
		...
		.flatMap(
			...
			} else return Mono.just(localState.split(",")[1]);
			})
		...
		.flatMap(
			...
			// We use the returned scope instead because users may have authorized fewer scopes
			// than requested
		...
		// We have no use of the datasource object during redirection, we merely send the response
		// as a success state
					...
					authenticationResponse -> {
						...
						if (!new HashSet<>(
							...
							return Mono.error(
								new AppsmithException(
									AppsmithError.AUTHENTICATION_FAILURE,
									"Please provide access to all the requested scopes to use the"
										+ " integration correctly."));
						}
						}
						datasource.getDatasourceConfiguration().setAuthentication(oAuth2);

						// When authentication scope is for specific sheets, we need to send token
						// and project id
						String accessToken = "";
						String projectID = "";
						...
						// for specific sheets, it needs to remain in as in progress until files are
						// selected
						// Once files are selected, client sets authentication status as SUCCESS, we
						// can find this code in
			...
			return WebClientUtils.create(
				...
				.flatMap(
					authenticationResponse -> {
						oAuth2.setAuthenticationResponse(authenticationResponse);
						datasource.getDatasourceConfiguration().setAuthentication(oAuth2);
						// We return the same object instead of the update value because the updates
						// value
