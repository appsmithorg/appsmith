...
public class AuthenticationServiceCEImpl implements AuthenticationServiceCE {
                ...
                .flatMap(datasource -> {
                    ...
                    return builder.build()
                            ...
                            .flatMap(response -> {
                                ...
                                return datasourceService.update(datasource.getId(), datasource, Boolean.FALSE);
                            });
                })
                ...
                .flatMap(integrationDTO -> {
                    return WebClientUtils.create(cloudServicesConfig.getBaseUrl() + "/api/v1/integrations/oauth/appsmith")
                            ...
                            .flatMap(tuple -> {
                                ...
                                return datasourceService.update(datasource.getId(), datasource, Boolean.FALSE).thenReturn(appsmithToken);
                            })
    ...
    public Mono<Datasource> getAccessTokenFromCloud(String datasourceId, String appsmithToken) {
        ...
        return datasourceMono
                ...
                .flatMap(datasource -> datasourceService.update(datasource.getId(), datasource, Boolean.FALSE))
                .onErrorMap(ConnectException.class,
                        error -> new AppsmithException(
                                AppsmithError.AUTHENTICATION_FAILURE,
                                "Unable to connect to Appsmith authentication server."
                    ...
                    return WebClientUtils.create(cloudServicesConfig.getBaseUrl() + "/api/v1/integrations/oauth/refresh")
                            ...
                            .flatMap(authenticationResponse -> {
                                ...
                                return datasourceService
                                        .update(datasource.getId(), datasource, Boolean.FALSE)
