...
public class CustomUserDataRepositoryCEImpl extends BaseAppsmithRepositoryImpl<UserData>
  ...
  public Mono<UpdateResult> removeIdFromRecentlyUsedList(
    ...
    Update update =
        new Update().pull(fieldName(QUserData.userData.recentlyUsedWorkspaceIds), workspaceId);
    if (!CollectionUtils.isEmpty(applicationIds)) {
      update =
          update.pullAll(
              fieldName(QUserData.userData.recentlyUsedAppIds), applicationIds.toArray());
    }
  ...
  public Flux<UserData> findPhotoAssetsByUserIds(Iterable<String> userId) {
    ...
    return queryAll(
        List.of(criteria), Optional.of(fieldsToInclude), Optional.empty(), Optional.empty());
  }
