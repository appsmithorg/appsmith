...
public class CustomUserDataRepositoryCEImpl extends BaseAppsmithRepositoryImpl<UserData>
    ...
    public Mono<UpdateResult> removeIdFromRecentlyUsedList(
        ...
        Update update =
                new Update()
                        .pull(fieldName(QUserData.userData.recentlyUsedWorkspaceIds), workspaceId);
        if (!CollectionUtils.isEmpty(applicationIds)) {
            update =
                    update.pullAll(
                            fieldName(QUserData.userData.recentlyUsedAppIds),
    ...
    /**
     * Fetches a list of UserData objects from DB where userId matches with the provided a list of
     * userId. The returned UserData objects will have only the userId and photoAssetId fields.
     *
    ...
    public Flux<UserData> findPhotoAssetsByUserIds(Iterable<String> userId) {
        // need to convert from Iterable to ArrayList because the "in" method of criteria takes a
        // collection as input
        Criteria criteria =
                where(fieldName(QUserData.userData.userId)).in(Lists.newArrayList(userId));
    ...
    public Mono<String> fetchMostRecentlyUsedWorkspaceId(String userId) {
        ...
        return mongoOperations
                .findOne(query, UserData.class)
                .map(
                        userData -> {
                            final List<String> recentlyUsedWorkspaceIds =
                                    userData.getRecentlyUsedWorkspaceIds();
                            return CollectionUtils.isEmpty(recentlyUsedWorkspaceIds)
                                    ? ""
                                    : recentlyUsedWorkspaceIds.get(0);
                        });
    }
}
