...
public Mono<List<User>> inviteUsers(InviteUsersDTO inviteUsersDTO, String originHeader) {
	...
	// Check if the current user has assign permissions to the permission group and permission group
	// is workspace's default permission group.
		...
		Flux.fromIterable(usernames)
			...
			.flatMap(
				...
				return getUserFromDbAndCheckIfUserExists
					.flatMap(
						existingUser -> {
							// The user already existed, just send an email informing that the user
							// has been added
							// to a new workspace
							log.debug(
								"Going to send email to user {} informing that the user has been"
