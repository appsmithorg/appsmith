...
public class UserAndAccessManagementServiceCEImpl implements UserAndAccessManagementServiceCE {
     ...
     * Send out an email informing the user to sign up with token. d. Follow the steps for User
     * which already exists 2. User exists : a. Add user to the workspace b. Add workspace to the
     * user
     *
    ...
    public Mono<List<User>> inviteUsers(InviteUsersDTO inviteUsersDTO, String originHeader) {
        ...
        if (!StringUtils.hasText(inviteUsersDTO.getPermissionGroupId())) {
            return Mono.error(
                    new AppsmithException(AppsmithError.INVALID_PARAMETER, FieldName.ROLE));
        }
                        ...
                        .filter(
                                permissionGroup ->
                                        permissionGroup
                                                        ...
                                                        .equals(Workspace.class.getSimpleName())
                                                && StringUtils.hasText(
                                                        permissionGroup.getDefaultDomainId()))
                        .switchIfEmpty(
                                Mono.error(
                                        new AppsmithException(
                                                AppsmithError.NO_RESOURCE_FOUND, FieldName.ROLE)))
                        .cache();
                        ...
                        .flatMap(
                                tuple -> {
                                    ...
                                    analyticsProperties.put(
                                            FieldName.NUMBER_OF_USERS_INVITED, numberOfUsers);
