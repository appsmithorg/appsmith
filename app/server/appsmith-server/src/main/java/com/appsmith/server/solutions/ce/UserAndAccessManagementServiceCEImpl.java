...
public class UserAndAccessManagementServiceCEImpl implements UserAndAccessManagementServiceCE {
  ...
  /**
   * 1. User doesn't exist : a. Create a new user. b. Set isEnabled to false c. Generate a token.
   * Send out an email informing the user to sign up with token. d. Follow the steps for User which
   * already exists 2. User exists : a. Add user to the workspace b. Add workspace to the user
   *
  ...
  public Mono<List<User>> inviteUsers(InviteUsersDTO inviteUsersDTO, String originHeader) {
    ...
    if (CollectionUtils.isEmpty(originalUsernames)) {
      return Mono.error(
          new AppsmithException(AppsmithError.INVALID_PARAMETER, FieldName.USERNAMES));
    }
    ...
    // Check if the current user has assign permissions to the permission group and permission
    // group is workspace's default permission group.
    Mono<PermissionGroup> permissionGroupMono =
        permissionGroupService
            .getById(
                inviteUsersDTO.getPermissionGroupId(),
            ...
            .switchIfEmpty(
                Mono.error(new AppsmithException(AppsmithError.NO_RESOURCE_FOUND, FieldName.ROLE)))
            .cache();
    ...
    Mono<Workspace> workspaceMono =
        permissionGroupMono
            .flatMap(
                permissionGroup -> workspaceService.getById(permissionGroup.getDefaultDomainId()))
            .cache();
    ...
    // Check if the invited user exists. If yes, return the user, else create a new user by
    // triggering
    // createNewUserAndSendInviteEmail. In both the cases, send the appropriate emails
    Mono<List<User>> inviteUsersMono =
        Flux.fromIterable(usernames)
            ...
            .flatMap(
                tuple -> {
                  ...
                  Mono<User> getUserFromDbAndCheckIfUserExists =
                      userRepository
                          .findByEmail(username)
                          .flatMap(
                              user -> {
                                return throwErrorIfUserAlreadyExistsInWorkspace(
                                        user, defaultPermissionGroups)
                  ...
                  return getUserFromDbAndCheckIfUserExists
                      .flatMap(
                          existingUser -> {
                            // The user already existed, just send an
                            // email informing that the user has been
                            // added
                            // to a new workspace
                            log.debug(
                                "Going to send email to user {}"
                                    + " informing that the user has"
                                    + " been added to new workspace"
                                    + " {}",
                                existingUser.getEmail(),
                                workspace.getName());

                            // Email template parameters initialization
                            // below.
                            Map<String, String> params =
                                userService.getEmailParams(
                                    workspace, currentUser, originHeader, false);
                      ...
                      .switchIfEmpty(
                          userService.createNewUserAndSendInviteEmail(
                              username,
                              originHeader,
                              workspace,
                              currentUser,
    ...
    Mono<PermissionGroup> bulkAddUserResultMono =
        Mono.zip(permissionGroupMono, inviteUsersMono)
            .flatMap(
                tuple -> {
                  PermissionGroup permissionGroup = tuple.getT1();
                  List<User> users = tuple.getT2();
                  return permissionGroupService.bulkAssignToUserAndSendEvent(
                      permissionGroup, users);
                })
            .cache();

    // Send analytics event
    Mono<Object> sendAnalyticsEventMono =
        Mono.zip(currentUserMono, inviteUsersMono)
            .flatMap(
                tuple -> {
                  ...
                  Map<String, Object> extraPropsForCloudHostedInstance =
                      Map.of(FieldName.USER_EMAILS, invitedUsers);
