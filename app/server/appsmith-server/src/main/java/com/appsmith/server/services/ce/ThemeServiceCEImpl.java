...
public Mono<Theme> updateTheme(String applicationId, String branchName, Theme resource) {
	return applicationService
		...
		.flatMap(
			application -> {
			// makes sure user has permission to edit application and an application exists by
			// this applicationId
						...
						return saveThemeMono
							.flatMap(
								savedTheme -> {
									...
									// current theme is neither a system theme nor app theme,
									// delete the user customizations
									return repository
										.delete(currentTheme)
										.then(
	...
	return repository
		...
		.flatMap(
			...
			if (theme.isSystemTheme()) { // it's a system theme, no need to copy
				...
				theme.setId(
					null); // setting id to null so that save method will create a new instance
		...
		.flatMap(
			...
			return editModeThemeMono.flatMap(
				editModeTheme -> {
					...
					} else { // a customized theme is set as edit mode theme, copy that theme for
					// published mode
					return saveThemeForApplication(
						application.getPublishedModeThemeId(),
						editModeTheme,
		...
		.flatMap(
			...
			if (currentTheme.isSystemTheme()) {
				...
				currentTheme.setPolicies(
					policyGenerator.getAllChildPolicies(
						application.getPolicies(), Application.class, Theme.class));
				// Not setting the application id in the theme because only the named themes have an
				// application id
...
public Mono<Theme> getOrSaveTheme(Theme theme, Application destApplication) {
	if (theme
		== null) { // this application was exported without theme, assign the legacy theme to it
...
public Mono<Application> importThemesToApplication(
	...
	return Mono.zip(editModeTheme, publishedModeTheme)
		.flatMap(
			...
			// also returning the updated application object so that theme id are available to the
			// next pipeline
			return applicationService
				.setAppTheme(
