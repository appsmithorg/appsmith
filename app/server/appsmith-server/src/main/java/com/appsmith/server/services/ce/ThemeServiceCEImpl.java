...
public class ThemeServiceCEImpl extends BaseService<ThemeRepositoryCE, Theme, String>
                              ...
                              .flatMap(
                                  savedTheme ->
                                      analyticsService.sendObjectEvent(
                                          AnalyticsEvents.APPLY, savedTheme));
                        }));
  }
        ...
        .flatMap(
            theme -> {
              if (theme.isSystemTheme()) { // it's a system theme, no need to copy
                ...
                theme.setId(null); // setting id to null so that save method will create
                // a new instance
        ...
        .flatMap(
            application -> {
              ...
              if (!StringUtils.hasLength(
                  application.getEditModeThemeId())) { // theme id is empty, use the
                // default theme
                editModeThemeMono = repository.getSystemThemeByName(Theme.LEGACY_THEME_NAME);
              } else { // theme id is not empty, fetch it by id
                editModeThemeMono =
                    repository.findById(application.getEditModeThemeId(), READ_THEMES);
              }

              return editModeThemeMono.flatMap(
                  editModeTheme -> {
                    if (editModeTheme.isSystemTheme()) { // system theme is set as edit
                      // mode theme
   ...
   * from the provided theme resource to the existing or newly created theme. It'll also update the
   * application if a new theme was created.
   *
  ...
  /**
   * This method will fetch a theme by id and delete this if it's not a system theme. When an app is
   * published with a customized theme, we store a copy of that theme so that changes are available
   * in published mode even user has changed the theme in edit mode. When user switches back to
   * another theme and publish the application where that app was previously published with a custom
   * theme, we should delete that copy. Otherwise there'll be a lot of orphan theme copies that were
   * set a published mode once but are used no more.
   *
  ...
  public Mono<Theme> archiveById(String themeId) {
    return repository
        ...
        .switchIfEmpty(
            Mono.error(
                new AppsmithException(
                    AppsmithError.NO_RESOURCE_FOUND, FieldName.APPLICATION, FieldName.THEME)))
        .flatMap(
            theme -> {
              if (StringUtils.hasLength(theme.getApplicationId())) { // only persisted themes are
  ...
  public Mono<Theme> updateName(String id, Theme themeDto) {
    return repository
        ...
        .switchIfEmpty(
            Mono.error(new AppsmithException(AppsmithError.NO_RESOURCE_FOUND, FieldName.THEME, id)))
        .flatMap(
            theme -> {
  ...
  /**
   * This method imports a theme from a JSON file to an application. The destination application can
   * already have a theme set or not. If no theme is set, it means the application is being created
   * from a JSON import, git import. In that case, we'll import the edit mode theme and published
   * mode theme from the JSON file and update the application. If the destination application
   * already has a theme, it means we're doing any of these Git operations - pull, merge, discard.
   * In this case, we'll decide based on this decision tree: - If current theme is a customized one
   * and source theme is also customized, replace the current theme properties with source theme
   * properties - If current theme is a customized one and source theme is system theme, set the
   * current theme to system and delete the old one - If current theme is system theme, update the
   * current theme as per source theme
   *
