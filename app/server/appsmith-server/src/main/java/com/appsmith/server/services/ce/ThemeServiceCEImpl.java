...
@Slf4j
public class ThemeServiceCEImpl extends BaseService<ThemeRepositoryCE, Theme, String>
    ...
    private String
            defaultThemeId; // acts as a simple cache so that we don't need to fetch from DB always
    ...
    public Mono<Theme> update(String s, Theme resource) {
        // we don't allow to update a theme by id, user can only update a theme under their
        // application
    ...
    @Override
    public Mono<Theme> getApplicationTheme(
                        ...
                        application -> {
                            ...
                            if (StringUtils.hasLength(themeId)) {
                                return repository
                                        ...
                                        .switchIfEmpty(
                                                repository.getSystemThemeByName(
    ...
    public Flux<Theme> getApplicationThemes(String applicationId, String branchName) {
        return applicationService
                ...
                .flatMapMany(
                        application ->
                                repository.getApplicationThemes(application.getId(), READ_THEMES));
    }
    ...
    public Mono<Theme> updateTheme(String applicationId, String branchName, Theme resource) {
        return applicationService
                ...
                .flatMap(
                        application -> {
                            // makes sure user has permission to edit application and an application
                            // exists by this applicationId
    ...
    @Override
    public Mono<Theme> changeCurrentTheme(
            String newThemeId, String applicationId, String branchName) {
        return applicationService
                .findByBranchNameAndDefaultApplicationId(
                        branchName, applicationId, applicationPermission.getEditPermission())
                .switchIfEmpty(
    ...
    public Mono<String> getDefaultThemeId() {
        if (StringUtils.isEmpty(defaultThemeId)) {
            return repository
                    .getSystemThemeByName(Theme.DEFAULT_THEME_NAME)
                    .map(
                            theme -> {
    ...
    public Mono<Theme> cloneThemeToApplication(String srcThemeId, Application destApplication) {
        return repository
                .findById(srcThemeId, READ_THEMES)
                .flatMap(
                        theme -> {
                            if (theme.isSystemTheme()) { // it's a system theme, no need to copy
                                ...
                                theme.setId(
                                        null); // setting id to null so that save method will create
                                // a new instance
                                theme.setApplicationId(null);
                                theme.setWorkspaceId(null);
                                theme.setPolicies(
                                        policyGenerator.getAllChildPolicies(
                                                destApplication.getPolicies(),
                                                Application.class,
                                                Theme.class));
        ...
        return applicationRepository
                ...
                .flatMap(
                        application -> {
                            ...
                            if (!StringUtils.hasLength(
                                    application
                                            .getEditModeThemeId())) { // theme id is empty, use the
                                // default theme
                                editModeThemeMono =
                                        repository.getSystemThemeByName(Theme.LEGACY_THEME_NAME);
                            } else { // theme id is not empty, fetch it by id
                                editModeThemeMono =
                                        repository.findById(
                                                application.getEditModeThemeId(), READ_THEMES);
                            }

                            return editModeThemeMono.flatMap(
                                    editModeTheme -> {
                                        if (editModeTheme
                                                .isSystemTheme()) { // system theme is set as edit
                                            // mode theme
                                            // Delete published mode theme if it was a copy of
                                            // custom theme
                                            return deletePublishedCustomizedThemeCopy(
                                                            application.getPublishedModeThemeId())
                                                    .then(
                                                            // Set the system theme id as edit and
                                                            // published mode theme id to
                                                            // application object
                                        ...
                                        } else { // a customized theme is set as edit mode theme,
                                            // copy that theme for published mode
                                            return saveThemeForApplication(
                                                    application.getPublishedModeThemeId(),
                                                    editModeTheme,
                                                    application,
    ...
    /**
     * Creates a new theme if Theme with provided themeId is a system theme. It sets the properties
     * from the provided theme resource to the existing or newly created theme. It'll also update
     * the application if a new theme was created.
     *
     * @param currentThemeId ID of the existing theme that might be updated
     * @param targetThemeResource new theme DTO that'll be stored as a new theme or override the
     *     existing theme
                ...
                .flatMap(
                        currentTheme -> {
                            ...
                            if (currentTheme.isSystemTheme()) {
                                ...
                                currentTheme.setId(
                                        null); // setting id to null will create a new theme
                                currentTheme.setSystemTheme(false);
                                currentTheme.setPolicies(
                                        policyGenerator.getAllChildPolicies(
                                                application.getPolicies(),
                                                Application.class,
                                                Theme.class));
                                // Not setting the application id in the theme because only the
                                // named themes have an application id
                                newThemeCreated = true;
                            }
                            return repository
                                    .save(currentTheme)
                                    .zipWith(Mono.just(newThemeCreated));
                        })
                .flatMap(
                        savedThemeTuple -> {
                            Theme theme = savedThemeTuple.getT1();
                            if (savedThemeTuple
                                    .getT2()) { // new theme created, update the application
                                ...
                                return Mono.just(theme); // old theme overwritten, no need to update
                                // application
                            }
                        });
    }
    ...
    @Override
    public Mono<Theme> persistCurrentTheme(
            String applicationId, String branchName, Theme resource) {
        ...
        return applicationService
                ...
                .flatMap(
                        application -> {
                            String themeId = application.getEditModeThemeId();
                            if (!StringUtils.hasLength(
                                    themeId)) { // theme id is not present, raise error
                                return Mono.error(
                ...
                .flatMap(
                        themeAndApplicationTuple -> {
                            ...
                            // A named theme differs from a non-named one by having an application
                            // id
    ...
    /**
     * This method will fetch a theme by id and delete this if it's not a system theme. When an app
     * is published with a customized theme, we store a copy of that theme so that changes are
     * available in published mode even user has changed the theme in edit mode. When user switches
     * back to another theme and publish the application where that app was previously published
     * with a custom theme, we should delete that copy. Otherwise there'll be a lot of orphan theme
     * copies that were set a published mode once but are used no more.
     *
    ...
    public Mono<Theme> archiveById(String themeId) {
        return repository
                ...
                .flatMap(
                        theme -> {
                            if (StringUtils.hasLength(
                                    theme.getApplicationId())) { // only persisted themes are
    ...
    public Mono<Theme> updateName(String id, Theme themeDto) {
        return repository
                ...
                .switchIfEmpty(
                        Mono.error(
                                new AppsmithException(
                                        AppsmithError.NO_RESOURCE_FOUND, FieldName.THEME, id)))
                .flatMap(
                        theme -> {
    ...
    public Mono<Theme> getOrSaveTheme(Theme theme, Application destApplication) {
        if (theme == null) { // this application was exported without theme, assign the legacy theme
            // to it
            return repository.getSystemThemeByName(
                    Theme.LEGACY_THEME_NAME); // return the default theme
    ...
    /**
     * This will archive themes related to the provided Application. It'll delete any theme that was
     * saved for this application. It'll also delete the draft themes for this Application.
     *
    ...
    public Mono<Application> archiveApplicationThemes(Application application) {
        return repository
                ...
                .then(
                        repository.archiveDraftThemesById(
                                application.getEditModeThemeId(),
                                application.getPublishedModeThemeId()))
    ...
    /**
     * This method imports a theme from a JSON file to an application. The destination application
     * can already have a theme set or not. If no theme is set, it means the application is being
     * created from a JSON import, git import. In that case, we'll import the edit mode theme and
     * published mode theme from the JSON file and update the application. If the destination
     * application already has a theme, it means we're doing any of these Git operations - pull,
     * merge, discard. In this case, we'll decide based on this decision tree: - If current theme is
     * a customized one and source theme is also customized, replace the current theme properties
     * with source theme properties - If current theme is a customized one and source theme is
     * system theme, set the current theme to system and delete the old one - If current theme is
     * system theme, update the current theme as per source theme
     *
     ...
     */
    @Override
    public Mono<Application> importThemesToApplication(
            Application destinationApp, ApplicationJson sourceJson) {
        Mono<Theme> editModeTheme =
                updateExistingAppThemeFromJSON(
                        destinationApp,
                        destinationApp.getEditModeThemeId(),
                        sourceJson.getEditModeTheme());

        Mono<Theme> publishedModeTheme =
                updateExistingAppThemeFromJSON(
                        destinationApp,
                        destinationApp.getPublishedModeThemeId(),
                        sourceJson.getPublishedTheme());
        ...
        return Mono.zip(editModeTheme, publishedModeTheme)
                .flatMap(
                        importedThemesTuple -> {
                            ...
                            // also returning the updated application object so that theme id are
                            // available to the next pipeline
                            return applicationService
                                    .setAppTheme(
    ...
    private Mono<Theme> updateExistingAppThemeFromJSON(
        ...
        return repository
                .findById(existingThemeId, READ_THEMES)
                .flatMap(
                        existingTheme -> {
                            if (existingTheme.isSystemTheme()) {
                                ...
                                if (themeFromJson.isSystemTheme()) {
                                    ...
                                    return repository.updateById(
                                            existingThemeId, themeFromJson, MANAGE_THEMES);
                                }
                            }
                        });
    }
