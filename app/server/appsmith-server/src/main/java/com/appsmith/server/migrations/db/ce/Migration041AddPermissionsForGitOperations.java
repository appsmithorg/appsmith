...
public class Migration041AddPermissionsForGitOperations {
    ...
    public void addPermissionForGitOperationsToExistingApplications() {
        ...
        */
        String policiesFieldPath = BaseAppsmithRepositoryCEImpl.fieldName(QBaseDomain.baseDomain.policies);
        String idFieldPath = BaseAppsmithRepositoryCEImpl.fieldName(QBaseDomain.baseDomain.id);
        String workspaceIdFieldPath = BaseAppsmithRepositoryCEImpl.fieldName(QApplication.application.workspaceId);
        ...
        do {
            ...
            Criteria criteria = Criteria.where(FieldName.DELETED_AT)
                    .exists(false)
                    .and(MIGRATION_FLAG_WORKSPACE_WITHOUT_GIT_PERMISSIONS)
                    .exists(true);
            Query selectWorkspacesQuery = Query.query(criteria).limit(batchSize);
            selectWorkspacesQuery.skip(updatedCount);
            selectWorkspacesQuery.fields().include(policiesFieldPath); // we only need the policies
            ...
            for (Workspace workspace : workspaceList) {
                ...
                // get the permission groups from the parent of the new permissions i.e. WORKSPACE_CREATE_APPLICATION
                Optional<Set<String>> optionalPermissionGroupsForManagePermission = workspace.getPolicies().stream()
                        .filter(policy ->
                                policy.getPermission().equals(AclPermission.WORKSPACE_CREATE_APPLICATION.getValue()))
                        .map(Policy::getPermissionGroups)
                        .findFirst();

                if (optionalPermissionGroupsForManagePermission.isPresent()) {
                    Set<String> parentPolicyPermissionGroups = optionalPermissionGroupsForManagePermission.get();
                    ...
                    for (Application application : applicationList) {
                        // add a new policy for git permissions and allow parentPolicyPermissionGroups
                        addGitPoliciesToPolicySet(application.getPolicies(), parentPolicyPermissionGroups);
                        ...
                        Query updateQuery =
                                Query.query(Criteria.where(idFieldPath).is(application.getId()));
