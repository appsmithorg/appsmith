...
public class CreateDBTablePageSolutionCEImpl implements CreateDBTablePageSolutionCE {
  ...
  public Mono<CRUDPageResponseDTO> createPageFromDBTable(
      String defaultPageId, CRUDPageResourceDTO pageResourceDTO, String branchName) {
    ...
    */
    ...
    if (pageResourceDTO.getTableName() == null || pageResourceDTO.getDatasourceId() == null) {
      return Mono.error(
          new AppsmithException(AppsmithError.INVALID_PARAMETER, "tableName and datasourceId"));
    } else {
      if (pageResourceDTO.getApplicationId() == null) {
        return Mono.error(
    ...
    Mono<Datasource> datasourceMono =
        datasourceService
            ...
            .switchIfEmpty(
                Mono.error(
                    new AppsmithException(
                        AppsmithError.INVALID_DATASOURCE, FieldName.DATASOURCE, datasourceId)));
    ...
    return datasourceMono
        ...
        .flatMap(
            tuple -> {
              ...
              if (pageList.isEmpty()) {
                return Mono.error(
                    new AppsmithException(
                        AppsmithError.NO_RESOURCE_FOUND, TEMPLATE_APPLICATION_FILE));
              }
                          ...
                          datasource1 -> {
                            ...
                            return StringUtils.equals(datasource1.getPluginId(), pluginRef)
                                ...
                                || (StringUtils.equals(
                                        ...
                                        Entity.POSTGRES_PLUGIN_PACKAGE_NAME)
              ...
              DatasourceConfigurationStructure templateDatasourceConfigurationStructure =
                  applicationJson.getDatasourceConfigurationStructureList().stream()
                      .filter(
                          configurationStructure ->
                              StringUtils.equals(
                                  configurationStructure.getDatasourceId(),
                                  templateDatasource.getName()))
                      .findAny()
                      .orElse(null);

              DatasourceStructure templateStructure =
                  templateDatasourceConfigurationStructure.getStructure();
        ...
        .flatMap(
            tuple -> {
              ...
              log.debug(
                  "Going to clone actions from template application for page {}", savedPageId);
              return cloneActionsFromTemplateApplication(
                  ...
                  .flatMap(
                      actionDTO ->
                          StringUtils.equals(actionDTO.getName(), SELECT_QUERY)
                          ...
                          .flatMap(
                              pageDTO -> {
                                ...
                                return sendGenerateCRUDPageAnalyticsEvent(
                                        crudPage, datasource, plugin.getName())
                                    .map(
                                        res -> {
  ...
  private Mono<NewPage> getOrCreatePage(
    ...
    */
    ...
    return applicationService
        ...
        .flatMap(
            pageDTO ->
                newPageService.findById(pageDTO.getId(), pagePermission.getEditPermission()));
  }
  ...
  /**
   * This function will clone actions from the template application and update action configuration
   * using mapped columns between the template datasource and datasource in context
   *
                      ...
                      pluginExecutor -> {
                        ...
                        actionDTO.setActionConfiguration(
                            deleteUnwantedWidgetReferenceInActions(
                                actionConfiguration, deletedWidgetNames));
                        return actionDTO;
                      })
    ...
    */
    ...
    if (searchColumn != null && !searchColumn.isEmpty()) {
      ...
      mappedTableColumns.put(
          DEFAULT_SEARCH_COLUMN, Objects.requireNonNullElse(tempSearchColumn, DELETE_FIELD));
    }
  ...
  private Map<String, String> mapKeys(
    ...
    */
    ...
    // Map keys for NoSQL datasources like MongoDB
      ...
      if (DatasourceStructure.TableType.COLLECTION.equals(sourceTable.getType())) {
        ...
        List<Column> autogeneratedColumns =
            destTable.getColumns().stream()
                ...
                // structure like primaryKey in SQL for MongoDB. We can safely
                // assume that these field will act as a
        ...
        // If column name "_id" is present in user's collection then map template datasource
        // key "_id" with column name ,
        // else map template datasource key "_id" to any autogenerated field from user's
        // collection
        if (destKey != null) {
          primaryKeyNameMap.put(sourceKey, destKey);
        } else {
          if (!CollectionUtils.isEmpty(autogeneratedColumns)) {
      ...
      if (FieldName.PRIMARY_COLUMNS.equals(key)) {
        ...
        for (String columnName : (Set<String>) primaryColumns.keySet()) {
          if (FieldName.MONGO_ESCAPE_ID.equals(columnName)) {
            ...
            if (FieldName.MONGO_ESCAPE_CLASS.equals(columnName)) {
              updateRequired = true;
        ...
        if (FieldName.DROP_DOWN_WIDGET.equals(widgetDsl.getAsString(FieldName.TYPE))
          ...
          if (FieldName.OPTIONS.equals(key)) {
            // This will update the options field to include all the column names as
            // label and value
            ...
            if (FieldName.DEFAULT_OPTION.equals(key)) {
              ...
              matcher.replaceAll(
                  field ->
                      ...
                      mappedColumnsAndTableNames.get(field.group()) == null
                          ? field.group()
                          : mappedColumnsAndTableNames.get(field.group()).replaceAll("\\W+", "_")));
        }
      }
    }
    ...
    */
    ...
    if (actionConfiguration.getBody() != null) {
      ...
      // condition
        ...
        if (actionConfiguration.getBody().matches("(?s).*,[\\W]*?(?i)WHERE.*")) {
          actionConfiguration.setBody(
  ...
  private void createSuccessMessageAndSetAsset(Plugin plugin, CRUDPageResponseDTO crudPage) {
    ...
    String updateWidget =
        Entity.S3_PLUGIN_PACKAGE_NAME.equals(plugin.getPackageName()) ? "Filepicker" : "Form";
  ...
  private Mono<CRUDPageResponseDTO> sendGenerateCRUDPageAnalyticsEvent(
      CRUDPageResponseDTO crudPage, Datasource datasource, String pluginName) {
