...
// Default SelectWidget dropdown value for MongoDB template page which will be used in find query
// for sort operator
private static final String MONGO_DEFAULT_DROPDOWN_VALUE = "1";

// This column will be used to map filter in Find and Select query. This particular field is added
// to have
	...
	*/
	...
	return datasourceMono
		...
		.flatMap(
					...
					.filter(
						datasource1 -> {
							...
							return StringUtils.equals(datasource1.getPluginId(), pluginRef)
								// In template resource we have used Postgresql as a representative
								// of all sql datasource
			...
			// We are supporting datasources for both with and without datasource structure. So if
			// datasource
			// structure is present then we can assign the mapping dynamically as per the template
			// datasource tables.
			// Those datasources for which we don't have structure like Google sheet etc we are
			// following a
			// protocol in template application that all the column headers should be named as
			// col1, col2,.... colx
			...
			if (templateStructure != null
				...
				int colCount = 1;
				// If the structure and tables not present in the template datasource then map the
				// columns from
				...
				// In template application we have used col1 - col5 so if users tables have less
				// number of
			...
			// Extract S3 bucket name from template application and map to users bucket. Bucket
			// name is stored at
	...
	*/
	...
	return Flux.fromIterable(templateActionList)
		.flatMap(
			...
			if (actionBody != null) {
				// If the primary key is autogenerated remove primaryKey's reference from
				// InsertQuery
			...
			if (!CollectionUtils.isEmpty(pluginSpecifiedTemplates)) {
				pluginSpecifiedTemplates.forEach(
					...
					if (property != null && property.getValue() instanceof String) {
						...
						// Replace template S3 bucket with user's  bucket. Here we can't apply
						// WORD_PATTERN
...
private Map<String, String> mapTableColumnNames(
	...
	*/
	...
	// Assign string type column as default search column. We are only supporting String matching
	// for filter clause
...
private Map<String, String> mapKeys(
	...
	*/
	...
	if (primaryKey != null) {
		...
		// In updated template we are using __primaryKey__ inside JsonForm to omit the field which
		// will prevent the
		// duplicate key exception
		primaryKeyNameMap.put(PRIMARY_KEY, destKey);

		// Check if the destKey is autogenerated, and save source column name which will be used to
		// remove reference
	...
	List<Column> autogeneratedColumns =
		destTable.getColumns().stream()
			// This makes sure only keys with instance of objectId will be considered as we don't
			// have definitive
			// structure like primaryKey in SQL for MongoDB. We can safely assume that these field
			// will act as a
	...
	// If column name "_id" is present in user's collection then map template datasource key "_id"
	// with column name ,
...
private JSONObject updateTemplateWidgets(
	...
	*/
	...
	// Handle special case for insert_modal for JSON form when auto-update is disabled for primary
	// key. Don't exclude
	...
	if (FieldName.PRIMARY_COLUMNS.equals(key)) {
		...
		// in SelectWidget except for SelectWidget with DefaultOptionValue SQL(DefaultValue :
		// "ASC")
		...
		widgetDsl.put(
			...
			matcher.replaceAll(
				field ->
					// Replace any special characters with "_" as all the special characters are
					// replaced with "_" in
					// table column widget
...
private ActionConfiguration deleteUnwantedWidgetReferenceInActions(
	...
	*/
	...
	// When the connected datasource have less number of columns than template datasource, delete
	// the unwanted fields
	/*
	Example 1: SQL
	...
	actionConfiguration
		...
		.forEach(
			property -> {
				...
				// This will remove the unwanted comma after fields deletion if present at the end
				// of body
				// "field1\","field2\",\n\t\"field3" \n,{non-word-characters})\n
