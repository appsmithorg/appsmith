...
public class CreateDBTablePageSolutionCEImpl implements CreateDBTablePageSolutionCE {
    ...
    private static final String TEMPLATE_AUTOGENERATED_PRIMARY_COLUMN = "col1";
        ...
        if (children != null) {
            for (Object obj : children) {
                ...
                if (!CollectionUtils.isEmpty(data)) {
                    ...
                    final String autogeneratedField = mappedColumnsAndTableNames.get(templateAutogeneratedKey);
                    if (FieldName.TABLE_WIDGET.equals(widgetType)
                        ...
                        if (FieldName.JSON_FORM_WIDGET.equals(widgetType)
                                && !StringUtils.isEmpty(autogeneratedField)) {
                            // Update widget for the field which is autogenerated as per DB structure from InsertQuery
                            Gson gson = new Gson();

                            JsonObject schema = gson.fromJson(child.toJSONString(), JsonObject.class)
                                    .getAsJsonObject(FieldName.SCHEMA);

                            JsonObject jsonFormSchemaChildren = schema != null
                                    ? schema.getAsJsonObject(FieldName.ROOT_SCHEMA) != null
                                        ? schema.getAsJsonObject(FieldName.ROOT_SCHEMA).getAsJsonObject(FieldName.CHILDREN)
                                        : null
                                    : null;
                            if (jsonFormSchemaChildren != null) {
                                ...
                                JsonObject primaryKeyChild = jsonFormSchemaChildren.getAsJsonObject(templateAutogeneratedKey);
                                primaryKeyChild.addProperty(FieldName.PLACEHOLDER_TEXT, "Autogenerated Field");
                                primaryKeyChild.addProperty(FieldName.IS_DISABLED, true);
                                primaryKeyChild.addProperty(FieldName.IS_REQUIRED, false);
                                //deletedWidgets.add(primaryKeyChild.get());
                            }
                        }
