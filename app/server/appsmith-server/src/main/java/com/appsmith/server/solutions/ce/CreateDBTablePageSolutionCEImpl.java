...
public class CreateDBTablePageSolutionCEImpl implements CreateDBTablePageSolutionCE {
    ...
    // Default SelectWidget dropdown value for SQL and Postgres template pages which will be used in
    // select query for sort operator
    private static final String SQL_DEFAULT_DROPDOWN_VALUE = "asc";

    // Default SelectWidget dropdown value for MongoDB template page which will be used in find
    // query for sort operator
    private static final String MONGO_DEFAULT_DROPDOWN_VALUE = "1";

    // This column will be used to map filter in Find and Select query. This particular field is
    // added to have
    ...
    // Widget fields those need to be mapped between template DB table and user's DB table in for
    ...
    // Currently, we only support string matching (like/ilike etc) for WHERE operator in SelectQuery
    // so the allowed
    ...
    private final Gson gson =
            new GsonBuilder()
                    .registerTypeAdapter(
                            DatasourceStructure.Key.class,
                            new DatasourceStructure.KeyInstanceCreator())
    ...
    public Mono<CRUDPageResponseDTO> createPageFromDBTable(
        ...
        if (Boolean.TRUE.equals(isTrueEnvironmentIdRequired)) {
            return datasourceService
                    ...
                    .flatMap(
                            workspaceId ->
                                    datasourceService.getTrueEnvironmentId(
                                            workspaceId, environmentId))
                    .flatMap(
                            trueEnvironmentId ->
                                    createPageFromDBTable(
                                            defaultPageId,
                                            pageResourceDTO,
                                            trueEnvironmentId,
                                            branchName));
    ...
    /**
     * This function will clone template page along with the actions. DatasourceStructure is used to
     * map the templateColumns with the datasource under consideration
     *
    ...
    public Mono<CRUDPageResponseDTO> createPageFromDBTable(
        ...
        */
        ...
        // All SQL datasources will be mapped to postgresql as actionBody will be same and the same
        // logic is used
        ...
        if (pageResourceDTO.getTableName() == null || pageResourceDTO.getDatasourceId() == null) {
            return Mono.error(
                    new AppsmithException(
                            AppsmithError.INVALID_PARAMETER, "tableName and datasourceId"));
        ...
        Mono<NewPage> pageMono =
                getOrCreatePage(defaultApplicationId, defaultPageId, tableName, branchName);
        ...
        return datasourceStorageMono
                .zipWhen(
                        datasourceStorage ->
                                Mono.zip(
                                        ...
                                        datasourceStructureSolution.getStructure(
                                                datasourceStorage, false)))
                .flatMap(
                        tuple -> {
                            ...
                            if (layout == null) {
                                return Mono.error(
                                        new AppsmithException(
                                                AppsmithError.NO_RESOURCE_FOUND, FieldName.PAGE));
                            }
                            layout.setId(null);
                            ...
                            DatasourceStorage templateDatasource =
                                    applicationJson.getDatasourceList().stream()
                                            .filter(
                                                    datasource1 -> {
                                                        ...
                                                        return StringUtils.equals(
                                                                        ...
                                                                        pluginRef)
                                                                // In template resource we have used
                                                                // Postgresql as a representative of
                                                                // all sql datasource
                                                                ...
                                                                || (StringUtils.equals(
                                                                                datasource1
                                                                                        .getPluginId(),
                                                                                Entity
                                                                                        .POSTGRES_PLUGIN_PACKAGE_NAME)
                                                                        && sqlPackageNames.contains(
                                                                                pluginRef));
                                                    })
                                            .findAny()
                                            .orElse(null);
                                            ...
                                            .filter(
                                                    configurationStructure ->
                                                            StringUtils.equals(
                                                                    configurationStructure
                                                                            .getDatasourceId(),
                                                                    templateDatasource.getName()))
                                            .findAny()
                                            .orElse(null);

                            DatasourceStructure templateStructure =
                                    templateDatasourceStorageStructure.getStructure();
                            // We are supporting datasources for both with and without datasource
                            // structure. So if datasource
                            // structure is present then we can assign the mapping dynamically as
                            // per the template datasource tables.
                            // Those datasources for which we don't have structure like Google sheet
                            // etc we are following a
                            // protocol in template application that all the column headers should
                            // be named as col1, col2,.... colx

                            String tableNameUsedInAction = tableName;
                            if (templateStructure != null
                                    && !CollectionUtils.isEmpty(templateStructure.getTables())) {
                                Table templateTable =
                                        templateStructure.getTables().stream()
                                                .filter(
                                                        table1 ->
                                ...
                                // Sample template body : SELECT * FROM public."templateTable" LIMIT
                                // 10;
                                if (!CollectionUtils.isEmpty(table.getTemplates())
                                        && table.getTemplates().get(0).getBody() != null) {
                                    final Pattern pattern =
                                            Pattern.compile("[^ ]*" + tableRef + "[^ ,]*");
                                    final Matcher matcher =
                                            pattern.matcher(table.getTemplates().get(0).getBody());

                                    tableNameUsedInAction =
                                            matcher.find() ? matcher.group(0) : tableName;
                                }
                                mappedColumnsAndTableName.putAll(
                                        mapTableColumnNames(
                                                templateTable,
                                                table,
                                                searchColumn,
                                ...
                                int colCount = 1;
                                // If the structure and tables not present in the template
                                // datasource then map the columns from
                                // template application where we are following the nomenclature of
                                // col1, col2, ...
                                if (!CollectionUtils.isEmpty(columns)) {
                                    ...
                                    // In template application we have used col1 - col5 so if users
                                    // tables have less number of
                                    // columns these fields need to be deleted
                                    if (colCount <= TEMPLATE_COLUMN_COUNT) {
                                        for (String column : columns) {
                                            mappedColumnsAndTableName.put(
                                                    "col" + colCount, DELETE_FIELD);
                                            colCount++;
                                        }
                                    }
                                }
                            ...
                            // Extract S3 bucket name from template application and map to users
                            // bucket. Bucket name is stored at
                            ...
                            log.debug(
                                    "Going to update layout for page {} and layout {}",
                                    savedPageId,
                                    layoutId);
                ...
                .flatMap(
                        tuple -> {
                            ...
                            log.debug(
                                    "Going to clone actions from template application for page {}",
                                    savedPageId);
                                            ...
                                            actionDTO ->
                                                    StringUtils.equals(
                                                                    ...
                                                                    || StringUtils.equals(
                                                                            actionDTO.getName(),
                                                                            LIST_QUERY)
                                                            ? layoutActionService.setExecuteOnLoad(
                                                                    actionDTO.getId(), true)
                                                            : Mono.just(actionDTO))
                                    .then(
                                            applicationPageService
                                                    ...
                                                    .flatMap(
                                                            pageDTO -> {
                                                                CRUDPageResponseDTO crudPage =
                                                                        new CRUDPageResponseDTO();
                                                                crudPage.setPage(pageDTO);
                                                                createSuccessMessageAndSetAsset(
                                                                        plugin, crudPage);
                                                                        ...
                                                                        .map(
                                                                                res -> {
                                                                                    PageDTO
                                                                                            sanitisedResponse =
                                                                                                    responseUtils
                                                                                                            .updatePageDTOWithDefaultResources(
                                                                                                                    res
                                                                                                                            .getPage());
                                                                                    crudPage
                                                                                            .setPage(
     ...
     * @return NewPage if not present already with the incremental suffix number to avoid duplicate
     *     application names
     */
                            ...
                            newPage -> {
                                ...
                                if (!CollectionUtils.isEmpty(layout.getWidgetNames())
                                    ...
                                    throw new AppsmithException(
                                            AppsmithError.INVALID_CRUD_PAGE_REQUEST,
                                            "please use empty layout");
                                }
                                return newPage;
                            });
        }
                ...
                .flatMap(
                        pages -> {
                            ...
                            for (PageDTO pageDTO : pages) {
                                if (pageDTO.getName()
                                        .matches("^" + Pattern.quote(pageName) + "\\d*$")) {
                                    long count = 1L;
                                    String pageCount =
                                            pageDTO.getName().substring(pageName.length());
                ...
                .flatMap(
                        pageDTO ->
                                newPageService.findById(
                                        pageDTO.getId(), pagePermission.getEditPermission()));
    }
    ...
    /**
     * This will fetch the template application resource which then act as a reference to clone
     * layouts and actions
     *
    ...
    /**
     * This function will clone actions from the template application and update action
     * configuration using mapped columns between the template datasource and datasource in context
     *
     ...
     * @param templateActionList Actions from the template application related to specific
     *     datasource
     * @param mappedColumns Mapped column names between template and resource table under
     *     consideration
     * @param deletedWidgetNames Deleted column ref when template application have more # of columns
     *     than the users table
        ...
        return Flux.fromIterable(templateActionList)
                .flatMap(
                        templateAction -> {
                            ...
                            if (actionBody != null) {
                                // If the primary key is autogenerated remove primaryKey's reference
                                // from InsertQuery
                                final String temp = mappedColumns.get(templateAutogeneratedKey);
                                if (!templateAutogeneratedKey.isEmpty()
                                        && INSERT_QUERY.equals(actionDTO.getName())) {
                                    mappedColumns.put(templateAutogeneratedKey, DELETE_FIELD);
                                }
                                String body =
                                        actionBody.replaceFirst(TEMPLATE_TABLE_NAME, tableName);
                                final Matcher matcher = WORD_PATTERN.matcher(body);
                                actionConfiguration.setBody(
                                        matcher.replaceAll(
                                                key ->
                                                        mappedColumns.get(key.group()) == null
                                                                ? key.group()
                            ...
                            if (!CollectionUtils.isEmpty(pluginSpecifiedTemplates)) {
                                pluginSpecifiedTemplates.forEach(
                                        property -> {
                                            if (property != null
                                                    && property.getValue() instanceof String) {
                                                if (Entity.S3_PLUGIN_PACKAGE_NAME.equals(
                                                                templateAction.getPluginId())
                                                        && mappedColumns.containsKey(
                                                                property.getValue().toString())) {
                                                    // Replace template S3 bucket with user's
                                                    // bucket. Here we can't apply WORD_PATTERN
                                                    // matcher as the bucket name can be
                                                    // test.appsmith etc
                                                    property.setValue(
                                                            mappedColumns.get(
                                                                    property.getValue()
                                                        ...
                                                        && !CollectionUtils.isEmpty(
                                                                pluginSpecificTemplateParams)
                                                        && pluginSpecificTemplateParams.get(
                                                                        property.getKey())
                                                                != null) {
                                                    property.setValue(
                                                            pluginSpecificTemplateParams.get(
                                    ...
                                    .map(
                                            pluginExecutor -> {
                                                ...
                                                actionDTO.setActionConfiguration(
                                                        deleteUnwantedWidgetReferenceInActions(
                                                                actionConfiguration,
                                                                deletedWidgetNames));
                                                return actionDTO;
                                            })
    ...
    /**
     * This function maps the column names between the template datasource table and the user's
     * table
     *
     * @param sourceTable provides keys for Map from column names
     * @param destTable provides values for Map from column names
     * @param searchColumn specific column provided to implement the filter for Select and Find
     *     query
     * @param tableColumns Specific columns provided by higher order function to act as values for
     *     Map
     * @return
     */
    private Map<String, String> mapTableColumnNames(
        ...
        */
        log.debug(
                "Mapping column names with template application for table {}", destTable.getName());
        Map<String, String> mappedTableColumns = new HashMap<>();

        // Assign string type column as default search column. We are only supporting String
        // matching for filter clause
        if (searchColumn != null && !searchColumn.isEmpty()) {
            mappedTableColumns.put(DEFAULT_SEARCH_COLUMN, searchColumn);
            sourceTable
                    .getColumns()
                    .removeIf(column -> DEFAULT_SEARCH_COLUMN.equals(column.getName()));
            destTable.getColumns().removeIf(column -> searchColumn.equals(column.getName()));
        } else {
            final String tempSearchColumn =
                    destTable.getColumns().stream()
                            .filter(
                                    column ->
                                            ALLOWED_TYPE_FOR_WHERE_CLAUSE.contains(
                                                    column.getType()
                                                            .replaceAll("[^A-Za-z]+", "")
                                                            .toLowerCase()))
                            .findAny()
            ...
            mappedTableColumns.put(
                    ...
                    Objects.requireNonNullElse(tempSearchColumn, DELETE_FIELD));
        }
        ...
        while (idx < sourceTableColumns.size() && idx < destTableColumns.size()) {
            if (!mappedTableColumns.containsKey(sourceTableColumns.get(idx).getName())) {
                mappedTableColumns.put(
                        sourceTableColumns.get(idx).getName(), destTableColumns.get(idx).getName());
            }
            idx++;
        }
    ...
    /**
     * This function maps the primary key names between the template datasource table and the user's
     * table
     *
    ...
    private Map<String, String> mapKeys(
            Table sourceTable, Table destTable, StringBuffer templateAutogeneratedKey) {
        ...
        */
        ...
        if (DatasourceStructure.TableType.TABLE.equals(sourceTable.getType())) {
            ...
            DatasourceStructure.Key primaryKey =
                    destTable.getKeys().stream()
            ...
            if (primaryKey != null) {
                ...
                // In updated template we are using __primaryKey__ inside JsonForm to omit the field
                // which will prevent the
                // duplicate key exception
                primaryKeyNameMap.put(PRIMARY_KEY, destKey);

                // Check if the destKey is autogenerated, and save source column name which will be
                // used to remove reference
                // from InsertQuery otherwise InsertQuery will fail with error : Trying to insert
                // autogenerated field
                templateAutogeneratedKey.append(
                        destTable.getColumns().stream()
                                .filter(
                                        column ->
                                                column.getIsAutogenerated() != null
        ...
        // Map keys for NoSQL datasources like MongoDB
            ...
            List<Column> autogeneratedColumns =
                    destTable.getColumns().stream()
                            // This makes sure only keys with instance of objectId will be
                            // considered as we don't have definitive
                            // structure like primaryKey in SQL for MongoDB. We can safely assume
                            // that these field will act as a
            ...
            // If column name "_id" is present in user's collection then map template datasource key
            // "_id" with column name ,
            // else map template datasource key "_id" to any autogenerated field from user's
            // collection
            if (destKey != null) {
                primaryKeyNameMap.put(sourceKey, destKey);
    ...
    private JSONObject updateTemplateWidgets(
            JSONObject widgetDsl,
            Map<String, String> mappedColumnsAndTableNames,
            String templateAutogeneratedKey) {
        ...
        */
        List<String> keys =
                widgetDsl.keySet().stream()
                        .filter(WIDGET_FIELDS::contains)
                        .collect(Collectors.toList());

        // This field will be used to check the default dropdown value for SelectWidget and only
        // required SelectWidget's
        // options will be updated
        String defaultDropdownValue =
                widgetDsl.containsKey(FieldName.DEFAULT_OPTION)
                        ? widgetDsl.getAsString(FieldName.DEFAULT_OPTION)
                        : "";

        // Handle special case for insert_modal for JSON form when auto-update is disabled for
        // primary key. Don't exclude
        ...
        for (String key : keys) {
            if (FieldName.PRIMARY_COLUMNS.equals(key)) {
                ...
                if (FieldName.OPTIONS.equals(key)) {
                    // This will update the options field to include all the column names as label
                    // and value
                    // in SelectWidget except for SelectWidget with DefaultOptionValue
                    // SQL(DefaultValue : "ASC")
                    // and Mongo(DefaultValue : "1") check template application layout for more
                    // details
                    List<String> dropdownOptions = new ArrayList<>();
                    mappedColumnsAndTableNames.forEach(
                            (colKey, colVal) -> {
                                if (colKey.toLowerCase().contains("col")
                                        && !colVal.equals(DELETE_FIELD)) {
                                    dropdownOptions.add(
                                            "\n{\n\t\"label\": \""
                                                    + colVal.replaceAll("\\W+", "_")
                                                    + "\",\n\t\"value\": \""
                                                    + colVal
                                                    + "\"\n}");
                                }
                ...
                widgetDsl.put(
                        ...
                        matcher.replaceAll(
                                field ->
                                        // Replace any special characters with "_" as all the
                                        // special characters are replaced with "_" in
                                        // table column widget
     ...
     * than the user connected datasource. Also it will replace the template column names with the
     * user connected datasource column names
     *
     * @param actionConfiguration resource which needs to be updated
     * @param deletedWidgetNames widgets for which references to be removed from the
     *     actionConfiguration
        ...
        */
        ...
        if (StringUtils.containsIgnoreCase(actionConfiguration.getBody(), "VALUES")) {
            ...
            actionConfiguration.setBody(
                    matcher.replaceAll(
                            field ->
                                    deletedWidgetNames.contains(field.group())
                                            ? DELETE_FIELD
                                            : field.group()));
        }

        // When the connected datasource have less number of columns than template datasource,
        ...
        if (actionConfiguration.getBody() != null) {
            ...
            // This will remove the unwanted comma after fields deletion if present at the end of
            // body
            // "field1\","field2\",\n\t\"field3" \n,{non-word-characters})\n => insertQuery
            if (actionConfiguration.getBody().matches("(?s).*,[\\W]*?\\).*")) {
                actionConfiguration.setBody(
                        actionConfiguration.getBody().replaceAll(",[\\W]*?\\)", ")"));
            }
            // "field1\","field2\",\n\t\"field3\" ,{non-word-characters => \n\t} WHERE => WHERE
            // condition
            else if (actionConfiguration.getBody().matches("(?s).*,[\\W]*?(?i)WHERE.*")) {
                actionConfiguration.setBody(
                        actionConfiguration.getBody().replaceAll(",[\\W]*?WHERE", "\nWHERE"));
            }
        }
                    ...
                    .forEach(
                            property -> {
                                if (property != null && property.getValue() instanceof String) {
                                    property.setValue(
                                            property.getValue().toString().replaceAll(regex, ""));
                                    // This will remove the unwanted comma after fields deletion if
                                    // present at the end of body
                                    // "field1\","field2\",\n\t\"field3" \n,{non-word-characters})\n
     ...
     * @param regex to replace the unwanted field this will be useful when the connected datasource
     *     have less number of columns than template datasource
     */
        ...
        for (Map.Entry<String, Object> property : formData.entrySet()) {
            if (property.getValue() != null) {
                if (property.getValue() instanceof String) {
                    ...
                    // This will remove the unwanted comma after fields deletion if present at the
                    // end of body
                    // "field1\","field2\",\n\t\"field3" \n,{non-word-characters})\n
                    if (property.getValue().toString().matches("(?s).*,[\\W]*?}.*")) {
                        property.setValue(
                                property.getValue().toString().replaceAll(",[\\W]*?}", "\n}"));
                    }
                }
                if (property.getValue() instanceof Map) {
                    removeUnwantedFieldRefFromFormData(
                            (Map<String, Object>) property.getValue(), regex);
                }
            }
        }
    }
    ...
    private void createSuccessMessageAndSetAsset(Plugin plugin, CRUDPageResponseDTO crudPage) {
        ...
        String displayWidget =
                Entity.S3_PLUGIN_PACKAGE_NAME.equals(plugin.getPackageName()) ? "List" : "Table";
        String updateWidget =
                Entity.S3_PLUGIN_PACKAGE_NAME.equals(plugin.getPackageName())
                        ? "Filepicker"
                        : "Form";
        ...
        String successMessage =
                "We have generated the <b>"
                        + displayWidget
                        + "</b> from the <b>"
                        + plugin.getName()
                        + " datasource</b>. You can use the <b>"
                        + updateWidget
                        + "</b> to modify it. Since all your data is already connected you can add"
                        + " more queries and modify the bindings";
                        ...
                        currentUser -> {
                            try {
                                ...
                                return analyticsService
                                        .sendEvent(
                                                AnalyticsEvents.GENERATE_CRUD_PAGE.getEventName(),
