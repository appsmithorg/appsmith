...
public class AstServiceCEImpl implements AstServiceCE {
    ...
    private final WebClient webClient =
            WebClientUtils.create(
                    ConnectionProvider.builder("rts-provider")
                            .maxConnections(100)
    ...
    public Flux<Tuple2<String, Set<String>>> getPossibleReferencesFromDynamicBinding(
        ...
        */
        ...
        // If RTS server is not accessible for this instance, it means that this is a slim container
        // set up
                    ...
                    .flatMap(
                            bindingValue -> {
                                return Mono.zip(
                                        ...
                                        Mono.just(
                                                new HashSet<>(
                                                        MustacheHelper.getPossibleParentsOld(
                                                                bindingValue))));
                            });
        }
        return webClient
                ...
                .body(
                        BodyInserters.fromValue(
                                new GetIdentifiersRequestBulk(bindingValues, evalVersion)))
                .retrieve()
                ...
                .flatMap(
                        tuple2 -> {
                            ...
                            return Mono.zip(
                                    Mono.just(bindingValues.get((int) currentIndex)),
    ...
    public Mono<Map<MustacheBindingToken, String>> refactorNameInDynamicBindings(
        ...
        return Flux.fromIterable(bindingValues)
                .flatMap(
                        bindingValue -> {
                            EntityRefactorRequest entityRefactorRequest =
                                    new EntityRefactorRequest(
                            ...
                            return webClient
                                    .post()
                                    .uri(
                                            commonConfig.getRtsBaseUrl()
                                                    + "/rts-api/v1/ast/entity-refactor")
                                    .contentType(MediaType.APPLICATION_JSON)
                                    .body(BodyInserters.fromValue(entityRefactorRequest))
                                    .retrieve()
                                    .toEntity(EntityRefactorResponse.class)
                                    .flatMap(
                                            entityRefactorResponseResponseEntity -> {
                                    ...
                                    .map(
                                            tuple -> {
                                                log.debug(
                                                        "Time elapsed since AST refactor call: {}"
                                                                + " ms",
                                                        tuple.getT1());
                                                if (tuple.getT1() > MAX_API_RESPONSE_TIME_IN_MS) {
                                                    log.debug(
                                                            "This call took longer than expected."
                                                                    + " The binding was: {}",
                                                            bindingValue);
                                                }
                                                return tuple.getT2();
                                    ...
                                    .flatMap(
                                            response ->
                                                    Mono.just(bindingValue)
                                                            .zipWith(Mono.just(response.script)))
                                    .onErrorResume(
                                            error -> {
                                                var temp = bindingValue;
                                                // If there is a problem with parsing and
                                                // refactoring this binding, we just ignore it and
                                                // move ahead
                                                // The expectation is that this binding would error
                                                // out during eval anyway
                                                return Mono.empty();
                                            });
                        })
    ...
    /**
     * Consider the following binding: ( function(ignoredAction1) { let a = ignoredAction1.data let
     * ignoredAction2 = { data: "nothing" } let b = ignoredAction2.data let c =
     * "ignoredAction3.data" // ignoredAction4.data return aPostAction.data }
     * )(anotherPostAction.data)
     *
     * <p>The values in the returned instance of GetIdentifiersResponseDetails will be: {
     * references: ["aPostAction.data", "anotherPostAction.data"], functionalParams:
     * ["ignoredAction1"], variables: ["ignoredAction2", "a", "b", "c"]
     */
    @NoArgsConstructor
    @AllArgsConstructor
    @Getter
