...
public class AstServiceCEImpl implements AstServiceCE {
    ...
    return webClient
        ...
        .flatMap(
            tuple2 -> {
              ...
              return Mono.zip(
                  Mono.just(bindingValues.get((int) currentIndex)), Mono.just(references));
            });
            ...
            bindingValue -> {
              ...
              return webClient
                  ...
                  .flatMap(
                      entityRefactorResponseResponseEntity -> {
                        if (HttpStatus.OK.equals(
                            entityRefactorResponseResponseEntity.getStatusCode())) {
                          return Mono.just(
                              Objects.requireNonNull(
                                  entityRefactorResponseResponseEntity.getBody()));
                        }
                        return Mono.error(
                  ...
                  .map(
                      tuple -> {
                        log.debug(
                            "Time elapsed since AST refactor call: {}" + " ms", tuple.getT1());
                        if (tuple.getT1() > MAX_API_RESPONSE_TIME_IN_MS) {
                          log.debug(
                              "This call took longer than expected." + " The binding was: {}",
                              bindingValue);
                        }
                        return tuple.getT2();
                      })
   ...
   * ignoredAction2 = { data: "nothing" } let b = ignoredAction2.data let c = "ignoredAction3.data"
   * // ignoredAction4.data return aPostAction.data } )(anotherPostAction.data)
   *
   * <p>The values in the returned instance of GetIdentifiersResponseDetails will be: { references:
   * ["aPostAction.data", "anotherPostAction.data"], functionalParams: ["ignoredAction1"],
