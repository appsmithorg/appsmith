...
public Mono<PageDTO> getPageByViewMode(NewPage newPage, Boolean viewMode) {
	...
	if (newPage.getPublishedPage() != null) {
		...
		// We are trying to fetch published page but it doesnt exist because the page hasn't been
		// published yet
		return Mono.error(
			new AppsmithException(
				AppsmithError.ACL_NO_RESOURCE_FOUND, FieldName.PAGE, newPage.getId()));
	}
	} else {
...
public Mono<ApplicationPagesDTO> findApplicationPagesByApplicationIdViewMode(
		...
		applicationService
			...
			.flatMap(
				...
				if (Boolean.TRUE.equals(view)) {
					...
					// We are trying to fetch published pages but they don't exist because the
					// application
	...
	Mono<String> defaultPageIdMono =
		applicationMono.map(
			...
			if (!StringUtils.hasLength(defaultPageId)
				...
				log.error(
					"application {} has no default page, returning first page as default",
					application.getId());
	...
	Mono<List<PageNameIdDTO>> pagesListMono =
		applicationMono
			...
			.flatMapMany(
				pageIds ->
					repository.findAllPageDTOsByIds(pageIds, pagePermission.getReadPermission()))
			.collectList()
			.flatMap(
				pagesFromDb -> Mono.zip(Mono.just(pagesFromDb), defaultPageIdMono, applicationMono))
			.flatMap(
				tuple -> {
				...
				for (NewPage pageFromDb : pagesFromDb) {
					...
					if (pageFromDb.getPublishedPage() == null) {
						// We are trying to fetch published page but it doesnt exist because the
	...
	Mono<Application> applicationMono =
		applicationService
			...
			.switchIfEmpty(
				Mono.error(
					new AppsmithException(
						AppsmithError.NO_RESOURCE_FOUND, FieldName.NAME, applicationName)))
			.cache();
...
public Mono<List<String>> findAllPageIdsInApplication(
	...
	return findNewPagesByApplicationId(applicationId, aclPermission)
		.flatMap(
			...
			// Looks like the page has been deleted in the `view` mode. Don't return the id for
			// this page.
			return Mono.empty();
			})
		.collectList();
...
public Mono<NewPage> archiveByIdEx(String id, Optional<AclPermission> permission) {
	Mono<NewPage> pageMono =
		this.findById(id, permission)
			.switchIfEmpty(
				Mono.error(
					new AppsmithException(AppsmithError.NO_RESOURCE_FOUND, FieldName.PAGE_ID, id)))
			.cache();
...
public Mono<String> findRootApplicationIdFromNewPage(String branchName, String defaultPageId) {
	...
	return getPageMono
		...
		.map(
			newPage -> {
			log.debug(
				"Retrieved possible application ids for page, picking the appropriate one now");
...
/**
* Returns a list of pages of an Application. If Application ID is present, it'll fetch all pages
* of that application in the provided mode. if Page ID is present, it'll fetch all pages of the
* corresponding Application. If both IDs are present, it'll use the Application ID only and
* ignore the Page ID
*
