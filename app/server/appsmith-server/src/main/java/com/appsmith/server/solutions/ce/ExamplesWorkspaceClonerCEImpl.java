...
private Mono<Workspace> cloneExamplesWorkspace(User user) {
	...
	// Don't create an examples workspace if the user already has some workspaces, perhaps because
	// they
...
public Mono<List<String>> cloneApplications(
	...
	return datasourceFlux
		.flatMap(
			...
			// forkWithConfiguration is dependent on application, here we are calling
			// cloneDatasource
			// for the workspace hence Boolean.TRUE is passed as the third parameter because by
			// default
			// user should have access to the datasource credentials already present in the
			// workspace
		...
		.flatMap(
			...
			// Extracting forkWithConfiguration to use below before resetting it for newly forked
			// app
			...
			// Setting the forkWithConfiguration and exportWithConfiguration to null for newly
			// forked app
				...
				.flatMap(
					savedPage -> {
						...
						return newActionService
							...
							// This call to `collectMap` will wait for all actions in all pages to
							// have been processed, and so the
											...
											unpublishedCollection
												...
												.forEach(
													...
													if (StringUtils.hasLength(oldActionId)
														...
														log.debug(
															"Unable to find action {} while forking"
																+ " inside ID map: {}",
															oldActionId,
															actionIdsMap);
													}
...
private Mono<Application> cloneApplicationDocument(Application application) {
	...
	// Clean the object so that it will be saved as a new application for the currently signed in
	// user.
