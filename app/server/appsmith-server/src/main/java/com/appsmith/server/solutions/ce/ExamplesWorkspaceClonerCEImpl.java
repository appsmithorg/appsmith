...
public class ExamplesWorkspaceClonerCEImpl implements ExamplesWorkspaceClonerCE {
  ...
  public Mono<Workspace> cloneExamplesWorkspace() {
    return sessionUserService.getCurrentUser().flatMap(this::cloneExamplesWorkspace);
  }

  /**
   * Clones the template workspace (as specified in config collection) for the given user. The given
   * user will be the owner of the cloned workspace. This method also assumes that the given user is
   * the same as the user in the current Spring session.
   *
  ...
  public Mono<List<String>> cloneApplications(
      String toWorkspaceId, Flux<Application> applicationFlux) {
    return cloneApplications(toWorkspaceId, applicationFlux, Flux.empty());
  }

  /**
   * Clone all applications (except deleted ones), including it's pages and actions from one
   * workspace into another. Also clones all datasources (not just the ones used by any
   * applications) in the given workspace.
   *
  ...
  public Mono<List<String>> cloneApplications(
    ...
    // A map of datasourceId => {a cached Mono that clones this datasource and yields the new
    // datasource }.
    ...
    return datasourceFlux
        .flatMap(
            datasource -> {
              ...
              final Mono<Datasource> clonerMono =
                  cloneDatasource(datasourceId, toWorkspaceId, Boolean.TRUE);
        ...
        .flatMap(
            application -> {
              ...
              // Setting the forkWithConfiguration and exportWithConfiguration to null
              // for newly forked app
              ...
              return doOnlyCloneApplicationObjectWithoutItsDependenciesAndReturnNonDeletedPages(
                                ...
                                newAction -> {
                                  ...
                                  if (datasourceInsideAction != null) {
                                    if (datasourceInsideAction.getId() != null) {
                                      ...
                                      if (!cloneDatasourceMonos.containsKey(datasourceId)) {
                                        cloneDatasourceMonos.put(
                                            datasourceId,
                                            cloneDatasource(
                                                    datasourceId, toWorkspaceId, forkWithConfig)
                                                .cache());
                                      }
                                      actionMono =
                                          cloneDatasourceMonos
                                              .get(datasourceId)
                                              .map(
                                                  newDatasource -> {
                                  ...
                                  return Mono.zip(
                                      actionMono
                                          .flatMap(
                                              actionDTO ->
                                                  layoutActionService.createAction(
                                                      actionDTO,
                                  ...
                                  return actionCollectionService
                                      ...
                                      .flatMap(
                                          actionCollection -> {
                                            ...
                                            unpublishedCollection.setDefaultResources(
                                                defaultResources);

                                            actionCollection.setWorkspaceId(toWorkspaceId);
                                            actionCollection.setApplicationId(
                                                savedPage.getApplicationId());

                                            DefaultResources defaultResources1 =
                                                new DefaultResources();
                                            ...
                                            // created
                                            // actionIds
                                            final Map<String, String> newActionIds =
                                                new HashMap<>();
                                            unpublishedCollection
                                                .getDefaultToBranchedActionIdsMap()
                                                .forEach(
                                                    (defaultActionId, oldActionId) -> {
                                      ...
                                      .then(
                                          Mono.zip(
                                              Mono.just(actionIdsMap),
                                              Mono.just(collectionIdsMap)));
                                });
                      });
            })
        .flatMap(
            tuple ->
                updateActionAndCollectionsIdsInClonedPages(
  ...
  private Flux<NewPage> updateActionAndCollectionsIdsInClonedPages(
   ...
   * (page and actions) Once the new application object is created, it adds the new application's id
   * into the list applicationIds
   *
   ...
   */
  private Flux<NewPage> doOnlyCloneApplicationObjectWithoutItsDependenciesAndReturnNonDeletedPages(
  ...
  public Mono<Datasource> cloneDatasource(
      String datasourceId, String toWorkspaceId, Boolean forkWithConfiguration) {
    final Mono<List<Datasource>> existingDatasourcesMono =
        datasourceRepository.findAllByWorkspaceId(toWorkspaceId).collectList();
        ...
        .flatMap(
            tuple -> {
              ...
              final AuthenticationDTO authentication =
                  templateDatasource.getDatasourceConfiguration() == null
                      ? null
                      : templateDatasource.getDatasourceConfiguration().getAuthentication();
              ...
              return Flux.fromIterable(existingDatasources)
                  .map(
                      ds -> {
                        final AuthenticationDTO auth =
                            ds.getDatasourceConfiguration() == null
                                ? null
                                : ds.getDatasourceConfiguration().getAuthentication();
                        if (auth != null) {
                  ...
                  .next() // Get the first matching datasource, we don't need more
                  // than one here.
                  .switchIfEmpty(
                      Mono.defer(
                          () -> {
                            // No matching existing datasource found, so
                            // create a new one.
                            Datasource newDs =
                                templateDatasource.fork(forkWithConfiguration, toWorkspaceId);
