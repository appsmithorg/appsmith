...
public Mono<ActionCollectionDTO> createCollection(ActionCollectionDTO collection) {
	...
	// If the collection name is unique, the action name will be guaranteed to be unique within that
	// collection
	return pageMono
		.flatMap(
			page -> {
		...
		.flatMap(
			...
			// Throw an error since the new action collection's name matches an existing action,
		...
		.flatMap(
			...
			// actionCollectionService would occur when the new collection is created by grouping
			// existing actions
		...
		.flatMap(
			...
			// Only store branchName, defaultApplicationId and defaultActionCollectionId for
			// ActionCollection level resource
				...
				actionCollectionService
					...
					.flatMap(
						savedActionCollection -> {
							// If the default collection is not set then current collection will be
							// the default one
			...
			return actionCollectionMono
				.map(
					actionCollection1 -> {
						actions.forEach(
							actionDTO -> {
							// Update all the actions in the list to belong to
							// actionCollectionService collection
						...
						actionDTO -> {
							...
							return newActionService
								...
								.onErrorResume(
									throwable -> {
									log.debug(
										"Failed to update collection name for action {} for"
											+ " collection with id: {}",
										actionDTO.getName(),
...
public Mono<ActionCollectionDTO> updateUnpublishedActionCollection(
	...
	// It is expected that client will be aware of defaultActionIds and not the branched (actual)
	// action ID
				...
				Flux.fromIterable(actionCollectionDTO.getActions())
					.flatMap(
						...
						if (actionDTO.getId() == null) {
							...
							// Client only knows about the default action ID, fetch branched action
							// id to update the action
					...
					.flatMap(
						...
						if (actionDTO.getId() == null) {
							...
							return layoutActionService
								...
								// return an empty action so that the filter can remove it from the
								// list
								.onErrorResume(
									throwable -> {
									log.debug(
...
public Mono<LayoutDTO> refactorAction(
	...
	return Mono.zip(refactorBranchedActionMono, branchedCollectionIdMono)
		.flatMap(
			...
			// As client is not aware of branch specific action IDs, replace default action IDs
			// within
