...
public class LayoutCollectionServiceCEImpl implements LayoutCollectionServiceCE {
    ...
    public Mono<ActionCollectionDTO> createCollection(ActionCollectionDTO collection) {
        ...
        // If the collection name is unique, the action name will be guaranteed to be unique within
        // that collection
        return pageMono.flatMap(
                        page -> {
                            ...
                            return layoutActionService.isNameAllowed(
                                    page.getId(), layout.getId(), collection.getName());
                        })
                .flatMap(
                        isNameAllowed -> {
                            // If the name is allowed, return list of actionDTOs for further
                            // processing
                            if (Boolean.TRUE.equals(isNameAllowed)) {
                                return Mono.justOrEmpty(collection.getActions())
                                        .defaultIfEmpty(List.of());
                            }
                            // Throw an error since the new action collection's name matches an
                            // existing action, widget or collection name.
                            return Mono.error(
                                    new AppsmithException(
                        ...
                        action -> {
                            if (action.getId() == null) {
                                ...
                                action.setFullyQualifiedName(
                                        collection.getName() + "." + action.getName());
                                ...
                                return layoutActionService
                                        ...
                                        // return an empty action so that this action is disregarded
                                        // from the list
                                        .onErrorResume(
                                                throwable -> {
                                                    log.debug(
                                                            "Failed to create action with name {}"
                                                                    + " for collection: {}",
                                                            action.getName(),
                                                            collection.getName());
                            ...
                            // actionCollectionService would occur when the new collection is
                            // created by grouping existing actions
                            // actionCollectionService could be a future enhancement for js editor
                            // templates,
                            // but is also useful for generic collections
                            // We do not expect to have to update the action at
                            // actionCollectionService point
                ...
                .flatMap(
                        tuple -> {
                            ...
                            // Only store branchName, defaultApplicationId and
                            // defaultActionCollectionId for ActionCollection level resource
                            DefaultResources defaults = new DefaultResources();
                            AppsmithBeanUtils.copyNewFieldValuesIntoOldObject(
                                    actionCollection.getDefaultResources(), defaults);
                                    ...
                                    actionCollectionService
                                            ...
                                            .flatMap(
                                                    savedActionCollection -> {
                                                        // If the default collection is not set then
                                                        // current collection will be the default
                                                        // one
                                                        ...
                                                        return actionCollectionService.save(
                                                                savedActionCollection);
                                                    })
                                            .flatMap(
                            ...
                            return actionCollectionMono
                                    .map(
                                            actionCollection1 -> {
                                                actions.forEach(
                                                        actionDTO -> {
                                                            // Update all the actions in the list to
                                                            // belong to actionCollectionService
                                                            // collection
                                                            actionDTO.setCollectionId(
                                                                    actionCollection1.getId());
                                                            if (StringUtils.isEmpty(
                                                                    actionDTO
                                                                            .getDefaultResources()
                                                                            .getCollectionId())) {
                                    ...
                                    .flatMap(
                                            tuple1 -> {
                                                final List<ActionDTO> actionDTOList =
                                                        tuple1.getT1();
                                                final ActionCollection actionCollection1 =
                                                        tuple1.getT2();
                                            ...
                                            updatedCollection ->
                                                    layoutActionService
                                                            .updatePageLayoutsByPageId(
                                                                    updatedCollection.getPageId())
                                                            .thenReturn(updatedCollection));
                        });
    }

    @Override
    public Mono<ActionCollectionDTO> createCollection(
            ActionCollectionDTO collection, String branchName) {
        if (collection.getId() != null) {
            return Mono.error(new AppsmithException(AppsmithError.INVALID_PARAMETER, FieldName.ID));
        }
                ...
                StringUtils.isEmpty(branchName)
                        ...
                        : newPageService
                                .findByBranchNameAndDefaultPageId(
                                        branchName, pageId, pagePermission.getReadPermission())
                                .map(NewPage::getId)
                                .cache();

        Mono<String> branchedActionCollectionIdMono =
                StringUtils.isEmpty(branchName)
        ...
        return branchedPageIdMono
                .flatMap(
                        branchedPageId ->
                                layoutActionService
                                        ...
                                        .flatMap(
                                                isNameAllowed -> {
                                                    // If the name is allowed, return list of
                                                    // actionDTOs for further processing
                                                    if (Boolean.TRUE.equals(isNameAllowed)) {
                                                        return branchedActionCollectionIdMono
                                                                .flatMap(
                                                                        collectionId ->
                                                                                actionCollectionService
                                                                                        .findActionCollectionDTObyIdAndViewMode(
                                                                                                collectionId,
                                                                                                false,
                                                                                                actionPermission
                                                                                                        .getEditPermission()));
                                                    }
                                                    // Throw an error since the new action
                                                    // collection's name matches an existing action,
                                                    // widget or collection name.
                                                    return Mono.error(
                                                            new AppsmithException(
                                                                    AppsmithError
                ...
                .flatMap(
                        branchedActionCollection -> {
                            ...
                            if (branchedActionCollection.getDefaultToBranchedActionIdsMap()
                                    != null) {
                                actionIds.putAll(
                                        branchedActionCollection
                                                .getDefaultToBranchedActionIdsMap());
                            }
                            if (branchedActionCollection.getDefaultToBranchedArchivedActionIdsMap()
                                    != null) {
                                actionIds.putAll(
                                        branchedActionCollection
                            ...
                            Flux<ActionDTO> actionUpdatesFlux =
                                    Flux.fromIterable(actionIds.values())
                                            .flatMap(
                                                    actionId ->
                                                            newActionService
                            ...
                            return actionUpdatesFlux
                                    .then(
                                            actionCollectionService.update(
                                                    branchedActionCollection.getId(),
                                                    branchedActionCollection))
                                    .then(branchedPageIdMono)
                                    .flatMap(
                                            branchedPageId ->
                                                    refactoringSolution
                                                            .refactorActionCollectionName(
                                                                    branchedActionCollection
                                                                            .getApplicationId(),
                                                                    branchedPageId,
                                                                    layoutId,
    ...
    public Mono<ActionCollectionDTO> moveCollection(
            ActionCollectionMoveDTO actionCollectionMoveDTO) {
        ...
        return Mono.zip(
                ...
                .flatMap(
                        tuple -> {
                            ...
                            if (actionCollectionDTO.getDefaultToBranchedActionIdsMap() != null) {
                                actionIds.putAll(
                                        actionCollectionDTO.getDefaultToBranchedActionIdsMap());
                            }
                            if (actionCollectionDTO.getDefaultToBranchedArchivedActionIdsMap()
                                    != null) {
                                actionIds.putAll(
                                        actionCollectionDTO
                                                .getDefaultToBranchedArchivedActionIdsMap());
                            ...
                            final Flux<ActionDTO> actionUpdatesFlux =
                                    Flux.fromIterable(actionIds.values())
                                            .flatMap(
                                                    actionId ->
                                                            newActionService
                                            ...
                                            .flatMap(
                                                    actionDTO -> {
                                                        ...
                                                        // Update default page ID in actions as per
                            ...
                            return actionUpdatesFlux
                                    ...
                                    .flatMap(
                                            actionDTOs ->
                                                    actionCollectionService.update(
                                                            collectionId, actionCollectionDTO))
                ...
                .flatMap(
                        tuple -> {
                            ...
                            return newPageService
                                    .findPageById(
                                            oldPageId, pagePermission.getEditPermission(), false)
                                    .flatMap(
                                            page -> {
    ...
    public Mono<ActionCollectionDTO> moveCollection(
        ...
        Mono<String> destinationPageMono =
                newPageService
                        .findByBranchNameAndDefaultPageId(
                                branchName,
                                actionCollectionMoveDTO.getDestinationPageId(),
        ...
        Mono<String> branchedCollectionMono =
                actionCollectionService
                        .findByBranchNameAndDefaultCollectionId(
                                branchName,
    ...
    public Mono<ActionCollectionDTO> updateUnpublishedActionCollection(
        ...
        Mono<ActionCollection> branchedActionCollectionMono =
                actionCollectionService
                        .findByBranchNameAndDefaultCollectionId(
                                branchName, id, actionPermission.getEditPermission())
                        .cache();

        // It is expected that client will be aware of defaultActionIds and not the branched
        // (actual) action ID
        final Set<String> validDefaultActionIds =
                actionCollectionDTO.getActions().stream()
                        .map(ActionDTO::getId)
                        .filter(Objects::nonNull)
                        .collect(Collectors.toUnmodifiableSet());
        final Set<String> archivedDefaultActionIds =
                actionCollectionDTO.getArchivedActions().stream()
                        .map(ActionDTO::getId)
                        .filter(Objects::nonNull)
                        .collect(Collectors.toUnmodifiableSet());
        ...
        final Mono<Map<String, String>> newValidActionIdsMono =
                branchedActionCollectionMono.flatMap(
                        branchedActionCollection ->
                                Flux.fromIterable(actionCollectionDTO.getActions())
                                        .flatMap(
                                                actionDTO -> {
                                                    ...
                                                    if (actionDTO.getId() == null) {
                                                        ...
                                                        actionDTO.setFullyQualifiedName(
                                                                actionCollectionDTO.getName()
                                                                        + "."
                                                                        + actionDTO.getName());
                                                        actionDTO.setPluginType(
                                                                actionCollectionDTO
                                                                        .getPluginType());
                                                        actionDTO.setPluginId(
                                                                actionCollectionDTO.getPluginId());
                                                        actionDTO.setDefaultResources(
                                                                branchedActionCollection
                                                                        .getDefaultResources());
                                                        actionDTO
                                                                .getDefaultResources()
                                                        ...
                                                        // actionCollectionService is a new action,
                                                        // we need to create one
                                                        return layoutActionService
                                                                .createSingleAction(
                                                                        actionDTO, Boolean.TRUE);
                                                    } else {
                                                        actionDTO.setCollectionId(null);
                                                        // Client only knows about the default
                                                        // action ID, fetch branched action id to
                                                        // update the action
                                                        Mono<String> branchedActionIdMono =
                                                                StringUtils.isEmpty(branchName)
                                                                        ? Mono.just(
                                                                                actionDTO.getId())
                                                                        : newActionService
        ...
        final Mono<Map<String, String>> newArchivedActionIdsMono =
                branchedActionCollectionMono.flatMap(
                        branchedActionCollection ->
                                Flux.fromIterable(actionCollectionDTO.getArchivedActions())
                                        .flatMap(
                                                actionDTO -> {
                                                    actionDTO.setCollectionId(
                                                            branchedActionCollection.getId());
                                                    ...
                                                    if (actionDTO.getId() == null) {
                                                        ...
                                                        return layoutActionService
                                                                .createSingleAction(
                                                                        actionDTO, Boolean.TRUE)
                                                                // return an empty action so that
                                                                // the filter can remove it from the
                                                                // list
                                                                .onErrorResume(
                                                                        throwable -> {
                                                                            log.debug(
        ...
        return branchedActionCollectionMono
                .map(
                        branchedActionCollection -> {
                            // From the existing collection, if an action id is not referenced at
                            // all anymore,
                            // this means the action has been somehow deleted
                            final Set<String> oldDefaultActionIds = new HashSet<>();
                            if (branchedActionCollection
                                            .getUnpublishedCollection()
                                            .getDefaultToBranchedActionIdsMap()
                                    != null) {
                                oldDefaultActionIds.addAll(
                                        branchedActionCollection
                                                .getUnpublishedCollection()
                                                .getDefaultToBranchedActionIdsMap()
                                                .keySet());
                ...
                .flatMap(
                        defaultActionId ->
                                newActionService
                                        ...
                                        // return an empty action so that the filter can remove it
                                        // from the list
                                        .onErrorResume(
                                                throwable -> {
                                                    log.debug(
                ...
                .flatMap(
                        actionCollection ->
                                actionCollectionService.update(
                                        actionCollection.getId(), actionCollection))
                ...
                .flatMap(
                        savedActionCollection ->
                                analyticsService.sendUpdateEvent(
                                        savedActionCollection,
                                        actionCollectionService.getAnalyticsProperties(
                                                savedActionCollection)))
                .flatMap(
                        actionCollection ->
                                actionCollectionService
                ...
                .zipWith(
                        ...
                        (branchedActionCollection, newPage) -> {
                            ...
                            if (newPage.getUnpublishedPage().getLayouts().size() > 0) {
                                // redundant check as the collection lies inside a layout. Maybe
                                // required for testcases
                                branchedActionCollection.setErrorReports(
    ...
    public Mono<LayoutDTO> refactorAction(
        ...
        final ActionCollectionDTO actionCollectionDTO =
                refactorActionNameInCollectionDTO.getActionCollection();
        ...
        return layoutDTOMono
                ...
                .flatMap(
                        actionCollection ->
                                actionCollectionService.update(
                                        actionCollectionDTO.getId(), actionCollection))
                .then(layoutDTOMono);
    }
    ...
    public Mono<LayoutDTO> refactorAction(
        ...
        RefactorActionNameDTO refactorActionNameDTO =
                refactorActionNameInCollectionDTO.getRefactorAction();
        ActionCollectionDTO defaultActionCollection =
                refactorActionNameInCollectionDTO.getActionCollection();
        ...
        Mono<RefactorActionNameDTO> refactorBranchedActionMono =
                newActionService
                        .findByBranchNameAndDefaultActionId(
                                branchName,
                                refactorActionNameDTO.getActionId(),
                                actionPermission.getEditPermission())
                        .map(
                                branchedAction -> {
                                    ...
                                    defaultActionCollection.setPageId(
                                            branchedAction.getUnpublishedAction().getPageId());
                                    defaultActionCollection.setApplicationId(
                                            branchedAction.getApplicationId());
                                    return refactorActionNameDTO;
                                });

        Mono<String> branchedCollectionIdMono =
                StringUtils.isEmpty(branchName)
        ...
        return Mono.zip(refactorBranchedActionMono, branchedCollectionIdMono)
                .flatMap(
                        tuple -> {
                            ...
                            // As client is not aware of branch specific action IDs, replace default
                            // action IDs within
                            ...
                            defaultActionCollection
                                    ...
                                    .forEach(
                                            actionDTO -> {
                                                actionDTO.setPageId(
                                                        defaultActionCollection.getPageId());
                                                actionDTO.setApplicationId(
                                                        defaultActionCollection.getApplicationId());
                                                actionDTO.setCollectionId(
                                                        defaultActionCollection.getId());
                                                actionDTO.setId(
                                                        defaultToBranchedActionIdMap.get(
                                                                actionDTO.getId()));
                                            });
                            if (!CollectionUtils.isNullOrEmpty(
                                    defaultActionCollection.getArchivedActions())) {
                                defaultActionCollection
                                        .getArchivedActions()
                                        .forEach(
                                                actionDTO -> {
