 ...
 * {@link Migration006EE01AddFlagToWorkspaceBeforeInviteUsersPolicyToApplicationPolicyMigration} to filter all the
 ...
 */
@Slf4j
@ChangeUnit(order = "007-ee-01", id = "add-invite-users-application-policy-to-application-policy", author = " ")
public class Migration007EE01AddInviteUsersApplicationPolicyToApplicationPolicies {
    ...
    public Migration007EE01AddInviteUsersApplicationPolicyToApplicationPolicies(MongoTemplate mongoTemplate) {
    ...
    public void addInviteUsersApplicationPolicyToApplicationPolicy() {
        ...
        try {
            ...
            Query workspaceQueryBatchedPerformantLimit1 =
                    Query.of(workspaceQueryBatchedPerformant).limit(1);
        ...
        log.debug(
                "Count of workspaces where applications will be migrated: {}, Query used: {}",
                countWorkspacesWhereApplicationsWillBeMigrated,
                workspaceQuery);

        mongoTemplate.stream(finalWorkspaceQuery, Workspace.class).forEach(this::migrateApplicationsInWorkspace);
    }
    ...
    private void migrateApplicationsInWorkspace(Workspace workspace) {
        ...
        Criteria criteriaApplicationsWhereInviteUsersPermissionDoesNotExist = Criteria.where("policies.permission")
        ...
        Query queryApplicationsWhereInviteUsersPermissionExists =
                new Query().addCriteria(criteriaApplicationsWhereInviteUsersPermissionExists);
        Query queryApplicationsWhereInviteUsersPermissionDoesNotExist =
                new Query().addCriteria(criteriaApplicationsWhereInviteUsersPermissionDoesNotExist);
        Query queryApplicationsWhichHaveInviteUsersPermissionWithWorkspacePermissionGroup =
        ...
        updateExistingInviteUsersToApplicationPolicy
                ...
                .each(workspace.getDefaultPermissionGroups().toArray());
        ...
        while (migrationAttempt <= migrationRetries
                && countApplicationsWhichHaveInviteUsersPermissionWithWorkspacePermissionGroup
                        != countAllApplicationsToBeUpdated) {

            mongoTemplate.updateMulti(
                    queryApplicationsWhereInviteUsersPermissionExists,
                    updateExistingInviteUsersToApplicationPolicy,
                    Application.class);

            mongoTemplate.updateMulti(
                    queryApplicationsWhereInviteUsersPermissionDoesNotExist,
                    updateAddInviteUsersToApplicationsPolicy,
                    Application.class);

            countApplicationsWhichHaveInviteUsersPermissionWithWorkspacePermissionGroup = mongoTemplate.count(
                    queryApplicationsWhichHaveInviteUsersPermissionWithWorkspacePermissionGroup, Application.class);

            migrationAttempt += 1;
        }

        if (countApplicationsWhichHaveInviteUsersPermissionWithWorkspacePermissionGroup
                != countAllApplicationsToBeUpdated) {
            String reasonForFailure =
                    "Failed to update policies for all the applications for workspaceId " + workspace.getId();
