...
/**
 * This class parses all headers that start with X-MDC-* and set them in the logger MDC. These MDC
 * parameters are also set in the response object before being sent to the user.
 */
@Component
@Slf4j
public class MDCFilter implements WebFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
        try {
            // Using beforeCommit here ensures that the function `addContextToHttpResponse` isn't
            // run immediately
            // It is only run when the response object is being created
            exchange.getResponse()
                    .beforeCommit(() -> addContextToHttpResponse(exchange.getResponse()));
            return ReactiveSecurityContextHolder.getContext()
                    .map(ctx -> ctx.getAuthentication().getPrincipal())
                    .flatMap(
                            principal -> {
                                final User user =
                                        principal instanceof User ? (User) principal : null;
                                return chain.filter(exchange)
                                        .contextWrite(
                                                ctx ->
                                                        addRequestHeadersToContext(
                                                                exchange.getRequest(), ctx, user));
                            });
        } finally {
            MDC.clear();
        }
    ...
    private Context addRequestHeadersToContext(
        ...
        final Map<String, String> contextMap =
                request.getHeaders().toSingleValueMap().entrySet().stream()
                        ...
                        .collect(
                                toMap(
                                        v -> v.getKey().substring((MDC_HEADER_PREFIX.length())),
                                        Map.Entry::getValue));
        ...
        // Setting the context map to the reactive context. This will be used in the reactive logger
        // to print the MDC
