...
public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
	try {
	// Using beforeCommit here ensures that the function `addContextToHttpResponse` isn't run
	// immediately
		...
		.flatMap(
			principal -> {
				...
				return chain
					.filter(exchange)
					.contextWrite(
						ctx -> addRequestHeadersToContext(exchange.getRequest(), ctx, user));
			});
	} finally {
	MDC.clear();
	}
...
private Context addRequestHeadersToContext(
	...
	final Map<String, String> contextMap =
		request.getHeaders().toSingleValueMap().entrySet().stream()
			...
			.collect(
				toMap(
					v -> v.getKey().substring((MDC_HEADER_PREFIX.length())), Map.Entry::getValue));
	...
	// Setting the context map to the reactive context. This will be used in the reactive logger to
	// print the MDC
