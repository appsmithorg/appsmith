    ...
    public void addSmtpEmailBodyType() {
        ...
        Query unpublishedActionsQuery =
                getQueryToFetchAllDomainObjectsWhichAreNotDeletedUsingPluginId(smtpPlugin)
                        .addCriteria(
                                where("unpublishedAction.actionConfiguration.formData.send")
                                        .exists(true))
                        .addCriteria(
                                where(
                                                "unpublishedAction.actionConfiguration.formData.send.bodyType")
                                        .exists(false));
        ...
        mongoTemplate.updateMulti(
                unpublishedActionsQuery, updateUnpublishedActions, NewAction.class);
        ...
        Query publishedActionsQuery =
                getQueryToFetchAllDomainObjectsWhichAreNotDeletedUsingPluginId(smtpPlugin)
                        .addCriteria(
                                where("publishedAction.actionConfiguration.formData.send")
                                        .exists(true))
                        .addCriteria(
                                where("publishedAction.actionConfiguration.formData.send.bodyType")
                                        .exists(false));
