...
/**
 * This class catches all the Exceptions and formats them into a proper ResponseDTO<ErrorDTO> object
...
public class GlobalExceptionHandler {
  ...
  private void doLog(Throwable error) {
    ...
    Sentry.configureScope(
        scope -> {
          /**
           * Send stack trace as a string message. This is a work around till it is figured out why
           * raw stack trace is not visible on Sentry dashboard.
           */
  ...
  /**
   * This function only catches the AppsmithException type and formats it into
   * ResponseEntity<ErrorDTO> object Ideally, we should only be throwing AppsmithException from our
   * code. This ensures that we can standardize and set proper error messages and codes.
   *
   * @param e AppsmithException that will be caught by the function
   * @param exchange ServerWebExchange contract in order to extract the response and set the http
   *     status code
   * @return Mono<ResponseDto < ErrorDTO>>
   */
  @ExceptionHandler
  @ResponseBody
  public Mono<ResponseDTO<ErrorDTO>> catchAppsmithException(
      AppsmithException e, ServerWebExchange exchange) {
    ...
    // Do special formatting for this error to run the message string into valid jsonified
    // string
    if (AppsmithError.INVALID_DYNAMIC_BINDING_REFERENCE
      ...
      response =
          new ResponseDTO<>(
              e.getHttpStatus(),
              new ErrorDTO(
                  e.getAppErrorCode(), e.getErrorType(), "{" + e.getMessage() + "}", e.getTitle()));
    } else {
      response =
          new ResponseDTO<>(
              e.getHttpStatus(),
  ...
  public Mono<ResponseDTO<ErrorDTO>> catchDuplicateKeyException(
    ...
    String conflictingObjectName =
        DuplicateKeyExceptionUtils.extractConflictingObjectName(e.getCause().getMessage());
    ResponseDTO<ErrorDTO> response =
        new ResponseDTO<>(
    ...
    if (e.getMethodParameter() != null) {
      errorMessage =
          "Malformed parameter '"
              ...
              + "' for "
              + e.getMethodParameter().getContainingClass().getSimpleName()
  ...
  @ExceptionHandler
  @ResponseBody
  public Mono<ResponseDTO<ErrorDTO>> catchPluginException(
      AppsmithPluginException e, ServerWebExchange exchange) {
    ...
    ResponseDTO<ErrorDTO> response =
        new ResponseDTO<>(
            appsmithError.getHttpErrorCode(),
            new ErrorDTO(
                appsmithError.getAppErrorCode(), e.getMessage(), e.getErrorType(), e.getTitle()));
  ...
  @ExceptionHandler
  @ResponseBody
  public Mono<ResponseDTO<ErrorDTO>> catchAccessDeniedException(
      AccessDeniedException e, ServerWebExchange exchange) {
  ...
  /**
   * This function catches the generic Exception class and is meant to be a catch all to ensure that
   * we don't leak any information to the client. Ideally, the function #catchAppsmithException
   * should be used
   *
  ...
  private Mono<ResponseDTO<ErrorDTO>> getResponseDTOMono(
      String urlPath, ResponseDTO<ErrorDTO> response) {
