...
public class GlobalExceptionHandler {
    ...
    private void doLog(Throwable error) {
        ...
        Sentry.configureScope(
                scope -> {
                    /**
                     * Send stack trace as a string message. This is a work around till it is
                     * figured out why raw stack trace is not visible on Sentry dashboard.
                     */
        ...
        if (error instanceof BaseException) {
            ...
            if (baseError.getErrorAction() == AppsmithErrorAction.LOG_EXTERNALLY) {
                ...
                user.setEmail(
                        baseError
                                .getContextMap()
                                .getOrDefault(MDCFilter.USER_EMAIL, "unknownUser"));
                Sentry.setUser(user);
     ...
     * ResponseEntity<ErrorDTO> object Ideally, we should only be throwing AppsmithException from
     * our code. This ensures that we can standardize and set proper error messages and codes.
     *
     ...
     * that we don't leak any information to the client. Ideally, the function
     * #catchAppsmithException should be used
     *
