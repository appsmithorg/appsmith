...
public class AuthenticationSuccessHandlerCE implements ServerAuthenticationSuccessHandler {
   ...
   * In the process, the client browser will also set the session ID in the cookie against the
   * server's API domain.
   *
      ...
      if (createDefaultApplication && isFromSignup) {
        redirectionMono =
            createDefaultApplication(defaultWorkspaceId, authentication)
                ...
                .flatMap(
                    defaultApplication ->
                        handleRedirect(webFilterExchange, defaultApplication, finalIsFromSignup));
      } else {
   ...
   * This is state variable contains a random generated key along with the referer header set in the
