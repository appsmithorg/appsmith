...
public Mono<Void> onAuthenticationSuccess(
	...
	// In case of OAuth2 based authentication, there is no way to identify if this was a user
	// signup (new user
	// creation) or if this was a login (existing user). What we do here to identify this, is an
	// approximation.
	// If and when we find a better way to do identify this, let's please move away from this
	// approximation.
	...
	// If user has previously signed up using password and now using OAuth as a sign in method we
	// are removing
	// form login method henceforth. This step is taken to avoid any security vulnerability in the
	// login flow
	// as we are not verifying the user emails at first sign up. In future if we implement the
	// email
		...
		redirectionMono =
			workspaceService
				...
				.map(
					pair -> {
					log.debug(
						"AuthenticationSuccessHandlerCE::Time taken to check if workspace"
							+ " creation allowed: {} ms",
						pair.getT1());
					return pair.getT2();
					})
					...
					if (isCreateWorkspaceAllowed) {
						return createDefaultApplication(defaultWorkspaceId, authentication)
							...
							.map(
								pair -> {
								log.debug(
									"AuthenticationSuccessHandlerCE::Time taken to create default"
										+ " application: {} ms",
									pair.getT1());
								return pair.getT2();
								})
...
*/
@SuppressWarnings(
	// Disabling this because although the reference in the Javadoc is to a private method, it is
	// still useful.
	"JavadocReference")
...
private Mono<Void> handleRedirect(
	...
	// On authentication success, we send a redirect to the client's home page. This ensures that
	// the session
	...
	return Mono.just(exchange.getRequest())
		...
		.map(
			...
			if (isFromSignup) {
				...
				// This redirectUrl will be used by the client to redirect after showing a welcome
				// page.
