...
public class AuthenticationSuccessHandlerCE implements ServerAuthenticationSuccessHandler {
    ...
    /**
     * On authentication success, we send a redirect to the endpoint that serve's the user's
     * profile. The client browser will follow this redirect and fetch the user's profile JSON from
     * the server. In the process, the client browser will also set the session ID in the cookie
    ...
    public Mono<Void> onAuthenticationSuccess(
        ...
        if (authentication instanceof OAuth2AuthenticationToken) {
            // In case of OAuth2 based authentication, there is no way to identify if this was a
            // user signup (new user
            // creation) or if this was a login (existing user). What we do here to identify this,
            // is an approximation.
            // If and when we find a better way to do identify this, let's please move away from
            // this approximation.
            ...
            // If user has previously signed up using password and now using OAuth as a sign in
            // method we are removing
            // form login method henceforth. This step is taken to avoid any security vulnerability
            // in the login flow
            // as we are not verifying the user emails at first sign up. In future if we implement
            // the email
                ...
                redirectionMono =
                        workspaceService
                                ...
                                .map(
                                        pair -> {
                                            log.debug(
                                                    "AuthenticationSuccessHandlerCE::Time taken to"
                                                        + " check if workspace creation allowed: {}"
                                                        + " ms",
                                                    pair.getT1());
                                            return pair.getT2();
                                        })
                                        ...
                                        isCreateWorkspaceAllowed -> {
                                            if (isCreateWorkspaceAllowed) {
                                                return createDefaultApplication(
                                                                defaultWorkspaceId, authentication)
                                                        .elapsed()
                                                        .map(
                                                                pair -> {
                                                                    log.debug(
                                                                            "AuthenticationSuccessHandlerCE::Time"
                                                                                + " taken to create"
                                                                                + " default"
                                                                                + " application: {}"
                                                                                + " ms",
                                            ...
                                            return handleOAuth2Redirect(
                                                    webFilterExchange, null, finalIsFromSignup);
                                        });
            } else {
                        ...
                        createDefaultApplication(defaultWorkspaceId, authentication)
                                ...
                                .map(
                                        pair -> {
                                            log.debug(
                                                    "AuthenticationSuccessHandlerCE::Time taken to"
                                                            + " create default application: {} ms",
                                                    pair.getT1());
                                            return pair.getT2();
                                        })
                                .flatMap(
                                        defaultApplication ->
                                                redirectHelper.handleRedirect(
                                                        webFilterExchange,
                                                        defaultApplication,
                                                        true));
            } else {
                redirectionMono =
                        redirectHelper.handleRedirect(webFilterExchange, null, finalIsFromSignup);
            }
        }
                ...
                .flatMap(
                        currentUser -> {
                            ...
                            if (isFromSignupFinal) {
                                ...
                                // This should hold the role of the user, e.g., `App Viewer`,
                                // `Developer`, etc.
                            ...
                            monos.add(
                                    analyticsService.sendObjectEvent(
                                            ...
                                            currentUser,
                                            Map.of(FieldName.MODE_OF_LOGIN, modeOfLogin)));
    ...
    protected Mono<Application> createDefaultApplication(
        ...
        if (defaultWorkspaceId == null) {
            ...
            applicationMono =
                    workspaceRepository
                            ...
                            .flatMap(
                                    workspaces -> {
                                        // Since this is the first application creation, the first
                                        // workspace would be the only
                                        // workspace user has access to, and would be user's default
                                        // workspace. Hence, we use this
                                        ...
                                        // In case no workspaces are found for the user, create a
                                        ...
                                        return userRepository
                                                ...
                                                .flatMap(
                                                        user ->
                                                                workspaceService.createDefault(
                                                                        new Workspace(), user))
                                                .map(
                                                        workspace -> {
                                                            application.setWorkspaceId(
                                                                    workspace.getId());
                                                            return application;
                                                        });
                                    });
        ...
        return applicationMono.flatMap(
                application1 -> applicationPageService.createApplication(application1));
    }

    /**
     * This function redirects the back to the client's page after a successful sign in/sign up
     * attempt by the user This is to transfer control back to the client because the OAuth2 dance
     * would have been performed by the server.
     *
     * <p>We extract the redirect url from the `state` key present in the request exchange object.
     * This is state variable contains a random generated key along with the referer header set in
     * the {@link CustomServerOAuth2AuthorizationRequestResolver#generateKey(ServerHttpRequest)}
     * function.
     *
     * @param webFilterExchange WebFilterExchange instance for the current request.
     ...
     */
    @SuppressWarnings(
            // Disabling this because although the reference in the Javadoc is to a private method,
            // it is still useful.
            "JavadocReference")
    private Mono<Void> handleOAuth2Redirect(
        ...
        String state =
                exchange.getRequest().getQueryParams().getFirst(Security.QUERY_PARAMETER_STATE);
        ...
        if (isFromSignup) {
            ...
            redirectUrl =
                    redirectHelper.buildSignupSuccessUrl(redirectUrl, addFirstTimeExperienceParam);
        }
