...
private Criteria notDeleted() {
	return new Criteria()
		.andOperator(
			new Criteria()
				.orOperator(
					where(FieldName.DELETED).exists(false), where(FieldName.DELETED).is(false)),
			new Criteria()
				.orOperator(
...
/**
* When `fieldName` is blank, this method will return the entire object. Otherwise, it will return
* only the value against the `fieldName` property in the matching object.
*/
@Override
...
public Mono<T> findByIdAndBranchName(ID id, String branchName) {
	// branchName will be ignored and this method is overridden for the services which are shared
	// across branches
...
public Flux<T> findAll(Example example, Sort sort) {
	...
	return ReactiveSecurityContextHolder.getContext()
		...
		.flatMapMany(
			...
			return mongoOperations.find(
				query, example.getProbeType(), entityInformation.getCollectionName());
			});
}
