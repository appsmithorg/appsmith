...
public class CustomServerOAuth2AuthorizationRequestResolverCE
            ...
            Mono.error(
                () ->
                    new ResponseStatusException(
                        HttpStatus.BAD_REQUEST, "Invalid client registration id")));
  }
  ...
  /**
   * This function sets the state query parameter sent to the OAuth2 resource server along with the
   * parameter of the referer which initiated this OAuth2 login. On successful login, we will
   * redirect back to the client's index page based on the referer so as to transfer control back to
   * it. If the referer is not available, we default to redirecting to the server's index page.
   *
   ...
   *     {@link PkceParameterNames#CODE_CHALLENGE_METHOD} are added to be used in the authorization
   *     request.
