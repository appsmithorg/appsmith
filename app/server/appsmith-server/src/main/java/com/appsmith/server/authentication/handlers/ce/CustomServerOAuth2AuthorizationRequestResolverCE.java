 ...
 * It has been copied so as to override the creation of the `state` query parameter sent to the
 * OAuth2 authentication server The only 2 functions that have been overridden from the base class
 * are: {@link #generateKey(ServerHttpRequest)} and {@link #authorizationRequest(ServerWebExchange,
 * ClientRegistration)}. We couldn't simply extend the base class because of the use of private
 * variables and methods to invoke these functions.
 */
@Slf4j
public class CustomServerOAuth2AuthorizationRequestResolverCE
        implements ServerOAuth2AuthorizationRequestResolver {

    /**
     * The name of the path variable that contains the {@link
     ...
     *
     * @param clientRegistrationRepository the repository to resolve the {@link ClientRegistration}
     * @param authorizationRequestMatcher the matcher that determines if the request is a match and
     *     extracts the {@link #DEFAULT_REGISTRATION_ID_URI_VARIABLE_NAME} from the path variables.
     * @param redirectHelper
     */
    public CustomServerOAuth2AuthorizationRequestResolverCE(
            ReactiveClientRegistrationRepository clientRegistrationRepository,
                            ...
                            if (MISSING_VALUE_SENTINEL.equals(clientRegistration.getClientId())) {
                                return Mono.error(
                                        new AppsmithException(
                                                AppsmithError.OAUTH_NOT_AVAILABLE,
        ...
        if (AuthorizationGrantType.AUTHORIZATION_CODE.equals(
            ...
            if (!commonConfig.getOauthAllowedDomains().isEmpty()) {
                if (commonConfig.getOauthAllowedDomains().size() == 1) {
                    // Incase there's only 1 domain, we can do a further optimization to let the
                    // user select a specific one
                    // from the list
            ...
            //        } else if
            // (AuthorizationGrantType.IMPLICIT.equals(clientRegistration.getAuthorizationGrantType())) {
            //            builder = OAuth2AuthorizationRequest.implicit();
        } else {
            throw new IllegalArgumentException(
                    "Invalid Authorization Grant type ("
                            + clientRegistration.getAuthorizationGrantType().getValue()
                            + ") for Client Registration with Id: "
                            + clientRegistration.getRegistrationId());
        }
    ...
    /**
     * This function sets the state query parameter sent to the OAuth2 resource server along with
     * the parameter of the referer which initiated this OAuth2 login. On successful login, we will
     * redirect back to the client's index page based on the referer so as to transfer control back
     * to it. If the referer is not available, we default to redirecting to the server's index page.
     *
        ...
        return redirectHelper
                ...
                .map(
                        redirectUrl -> {
                            ...
                            stateKey =
                                    stateKey + "@" + Security.STATE_PARAMETER_ORIGIN + redirectUrl;
                            return stateKey;
                        });
    }
     ...
     * - action (e.g. login) <br>
     *
     * <p>Null variables are provided as empty strings.
     *
     * <p>Default redirectUriTemplate is: {@link
    ...
    private static String expandRedirectUri(
            ServerHttpRequest request, ClientRegistration clientRegistration) {
        Map<String, String> uriVariables = new HashMap<>();
        uriVariables.put(
                DEFAULT_REGISTRATION_ID_URI_VARIABLE_NAME, clientRegistration.getRegistrationId());
     ...
     *
     * @param attributes where the {@link OidcParameterNames#NONCE} is stored for the authentication
     *     request
     ...
     * @since 5.2
     */
    private void addNonceParameters(
     ...
     * Token Requests
     *
     * @param attributes where {@link PkceParameterNames#CODE_VERIFIER} is stored for the token
     *     request
     * @param additionalParameters where {@link PkceParameterNames#CODE_CHALLENGE} and, usually,
     *     {@link PkceParameterNames#CODE_CHALLENGE_METHOD} are added to be used in the
