...
public ActionDTO curlToAction(String command) throws AppsmithException {
	...
	// 1. lex: Split the string into tokens, respecting the quoting semantics of a POSIX-compliant
	// shell.
...
public List<String> normalize(List<String> tokens) {
	...
	if ("-d".equals(token) || "--data-ascii".equals(token) || "--data-raw".equals(token)) {
		...
		// We skip the `--url` argument since it's superfluous and URLs are directly sniffed out of
		// the argument
...
public ActionDTO parse(List<String> tokens) throws AppsmithException {
	...
	if (ARG_REQUEST.equals(state)) {
		...
		contentType = parts[1];
		// part[0] is already set to content-type, however, it might not have consistent casing.
		// hence resetting it to a HTTP standard.
		...
		// This is an option, in cURL's terminology. The next token would be the value of this
		// option.
		state = token;
	...
	if (contentType != null) {
		...
		// Setting the apiContentType to the content type detected by guessing the elements from
		// -f/ --form flag or -d/ --data flag
		// required for RestAPI calls with GET method having body.
		actionConfiguration.setFormData(Map.of(API_CONTENT_TYPE_KEY, contentType));
	}
	}
