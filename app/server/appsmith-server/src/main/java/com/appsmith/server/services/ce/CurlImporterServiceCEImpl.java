...
public class CurlImporterServiceCEImpl extends BaseApiImporter implements CurlImporterServiceCE {
  ...
  public ActionDTO curlToAction(String command) throws AppsmithException {
    ...
    // 1. lex: Split the string into tokens, respecting the quoting semantics of a
    // POSIX-compliant shell.
    // 2. normalize: Normalize all the command line arguments of a curl command, into their
    // long-form versions.
    //    E.g., `-X` into `--request`.
   ...
   * @return List of String tokens. The tokens don't include quote characters that were use to
   *     delineate the tokens.
   */
              ...
              if (currentChar == '\\') {
                ...
                if (currentChar == '\n' || currentChar == '#') {
                  ...
                  if (currentChar == ' ' || currentChar == '\t') {
                    // White space lying around between arguments. This
                    // delineates tokens.
  ...
  /**
   * Normalizes curl command arguments. For example, inputs like `-XGET`, `-X GET`, `--request GET`
   * are all converted to look like `--request GET`.
   *
          ...
          if ("-F".equals(token)) {
            ...
            if ("-H".equals(token)) {
              ...
              if (token.startsWith("-H")) {
                // `-HContent-Type:application/json` -> `--header
                    ...
                    if ("-b".equals(token)) {
                      ...
                      if ("-u".equals(token)) {
                        ...
                        if ("-A".equals(token)) {
                          ...
                          if (!"--url".equals(token)) {
                            // We skip the `--url` argument since it's
                            // superfluous and URLs are directly sniffed
                            // out of the argument
                            // list. The `--url` argument holds no
                            // special significance in cURL.
  ...
  public ActionDTO parse(List<String> tokens) throws AppsmithException {
      ...
      if (ARG_REQUEST.equals(state)) {
        ...
        if (ARG_HEADER.equals(state)) {
          ...
          if ("content-type".equalsIgnoreCase(parts[0])) {
            contentType = parts[1];
            // part[0] is already set to content-type, however, it might not have
            // consistent casing. hence resetting it to a HTTP standard.
            parts[0] = HttpHeaders.CONTENT_TYPE;
            // Setting the apiContentType to the content-type detected in the header
            // with the key word content-type.
          ...
          if (ARG_DATA.equals(state)) {
            // The `token` is next to `--data`.
            dataParts.add(token);

          } else {
            if ("--data-urlencode".equals(state)) {
              ...
              if (ARG_FORM.equals(state)) {
                ...
                if (ARG_COOKIE.equals(state)) {
                  ...
                  if (ARG_USER.equals(state)) {
                    ...
                    if (ARG_USER_AGENT.equals(state)) {
                      // The `token` is next to `--user-agent`.
                      headers.add(new Property("User-Agent", token));

                    } else {
                      if (token.startsWith("-")) {
                        // This is an option, in cURL's terminology. The
                        // next token would be the value of this option.
                        state = token;
                        isStateProcessed = false;

                      } else {
                        // Anything that doesn't start with `-` would be
                        // treated as a URL by cURL.
                        try {
                          trySaveURL(action, token);
                        } catch (MalformedURLException | URISyntaxException e) {
                          // Ignore this argument. May be there's a valid
                          // URL later down the arguments list.
                        }
                      }
                    }
                  }
    ...
    if (contentType == null) {
      ...
      if (contentType != null) {
        ...
        // Setting the apiContentType to the content type detected by guessing the elements
        // from  -f/ --form flag or -d/ --data flag
        // required for RestAPI calls with GET method having body.
        actionConfiguration.setFormData(Map.of(API_CONTENT_TYPE_KEY, contentType));
      }
    }
  ...
  private void trySaveURL(ActionDTO action, String token)
