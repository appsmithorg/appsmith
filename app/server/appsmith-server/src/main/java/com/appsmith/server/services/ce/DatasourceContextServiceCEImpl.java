...
public class DatasourceContextServiceCEImpl implements DatasourceContextServiceCE {
     ...
     * that created connection to a datasource - which resulted in a data race condition resulting
     * in multiple orphan connections. Ref: https://github.com/appsmithorg/appsmith/issues/14117
     * This method caches the result from the source publisher and forces concurrent subscriptions
     * to re-use the cached value. Hence, even if multiple threads subscribe to the same source
     * publisher they get the pre-computed cached value instead of creating a new connection for
     * each subscription of the source publisher.
     *
     * @param datasourceStorage - datasource storage for which a new datasource context / connection
     *     needs to be created
    ...
    public Mono<Object> updateDatasourceAndSetAuthentication(
            Object connection, DatasourceStorage datasourceStorage) {
        ...
        return pluginExecutorHelper
                ...
                .flatMap(
                        pluginExecutor -> {
                            ...
                            /**
                             * Keep one monitor object against each datasource id. The synchronized
                             * method `getCachedDatasourceContextMono` would then acquire lock on
                             * the monitor object which is unique for each datasourceId hence
                             * ensuring that if competing threads want to create datasource context
                             * on different datasource id then they are not blocked on each other
                             * and can run concurrently. Only threads that want to create a new
                             * datasource context on the same datasource id would be synchronized.
                             */
                            Object monitor = new Object();
                            if (datasourceContextIdentifier.isKeyValid()) {
                                if (datasourceContextSynchronizationMonitorMap.get(
                                ...
                                monitor =
                                        datasourceContextSynchronizationMonitorMap.get(
                                                datasourceContextIdentifier);
                            }
    ...
    public Mono<DatasourceContext<?>> getDatasourceContext(DatasourceStorage datasourceStorage) {
        ...
        if (datasourceId == null) {
            log.debug(
                    "This is a dry run or an embedded datasourceStorage. The datasourceStorage"
                            + " context would not exist in this scenario");
        } else {
    ...
    public <T> Mono<T> retryOnce(
            DatasourceStorage datasourceStorage, Function<DatasourceContext<?>, Mono<T>> task) {

        final Mono<T> taskRunnerMono =
                Mono.justOrEmpty(datasourceStorage)
    ...
    public Mono<DatasourceContext<?>> deleteDatasourceContext(DatasourceStorage datasourceStorage) {
        ...
        DatasourceContext<?> datasourceContext =
                datasourceContextMap.get(datasourceContextIdentifier);
        ...
        return pluginExecutorHelper
                ...
                .map(
                        pluginExecutor -> {
                            log.info(
                                    "Clearing datasource context for datasource storage ID {}.",
    ...
    public Mono<DatasourceContext<?>> getDatasourceContext(
            DatasourceStorage datasourceStorage, Plugin plugin) {
    ...
    public Mono<DatasourceContext<?>> getRemoteDatasourceContext(
            Plugin plugin, DatasourceStorage datasourceStorage) {
     ...
     */
    @Override
    public DatasourceContextIdentifier initializeDatasourceContextIdentifier(
            DatasourceStorage datasourceStorage) {
        return new DatasourceContextIdentifier(
                datasourceStorage.getDatasourceId(), FieldName.UNUSED_ENVIRONMENT_ID);
    }
}
