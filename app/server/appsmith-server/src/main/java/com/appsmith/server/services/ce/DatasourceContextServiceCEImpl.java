...
public class DatasourceContextServiceCEImpl implements DatasourceContextServiceCE {
  ...
  // DatasourceContextIdentifier contains datasourceId & environmentId which is mapped to
  // DatasourceContext
  protected final Map<DatasourceContextIdentifier, Mono<? extends DatasourceContext<?>>>
      datasourceContextMonoMap;
  protected final Map<DatasourceContextIdentifier, Object>
  ...
  /**
   * This method defines a critical section that can be executed only by one thread at a time per
   * datasource id - i .e. if two threads want to create datasource for different datasource ids
   * then they would not be synchronized. Earlier multiple threads could subscribe to a publisher
   * that created connection to a datasource - which resulted in a data race condition resulting in
   * multiple orphan connections. Ref: https://github.com/appsmithorg/appsmith/issues/14117 This
   * method caches the result from the source publisher and forces concurrent subscriptions to
   * re-use the cached value. Hence, even if multiple threads subscribe to the same source publisher
   * they get the pre-computed cached value instead of creating a new connection for each
   * subscription of the source publisher.
   *
   * @param datasource - datasource for which a new datasource context / connection needs to be
   *     created
   * @param pluginExecutor - plugin executor associated with the datasource's plugin
   * @param monitor - unique monitor object per datasource id. Lock is acquired on this monitor
   *     object.
   * @param datasourceContextIdentifier - key for the datasourceContextMaps.
   * @return a cached source publisher which upon subscription produces / returns the latest
  ...
  public Mono<? extends DatasourceContext<?>> getCachedDatasourceContextMono(
    ...
    synchronized (monitor) {
      ...
      Mono<Object> connectionMonoCache =
          pluginExecutor.datasourceCreate(datasource.getDatasourceConfiguration()).cache();

      Mono<DatasourceContext<Object>> datasourceContextMonoCache =
          connectionMonoCache
              .flatMap(
                  connection ->
                      updateDatasourceAndSetAuthentication(
                          connection, datasource, datasourceContextIdentifier))
              .map(
                  connection -> {
  ...
  public Mono<Object> updateDatasourceAndSetAuthentication(
      Object connection,
      Datasource datasource,
      DatasourceContextIdentifier datasourceContextIdentifier) {
    ...
    if (connection instanceof UpdatableConnection) {
      ...
      datasource
          ...
          .setAuthentication(
              ((UpdatableConnection) connection)
  ...
  protected Mono<Datasource> retrieveDatasourceFromDB(
      Datasource datasource, DatasourceContextIdentifier datasourceContextIdentifier) {
    if (datasourceContextIdentifier.isKeyValid()) {
  ...
  protected Mono<DatasourceContext<?>> createNewDatasourceContext(
    ...
    Mono<Datasource> datasourceMono =
        retrieveDatasourceFromDB(datasource, datasourceContextIdentifier);
    ...
    return datasourceMono
            ...
            datasource1 -> {
              Mono<Plugin> pluginMono =
                  datasourceMono.flatMap(
                      resource -> pluginService.findById(resource.getPluginId()));

              // Datasource Context has not been created for this resource on this
              // machine. Create one now.
        ...
        .flatMap(
            objects -> {
               ...
               * `getCachedDatasourceContextMono` would then acquire lock on the monitor object
               * which is unique for each datasourceId hence ensuring that if competing threads want
               * to create datasource context on different datasource id then they are not blocked
               * on each other and can run concurrently. Only threads that want to create a new
               * datasource context on the same datasource id would be synchronized.
               */
              Object monitor = new Object();
              if (datasourceContextIdentifier.isKeyValid()) {
                if (datasourceContextSynchronizationMonitorMap.get(datasourceContextIdentifier)
                    == null) {
                  synchronized (this) {
                    if (datasourceContextSynchronizationMonitorMap.get(datasourceContextIdentifier)
                        == null) {
                      datasourceContextSynchronizationMonitorMap.put(
                          datasourceContextIdentifier, new Object());
                    }
                  }
                }

                monitor =
                    datasourceContextSynchronizationMonitorMap.get(datasourceContextIdentifier);
              }

              return getCachedDatasourceContextMono(
                  datasource1, pluginExecutor, monitor, datasourceContextIdentifier);
            });
  }
  ...
  public boolean getIsStale(
    ...
    return datasourceId != null
        ...
        && datasource
            .getUpdatedAt()
            .isAfter(datasourceContextMap.get(datasourceContextIdentifier).getCreationTime());
  }
  ...
  public boolean isValidDatasourceContextAvailable(
    ...
    return datasourceContextMap.get(datasourceContextIdentifier) != null
        // The following condition happens when there's a timeout in the middle of
        // destroying a connection and
  ...
  public Mono<DatasourceContext<?>> getDatasourceContext(
    ...
    if (datasourceId == null) {
      log.debug(
          "This is a dry run or an embedded datasource. The datasource context would not"
              + " exist in this scenario");
    } else {
      if (isValidDatasourceContextAvailable(datasource, datasourceContextIdentifier)) {
  ...
  @Override
  public <T> Mono<T> retryOnce(
      Datasource datasource,
      DatasourceContextIdentifier datasourceContextIdentifier,
      Map<String, BaseDomain> environmentMap,
      Function<DatasourceContext<?>, Mono<T>> task) {

    final Mono<T> taskRunnerMono =
        Mono.justOrEmpty(datasource)
            .flatMap(
                datasource1 ->
                    getDatasourceContext(datasource1, datasourceContextIdentifier, environmentMap))
            // Now that we have the context (connection details), call the task.
            .flatMap(task);

    return taskRunnerMono.onErrorResume(
        StaleConnectionException.class,
        error -> {
          log.info("Looks like the connection is stale. Retrying with a fresh context.");
          return deleteDatasourceContext(datasourceContextIdentifier).then(taskRunnerMono);
        });
  }

  @Override
  public Mono<DatasourceContext<?>> deleteDatasourceContext(
      DatasourceContextIdentifier datasourceContextIdentifier) {
        ...
        .zipWhen(
            datasource1 ->
                pluginExecutorHelper.getPluginExecutor(
                    pluginService.findById(datasource1.getPluginId())))
        .map(
            tuple -> {
  ...
  // We can afford to make this call all the time since we already have all the info we need in
  // context
  @Override
  public Mono<DatasourceContext<?>> getRemoteDatasourceContext(
      Plugin plugin, Datasource datasource) {
        ...
        .map(
            instanceId -> {
              ...
              executePluginDTO.setDatasource(
                  new DatasourceDTO(datasource.getId(), datasource.getDatasourceConfiguration()));
  ...
  /**
   * Generates the custom key that is used in: datasourceContextMap datasourceContextMonoMap
   * datasourceContextSynchronizationMonitorMap
   *
   * @param datasource
