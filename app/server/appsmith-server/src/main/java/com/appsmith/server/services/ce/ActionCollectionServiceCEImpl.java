...
public class ActionCollectionServiceCEImpl
    extends BaseService<ActionCollectionRepository, ActionCollection, String>
  ...
  public Mono<ActionCollection> findByIdAndBranchName(String id, String branchName) {
    ...
    return this.findByBranchNameAndDefaultCollectionId(
  ...
  public Flux<ActionCollectionDTO> getPopulatedActionCollectionsByViewMode(
    ...
    return this.getActionCollectionsByViewMode(params, viewMode)
        .flatMap(
            actionCollectionDTO ->
                this.populateActionCollectionByViewMode(actionCollectionDTO, viewMode));
  }
  ...
  public Mono<ActionCollectionDTO> populateActionCollectionByViewMode(
    ...
    return Mono.just(actionCollectionDTO1)
        .flatMap(
            actionCollectionDTO ->
                Flux.fromIterable(actionCollectionDTO.getDefaultToBranchedActionIdsMap().values())
                    .mergeWith(
                        Flux.fromIterable(
                            actionCollectionDTO
                    ...
                    .flatMap(
                        actionsList ->
                            splitValidActionsByViewMode(
                                actionCollectionDTO, actionsList, viewMode)));
  }

  /**
   * This method splits the actions associated to an action collection into valid and archived
   * actions
   */
  @Override
        ...
        .flatMapMany(
            branchedApplicationId ->
                repository
                    ...
                    // Filter out all the action collections which haven't been
                    // published
                    .flatMap(
                        actionCollection -> {
                    ...
                    .flatMap(
                        actionCollection -> {
                          ActionCollectionViewDTO actionCollectionViewDTO =
                              new ActionCollectionViewDTO();
                          ...
                          actionCollectionViewDTO.setApplicationId(
                          ...
                          // actionCollection.defaultResources contains
                          // appId, collectionId and branch(optional).
                          // Default pageId will be taken from
                          // publishedCollection.defaultResources
                          DefaultResources defaults = actionCollection.getDefaultResources();
                          // Consider a situation when collection is not
                          // published but user is viewing in deployed
                          // mode
                          if (publishedCollection.getDefaultResources() != null
                              && defaults != null) {
                            defaults.setPageId(
                                publishedCollection.getDefaultResources().getPageId());
                          } else {
                            log.debug(
                                "Unreachable state, unable to find"
                                    + " default ids for"
                                    + " actionCollection: {}",
  ...
  public Flux<ActionCollectionDTO> getActionCollectionsByViewMode(
    ...
    if (params.getFirst(FieldName.APPLICATION_ID) != null) {
      ...
      return applicationService
          ...
          .flatMap(
              actionCollection -> generateActionCollectionByViewMode(actionCollection, viewMode));
    }
            ...
            .switchIfEmpty(
                Mono.error(
                    new AppsmithException(
                        AppsmithError.ACL_NO_RESOURCE_FOUND, FieldName.ACTION_COLLECTION, id)))
            .cache();
    ...
    return actionCollectionMono
        .map(
            dbActionCollection -> {
              ...
              // No need to save defaultPageId at actionCollection level as this will
              // be stored inside the
              // actionCollectionDTO
              DefaultResourcesUtils.createDefaultIdsOrUpdateWithGivenResourceIds(
                  dbActionCollection, dbActionCollection.getDefaultResources().getBranchName());
        ...
        .flatMap(
            actionCollection ->
                this.generateActionCollectionByViewMode(actionCollection, false)
                    .flatMap(
                        actionCollectionDTO1 ->
  ...
  public Mono<ActionCollectionDTO> deleteUnpublishedActionCollection(String id) {
    return deleteUnpublishedActionCollectionEx(
        id, Optional.of(actionPermission.getDeletePermission()));
  }
    ...
    Mono<ActionCollection> actionCollectionMono =
        repository
            ...
            .switchIfEmpty(
                Mono.error(
                    new AppsmithException(
                        AppsmithError.NO_RESOURCE_FOUND, FieldName.ACTION_COLLECTION, id)));
    return actionCollectionMono
        .flatMap(
            toDelete -> {
              ...
              if (toDelete.getPublishedCollection() != null) {
                ...
                modifiedActionCollectionMono =
                    Flux.fromIterable(
                                        ...
                                        throwable -> {
                                          log.debug(
                                              "Failed"
                                                  ...
                                                  + " {}",
                                              actionId,
                                              toDelete.getUnpublishedCollection().getName());
                        ...
                        .flatMap(
                            modifiedActionCollection -> {
                              return analyticsService.sendArchiveEvent(
                                  modifiedActionCollection,
                                  getAnalyticsProperties(modifiedActionCollection));
                            });
              } else {
                // This actionCollection was never published. This document can be
                // safely archived
  ...
  public Mono<ActionCollectionDTO> deleteUnpublishedActionCollection(String id, String branchName) {
    Mono<String> branchedCollectionId =
        StringUtils.isEmpty(branchName)
            ? Mono.just(id)
            : this.findByBranchNameAndDefaultCollectionId(
  ...
  public Mono<ActionCollectionDTO> generateActionCollectionByViewMode(
    ...
    if (TRUE.equals(viewMode)) {
      if (actionCollection.getPublishedCollection() != null) {
        ...
        // We are trying to fetch published action but it doesnt exist because the action
  ...
  public Mono<ActionCollection> archiveById(String id) {
    ...
    return actionCollectionMono
        .map(
            actionCollection -> {
              ...
              if (unpublishedCollection != null) {
                ...
                actionIds.addAll(
                    unpublishedCollection.getDefaultToBranchedArchivedActionIdsMap().values());
              }
              if (publishedCollection != null
                  && !CollectionUtils.isEmpty(
                      publishedCollection.getDefaultToBranchedActionIdsMap())) {
                actionIds.addAll(publishedCollection.getDefaultToBranchedActionIdsMap().values());
                actionIds.addAll(
                    publishedCollection.getDefaultToBranchedArchivedActionIdsMap().values());
              }
              return actionIds;
            })
        ...
        .flatMap(
            deletedActionCollection ->
                analyticsService.sendDeleteEvent(
  ...
  public Map<String, Object> getAnalyticsProperties(ActionCollection savedActionCollection) {
    final ActionCollectionDTO unpublishedCollection =
        savedActionCollection.getUnpublishedCollection();
    Map<String, Object> analyticsProperties = new HashMap<>();
    analyticsProperties.put(
        "actionCollectionName", ObjectUtils.defaultIfNull(unpublishedCollection.getName(), ""));
    analyticsProperties.put(
        "applicationId", ObjectUtils.defaultIfNull(savedActionCollection.getApplicationId(), ""));
    analyticsProperties.put(
        "pageId", ObjectUtils.defaultIfNull(unpublishedCollection.getPageId(), ""));
    analyticsProperties.put(
        "orgId", ObjectUtils.defaultIfNull(savedActionCollection.getWorkspaceId(), ""));
