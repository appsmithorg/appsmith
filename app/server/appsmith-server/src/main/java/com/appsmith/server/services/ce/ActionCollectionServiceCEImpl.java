			...
			branchedApplicationId ->
				repository
					...
					.flatMap(
						...
						// actionCollection.defaultResources contains appId, collectionId and
						// branch(optional).
						...
						// Consider a situation when collection is not published but user is
						// viewing in deployed mode
						if (publishedCollection.getDefaultResources() != null
							...
							log.debug(
								"Unreachable state, unable to find default ids for"
									+ " actionCollection: {}",
...
public Mono<ActionCollectionDTO> update(String id, ActionCollectionDTO actionCollectionDTO) {
	...
	return actionCollectionMono
		.map(
			...
			// No need to save defaultPageId at actionCollection level as this will be stored
			// inside the
			// actionCollectionDTO
						...
						.flatMap(
							actionId ->
								newActionService
									...
									// return an empty action so that the filter can remove it from
									// the list
									.onErrorResume(
										throwable -> {
										log.debug(
...
public Mono<ActionCollectionDTO> generateActionCollectionByViewMode(
	...
	if (actionCollection.getPublishedCollection() != null) {
		...
		// We are trying to fetch published action but it doesnt exist because the action hasn't
		// been published yet
		return Mono.empty();
	}
	} else {
