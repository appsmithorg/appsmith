...
public class ActionCollectionServiceCEImpl
    ...
    public Flux<ActionCollection> saveAll(List<ActionCollection> collections) {
        collections.forEach(
                collection -> {
                    if (collection.getGitSyncId() == null) {
                        collection.setGitSyncId(
                                collection.getApplicationId() + "_" + new ObjectId());
                    }
                });
                ...
                .map(
                        actionCollectionDTO1 -> {
                            ...
                            if (collect.size() == 1) {
                                ...
                                actionCollectionDTO.setPluginType(
                                        actionsList.get(0).getPluginType());
                            }
                            actionsList.forEach(
                                    action -> {
                                ...
                                repository
                                        ...
                                        .flatMap(
                                                actionCollection -> {
                                                    ...
                                                    actionCollectionViewDTO.setName(
                                                            publishedCollection.getName());
                                                    actionCollectionViewDTO.setPageId(
                                                            publishedCollection.getPageId());
                                                    actionCollectionViewDTO.setApplicationId(
                                                            actionCollection.getApplicationId());
                                                    actionCollectionViewDTO.setVariables(
                                                            publishedCollection.getVariables());
                                                    actionCollectionViewDTO.setBody(
                                                            publishedCollection.getBody());
                                                    ...
                                                    // mode
                                                    if (publishedCollection.getDefaultResources()
                                                                    != null
                                                            && defaults != null) {
                                                        defaults.setPageId(
                                                                publishedCollection
                                                    ...
                                                    actionCollectionViewDTO.setDefaultResources(
                                                            defaults);
    ...
    public Mono<ActionCollection> archiveById(String id) {
        ...
        return actionCollectionMono
                        ...
                        actionId ->
                                newActionService
                                        ...
                                        .onErrorResume(
                                                throwable -> {
                                                    log.debug(
                                                            "Failed to delete action with id {} for"
                                                                    + " collection with id: {}",
                                                            actionId,
                                                            id);
                                                    log.error(throwable.getMessage());
                ...
                .flatMap(
                        actionCollection ->
                                repository.archive(actionCollection).thenReturn(actionCollection))
                .flatMap(
                        deletedActionCollection ->
                                analyticsService.sendDeleteEvent(
                                        deletedActionCollection,
                                        getAnalyticsProperties(deletedActionCollection)));
    }
    ...
    public Map<String, Object> getAnalyticsProperties(ActionCollection savedActionCollection) {
        ...
        analyticsProperties.put(
                ...
                ObjectUtils.defaultIfNull(unpublishedCollection.getName(), ""));
        analyticsProperties.put(
                "applicationId",
                ObjectUtils.defaultIfNull(savedActionCollection.getApplicationId(), ""));
        analyticsProperties.put(
