...
public class UserWorkspaceServiceTest {
  ...
  public void leaveWorkspace_WhenUserExistsInWorkspace_RemovesUser() {
    ...
    Application savedApplication =
        applicationPageService.createApplication(application, testWorkspace.getId()).block();
    ...
    StepVerifier.create(updateUserRoleMono)
        .assertNext(
            userRole1 -> {
              ...
              assertEquals(
                  developerPermissionGroup.getName(), userRole1.getRoles().get(0).getName());
              assertEquals(
                  Workspace.class.getSimpleName(), userRole1.getRoles().get(0).getEntityType());
            })
        .verifyComplete();
  }
