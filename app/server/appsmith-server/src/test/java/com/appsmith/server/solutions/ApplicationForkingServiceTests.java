...
/**
 * Always run the complete test suite here as TCs have dependency between them. We are controlling
 * this with FixMethodOrder(MethodSorters.NAME_ASCENDING) to run the TCs in a sequence. Running TC
 * in a sequence is a bad practice for unit TCs but here we are actually executing a integration
 * test where we are first inviting user and then forking the application
 */
@Slf4j
...
public class ApplicationForkingServiceTests {
    ...
    @Autowired private ApplicationForkingService applicationForkingService;
    ...
    @Autowired private ImportExportApplicationService importExportApplicationService;
    ...
    public void setup() {
        ...
        layoutActionService
                ...
                .block();
        // Invite "usertest@usertest.com" with VIEW access, api_user will be the admin of
        // sourceWorkspace and we are
        // controlling this with @FixMethodOrder(MethodSorters.NAME_ASCENDING) to run the TCs in a
        // sequence.
        // Running TC in a sequence is a bad practice for unit TCs but here we are testing the
        // invite user and then fork
        // application as a part of this flow.
        // We need to test with VIEW user access so that any user should be able to fork template
        // applications
        PermissionGroup permissionGroup =
                permissionGroupService
                        .getByDefaultWorkspace(
                                sourceWorkspace, AclPermission.READ_PERMISSION_GROUP_MEMBERS)
                        .collectList()
                        .block()
                        .stream()
                        .filter(
    ...
    public void test2_forkApplicationWithReadApplicationUserAccess() {
        ...
        // copy all the actions and collections. This process may take time and since some of the
        // test cases in
        // ApplicationForkingServiceTests observed failure in the CI due to timeoutException, to
        // unblock this temporarily,
        // synchronous block() is being used until it is fixed.
        // TODO: Investigate working of applicationForkingService.forkApplicationToWorkspace()
        // further and fix the timeoutException.
    ...
    public void test4_validForkApplication_cancelledMidWay_createValidApplication() {
        ...
        applicationForkingService
                .forkApplicationToWorkspaceWithEnvironment(
                        sourceAppId, targetWorkspace.getId(), sourceEnvironmentId)
                .timeout(Duration.ofMillis(10))
                .subscribe();

        // Wait for fork to complete
        Mono<Application> forkedAppFromDbMono =
                Mono.just(targetWorkspace)
                        .flatMap(
                                workspace -> {
                                    try {
                                        // Before fetching the forked application, sleep for 5
                                        // seconds to ensure that the forking finishes
                                        Thread.sleep(5000);
    ...
    public void
            forkApplicationToWorkspace_WhenAppHasUnsavedThemeCustomization_ForkedWithCustomizations() {
        // Fork application is currently a slow API because it needs to create application, clone
        // all the pages, and then
        // copy all the actions and collections. This process may take time and since some of the
        // test cases in
        ...
        String createdSrcDefaultEnvironmentId =
                workspaceService.getDefaultEnvironmentId(createdSrcWorkspace.getId()).block();

        Application srcApplication = new Application();
        srcApplication.setName("app_" + uniqueString);
        Application createdSrcApplication =
        ...
        Application forkedApplication =
                applicationForkingService
                        .forkApplicationToWorkspaceWithEnvironment(
                                createdSrcApplication.getId(),
                                createdDestWorkspace.getId(),
                                createdSrcDefaultEnvironmentId)
                        .block();

        Theme forkedApplicationEditModeTheme =
                themeService
                        .getApplicationTheme(forkedApplication.getId(), ApplicationMode.EDIT, null)
                        .block();
        Theme forkedApplicationPublishedModeTheme =
                themeService
                        .getApplicationTheme(
                                forkedApplication.getId(), ApplicationMode.PUBLISHED, null)
                        .block();

        final Mono<Tuple4<Theme, Theme, Application, Application>> tuple4Mono =
                Mono.zip(
        ...
        StepVerifier.create(tuple4Mono)
                .assertNext(
                        objects -> {
                            ...
                            assertThat(forkedApp.getEditModeThemeId())
                                    .isEqualTo(editModeTheme.getId());
                            assertThat(forkedApp.getPublishedModeThemeId())
                                    .isEqualTo(publishedModeTheme.getId());
                            assertThat(forkedApp.getEditModeThemeId())
                                    .isNotEqualTo(forkedApp.getPublishedModeThemeId());

                            // published mode should have the custom theme as we publish after
                            // forking the app
                            assertThat(publishedModeTheme.isSystemTheme()).isFalse();
                            // published mode theme will have no application id and org id set as
                            // the customizations were not saved
                            ...
                            // edit mode theme will have no application id and org id set as the
                            // customizations were not saved
                            ...
                            assertThat(editModeTheme.getDisplayName())
                                    .isEqualTo("theme_" + uniqueString);
                            assertThat(publishedModeTheme.getDisplayName())
                                    .isEqualTo("theme_" + uniqueString);

                            // forked application should have a new edit mode theme created, should
                            // not be same as src app theme
                            assertThat(srcApp.getEditModeThemeId())
                                    .isNotEqualTo(forkedApp.getEditModeThemeId());
    ...
    public void forkApplicationToWorkspace_WhenAppHasSystemTheme_SystemThemeSet() {
        ...
        String createdSrcDefaultEnvironmentId =
                workspaceService.getDefaultEnvironmentId(createdWorkspace.getId()).block();
        ...
        Application forkedApplication =
                applicationForkingService
                        .forkApplicationToWorkspaceWithEnvironment(
                                ...
                                createdSrcDefaultEnvironmentId)
                        .block();
        Theme forkedApplicationTheme =
                themeService
                        .getApplicationTheme(forkedApplication.getId(), ApplicationMode.EDIT, null)
                        .block();

        Mono<Tuple3<Theme, Application, Application>> tuple3Mono =
                Mono.zip(
                        Mono.just(forkedApplicationTheme),
                        Mono.just(forkedApplication),
                        Mono.just(createdSrcApplication));
        ...
        StepVerifier.create(tuple3Mono)
                .assertNext(
                        objects -> {
                            ...
                            // edit mode theme will have no application id and org id set as it's
                            // system theme
                            assertThat(editModeTheme.getWorkspaceId()).isNullOrEmpty();
                            assertThat(editModeTheme.getApplicationId()).isNullOrEmpty();

                            // forked theme should be default theme
                            assertThat(editModeTheme.getName())
                                    .isEqualToIgnoringCase(Theme.DEFAULT_THEME_NAME);

                            // forked application should have same theme set
                            assertThat(srcApp.getEditModeThemeId())
                                    .isEqualTo(forkedApp.getEditModeThemeId());
                        })
                .verifyComplete();
    }
    ...
    public void forkApplicationToWorkspace_WhenAppHasCustomSavedTheme_NewCustomThemeCreated() {
        ...
        Mono<Tuple4<Theme, Theme, Application, Application>> tuple4Mono =
                Mono.zip(
                        Mono.just(forkedApplicationEditModeTheme),
                        Mono.just(forkedApplicationPublishedModeTheme),
                        Mono.just(forkedApplication),
        ...
        StepVerifier.create(tuple4Mono)
                .assertNext(
                        objects -> {
                            ...
                            // edit mode theme will have application id and org id set as the
                            // customizations were saved
                            assertThat(editModeTheme.getWorkspaceId()).isNullOrEmpty();
                            assertThat(editModeTheme.getApplicationId()).isNullOrEmpty();
    ...
    public void forkApplication_deletePageAfterBeingPublished_deletedPageIsNotCloned() {
        ...
        final String pageId =
                Objects.requireNonNull(applicationPageService.createPage(pageDTO).block()).getId();
        ...
        StepVerifier.create(
                ...
                .assertNext(
                        tuple -> {
                            ...
                            forkedPages.forEach(
                                    newPage ->
                                            assertThat(newPage.getUnpublishedPage().getName())
                                                    .isNotEqualTo(pageDTO.getName()));
                            NewPage deletedPage =
                                    originalPages.stream()
                                            .filter(
                                                    newPage ->
                                                            pageDTO.getName()
                                                                    .equals(
                            ...
                            assertThat(deletedPage.getUnpublishedPage().getName())
                                    .isEqualTo(pageDTO.getName());
                        })
                .verifyComplete();
    }
    ...
    public void forkGitConnectedApplication_defaultBranchUpdated_forkDefaultBranchApplication() {
        ...
        Application createdBranchApplication =
                applicationPageService
    ...
    public void forkApplication_WhenContainsInternalFields_InternalFieldsNotForked() {
        ...
        assert srcWorkspace != null;
        Application srcApp =
                applicationPageService.createApplication(application, srcWorkspace.getId()).block();
    ...
    private Mono<Tuple2<Application, String>> forkApplicationSetup(
        ...
        DatasourceStorage datasourceStorage =
                new DatasourceStorage(datasource, srcDefaultEnvironmentId);
    ...
    @WithUserDetails(value = "api_user")
    public void
            forkApplication_withForkWithConfigurationFalseAndDatasourceUsed_IsPartialImportTrue() {
        ...
        String srcDefaultEnvironmentId =
                workspaceService.getDefaultEnvironmentId(srcApp.getWorkspaceId()).block();

        Mono<ApplicationImportDTO> resultMono =
                applicationForkingService
                        .forkApplicationToWorkspaceWithEnvironment(
                                srcApp.getId(), targetWorkspaceId, srcDefaultEnvironmentId)
                        .flatMap(
                                application ->
                                        importExportApplicationService.getApplicationImportDTO(
                                                application.getId(),
        ...
        StepVerifier.create(resultMono)
                .assertNext(
                        forkedApplicationImportDTO -> {
                            Application forkedApplication =
                                    forkedApplicationImportDTO.getApplication();
                            assertThat(forkedApplication).isNotNull();
    ...
    @WithUserDetails(value = "api_user")
    public void
            forkApplication_withForkWithConfigurationFalseAndDatasourceNotUsed_IsPartialImportFalse() {
        Tuple2<Application, String> forkApplicationSetupResponse =
