...
public void setup() {
	...
	// sequence.
	// Running TC in a sequence is a bad practice for unit TCs but here we are testing the invite
	// user and then fork
...
public void test2_forkApplicationWithReadApplicationUserAccess() {
	...
	// the pages, and then
	// copy all the actions and collections. This process may take time and since some of the test
	// cases in
	// ApplicationForkingServiceTests observed failure in the CI due to timeoutException, to unblock
	// this temporarily,
	// synchronous block() is being used until it is fixed.
	// TODO: Investigate working of applicationForkingService.forkApplicationToWorkspace() further
	// and fix the timeoutException.
	...
	Mono<Application> forkedAppFromDbMono =
		Mono.just(targetWorkspace)
			.flatMap(
				workspace -> {
				try {
					// Before fetching the forked application, sleep for 5 seconds to ensure that
					// the forking finishes
					Thread.sleep(5000);
...
public void
	...
	StepVerifier.create(tuple4Mono)
		.assertNext(
			...
			// published mode theme will have no application id and org id set as the
			...
			// edit mode theme will have no application id and org id set as the customizations
			// were not saved
			...
			// forked application should have a new edit mode theme created, should not be same as
			// src app theme
...
public void forkApplicationToWorkspace_WhenAppHasCustomSavedTheme_NewCustomThemeCreated() {
	...
	StepVerifier.create(tuple4Mono)
		.assertNext(
			...
			// edit mode theme will have application id and org id set as the customizations were
			// saved
			assertThat(editModeTheme.getWorkspaceId()).isNullOrEmpty();
			assertThat(editModeTheme.getApplicationId()).isNullOrEmpty();
