    ...
    public void setup() {
        ...
        Workspace workspace =
                userMono.flatMap(user -> workspaceService.createDefault(new Workspace(), user))
                        .switchIfEmpty(
                                Mono.error(new Exception("createDefault is returning empty!!")))
                        .block();
    }
    ...
    public void verifyFindByDatasourceId() {
        ...
        DatasourceStorage datasourceStorageOne =
                new DatasourceStorage(
                        datasourceId, environmentIdOne, datasourceConfiguration, null, null, null);

        DatasourceStorage datasourceStorageTwo =
                new DatasourceStorage(
                        datasourceId, environmentIdTwo, datasourceConfiguration, null, null, null);
        ...
        StepVerifier.create(datasourceStorageFlux)
                .assertNext(
                        datasourceStorage -> {
                            ...
                            assertThat("mockEnvironmentIdOne")
                                    .isEqualTo(datasourceStorage.getEnvironmentId());
                        })
                .assertNext(
                        datasourceStorage -> {
                            assertThat(datasourceStorage).isNotNull();
                            assertThat(datasourceId).isEqualTo(datasourceStorage.getDatasourceId());
                            assertThat("mockEnvironmentIdTwo")
                                    .isEqualTo(datasourceStorage.getEnvironmentId());
                        })
                .verifyComplete();
    }
