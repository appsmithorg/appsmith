...
public class UserGroupRepositoryTestEE {
    ...
    public void testGetUserGroupsWithParamsPaginated_multipleNames_allExistingNamesShouldCome() {
        ...
        PagedDomain<UserGroup> pagedUsers = userGroupRepository.findUserGroupsWithParamsPaginated(2, 0, emailsForFilter, List.of(), Optional.empty()).block();
        assertThat(pagedUsers.getCount()).isEqualTo(2);
        assertThat(pagedUsers.getTotal()).isEqualTo(2);

        Optional<UserGroup> optionalUserGroup1 = pagedUsers.getContent().stream().filter(user -> user.getName().equals(userGroup1.getName())).findFirst();
        Optional<UserGroup> optionalUserGroup2 = pagedUsers.getContent().stream().filter(user -> user.getName().equals(userGroup2.getName())).findFirst();
        Optional<UserGroup> optionalUserGroup3 = pagedUsers.getContent().stream().filter(user -> user.getName().equals(nonExistingGroupName)).findFirst();
        ...
        userGroupRepository.deleteAllById(List.of(createdUserGroup1.getId(), createdUserGroup2.getId())).block();
    }

    @Test
    @WithUserDetails(value = "api_user")
    public void testGetUserGroupsWithParamsPaginated_multipleNamesCaseInsensitive_allExistingNamesShouldCome() {
        String testName = "testGetUserGroupsWithParamsPaginated_multipleNamesCaseInsensitive_allExistingNamesShouldCome";
        UserGroup userGroup1 = new UserGroup();
        userGroup1.setName(testName + "_1");
        UserGroup createdUserGroup1 = userGroupRepository.save(userGroup1).block();
        ...
        List<String> emailsForFilter = List.of(userGroup1.getName().toUpperCase(), userGroup2.getName().toUpperCase(), nonExistingGroupName.toUpperCase());
