...
public void createApplicationWithDuplicateName() {
	...
	// Creating second App with same name "ApplicationServiceTest TestApp" but due to duplicate name
	// its resultant
...
public void getAllApplicationsForHome() {
	...
	StepVerifier.create(allApplications)
		.assertNext(
			...
			// In case of anonymous user, we should have errored out. Assert that the user is not
			// anonymous.
		...
		.assertNext(
			...
			actionCollectionList.forEach(
				actionCollection -> {
					...
					// We should have single entry as other action is deleted from the parent
					// application
	...
	Mono<Application> clonedAppFromDbMono =
		Mono.just(originalApplication)
			.flatMap(
				originalApp -> {
				try {
					// Before fetching the cloned application, sleep for 5 seconds to ensure that
					// the cloning finishes
					Thread.sleep(5000);
	...
	Mono<Application> applicationFromDbPostViewChange =
		Mono.just(originalApplication)
			.flatMap(
				originalApp -> {
				try {
					// Before fetching the public application, sleep for 5 seconds to ensure that
					// the updating
