...
public class UserWorkspaceServiceUnitTest {
    ...
    public void getWorkspaceMembers_WhenRoleIsNull_ReturnsEmptyList() {
        ...
        /**
         * Removing the Default Workspace ID from auto-created permission groups so that while
         * fetching the Users and Groups, we should get empty list.
         */
        Workspace createdWorkspace = workspaceService.create(testWorkspace).block();
        List<PermissionGroup> autoCreatedPermissionGroups =
                permissionGroupRepository
                        .findByDefaultDomainIdAndDefaultDomainType(
                                createdWorkspace.getId(), Workspace.class.getSimpleName())
                        .flatMap(
                                permissionGroup -> {
        ...
        Mono<List<MemberInfoDTO>> workspaceMembers =
                userWorkspaceService.getWorkspaceMembers(testWorkspace.getId());
    ...
    public void getWorkspaceMembers_WhenNoOrgFound_ThrowsException() {
        ...
        Mono<List<MemberInfoDTO>> workspaceMembers =
                userWorkspaceService.getWorkspaceMembers(sampleWorkspaceId);
        StepVerifier.create(workspaceMembers)
                .expectErrorMessage(
                        AppsmithError.NO_RESOURCE_FOUND.getMessage(
                                FieldName.WORKSPACE, sampleWorkspaceId))
                .verify();
    }
    ...
    public void getWorkspaceMembers_WhenUserHasProfilePhotoForOneWorkspace_ProfilePhotoIncluded() {
        ...
        Mono<List<MemberInfoDTO>> listMono =
                userDataService
                        .getForCurrentUser()
                        .flatMap(
                                userData -> {
                        ...
                        .flatMap(
                                createdWorkspace ->
                                        userWorkspaceService.getWorkspaceMembers(
                                                createdWorkspace.getId()));

        StepVerifier.create(listMono)
                .assertNext(
                        workspaceMemberInfoDTOS -> {
                            assertThat(workspaceMemberInfoDTOS.size()).isEqualTo(1);
                            assertThat(workspaceMemberInfoDTOS.get(0).getPhotoId())
                                    .isEqualTo("sample-photo-id");
                        })
                .verifyComplete();
    }
    ...
    @WithUserDetails(value = "api_user")
    public void
            getWorkspaceMembers_WhenUserHasProfilePhotoForMultipleWorkspace_ProfilePhotoIncluded() {
        // create workspace
        ...
        Mono<Map<String, List<MemberInfoDTO>>> mapMono =
                userDataService
                        .getForCurrentUser()
                        .flatMap(
                                userData -> {
                ...
                .assertNext(
                        workspaceMemberInfoDTOSMap -> {
                            ...
                            workspaceMemberInfoDTOSMap
                                    ...
                                    .forEach(
                                            workspaceMemberInfoDTOS -> {
                                                // should have one entry for the creator member
                                                // only, get that
                                                MemberInfoDTO workspaceMemberInfoDTO =
                                                        workspaceMemberInfoDTOS.get(0);
                                                // we already set profile photo for the current
                                                // user, check it exists in response
                                                assertThat(workspaceMemberInfoDTO.getPhotoId())
                                                        .isEqualTo("sample-photo-id");
                                            });
                        })
                .verifyComplete();
