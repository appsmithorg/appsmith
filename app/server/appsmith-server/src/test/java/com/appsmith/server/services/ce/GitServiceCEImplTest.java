...
@MockBean GitFileUtils gitFileUtils;
@MockBean ImportExportApplicationService importExportApplicationService;
@MockBean GitExecutor gitExecutor;
@MockBean EmailConfig emailConfig;
@MockBean GitCloudServicesUtils gitCloudServicesUtils;
@MockBean GitDeployKeysRepository gitDeployKeysRepository;
...
@MockBean RedisUtils redisUtils;

@BeforeEach
public void setup() {
	gitService =
		new GitServiceCEImpl(
			userService,
			userDataService,
			sessionUserService,
			applicationService,
...
public void isRepoLimitReached_connectedAppCountIsLessThanLimit_Success() {
	...
	doReturn(Mono.just(1L))
		.when(gitService1)
		.getApplicationCountWithPrivateRepo(Mockito.any(String.class));
...
// This test is to check if the limit is reached when the count of connected apps is more than the
// limit
// This happens when public visible git repo is synced with application and then the visibility is
// changed
@Test
