...
public class SeedMongoData {
  ...
  ApplicationRunner init(
    ...
    Policy manageWorkspaceAppPolicy =
        Policy.builder()
            .permission(WORKSPACE_MANAGE_APPLICATIONS.getValue())
            .users(Set.of(API_USER_EMAIL))
            .build();

    Policy exportWorkspaceAppPolicy =
        Policy.builder()
    ...
    Policy managePagePolicy =
        Policy.builder().permission(MANAGE_PAGES.getValue()).users(Set.of(API_USER_EMAIL)).build();

    Policy readPagePolicy =
        Policy.builder().permission(READ_PAGES.getValue()).users(Set.of(API_USER_EMAIL)).build();
    ...
    Policy readApiUserPolicy =
        Policy.builder().permission(READ_USERS.getValue()).users(Set.of(API_USER_EMAIL)).build();

    Policy manageApiUserPolicy =
        Policy.builder().permission(MANAGE_USERS.getValue()).users(Set.of(API_USER_EMAIL)).build();

    Policy readTestUserPolicy =
        Policy.builder().permission(READ_USERS.getValue()).users(Set.of(TEST_USER_EMAIL)).build();

    Policy readAdminUserPolicy =
        Policy.builder().permission(READ_USERS.getValue()).users(Set.of(ADMIN_USER_EMAIL)).build();

    Policy readDevUserPolicy =
        Policy.builder().permission(READ_USERS.getValue()).users(Set.of(DEV_USER_EMAIL)).build();
    ...
    Object[][] userData = {
      ...
      },
      {
        ...
        UserState.ACTIVATED,
        new HashSet<>(
            Arrays.asList(userManageWorkspacePolicy, readApiUserPolicy, manageApiUserPolicy))
      },
      {
        "admin test",
        ADMIN_USER_EMAIL,
                      ...
                      .flatMap(
                          savedPermissionGroup -> {
                            ...
                            User updatedWithPolicies =
                                policyUtils.addPoliciesToExistingObject(crudUserPolicies, user);
    ...
    Flux<Workspace> workspaceFlux =
        mongoTemplate
            .find(
                new Query()
                    .addCriteria(
                        where("name").in(pluginData[0][0], pluginData[1][0], pluginData[2][0])),
                Plugin.class)
    ...
    Flux<User> addUserWorkspaceFlux =
        workspaceFlux1.flatMap(
            workspace ->
                userFlux.flatMap(
                    user -> {
    ...
    Mono<Workspace> workspaceByNameMono =
        mongoTemplate
    ...
    Mono<Application> appByNameMono =
        mongoTemplate
    ...
    return args -> {
      workspaceFlux1
          ...
          // Query the seed data to get the workspaceId (required for application
          // creation)
          ...
          .flatMapMany(
              ...
              // Query the seed data to get the applicationId (required for page
              // creation)
              )
          .then(appByNameMono)
