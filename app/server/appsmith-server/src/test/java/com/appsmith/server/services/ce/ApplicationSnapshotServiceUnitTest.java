...
public void createApplicationSnapshot_WhenApplicationTooLarge_SnapshotCreatedSuccessfully() {
	...
	StepVerifier.create(
			applicationSnapshotService.createApplicationSnapshot(defaultAppId, branchName))
		.assertNext(
			aBoolean -> {
...
public void restoreSnapshot_WhenSnapshotHasMultipleChunks_RestoredSuccessfully() {
	...
	List<ApplicationSnapshot> snapshots =
		List.of(
			...
			createSnapshot(
				branchedAppId, copyOfRange(jsonStringBytes, chunkSize, chunkSize * 2), 2));
...
public void restoreSnapshot_WhenApplicationHasDefaultPageIds_IdReplacedWithDefaultPageId() {
	...
	List<ApplicationSnapshot> snapshots =
		List.of(createSnapshot(branchedAppId, jsonStringBytes, 1));
