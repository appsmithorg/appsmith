...
public class ApplicationSnapshotServiceUnitTest {
    ...
    @MockBean ApplicationSnapshotRepository applicationSnapshotRepository;

    @Autowired ApplicationSnapshotService applicationSnapshotService;
    ...
    public void createApplicationSnapshot_WhenApplicationTooLarge_SnapshotCreatedSuccessfully() {
        ...
        StepVerifier.create(
                        applicationSnapshotService.createApplicationSnapshot(
                                defaultAppId, branchName))
                .assertNext(
                        aBoolean -> {
    ...
    public void restoreSnapshot_WhenSnapshotHasMultipleChunks_RestoredSuccessfully() {
        ...
        List<ApplicationSnapshot> snapshots =
                List.of(
                        createSnapshot(
                                branchedAppId,
                                copyOfRange(jsonStringBytes, chunkSize * 2, jsonStringBytes.length),
                                3),
                        createSnapshot(
                                branchedAppId, copyOfRange(jsonStringBytes, 0, chunkSize), 1),
                        createSnapshot(
                                branchedAppId,
                                copyOfRange(jsonStringBytes, chunkSize, chunkSize * 2),
                                2));
    ...
    public void restoreSnapshot_WhenApplicationConnectedToGit_ReturnApplicationWithDefaultIds() {
        ...
        // create the application json because we need to mock the snapshot with a byte array of
        // this json
        ApplicationJson applicationJson = new ApplicationJson();
        applicationJson.setExportedApplication(application);
        String jsonString = gson.toJson(applicationJson);
        ...
        ApplicationSnapshot applicationSnapshot =
                createSnapshot("branched-app-id", jsonStringBytes, 1);
        ...
        // mock so that this application snapshot is returned when queried with branched application
        // id
        ...
        StepVerifier.create(
                        applicationSnapshotService.restoreSnapshot("default-app-id", "development"))
                .assertNext(
                        application1 -> {
                            assertThat(application1.getName()).isEqualTo(application.getName());
