	...
	Mono<Datasource> datasourceMono =
		Mono.just(createdDatasource)
			.flatMap(
				original -> {
				Datasource datasource1 = new Datasource();
				// Here we still need to send some object of authentication type to make sure that
				// the entire object is not replaced by null
...
public void checkEncryptionOfAuthenticationDTOAfterRemoval() {
	...
	// We want the entire authentication object to be discarded here to avoid reusing any sensitive
	// data across types
	Mockito.when(pluginExecutorHelper.getPluginExecutor(Mockito.any()))
		.thenReturn(Mono.just(new MockPluginExecutor()));
	...
	Mono<Datasource> datasourceMono =
		Mono.just(createdDatasource)
			.flatMap(
				original -> {
				Datasource datasource1 = new Datasource();
				// Here we abstain from sending an authentication object to remove the field from
...
public void get_WhenDatasourcesPresent_SortedAndIsRecentlyCreatedFlagSet() {
	...
	Workspace toCreate = new Workspace();
	toCreate.setName(
		"DatasourceServiceTest : get_WhenDatasourcesPresent_SortedAndIsRecentlyCreatedFlagSet");
