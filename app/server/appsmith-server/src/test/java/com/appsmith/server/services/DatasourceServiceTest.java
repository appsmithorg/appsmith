...
public class DatasourceServiceTest {
        ...
        .expectErrorMatches(
            throwable ->
                throwable instanceof AppsmithException
                    && throwable
                        .getMessage()
                        .equals(AppsmithError.INVALID_PARAMETER.getMessage(FieldName.ID)))
        .verify();
  }
        ...
        .assertNext(
            createdDatasource -> {
              ...
              assertThat(createdDatasource.getPolicies())
                  .containsAll(
                      Set.of(
                          manageDatasourcePolicy, readDatasourcePolicy,
        ...
        .assertNext(
            createdDatasource -> {
              ...
              assertThat(
                  ...
                  .isEqualTo("ssl_key_file_id2");
              assertThat(
                      createdDatasource.getDatasourceConfiguration().getAuthentication()
                          instanceof OAuth2)
                  .isTrue();
            })
        .verifyComplete();
  ...
  public void testDatasourceValid() {
    ...
    Mono<DatasourceTestResult> testResultMono =
        datasourceMono.flatMap(datasource1 -> datasourceService.testDatasource(datasource1, null));
        ...
        .assertNext(
            savedDatasource -> {
              DBAuth authentication =
                  (DBAuth) savedDatasource.getDatasourceConfiguration().getAuthentication();
  ...
  public void checkEncryptionOfAuthenticationDTOAfterUpdate() {
        ...
        Mono.just(createdDatasource)
            .flatMap(
                original -> {
                  ...
                  datasource1.setName(
                      "New Name for update to test that encryption is still" + " correct");
    ...
    StepVerifier.create(datasourceMono)
        .assertNext(
            updatedDatasource -> {
              DBAuth authentication =
                  (DBAuth) updatedDatasource.getDatasourceConfiguration().getAuthentication();

              assertThat(authentication.getUsername()).isEqualTo(username);
              assertThat(password)
                  .isEqualTo(encryptionService.decryptString(authentication.getPassword()));
            })
        .verifyComplete();
  }
        ...
        Mono.just(createdDatasource)
            .flatMap(
                original -> {
                  ...
                  datasource1.setName(
                      "New Name for update to test that encryption is now" + " gone");
                  return datasourceService.update(original.getId(), datasource1);
                });
        ...
        .assertNext(
            createdDatasource -> {
              ...
              assertThat(createdDatasource.getPolicies())
                  .containsAll(
                      Set.of(
                          manageDatasourcePolicy, readDatasourcePolicy, executeDatasourcePolicy));
            })
        .verifyComplete();
  }
    ...
    StepVerifier.create(datasourceMono)
        .assertNext(
            createdDatasource -> {
              ...
              assertThat(createdDatasource.getName()).isEqualTo(datasource.getName());
              assertThat(createdDatasource.getInvalids()).isEmpty();
              assertThat(createdDatasource.getDatasourceConfiguration().getEndpoints())
