...
public class CustomNotificationRepositoryImplTest {
    ...
    public void updateIsReadByForUsernameAndIdList_WhenUsernameNotMatched_UpdatesNone() {
        Mono<Notification> saveMono1 =
                notificationRepository.save(createNotification("abc", false));
        Mono<Notification> saveMono2 =
                notificationRepository.save(createNotification("efg", false));

        // create the notifications and then try to update them by different username
        Mono<Tuple2<Notification, Notification>> tuple2Mono =
                Mono.zip(saveMono1, saveMono2)
                        .flatMap(
                                objects -> {
        ...
        Mono<List<Notification>> listMono =
                tuple2Mono.flatMap(
                        objects -> {
                            Notification n1 = objects.getT1();
                            Notification n2 = objects.getT2();
    ...
    public void updateIsReadByForUsername_WhenForUsernameMatched_UpdatesMatchedOnes() {
        ...
        Mono<Notification> saveMono2 =
                notificationRepository.save(createNotification("abc", false));
        Mono<Notification> saveMono3 =
                notificationRepository.save(createNotification("efg", false));

        // create the notifications and then try to update them by same username
        Mono<Tuple3<Notification, Notification, Notification>> tuple2Mono =
                Mono.zip(saveMono1, saveMono2, saveMono3)
                        .flatMap(
                                objects ->
        ...
        // now get the notifications we created
        Mono<Map<String, Collection<Notification>>> mapMono =
                tuple2Mono.flatMap(
                        objects -> {
                            Notification n1 = objects.getT1();
                            Notification n2 = objects.getT2();
        ...
        StepVerifier.create(mapMono)
                .assertNext(
                        notificationCollectionMap -> {
                            assertEquals(
                                    2,
                                    notificationCollectionMap
                                            .size()); // should contain map of two keys
