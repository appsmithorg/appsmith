...
public class WorkspaceServiceTest {
        ...
        .assertNext(
            tuple -> {
              ...
              adminPermissionGroup.getPolicies().stream()
                  .filter(
                      policy -> policy.getPermission().equals(ASSIGN_PERMISSION_GROUPS.getValue()))
                  .findFirst()
                  .ifPresent(
                      policy ->
              ...
              adminPermissionGroup.getPolicies().stream()
                  .filter(
                      policy ->
                          policy.getPermission().equals(READ_PERMISSION_GROUP_MEMBERS.getValue()))
                  .findFirst()
                  .ifPresent(
                      policy ->
                          assertThat(policy.getPermissionGroups())
              ...
              adminPermissionGroup.getPolicies().stream()
                  .filter(
                      policy ->
                          policy.getPermission().equals(UNASSIGN_PERMISSION_GROUPS.getValue()))
                  .findFirst()
                  .ifPresent(
                      policy ->
                          assertThat(policy.getPermissionGroups())
  ...
  /**
   * This test tests for updating the workspace with an empty name. The workspace name should not be
   * empty.
   */
  @Test
  @WithUserDetails(value = "api_user")
  public void inValidupdateWorkspaceEmptyName() {
    ...
    Policy manageWorkspacePolicy =
        Policy.builder().permission(MANAGE_WORKSPACES.getValue()).users(Set.of("api_user")).build();
  ...
  public void getAllUserRolesForWorkspaceDomainAsAdministrator() {
    Mono<List<PermissionGroupInfoDTO>> userRolesForWorkspace =
        workspaceService
            ...
            .flatMap(
                createdWorkspace ->
                    workspaceService.getPermissionGroupsForWorkspace(createdWorkspace.getId()));
   ...
   * workspace as an Admin. The existing applications in the workspace should now have the new user
   * be included in both manage:applications and read:applications policies.
   */
  @Test
  @WithUserDetails(value = "api_user")
  public void addUserToWorkspaceAsAdminAndCheckApplicationAndDatasourcePermissions() {
    ...
    Mono<Workspace> userAddedToWorkspaceMono =
        Mono.zip(workspaceMono, adminPermissionGroupMono)
            .flatMap(
                tuple -> {
                  Workspace workspace1 = tuple.getT1();
    ...
    Mono<Application> readApplicationByNameMono =
        applicationService
            ...
            .switchIfEmpty(
                Mono.error(
                    new AppsmithException(AppsmithError.NO_RESOURCE_FOUND, "application by name")));
    ...
    Mono<Workspace> readWorkspaceByNameMono =
        workspaceRepository
            ...
            .switchIfEmpty(
                Mono.error(
                    new AppsmithException(AppsmithError.NO_RESOURCE_FOUND, "workspace by name")));
        ...
        workspaceMono.flatMap(
            workspace1 ->
                datasourceRepository
                    ...
                    .switchIfEmpty(
                        Mono.error(
                            new AppsmithException(AppsmithError.NO_RESOURCE_FOUND, "Datasource"))));
    ...
    Mono<Tuple6<Application, Workspace, Datasource, List<PermissionGroup>, User, User>> testMono =
        workspaceMono
  ...
  public void addNewUsersBulkToWorkspaceAsViewer() {
    ...
    StepVerifier.create(
            Mono.zip(
                userAddedToWorkspaceMono, workspaceAfterUpdateMono, viewerGroupMonoAfterInvite))
        .assertNext(
            tuple -> {
            ...
            throwable ->
                throwable instanceof AppsmithException
                    && throwable
                        .getMessage()
                        .equals(
                            AppsmithError.USER_ALREADY_EXISTS_IN_WORKSPACE.getMessage(
        ...
        .assertNext(
            appsmithRoles -> {
              ...
              assertThat(appsmithRoles)
                  .containsAll(Set.of(administratorRole, developerRole, viewerRole));
            })
        .verifyComplete();
  }
            ...
            throwable ->
                throwable instanceof AppsmithException
                    && throwable
                        .getMessage()
                        .equals(
                            AppsmithError.NO_RESOURCE_FOUND.getMessage(FieldName.WORKSPACE, "")))
        .verify();
  }
  ...
  public void save_WhenNameIsPresent_SlugGenerated() {
    String uniqueString =
        UUID.randomUUID().toString(); // to make sure name is not conflicted with other tests
    Workspace workspace = new Workspace();
            ...
            tuple -> {
              ...
              for (PermissionGroup permissionGroup : permissionGroups) {
                ...
                if (name.startsWith(ADMINISTRATOR)) {
                  assertThat(name)
                      .isEqualTo(generateDefaultRoleNameForResource(ADMINISTRATOR, newName));
                } else {
                  if (name.startsWith(DEVELOPER)) {
                    assertThat(name)
                        .isEqualTo(generateDefaultRoleNameForResource(DEVELOPER, newName));
                  } else {
                    if (name.startsWith(VIEWER)) {
                      assertThat(name)
                          .isEqualTo(generateDefaultRoleNameForResource(VIEWER, newName));
                    }
                  }
                }
              }
