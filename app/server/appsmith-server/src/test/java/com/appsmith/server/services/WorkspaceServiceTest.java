...
public class WorkspaceServiceTest {
    ...
    @Autowired RoleGraph roleGraph;
    @Autowired MongoTemplate mongoTemplate;
    Workspace workspace;
    @Autowired private AssetRepository assetRepository;
    @Autowired private PermissionGroupRepository permissionGroupRepository;
    @Autowired private UserAndAccessManagementService userAndAccessManagementService;
    ...
    public void createDefaultWorkspace() {
        ...
        Mono<Set<PermissionGroup>> defaultPermissionGroupMono =
                Mono.just(workspace)
                        .flatMap(
                                workspace1 -> {
                                    Set<String> defaultPermissionGroups =
                                            workspace1.getDefaultPermissionGroups();
                                    return permissionGroupRepository
                                            .findAllById(defaultPermissionGroups)
                                            .collect(Collectors.toSet());
                                });

        Mono<Set<PermissionGroup>> userPermissionGroupsSetMono =
                userMono.flatMapMany(
                                user ->
                                        permissionGroupRepository.findByAssignedToUserIdsIn(
                                                user.getId()))
                        .collect(Collectors.toSet());
        ...
        StepVerifier.create(
                        Mono.zip(
                                Mono.just(workspace),
                                userMono,
                                defaultPermissionGroupMono,
                                userPermissionGroupsSetMono))
                .assertNext(
                        tuple -> {
                            ...
                            Set<String> userPermissionGroupIds =
                                    userPermissionGroups.stream()
                                            .map(PermissionGroup::getId)
                                            .collect(Collectors.toSet());

                            assertThat(userPermissionGroupIds)
                                    .contains(adminPermissionGroup.getId());
                        })
                .verifyComplete();
    }
        ...
        PermissionGroup permissionGroup =
                permissionGroupRepository
                        .findByDefaultDomainIdAndDefaultDomainType(
                                defaultWorkspace.getId(), Workspace.class.getSimpleName())
    ...
    public void validCreateWorkspaceTest() {
        ...
        Mono<Workspace> workspaceResponse =
                workspaceService
                        .create(workspace)
        ...
        StepVerifier.create(Mono.zip(workspaceResponse, userMono, defaultPermissionGroupsMono))
                .assertNext(
                        tuple -> {
                            ...
                            Policy workspaceCreateApplicationPolicy =
                                    Policy.builder()
                                            .permission(
                                                    AclPermission.WORKSPACE_CREATE_APPLICATION
                                                            .getValue())
                                            .permissionGroups(
                                                    Set.of(
                                                            adminPermissionGroup.getId(),
                            ...
                            Policy workspaceCreateDataSourcePolicy =
                                    Policy.builder()
                                            .permission(
                                                    AclPermission.WORKSPACE_CREATE_DATASOURCE
                                                            .getValue())
                                            .permissionGroups(
                                                    Set.of(
                                                            adminPermissionGroup.getId(),
                            ...
                            Policy deleteWorkspacePolicy =
                                    Policy.builder()
                                            .permission(AclPermission.DELETE_WORKSPACES.getValue())
                                            .permissionGroups(Set.of(adminPermissionGroup.getId()))
                                            .build();

                            Policy workspaceDeleteApplicationaPolicy =
                                    Policy.builder()
                                            .permission(
                                                    AclPermission.WORKSPACE_DELETE_APPLICATIONS
                                                            .getValue())
                                            .permissionGroups(
                                                    Set.of(
                                                            adminPermissionGroup.getId(),
                            ...
                            Policy workspaceDeleteDatasourcesPolicy =
                                    Policy.builder()
                                            .permission(
                                                    AclPermission.WORKSPACE_DELETE_DATASOURCES
                                                            .getValue())
                                            .permissionGroups(
                                                    Set.of(
                                                            adminPermissionGroup.getId(),
                            ...
                            assertThat(workspace1.getSlug())
                                    .isEqualTo(TextUtils.makeSlug(workspace.getName()));
                            assertThat(workspace1.getEmail()).isEqualTo("api_user");
    ...
    public void validUpdateWorkspace() {
        ...
        Mono<Workspace> updateWorkspace =
                createWorkspace.flatMap(
                        t -> {
        ...
        StepVerifier.create(Mono.zip(updateWorkspace, defaultPermissionGroupsMono))
                .assertNext(
                        tuple -> {
                            ...
                            assertThat(t.getPolicies())
                                    .contains(manageWorkspaceAppPolicy, manageWorkspacePolicy);
                        })
                .verifyComplete();
    }
    ...
    void validUpdateCorrectWorkspace() {
        ...
        final Mono<Tuple2<Workspace, Workspace>> updateWorkspace =
                Mono.zip(createWorkspace, createOtherWorkspace)
                        .flatMap(
                                t -> {
    ...
    /**
     * This test tests for updating the workspace with an empty name. The workspace name should not
     * be empty.
     */
    @Test
    @WithUserDetails(value = "api_user")
    ...
    public void createDuplicateNameWorkspace() {
        ...
        Mono<Workspace> secondWorkspaceCreation =
                firstWorkspaceCreation.then(workspaceService.create(secondWorkspace));

        StepVerifier.create(Mono.zip(firstWorkspaceCreation, secondWorkspaceCreation))
                .assertNext(
                        WorkspacesTuple -> {
                            assertThat(WorkspacesTuple.getT1().getName())
                                    .isEqualTo("Really good org");
                            assertThat(WorkspacesTuple.getT1().getSlug())
                                    .isEqualTo("really-good-org");

                            assertThat(WorkspacesTuple.getT2().getName())
                                    .isEqualTo("Really good org");
                            assertThat(WorkspacesTuple.getT2().getSlug())
                                    .isEqualTo("really-good-org");
                        })
                .verifyComplete();
    }
    ...
    public void getAllUserRolesForWorkspaceDomainAsAdministrator() {
        Mono<List<PermissionGroupInfoDTO>> userRolesForWorkspace =
                workspaceService
                        ...
                        .flatMap(
                                createdWorkspace ->
                                        workspaceService.getPermissionGroupsForWorkspace(
                            ...
                            assertThat(userGroupInfos)
                                    .anyMatch(
                                            userGroupInfo ->
                                                    userGroupInfo
                                                            .getName()
                                                            .startsWith(FieldName.VIEWER));
                            assertThat(userGroupInfos)
                                    .anyMatch(
                                            userGroupInfo ->
                                                    userGroupInfo
                                                            .getName()
                                                            .startsWith(FieldName.DEVELOPER));
                        })
                .verifyComplete();
    }
    ...
    public void getAllMembersForWorkspace() {
        ...
        String developerPermissionGroupId =
                permissionGroups.stream()
        ...
        String viewerPermissionGroupId =
                permissionGroups.stream()
        ...
        Mono<List<MemberInfoDTO>> usersMono =
                userWorkspaceService.getWorkspaceMembers(createdWorkspace.getId());
        ...
        StepVerifier.create(usersMono)
                .assertNext(
                        users -> {
                            ...
                            // Assert that the members are sorted by the permission group and then
                            // email
                            ...
                            assertThat(userAndGroupDTO.getRoles().get(0).getName())
                                    .startsWith(ADMINISTRATOR);
    ...
    /**
     * This test tests for a new user being added to a workspace as an admin. The new user must be
     * created at after invite flow and the new user must be disabled.
     */
    @Test
    @WithUserDetails(value = "api_user")
    public void addNewUserToWorkspaceAsAdmin() {
        ...
        List<PermissionGroup> permissionGroupsAfterInvite =
                permissionGroupRepository
                        .findAllById(workspace.getDefaultPermissionGroups())
                        .collectList()
                        .block();
        ...
        PermissionGroup adminPermissionGroup =
                permissionGroupsAfterInvite.stream()
                        .filter(
                                permissionGroup ->
                                        permissionGroup.getName().startsWith(ADMINISTRATOR))
                        .findFirst()
        ...
        assertThat(adminPermissionGroup.getAssignedToUserIds())
                .containsAll(Set.of(newUser.getId(), api_user.getId()));
    ...
    /**
     * This test tests for a new user being added to an workspace as viewer. The new user must be
     * created at after invite flow and the new user must be disabled.
     */
    @Test
    @WithUserDetails(value = "api_user")
    public void addNewUserToWorkspaceAsViewer() {
        ...
        assertThat(viewerPermissionGroup.getAssignedToUserIds())
                .containsAll(Set.of(newUser.getId()));
        // assert that api_user has admin role
        assertThat(adminPermissionGroup.getAssignedToUserIds())
                .containsAll(Set.of(api_user.getId()));
    ...
    /**
     * This test checks for application and datasource permissions if a user is invited to the
     * workspace as an Admin. The existing applications in the workspace should now have the new
     * user be included in both manage:applications and read:applications policies.
     */
    @Test
    @WithUserDetails(value = "api_user")
    public void addUserToWorkspaceAsAdminAndCheckApplicationAndDatasourcePermissions() {
        ...
        Mono<PermissionGroup> adminPermissionGroupMono =
                permissionGroupFlux
                        .filter(
                                permissionGroup ->
                                        permissionGroup.getName().startsWith(ADMINISTRATOR))
        ...
        Mono<Application> applicationMono =
                applicationPageService.createApplication(application, workspace1.getId());
        ...
        Mono<Workspace> userAddedToWorkspaceMono =
                adminPermissionGroupMono
                        .flatMap(
                                adminPermissionGroup -> {
                                    ...
                                    return userAndAccessManagementService.inviteUsers(
                                            inviteUsersDTO, origin);
                                })
                        .flatMap(
                                tuple ->
                                        workspaceService.findById(
                                                workspace1.getId(), READ_WORKSPACES));

        Mono<Application> readApplicationByNameMono =
                applicationService
        ...
        Mono<Workspace> readWorkspaceByNameMono =
                workspaceRepository
        ...
        Mono<Datasource> readDatasourceByNameMono =
                datasourceRepository
                        .findByNameAndWorkspaceId(
                                "test datasource", workspace1.getId(), READ_DATASOURCES)
                        .switchIfEmpty(
                                Mono.error(
                                        new AppsmithException(
                                                AppsmithError.NO_RESOURCE_FOUND, "Datasource")));
        ...
        Mono<Tuple6<Application, Workspace, Datasource, List<PermissionGroup>, User, User>>
                testMono =
                        Mono.zip(applicationMono, datasourceMono)
                                // Now add the user
                                .then(userAddedToWorkspaceMono)
                                // Read application, workspace and datasource now to confirm the
                                // policies.
                                .then(
                                        Mono.zip(
        ...
        StepVerifier.create(testMono)
                .assertNext(
                        tuple -> {
                            ...
                            // Now assert that the application and datasource have correct
    ...
    /**
     * This test checks for application permissions if a user is invited to the workspace as a
     * Viewer. The existing applications in the workspace should now have the new user be included
     * in both manage:applications and read:applications policies.
     */
    @Test
    @WithUserDetails(value = "api_user")
    public void addUserToWorkspaceAsViewerAndCheckApplicationPermissions() {
        ...
        Flux<PermissionGroup> permissionGroupFlux =
                workspaceMono.flatMapMany(
                        workspace1 ->
                                permissionGroupRepository.findAllById(
                                        workspace1.getDefaultPermissionGroups()));

        Mono<PermissionGroup> viewerPermissionGroupMono =
                permissionGroupFlux
                        .filter(permissionGroup -> permissionGroup.getName().startsWith(VIEWER))
                        .single();
                ...
                workspaceMono.flatMap(
                        workspace1 -> {
                            ...
                            return applicationPageService.createApplication(
                                    application, workspace1.getId());
                        });

        Mono<Workspace> userAddedToWorkspaceMono =
                Mono.zip(workspaceMono, viewerPermissionGroupMono)
                        .flatMap(
                                tuple -> {
                                    Workspace workspace1 = tuple.getT1();
        ...
        Mono<Tuple5<Application, Workspace, List<PermissionGroup>, User, User>> testMono =
                workspaceMono
        ...
        StepVerifier.create(testMono)
                .assertNext(
                        tuple -> {
                            ...
                            assertThat(adminPermissionGroup.getAssignedToUserIds())
                                    .contains(api_user.getId());
                            assertThat(viewerPermissionGroup.getAssignedToUserIds())
                                    .contains(userTest.getId());
    ...
    public void addNewUsersBulkToWorkspaceAsViewer() {
        ...
        Mono<PermissionGroup> viewerGroupMono =
                workspaceMono
                        .flatMapMany(
                                workspace1 ->
                                        permissionGroupRepository.findAllById(
        ...
        Mono<List<User>> userAddedToWorkspaceMono =
                viewerGroupMono
                        .flatMap(
                                permissionGroup -> {
                                    // Add user to workspace
        ...
        Mono<Workspace> readWorkspaceMono =
                workspaceRepository.findByName("Add Bulk Viewers to Test Workspace");
        ...
        StepVerifier.create(
                        Mono.zip(
                                userAddedToWorkspaceMono,
                                workspaceAfterUpdateMono,
                                viewerGroupMonoAfterInvite))
                .assertNext(
                        tuple -> {
                            ...
                            assertThat(workspace1).isNotNull();
                            assertThat(workspace1.getName())
                                    .isEqualTo("Add Bulk Viewers to Test Workspace");
    ...
    public void addUserToWorkspaceIfUserAlreadyMember_throwsError() {
        ...
        Mono<PermissionGroup> developerPermissionGroupMono =
                permissionGroupFlux
                        .filter(permissionGroup -> permissionGroup.getName().startsWith(DEVELOPER))
                        .single();
        ...
        Mono<List<User>> userAddedToWorkspaceTwiceMono =
                adminPermissionGroupMono
                        .flatMap(
                                adminPermissionGroup -> {
                                                ...
                                                .equals(
                                                        AppsmithError
                                                                ...
                                                                .getMessage(
                                                                        ...
                                                                        "Administrator -"
                                                                            + " addUserToWorkspaceIfUserAlreadyMember_throwsError")))
                .verify();
    }
        ...
        StepVerifier.create(Mono.just(roles))
                .assertNext(
                        appsmithRoles -> {
                            ...
                            assertThat(appsmithRoles)
                                    .containsAll(
                                            Set.of(administratorRole, developerRole, viewerRole));
                        })
                .verifyComplete();
    }
        ...
        StepVerifier.create(Mono.just(roles))
                .assertNext(
                        appsmithRoles -> {
                            assertThat(appsmithRoles).isNotNull();
                            assertThat(appsmithRoles)
                                    .containsAll(Set.of(developerRole, viewerRole));
                        })
                .verifyComplete();
    }
    ...
    public void uploadWorkspaceLogo_nullFilePart() throws IOException {
        ...
        final Mono<Workspace> resultMono =
                createWorkspace.flatMap(
                        workspace -> workspaceService.uploadLogo(workspace.getId(), null));
                        ...
                        throwable ->
                                throwable instanceof AppsmithException
                                        && throwable
                                                .getMessage()
                                                .equals(
                                                        AppsmithError.PAYLOAD_TOO_LARGE.getMessage(
    ...
    public void testUpdateAndDeleteLogo_validLogo() throws IOException {
        ...
        final Mono<Tuple2<Workspace, Asset>> resultMono =
                createWorkspace.flatMap(
                        workspace ->
                                workspaceService
        ...
        StepVerifier.create(resultMono)
                .assertNext(
                        tuple -> {
                            ...
                            assertThat(workspaceWithLogo.getLogoUrl())
                                    .contains(workspaceWithLogo.getLogoAssetId());
        ...
        Mono<Workspace> deleteLogo =
                createWorkspace.flatMap(
                        workspace -> workspaceService.deleteLogo(workspace.getId()));
        StepVerifier.create(deleteLogo)
                .assertNext(
                        x -> {
    ...
    public void delete_WhenWorkspaceHasApp_ThrowsException() {
        ...
        Mono<Workspace> deleteWorkspaceMono =
                workspaceService
                        .create(workspace)
                        .flatMap(
                                savedWorkspace -> {
    ...
    public void delete_WithoutManagePermission_ThrowsException() {
        ...
        StepVerifier.create(deleteWorkspaceMono).expectError(AppsmithException.class).verify();
    }
    ...
    public void save_WhenNameIsPresent_SlugGenerated() {
        String uniqueString =
                UUID.randomUUID()
                        .toString(); // to make sure name is not conflicted with other tests
        Workspace workspace = new Workspace();
        ...
        StepVerifier.create(workspaceMono)
                .assertNext(
                        savedWorkspace -> {
                            assertThat(savedWorkspace.getSlug())
                                    .isEqualTo(TextUtils.makeSlug(finalName));
                        })
                .verifyComplete();
    }
                        ...
                        .flatMap(
                                savedWorkspace -> {
                                    ...
                                    return workspaceService.update(
                                            savedWorkspace.getId(), workspaceDto);
                                });
        ...
        StepVerifier.create(workspaceMono)
                .assertNext(
                        savedWorkspace -> {
                            ...
                            assertThat(savedWorkspace.getSlug())
                                    .isEqualTo(TextUtils.makeSlug(initialName));
                        })
                .verifyComplete();
    }
    ...
    public void update_NewName_GroupNamesUpdated() {
        ...
        Mono<Workspace> workspaceNameUpdateMono =
                workspaceService
                        .create(workspace)
                        .flatMap(
                                savedWorkspace -> {
        ...
        Mono<List<PermissionGroup>> permissionGroupsMono =
                workspaceNameUpdateMono.flatMap(
                        savedWorkspace ->
                                permissionGroupRepository
                                        .findAllById(savedWorkspace.getDefaultPermissionGroups())
                                        .collectList());
        ...
        StepVerifier.create(Mono.zip(workspaceNameUpdateMono, permissionGroupsMono))
                .assertNext(
                        tuple -> {
                            ...
                            List<PermissionGroup> permissionGroups = tuple.getT2();
                            assertThat(savedWorkspace.getSlug())
                                    .isEqualTo(TextUtils.makeSlug(newName));
                            ...
                            for (PermissionGroup permissionGroup : permissionGroups) {
                                ...
                                if (name.startsWith(ADMINISTRATOR)) {
                                    assertThat(name)
                                            .isEqualTo(
                                                    generateDefaultRoleNameForResource(
    ...
    void testWorkspaceUpdate_checkAdditionalFieldsArePresentAfterUpdate() {
        ...
        Query queryWorkspace =
                new Query(
                        Criteria.where(fieldName(QWorkspace.workspace.id))
                                .is(createdWorkspace.getId()));
        UpdateResult updateResult =
                mongoTemplate.updateMulti(
                        queryWorkspace, updateAddAdditionalField, Workspace.class);
        ...
        Criteria criteriaAdditionalField =
                new Criteria()
                        .andOperator(
                                Criteria.where(fieldName(QWorkspace.workspace.id))
                                        .is(createdWorkspace.getId()),
        ...
        Workspace updatedWorkspace =
                workspaceService.update(createdWorkspace.getId(), updateWorkspace).block();
