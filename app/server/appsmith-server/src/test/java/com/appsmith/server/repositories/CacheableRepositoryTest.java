...
public class CacheableRepositoryTest {
    ...
    @Autowired CacheableRepositoryHelper cacheableRepositoryHelper;
    ...
    public void getUserPermissionsTest_onPermissionGroupDelete_valid() {
        ...
        PermissionGroup adminPg =
                defaultPermissionGroups.stream()
                        .filter(pg -> pg.getName().startsWith(ADMINISTRATOR))
                        .findFirst()
                        .get();

        Mono<Set<String>> permissionGroupsOfUserMono =
                cacheableRepositoryHelper.getPermissionGroupsOfUser(api_user);

        // Assert that the user has the ADMINISTRATOR permission group
        StepVerifier.create(permissionGroupsOfUserMono)
                .assertNext(
                        permissionGroupsOfUser -> {
                            assertThat(permissionGroupsOfUser).contains(adminPg.getId());
                        })
                .verifyComplete();

        // Now delete the workspace and assert that user permission groups does not contain the
        // admin pg
