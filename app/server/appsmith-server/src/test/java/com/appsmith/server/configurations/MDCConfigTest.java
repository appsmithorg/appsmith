                ...
                .flatMap(
                        (firstValue) -> {
                            ...
                            return Mono.fromSupplier(() -> "secondString")
                                    .doOnEach(
                                            (secondValue) -> {
                                                final String secondThread =
                                                        MDC.get(MDCFilter.THREAD);
                                                log.debug(
                                                        "Second thread -> {} ; Context thread -> {}"
                                                                + " ; on signal {}",
                                                        Thread.currentThread().getName(),
                                                        secondThread,
                                                        secondValue.getType());
                                                ...
                                                Assertions.assertEquals(
                                                        ...
                                                        Thread.currentThread().getName());
                                                Assertions.assertEquals(
                                                        MDC.get("constantKey"), "constantValue");
                                            })
                            ...
                            return Mono.error(
                                    ...
                                    .doOnEach(
                                            (errorValue) -> {
                                                ...
                                                Assertions.assertEquals(
                                                        errorValue.getType(), SignalType.ON_ERROR);

                                                final Map<String, String> contextMap =
                                                        ((BaseException) errorValue.get())
                                                                .getContextMap();
                                                Assertions.assertEquals(
                                                        contextMap.get("constantKey"),
                                                        "constantValue");
                                                Assertions.assertEquals(
                                                        contextMap.get(MDCFilter.THREAD),
                                                        secondThread);
                                            })
                                    .subscribeOn(Schedulers.boundedElastic());
                        })
