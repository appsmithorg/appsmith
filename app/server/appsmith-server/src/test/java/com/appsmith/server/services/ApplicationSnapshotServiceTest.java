...
public class ApplicationSnapshotServiceTest {
    ...
    @Autowired private ApplicationSnapshotService applicationSnapshotService;
    ...
    public void createApplicationSnapshot_WhenNoPreviousSnapshotExists_NewCreated() {
        ...
        Mono<ApplicationSnapshot> snapshotMono =
                workspaceService
                        ...
                        .flatMap(
                                applicationId ->
                                        applicationSnapshotService.getWithoutDataByApplicationId(
                                                applicationId, null));
    ...
    public void createApplicationSnapshot_WhenGitBranchExists_SnapshotCreatedWithBranchedAppId() {
        ...
        Mono<Tuple2<ApplicationSnapshot, Application>> tuple2Mono =
                workspaceService
                        .create(workspace)
                        .flatMap(
                                createdWorkspace -> {
                                    ...
                                    // this app will have default app id=testDefaultAppId and branch
                                    ...
                                    testApplication.setGitApplicationMetadata(
    ...
    public void createApplicationSnapshot_OlderSnapshotExists_OlderSnapshotsRemoved() {
        ...
        Flux<ApplicationSnapshot> applicationSnapshotFlux =
                workspaceService
                        .create(workspace)
                        .flatMap(
                                createdWorkspace -> {
    ...
    @WithUserDetails("api_user")
    public void
            restoreSnapshot_WhenNewPagesAddedAfterSnapshotTaken_NewPagesRemovedAfterSnapshotIsRestored() {
        String uniqueString = UUID.randomUUID().toString();
        String testDefaultAppId = "default-app-" + uniqueString;
        String testBranchName = "hello/world";
                ...
                workspaceService
                        ...
                        .flatMap(
                                createdWorkspace -> {
                                    ...
                                    return applicationPageService.createApplication(
                                            testApplication, workspace.getId());
                                })
                        .cache();

        Mono<ApplicationPagesDTO> pagesBeforeSnapshot =
        ...
        Mono<ApplicationPagesDTO> pagesAfterSnapshot =
                applicationMono
                        .flatMap(
                        ...
                        .flatMap(
                                application -> { // add a new page to the application
                                    ...
                                    return applicationPageService
                                            .createPage(pageDTO)
                                            .then(
                                                    applicationSnapshotService.restoreSnapshot(
                                                            application.getId(), null))
                                            .then(
                                                    newPageService.findApplicationPages(
                                                            application.getId(),
        ...
        Mono<Tuple2<ApplicationPagesDTO, ApplicationPagesDTO>> tuple2Mono =
                pagesBeforeSnapshot.flatMap(
                        applicationPagesDTO ->
                                pagesAfterSnapshot.zipWith(Mono.just(applicationPagesDTO)));
    ...
    public void restoreSnapshot_WhenSuccessfullyRestored_SnapshotDeleted() {
        ...
        Flux<ApplicationSnapshot> snapshotFlux =
                workspaceService
                        .create(workspace)
                        .flatMap(
                                createdWorkspace -> {
