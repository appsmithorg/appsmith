...
public void testGetAuthorizationCodeURL_missingDatasource() {
	Mono<String> authorizationCodeUrlMono =
		authenticationService.getAuthorizationCodeURLForGenericOauth2(
			"invalidId", "irrelevantPageId", null);
			...
			throwable ->
				throwable instanceof AppsmithException
					&& ((AppsmithException) throwable)
						.getError()
						.equals(AppsmithError.NO_RESOURCE_FOUND)
					&& throwable
						.getMessage()
						.equalsIgnoreCase("Unable to find datasource invalidId"))
		.verify();
}
	...
	Mono<String> authorizationCodeUrlMono =
		datasourceMono
			...
			.flatMap(
				datasourceId ->
					authenticationService.getAuthorizationCodeURLForGenericOauth2(
						datasourceId, "irrelevantPageId", null));
		...
		.expectErrorMatches(
			throwable ->
				throwable instanceof AppsmithException
					&& ((AppsmithException) throwable)
						.getError()
						.equals(AppsmithError.INVALID_PARAMETER)
					&& throwable
						.getMessage()
						.equalsIgnoreCase("Please enter a valid parameter authentication."))
		.verify();
}
...
public void testGetAuthorizationCodeURL_validDatasource() {
	...
	pluginMono
		.flatMap(
			plugin -> {
			return pluginService.installPlugin(
				PluginWorkspaceDTO.builder()
	...
	Mono<String> authorizationCodeUrlMono =
		authenticationService.getAuthorizationCodeURLForGenericOauth2(
			datasourceId1, pageDto.getId(), httpRequest);
				...
				.matches(
					Pattern.compile(
						"AuthorizationURL"
							...
							+ "&state="
							+ String.join(
								",", pageDto.getId(), datasourceId1, "https://mock.origin.com")
