...
public class GitFileUtilsTest {
    ...
    @MockBean FileInterface fileInterface;
    @Autowired GitFileUtils gitFileUtils;
    @Autowired AnalyticsService analyticsService;
    @Autowired SessionUserService userService;
    @Autowired Gson gson;
    ...
    @Test
    public void
            getSerializableResource_allEntitiesArePresentForApplication_keysIncludesSeparator() {
        ApplicationJson validAppJson = createAppJson(filePath).block();
        ApplicationGitReference applicationGitReference =
                gitFileUtils.createApplicationReference(validAppJson);

        List<String> pageNames =
                validAppJson.getPageList().stream()
                        .map(newPage -> newPage.getUnpublishedPage().getName())
                        .collect(Collectors.toList());

        List<String> actionNames =
                validAppJson.getActionList().stream()
                        .map(
                                newAction ->
                                        newAction
        ...
        List<String> collectionNames =
                validAppJson.getActionCollectionList().stream()
                        .map(
                                actionCollection ->
    ...
    public void getSerializableResource_withDeletedEntities_excludeDeletedEntities() {
        ...
        ActionCollection deletedCollection =
                validAppJson
        ...
        for (Map.Entry<String, Object> entry : actionsCollections.entrySet()) {
            ...
            assertThat(deletedCollection.getUnpublishedCollection().getName())
                    .isNotEqualTo(collectionName);
        }
    ...
    public void saveApplicationToLocalRepo_allResourcesArePresent_removePublishedResources() {
        ...
        Mockito.when(
                        fileInterface.saveApplicationToGitRepo(
                ...
                .assertNext(
                        path -> {
                            validAppJson
                                    ...
                                    .forEach(
                                            newPage -> {
                                                assertThat(newPage.getUnpublishedPage())
                                                        .isNotNull();
                            ...
                            validAppJson
                                    ...
                                    .forEach(
                                            newAction -> {
                                                assertThat(newAction.getUnpublishedAction())
                                                        .isNotNull();
    ...
    @WithUserDetails(value = "api_user")
    public void
            reconstructApplicationFromLocalRepo_allResourcesArePresent_getClonedPublishedResources() {
        ...
        Mockito.when(
                        fileInterface.reconstructApplicationReferenceFromGitRepo(
                                Mockito.anyString(),
                                Mockito.anyString(),
                                Mockito.anyString(),
                                Mockito.anyString()))
                            ...
                            applicationJson1
                                    ...
                                    .forEach(
                                            newPage -> {
                                                assertThat(newPage.getUnpublishedPage())
                                                        .isNotNull();
                                                // Both DTOs will not be equal as we are creating a
                                                // deep copy for the published version from
                                                // unpublished version
                                                assertThat(newPage.getPublishedPage())
                                                        .isNotEqualTo(newPage.getUnpublishedPage());

                                                // Check if the published versions are deep copy of
                                                // the unpublished version and updating any
                                                // will not affect the other
                                                final String unpublishedName =
                                                        newPage.getUnpublishedPage().getName();
                                                newPage.getUnpublishedPage().setName("updatedName");

                                                assertThat(newPage.getPublishedPage().getName())
                            ...
                            applicationJson1
                                    ...
                                    .forEach(
                                            newAction -> {
                                                assertThat(newAction.getUnpublishedAction())
                                                        .isNotNull();
                                                assertThat(newAction.getPublishedAction())
                                                        .isNotEqualTo(
                                                ...
                                                newAction
                                                        .getUnpublishedAction()
                                                        .setName("updatedName");
                                                assertThat(newAction.getPublishedAction().getName())
                                                        .isEqualTo(unpublishedName);
                                                assertThat(
                                                                newAction
                                                                        .getUnpublishedAction()
