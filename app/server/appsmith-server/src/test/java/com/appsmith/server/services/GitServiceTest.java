					...
					&& throwable
						...
						.equals(
							AppsmithError.GIT_ACTION_FAILED.getMessage(
								"checkout",
								"origin/branchInLocal already exists in local - branchInLocal")))
		.verify();
}
	...
	Mono<Application> gitConnectedAppFromDbMono =
		Mono.just(application1)
			.flatMap(
				application -> {
				try {
					// Before fetching the git connected application, sleep for 5 seconds to ensure
					// that the clone
					// completes
					Thread.sleep(5000);
...
// We are only testing git level operations from this testcase. For testcases related to scenarios
// like
// 1. Resource is added
