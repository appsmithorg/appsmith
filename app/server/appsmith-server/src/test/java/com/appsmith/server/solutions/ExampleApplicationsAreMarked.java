...
public class ExampleApplicationsAreMarked {
    ...
    @SpyBean private ConfigService configService;

    @SpyBean private InstanceConfig instanceConfig;
    ...
    public void exampleApplicationsAreMarked() {
        ...
        final Mono<List<Application>> resultMono =
                Mono.zip(workspaceService.create(newWorkspace), sessionUserService.getCurrentUser())
                        .flatMap(
                                tuple -> {
                                    ...
                                    Mockito.when(configService.getTemplateWorkspaceId())
                                            .thenReturn(Mono.just(workspace.getId()));

                                    // Create 4 applications inside the example workspace but only
                                    // mark three applications as example
                                    ...
                                    Mockito.when(configService.getTemplateApplications())
                                            .thenReturn(
                                                    Flux.fromIterable(List.of(app1, app2, app3)));
                        ...
                        .flatMapMany(
                                workspaceId ->
                                        applicationService.findByWorkspaceId(
                                                workspaceId, READ_APPLICATIONS))
                        .collectList();
