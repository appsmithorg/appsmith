...
public class PluginServiceTest {
    ...
    public void checkPluginExecutor() {
        Mono<ActionExecutionResult> executeMono =
                pluginExecutor.execute(
                        new Object(), new DatasourceConfiguration(), new ActionConfiguration());
    ...
    // The datasource form config is mandatory for plugins. Hence we expect an error when that file
    // is not present
    @Test
    public void getPluginFormWithNullFormConfig() {
        ...
        Mockito.when(pluginService.loadPluginResource(Mockito.anyString(), eq("form.json")))
                .thenReturn(
                        Mono.error(
                                new AppsmithException(AppsmithError.PLUGIN_LOAD_FORM_JSON_FAIL)));
    ...
    // The editor form config is not mandatory for plugins. The function should return successfully
