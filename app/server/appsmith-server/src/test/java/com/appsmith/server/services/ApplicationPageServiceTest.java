...
public class ApplicationPageServiceTest {
     ...
     *
     * @param uniquePrefix unique string that'll be added as prefix to org and app names to avoid
     *     name collision
    ...
    public void deleteUnpublishedPage_WhenPageDeleted_ApplicationEditDateSet() {
        Mono<Application> applicationMono =
                createPageMono(UUID.randomUUID().toString())
                        .flatMap(
                                pageDTO -> {
                                    Application application = new Application();
                                    application.setLastEditedAt(
                                    ...
                                    return applicationRepository
                                            ...
                                            .then(
                                                    applicationPageService.deleteUnpublishedPage(
                                                            pageDTO.getId()))
                                            .then(
    ...
    public void cloneApplication_WhenClonedSuccessfully_ApplicationIsPublished() {
        Mono<Application> applicationMono =
                createPageMono(UUID.randomUUID().toString())
                        .flatMap(
                                pageDTO ->
                                        applicationPageService.cloneApplication(
                                                pageDTO.getApplicationId(), null));
