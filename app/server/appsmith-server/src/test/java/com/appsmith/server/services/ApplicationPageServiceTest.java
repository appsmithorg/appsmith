...
*
* @param uniquePrefix unique string that'll be added as prefix to org and app names to avoid name
*     collision
...
public void cloneApplication_WhenClonedSuccessfully_ApplicationIsPublished() {
	Mono<Application> applicationMono =
		createPageMono(UUID.randomUUID().toString())
			.flatMap(
				pageDTO ->
					applicationPageService.cloneApplication(pageDTO.getApplicationId(), null));
