...
public void updateShouldNotResetUserSetOnLoad() {
	...
	Mono<ActionDTO> setExecuteOnLoadMono =
		newActionMono.flatMap(
			savedAction -> layoutActionService.setExecuteOnLoad(savedAction.getId(), true));
			...
			assertThat(
					updatedAction.getInvalids().stream()
						.anyMatch(
							errorMsg ->
								errorMsg.contains(
									"'Query timeout' field must be an integer between"
										+ " 0 and 60000")))
				.isTrue();
			})
		.verifyComplete();
...
public void testCreateActionWithOutOfRangeTimeout() {
	...
	Mono<ActionDTO> actionMono =
		layoutActionService
			.createSingleAction(action, Boolean.FALSE)
			.flatMap(
				createdAction -> newActionService.findById(createdAction.getId(), READ_ACTIONS))
...
public void getActionsExecuteOnLoadPaginatedApi() {
		...
		pageMono
			.flatMap(
				...
				action.setDynamicBindingPathList(
					List.of(new Property("next", null), new Property("prev", null)));
			...
			.flatMap(
				...
				dynamicBindingsPathList.addAll(
					List.of(new JSONObject(Map.of("key", "bindingPath"))));
	...
	Mockito.when(
		...
		.thenReturn(
			Flux.just(Tuples.of("paginatedApi.data", new HashSet<>(Set.of("paginatedApi.data")))));
	Mockito.when(
			astService.getPossibleReferencesFromDynamicBinding(
		...
		.thenReturn(
			Flux.just(
				Tuples.of(
					"paginatedApi.data.prev", new HashSet<>(Set.of("paginatedApi.data.prev")))));
	Mockito.when(
			astService.getPossibleReferencesFromDynamicBinding(
		...
		.thenReturn(
			Flux.just(
				Tuples.of(
					"paginatedApi.data.next", new HashSet<>(Set.of("paginatedApi.data.next")))));
