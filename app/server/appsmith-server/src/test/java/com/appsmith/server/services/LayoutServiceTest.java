...
public class LayoutServiceTest {
                ...
                pageMono.flatMap(
                                page1 -> {
                                    ...
                                    action.setDynamicBindingPathList(
                                            List.of(new Property("body", null)));
                                    monos.add(
                                            layoutActionService.createSingleAction(
                                                    action, Boolean.FALSE));
                        ...
                        .flatMap(
                                tuple2 -> {
                                    ...
                                    obj.put(
                                            "dynamicDB2",
                                            List.of("{{ anotherDBAction.data.optional }}"));
                                    obj.put(
                                            "tableWidget",
                                            new JSONObject(
                                                    Map.of(
    ...
    public void getActionsExecuteOnLoadWithAstLogic_withAllTypesOfActionReferences() {
        ...
        Mockito.when(
                ...
                .thenReturn(
                        Flux.just(
                                Tuples.of(
                                        "anotherPostAction.run()",
                                        new HashSet<>(Set.of("anotherPostAction.run")))));
        Mockito.when(
                        astService.getPossibleReferencesFromDynamicBinding(
                                List.of("Collection.anAsyncCollectionActionWithoutCall.data"),
                                EVALUATION_VERSION))
                .thenReturn(
                        Flux.just(
                                Tuples.of(
        ...
        Mockito.when(
                        astService.getPossibleReferencesFromDynamicBinding(
                                List.of("Collection.aSyncCollectionActionWithoutCall.data"),
                                EVALUATION_VERSION))
                .thenReturn(
                        Flux.just(
                                Tuples.of(
        ...
        Mockito.when(
                        astService.getPossibleReferencesFromDynamicBinding(
                                List.of("Collection.anAsyncCollectionActionWithCall()"),
                                EVALUATION_VERSION))
                .thenReturn(
                        Flux.just(
                                Tuples.of(
        ...
        Mockito.when(
                        astService.getPossibleReferencesFromDynamicBinding(
                                List.of("Collection.aSyncCollectionActionWithCall()"),
                                EVALUATION_VERSION))
                .thenReturn(
                        Flux.just(
                                Tuples.of(
        ...
        Mockito.when(
                ...
                .thenReturn(
                        Flux.just(
                                Tuples.of(
                                        "Collection.data()",
                                        new HashSet<>(Set.of("Collection.data")))));
        Mockito.when(
                        astService.getPossibleReferencesFromDynamicBinding(
                ...
                .thenReturn(
                        Flux.just(
                                Tuples.of(
                                        "Collection2.data()",
                                        new HashSet<>(Set.of("Collection2.data")))));
        Mockito.when(
                        astService.getPossibleReferencesFromDynamicBinding(
                ...
                .thenReturn(
                        Flux.just(
                                Tuples.of(
                                        "hiddenAction1.data",
                                        new HashSet<>(Set.of("hiddenAction1.data")))));
        Mockito.when(
                        astService.getPossibleReferencesFromDynamicBinding(
                ...
                .thenReturn(
                        Flux.just(
                                Tuples.of(
                                        "hiddenAction2.data",
                                        new HashSet<>(Set.of("hiddenAction2.data")))));
        Mockito.when(
                        astService.getPossibleReferencesFromDynamicBinding(
                ...
                .thenReturn(
                        Flux.just(
                                Tuples.of(
                                        "hiddenAction4.data",
                                        new HashSet<>(Set.of("hiddenAction4.data")))));
        Mockito.when(
                        astService.getPossibleReferencesFromDynamicBinding(
                ...
                .thenReturn(
                        Flux.just(
                                Tuples.of(
                                        "hiddenAction5.data",
                                        new HashSet<>(Set.of("hiddenAction5.data")))));
     ...
     * load. This test asserts the following : 1. All the actions which must be executed on page
     * load have been recognized correctly 2. The sequence of the action execution takes into
     * account the action dependencies 3. An action which has been marked to not execute on page
     * load does not get added to the on page load order 4. Async and sync JS functions when called
     * with a .data reference are marked to run on load 5. Async and sync JS function when called as
     * function calls are ignored to be marked on page load 6. A string that happens to match an
     * action reference IS NOT marked to run on page load
     */
    @Test
    @WithUserDetails(value = "api_user")
    public void getActionsExecuteOnLoadWithAstLogic() {
        ...
        Mockito.when(
                        astService.getPossibleReferencesFromDynamicBinding(
                                List.of("\"anIgnoredAction.data:\" + aGetAction.data"),
                                EVALUATION_VERSION))
                .thenReturn(
                        Flux.just(
                                Tuples.of(
        ...
        Mockito.when(
                ...
                .thenReturn(
                        Flux.just(
                                Tuples.of(
                                        "aTableAction.data.child",
                                        new HashSet<>(Set.of("aTableAction.data.child")))));
        Mockito.when(
                        astService.getPossibleReferencesFromDynamicBinding(
                                List.of("Collection.anAsyncCollectionActionWithoutCall.data"),
                                EVALUATION_VERSION))
        ...
        StepVerifier.create(testMono)
                .assertNext(
                        layout -> {
                            ...
                            Set<String> secondSetPageLoadActions =
                                    Set.of("aTableAction", "anotherDBAction");
    ...
    /**
     * This test is meant for the older string based on page load logic that persists today for
     * older deployments. The expectation is the same as the previous test, except that only valid
     * global references are marked to run on page load. (Please refer point #6 in this list) This
     * test adds some actions in the page and attaches a few of those in the dynamic bindings in the
     * widgets in the layout. An action attached in the widget also has two dependencies on other
     * actions. One of those has been explicitly marked to NOT run on page load. This test asserts
     * the following : 1. All the actions which must be executed on page load have been recognized
     * correctly 2. The sequence of the action execution takes into account the action dependencies
     * 3. An action which has been marked to not execute on page load does not get added to the on
     * page load order 4. Async and sync JS functions when called with a .data reference are marked
     * to run on load 5. Async and sync JS function when called as function calls are ignored to be
     * marked on page load 6. A string that happens to match an action reference DOES GET
     * INCORRECTLY marked to run on page load
     */
    @Test
    @WithUserDetails(value = "api_user")
                        ...
                        .flatMap(
                                tuple2 -> {
                                    ...
                                    dynamicBindingsPathList.add(
                                            new JSONObject(
                                                    Map.of("key", "dynamicGet_IncorrectKey")));
                                ...
                                oldParent =
                                        objectMapper.readTree(
                                                "{\"widgetName\":\"testWidget\",\"dynamicBindingPathList\":[{\"key\":\"dynamicGet_IncorrectKey\"}],\"widgetId\":\"id\",\"another\":\"Hello"
                                                    + " people of the {{input1.text}}"
                                                    + " planet!\",\"dynamicGet\":\"some dynamic"
