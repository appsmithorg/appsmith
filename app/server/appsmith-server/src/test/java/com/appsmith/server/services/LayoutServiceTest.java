		...
		.expectErrorMatches(
			throwable ->
				throwable instanceof AppsmithException
					&& throwable
						.getMessage()
						.equals(AppsmithError.INVALID_PARAMETER.getMessage(FieldName.PAGE_ID)))
		.verify();
}
...
public void createValidLayout() {
	...
	Mono<Application> applicationMono =
		applicationPageService.createApplication(application, workspaceId);
	...
	Mono<Layout> layoutMono =
		pageMono.flatMap(page -> layoutService.createLayout(page.getId(), testLayout));
			...
			throwable ->
				throwable instanceof AppsmithException
					&& throwable
						.getMessage()
						.equals(
							AppsmithError.ACL_NO_RESOURCE_FOUND.getMessage(
...
public void updateLayoutValidPageId() {
	...
	Mono<Layout> startLayoutMono =
		pageMono.flatMap(page -> layoutService.createLayout(page.getId(), testLayout));

	Mono<LayoutDTO> updatedLayoutMono =
		Mono.zip(pageMono, startLayoutMono)
			.flatMap(
				tuple -> {
	...
	Mono<LayoutDTO> testMono =
		pageMono
			.flatMap(
				...
				// Action aPostActionWithAutoExec depends on [aPostSecondaryAction,
				// aPostTertiaryAction]
				action = new ActionDTO();
				...
				action
					...
					.setBody(
						"this won't be auto-executed: {{aPostSecondaryAction.data}}, but this one"
							+ " will be: {{aPostTertiaryAction.data}}.");
				action.setPageId(page1.getId());
				action.setDatasource(datasource);
			...
			.flatMap(
				...
				obj.putAll(
					Map.of(
						"collection1Key",
							"some dynamic {{Collection.anAsyncCollectionActionWithoutCall.data}}",
						"collection2Key",
							"some dynamic {{Collection.aSyncCollectionActionWithoutCall.data}}",
						"collection3Key",
							"some dynamic {{Collection.anAsyncCollectionActionWithCall()}}",
						// only add sync function call dependencies in the dependency tree. sync
						// call would be done during eval.
						"collection4Key",
							"some dynamic {{Collection.aSyncCollectionActionWithCall()}}"));
				obj.put(
					"dynamicDB",
					new JSONObject(
						Map.of("test", "child path {{aDBAction.data[0].irrelevant}}")));
				...
				dynamicBindingsPathList.addAll(
					List.of(
...
private Mono<LayoutDTO> createAppWithAllTypesOfReferencesForExecuteOnLoad(
	Mono<PageDTO> pageMono) {
...
public void getActionsExecuteOnLoadWithAstLogic_withAllTypesOfActionReferences() {
	...
	Mockito.when(
			astService.getPossibleReferencesFromDynamicBinding(
				List.of("aGetAction.data"), EVALUATION_VERSION))
		.thenReturn(
			Flux.just(Tuples.of("aGetAction.data", new HashSet<>(Set.of("aGetAction.data")))));
	Mockito.when(
			astService.getPossibleReferencesFromDynamicBinding(
		...
		.thenReturn(
			Flux.just(
				Tuples.of(
					"aPostAction.data.users.name",
					new HashSet<>(Set.of("aPostAction.data.users.name")))));
	Mockito.when(
			astService.getPossibleReferencesFromDynamicBinding(
		...
		.thenReturn(
			Flux.just(
				Tuples.of(
					"anotherPostAction.run()", new HashSet<>(Set.of("anotherPostAction.run")))));
	Mockito.when(
			astService.getPossibleReferencesFromDynamicBinding(
	...
	Mockito.when(
		...
		.thenReturn(
			Flux.just(Tuples.of("Collection.data()", new HashSet<>(Set.of("Collection.data")))));
	Mockito.when(
			astService.getPossibleReferencesFromDynamicBinding(
				List.of("Collection2.data()"), EVALUATION_VERSION))
		.thenReturn(
			Flux.just(Tuples.of("Collection2.data()", new HashSet<>(Set.of("Collection2.data")))));
	Mockito.when(
			astService.getPossibleReferencesFromDynamicBinding(
	...
	Mockito.when(
		...
		.thenReturn(
			Flux.just(
				Tuples.of("hiddenAction5.data", new HashSet<>(Set.of("hiddenAction5.data")))));
	...
	StepVerifier.create(testMono)
		.assertNext(
			...
			Set<String> thirdSetPageLoadActions =
				Set.of("Collection.anAsyncCollectionActionWithoutCall");
			...
			assertThat(
					layout.getLayoutOnLoadActions().get(1).stream()
						.map(DslActionDTO::getName)
						.collect(Collectors.toSet()))
				.hasSameElementsAs(secondSetPageLoadActions);
			assertThat(
					layout.getLayoutOnLoadActions().get(2).stream()
						.map(DslActionDTO::getName)
						.collect(Collectors.toSet()))
	...
	Mono<Tuple2<ActionDTO, ActionDTO>> actionDTOMono =
		pageMono.flatMap(
			page -> {
			return newActionService
				.findByUnpublishedNameAndPageId(
					"aGetAction", page.getId(), AclPermission.MANAGE_ACTIONS)
				.zipWith(
					newActionService.findByUnpublishedNameAndPageId(
...
/**
* This test is meant for the newer AST based on page load logic that is meant to work with fat
* container deployments. This test adds some actions in the page and attaches a few of those in
* the dynamic bindings in the widgets in the layout. An action attached in the widget also has
* two dependencies on other actions. One of those has been explicitly marked to NOT run on page
* load. This test asserts the following : 1. All the actions which must be executed on page load
* have been recognized correctly 2. The sequence of the action execution takes into account the
* action dependencies 3. An action which has been marked to not execute on page load does not get
* added to the on page load order 4. Async and sync JS functions when called with a .data
* reference are marked to run on load 5. Async and sync JS function when called as function calls
* are ignored to be marked on page load 6. A string that happens to match an action reference IS
* NOT marked to run on page load
*/
@Test
@WithUserDetails(value = "api_user")
public void getActionsExecuteOnLoadWithAstLogic() {
	...
	Mockito.when(
		...
		.thenReturn(
			Flux.just(
				Tuples.of(
					bindingValue,
					new HashSet<>(Set.of("aPostAction.data", "anotherPostAction.data")))));
	Mockito.when(
			astService.getPossibleReferencesFromDynamicBinding(
		...
		.thenReturn(
			Flux.just(
				Tuples.of(
					...
					new HashSet<>(Set.of("aPostActionWithAutoExec.data")))));
	Mockito.when(
			astService.getPossibleReferencesFromDynamicBinding(
		...
		.thenReturn(
			Flux.just(
				Tuples.of(
					...
					new HashSet<>(Set.of("aDBAction.data[0].irrelevant")))));
	Mockito.when(
			astService.getPossibleReferencesFromDynamicBinding(
	...
	Mockito.when(
		...
		.thenReturn(
			Flux.just(
				Tuples.of(
					"aTableAction.data.child", new HashSet<>(Set.of("aTableAction.data.child")))));
	Mockito.when(
			astService.getPossibleReferencesFromDynamicBinding(
				List.of("Collection.anAsyncCollectionActionWithoutCall.data"), EVALUATION_VERSION))
		.thenReturn(
		...
		.thenReturn(
			Flux.just(
				Tuples.of(
					...
					new HashSet<>(Set.of("aPostTertiaryAction.data")))));
	Mockito.when(
			astService.getPossibleReferencesFromDynamicBinding(
		...
		.thenReturn(
			Flux.just(
				Tuples.of(
					...
					new HashSet<>(Set.of("aPostSecondaryAction.data")))));
	StepVerifier.create(testMono)
		.assertNext(
			layout -> {
			assertThat(layout).isNotNull();
			...
			Set<String> fourthSetPageLoadActions =
				Set.of(
					"aPostActionWithAutoExec", "Collection.anAsyncCollectionActionWithoutCall");
			assertThat(
					layout.getLayoutOnLoadActions().get(0).stream()
						.map(DslActionDTO::getName)
						.collect(Collectors.toSet()))
			...
			assertThat(
				...
				.hasSameElementsAs(thirdSetPageLoadActions);
			assertThat(
					layout.getLayoutOnLoadActions().get(3).stream()
						.map(DslActionDTO::getName)
						.collect(Collectors.toSet()))
...
/**
* This test is meant for the older string based on page load logic that persists today for older
* deployments. The expectation is the same as the previous test, except that only valid global
* references are marked to run on page load. (Please refer point #6 in this list) This test adds
* some actions in the page and attaches a few of those in the dynamic bindings in the widgets in
* the layout. An action attached in the widget also has two dependencies on other actions. One of
* those has been explicitly marked to NOT run on page load. This test asserts the following : 1.
* All the actions which must be executed on page load have been recognized correctly 2. The
* sequence of the action execution takes into account the action dependencies 3. An action which
* has been marked to not execute on page load does not get added to the on page load order 4.
* Async and sync JS functions when called with a .data reference are marked to run on load 5.
* Async and sync JS function when called as function calls are ignored to be marked on page load
* 6. A string that happens to match an action reference DOES GET INCORRECTLY marked to run on
* page load
*/
@Test
@WithUserDetails(value = "api_user")
public void getActionsExecuteOnLoadWithoutAstLogic() {
	Mockito.when(pluginExecutorHelper.getPluginExecutor(Mockito.any()))
		.thenReturn(Mono.just(new MockPluginExecutor()));
	Mockito.when(
			astService.getPossibleReferencesFromDynamicBinding(Mockito.anyList(), Mockito.anyInt()))
		.thenCallRealMethod();
	...
	Mono<LayoutDTO> testMono =
		Mono.just(page)
			...
			.flatMap(
				...
				JSONArray dynamicBindingsPathList = new JSONArray();
				dynamicBindingsPathList.addAll(
					List.of(new JSONObject(Map.of("key", "dynamicGet_IncorrectKey"))));
			...
			try {
				oldParent =
					objectMapper.readTree(
						"{\"widgetName\":\"testWidget\",\"dynamicBindingPathList\":[{\"key\":\"dynamicGet_IncorrectKey\"}],\"widgetId\":\"id\",\"another\":\"Hello"
							+ " people of the {{input1.text}} planet!\",\"dynamicGet\":\"some"
							+ " dynamic"
							+ " {{aGetAction.data}}\",\"type\":\"test_type\",\"key\":\"value-updated\"}");
			} catch (JsonProcessingException e) {
				Assertions.fail("Incorrect initialization of expected DSL");
			}
			assertThat(throwable.getMessage())
				.isEqualTo(
					AppsmithError.INVALID_DYNAMIC_BINDING_REFERENCE.getMessage(
						"test_type",
						"testWidget",
						"id",
...
public void testIncorrectMustacheExpressionInBindingInDsl() {
	...
	Mono<LayoutDTO> testMono =
		Mono.just(page)
			...
			.flatMap(
				...
				JSONArray dynamicBindingsPathList = new JSONArray();
				dynamicBindingsPathList.addAll(
					List.of(new JSONObject(Map.of("key", "dynamicGet"))));
	...
	StepVerifier.create(testMono)
		.assertNext(
			...
			// Since this is still a bad mustache binding, we couldn't have extracted the action
			// name
