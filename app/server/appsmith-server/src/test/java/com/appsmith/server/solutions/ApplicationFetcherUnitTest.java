...
public void getAllApplications_gitConnectedAppScenarios_OnlyTheDefaultBranchedAppIsReturned() {
	...
	Mono<UserHomepageDTO> userHomepageDTOMono =
		applicationFetcher
			...
			// After choosing the any app randomly to connect to git, Generate keys and stop the
			// process
			.flatMap(
				application ->
					applicationService.createOrUpdateSshKeyPair(application.getId(), null))
	...
	// For connect and create branch flow scenarios where - defaultBranchName is somehow not saved
	// in DB
	userHomepageDTOMono =
		applicationFetcher
			.getAllApplications()
			...
			.flatMap(
				application -> {
				// Create a new branched App resource in the same org and verify that branch App
...
public void getAllApplications_WhenUserHasRecentOrgAndApp_RecentEntriesComeFirst() {
	...
	StepVerifier.create(applicationFetcher.getAllApplications())
		.assertNext(
			...
			// apps under first org should be sorted as org-2-app-2, org-2-app-1, org-2-app-3,
			// org-2-app-4
			checkAppsAreSorted(
