...
public class MustacheHelperTest {
        ...
        checkTokens(
                ...
                Arrays.asList(
                        new MustacheBindingToken(
                                "{{data.map(datum => {return {id: datum}})}}", 0, true)));
        checkKeys(
                "{{data.map(datum => {return {id: datum}})}}",
                Set.of(
                        new MustacheBindingToken(
                                "data.map(datum => {return {id: datum}})", 2, false)));
    }
    ...
    public void braceDances1() {
        check(
                ...
                Set.of(
                        new MustacheBindingToken("", 2, false),
        ...
        check(
                "}} {{",
                Arrays.asList(
                        new MustacheBindingToken("}} ", 0, false),
                        new MustacheBindingToken("{{", 3, false)),
                Set.of());
    }
    ...
    public void singleQuotedStringsWithBraces() {
        ...
        check(
                ...
                Arrays.asList(
                        new MustacheBindingToken(
                                "{{ 'Interpolation uses {{ and }} delimiters' }}", 0, true)),
                Set.of(
                        new MustacheBindingToken(
                                " 'Interpolation uses {{ and }} delimiters' ", 2, false)));
    }
    ...
    public void doubleQuotedStringsWithBraces() {
        ...
        check(
                "{{ \"I have {{{ three braces\" }}",
                Arrays.asList(
                        new MustacheBindingToken("{{ \"I have {{{ three braces\" }}", 0, true)),
                Set.of(new MustacheBindingToken(" \"I have {{{ three braces\" ", 2, false)));
        check(
                "{{ \"The } char is a brace\" }}",
                Arrays.asList(new MustacheBindingToken("{{ \"The } char is a brace\" }}", 0, true)),
    ...
    public void quotedStringsWithEscapes() {
        ...
        check(
                "{{ \"Escaped \\\" character\" }}",
                Arrays.asList(
                        new MustacheBindingToken("{{ \"Escaped \\\" character\" }}", 0, true)),
                Set.of(new MustacheBindingToken(" \"Escaped \\\" character\" ", 2, false)));
        check(
                "{{ `Escaped \\` character` }}",
                Arrays.asList(new MustacheBindingToken("{{ `Escaped \\` character` }}", 0, true)),
    ...
    public void allDatasourceConfigurationFields() {
        ...
        assertThat(configuration.getConnection().getDefaultDatabaseName())
                .isEqualTo("rendered dbName");
    ...
    public void whenBindingFoundWithoutValue_doNotReplaceWithNull() {
        ...
        assertThat(rendered)
                .isEqualTo(
                        "HL7 Result: CODE 5 - CRITICAL VALUE - {{institution}}"
                                + " {{accessionNumber}}");
    }
}
