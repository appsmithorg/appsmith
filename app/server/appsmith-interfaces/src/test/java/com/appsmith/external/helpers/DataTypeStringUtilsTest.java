...
public class DataTypeStringUtilsTest {
  ...
  public void checkStringDataType() {
    ...
    // JSON.
    String stringData =
        "2.1 In order to understand recursion, one must first understand recursion."
            + " -Anonymous";
    AppsmithType appsmithType =
  ...
  public void testJsonStrictParsing() {
    ...
    assertThat(DataType.STRING)
        .isEqualByComparingTo(
            DataTypeServiceUtils.getAppsmithType(ClientDataType.STRING, "}{}").type());
    // Top-level values of any type. With strict parsing, the top-level value must be an object
    // or an array.
    assertThat(DataType.STRING)
        .isEqualByComparingTo(
            DataTypeServiceUtils.getAppsmithType(ClientDataType.STRING, "").type());
