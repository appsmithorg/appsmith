...
public interface CrudTemplateService {
  ...
  /**
   * This method will recursively replace the column names from template table to user provided
   * table This generic implementation makes sure that CRUD generation for each plugin goes through
   * by default The guideline to follow to figure out if this implementation applies to the plugin
   * is: - The plugin follows pre-UQI data structure for queries - The plugin has a datasource
   * structure that is used to generate query templates
   *
   ...
   */
  default void updateCrudTemplateFormData(
    ...
    for (Map.Entry<String, Object> property : formData.entrySet()) {
      if (property.getValue() != null) {
        if (property.getKey() != null
          ...
          // Recursively replace the column names from template table with user provided
          // table using mappedColumns
          if (property.getValue() instanceof String) {
            final String replacedValue =
                replaceMappedColumnInStringValue(mappedColumns, property.getValue());
            property.setValue(replacedValue);
          }
          if (property.getValue() instanceof Map) {
            updateCrudTemplateFormData(
