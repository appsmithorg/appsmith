...
@Slf4j
@Extension
public class BaseRestApiPluginExecutor
	implements PluginExecutor<APIConnection>, SmartSubstitutionInterface {
...
protected BaseRestApiPluginExecutor(SharedConfig sharedConfig) {
	...
	this.EXCHANGE_STRATEGIES =
		ExchangeStrategies.builder()
			.codecs(
				configurer ->
					configurer.defaultCodecs().maxInMemorySize(sharedConfig.getCodecSize()))
			.build();
}
...
public Mono<DatasourceTestResult> testDatasource(
	...
	// At this point, the URL can be invalid because of mustache template keys inside it. Hence,
	// connecting to
	// and verifying the URL isn't feasible. Since validation happens just before testing, and since
	// validation
	// checks if a URL is present, there's nothing left to do here, but return a successful
	// response.
...
public Mono<ActionExecutionResult> execute(
	...
	// Unused function
	return Mono.error(
		new AppsmithPluginException(AppsmithPluginError.PLUGIN_ERROR, "Unsupported Operation"));
}
