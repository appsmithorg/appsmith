...
public class APIConnectionFactory {
  ...
  public static Mono<APIConnection> createConnection(
    ...
    if (authentication instanceof OAuth2) {
      if (OAuth2.Type.CLIENT_CREDENTIALS.equals(((OAuth2) authentication).getGrantType())) {
        ...
        if (OAuth2.Type.AUTHORIZATION_CODE.equals(((OAuth2) authentication).getGrantType())) {
          if (!Boolean.TRUE.equals(authentication.getIsAuthorized())) {
            return Mono.error(
                new AppsmithPluginException(
                    AppsmithPluginError.PLUGIN_DATASOURCE_ARGUMENT_ERROR,
                    "Please authorize datasource"));
          }
      ...
      if (authentication instanceof BasicAuth) {
        ...
        if (authentication instanceof ApiKeyAuth) {
          ...
          if (authentication instanceof BearerTokenAuth) {
