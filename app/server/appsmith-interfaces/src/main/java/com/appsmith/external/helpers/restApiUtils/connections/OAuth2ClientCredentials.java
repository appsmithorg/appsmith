...
public class OAuth2ClientCredentials extends APIConnection implements UpdatableConnection {
    ...
    public static Mono<OAuth2ClientCredentials> create(
        ...
        return Mono.just(oAuth2)
                ...
                .filter(
                        x ->
                                x.getAuthenticationResponse() != null
                ...
                .flatMap(
                        token -> {
                            ...
                            connection.setExpiresAt(
                                    token.getAuthenticationResponse().getExpiresAt());
    ...
    private Mono<OAuth2> generateOAuth2Token(DatasourceConfiguration datasourceConfiguration) {
        ...
        final WebClient.Builder webClientBuilder =
                WebClientUtils.builder(securedHttpClient)
                        .defaultHeader(
                ...
                .map(
                        mappedResponse -> {
                            ...
                            if (issuedAtResponse != null) {
                                issuedAt =
                                        Instant.ofEpochSecond(
                                                Long.parseLong((String) issuedAtResponse));
                            }

                            // We expect at least one of the following to be present
                            Object expiresAtResponse =
                                    mappedResponse.get(Authentication.EXPIRES_AT);
                            Object expiresInResponse =
                                    mappedResponse.get(Authentication.EXPIRES_IN);
                            ...
                            authenticationResponse.setToken(
                                    String.valueOf(
                                            mappedResponse.get(Authentication.ACCESS_TOKEN)));
                    ...
                    ClientRequest.from(clientRequest)
                            .headers(
                                    headers ->
                                            headers.set(
                                                    "Authorization",
                                                    finalHeaderPrefix + this.getToken()))
                            .build());
        } else {
            final URI url =
                    UriComponentsBuilder.fromUri(clientRequest.url())
    ...
    private BodyInserters.FormInserter<String> clientCredentialsTokenBody(OAuth2 oAuth2) {
        BodyInserters.FormInserter<String> body =
                BodyInserters.fromFormData(
                        Authentication.GRANT_TYPE, Authentication.CLIENT_CREDENTIALS);
        ...
        if (!CollectionUtils.isEmpty(oAuth2.getScope())) {
            body.with(
                    ...
                    StringUtils.collectionToDelimitedString(oAuth2.getScope(), " "));
        }
        // Custom Token Parameters
        if (oAuth2.getCustomTokenParameters() != null) {
            oAuth2.getCustomTokenParameters()
                    .forEach(params -> body.with(params.getKey(), params.getValue().toString()));
        }
        return body;
    }
