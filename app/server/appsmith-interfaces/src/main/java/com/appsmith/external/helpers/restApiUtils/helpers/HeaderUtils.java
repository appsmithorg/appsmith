...
public class HeaderUtils {
    ...
    public String getRequestContentType(ActionConfiguration actionConfiguration,
        ...
        if(reqContentType.isEmpty() && actionConfiguration.getAutoGeneratedHeaders() != null){
            reqContentType = getRequestContentTypeFromHeaders(actionConfiguration.getAutoGeneratedHeaders());
        }
    ...
    public String getRequestContentType(ActionConfiguration actionConfiguration) {
        if(actionConfiguration.getAutoGeneratedHeaders() != null) {
            return getRequestContentTypeFromHeaders(actionConfiguration.getAutoGeneratedHeaders());
        }

        if(actionConfiguration.getHeaders() != null){
            return getRequestContentTypeFromHeaders(actionConfiguration.getHeaders());
        }
        return "";
    }
    ...
    public void setContentTypeHeader(ActionConfiguration actionConfiguration){
        if(actionConfiguration.getAutoGeneratedHeaders() == null){
            return;
        }
        if(actionConfiguration.getHeaders() == null){
            actionConfiguration.setHeaders(Arrays.asList(new Property(HttpHeaders.CONTENT_TYPE,
                    getRequestContentTypeFromHeaders(actionConfiguration.getAutoGeneratedHeaders()))));
        } else if(!actionConfiguration.getHeaders().stream().filter(s -> s.getKey().
                equals(HttpHeaders.CONTENT_TYPE)).toList().isEmpty()){
            actionConfiguration.getHeaders().add(new Property(HttpHeaders.CONTENT_TYPE,
                    getRequestContentTypeFromHeaders(actionConfiguration.getAutoGeneratedHeaders())));
        }
    }
}
