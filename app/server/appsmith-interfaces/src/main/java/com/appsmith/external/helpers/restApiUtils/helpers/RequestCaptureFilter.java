...
public class RequestCaptureFilter implements ExchangeFilterFunction {
    ...
    public ActionExecutionRequest populateRequestFields(ActionExecutionRequest existing) {
        ...
        if(existing.getAutoGeneratedHeaders() != null &&
                !((JsonNode) existing.getAutoGeneratedHeaders()).isEmpty()) {
            String autoGeneratedContentType = ((JsonNode) existing.getAutoGeneratedHeaders()).
                    get(HttpHeaders.CONTENT_TYPE.toLowerCase()).toString();
            if (autoGeneratedContentType.toLowerCase().contains(MULTIPART_FORM_DATA_VALUE.toLowerCase())) {
    ...
    public static ActionExecutionRequest populateRequestFields(ActionConfiguration actionConfiguration,
        ...
        if (actionConfiguration.getAutoGeneratedHeaders() != null) {
            MultiValueMap<String, String> reqMultiMap = CollectionUtils.toMultiValueMap(new LinkedCaseInsensitiveMap<>(8, Locale.ENGLISH));

            actionConfiguration.getAutoGeneratedHeaders()
                    .forEach(header -> {
                        if (HttpHeaders.CONTENT_TYPE.equalsIgnoreCase(header.getKey())) {
                            reqContentType.set((String) header.getValue());
                        }
            ...
            actionExecutionRequest.setAutoGeneratedHeaders(objectMapper.valueToTree(reqMultiMap));
        }
