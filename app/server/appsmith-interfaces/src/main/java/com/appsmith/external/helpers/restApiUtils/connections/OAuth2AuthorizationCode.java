	...
	return webClient
		...
		.map(
			...
			authenticationResponse.setToken(
				String.valueOf(mappedResponse.get(Authentication.ACCESS_TOKEN)));
...
private BodyInserters.FormInserter<String> getTokenBody(OAuth2 oAuth2) {
	BodyInserters.FormInserter<String> body =
		BodyInserters.fromFormData(Authentication.GRANT_TYPE, Authentication.REFRESH_TOKEN)
			.with(
				Authentication.REFRESH_TOKEN, oAuth2.getAuthenticationResponse().getRefreshToken());
	...
	body.with(
		Authentication.SCOPE, StringUtils.collectionToDelimitedString(oAuth2.getScope(), " "));
	}
	return body;
}
