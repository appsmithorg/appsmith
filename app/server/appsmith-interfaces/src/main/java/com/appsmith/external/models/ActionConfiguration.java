...
/**
* This field is supposed to hold a set of paths that are expected to contain bindings that refer
* to the same action object i.e. a cyclic reference. e.g. A GraphQL API response can contain
* pagination cursors that are required to be configured in the pagination tab of the same API. We
* don't want to treat these cyclic references as cyclic dependency errors.
*/
@Transient Set<String> selfReferencingDataPaths = new HashSet<>();
...
public void setTimeoutInMillisecond(String timeoutInMillisecond) {
	...
	log.debug(
		"Failed to convert timeout request parameter to Integer. Setting it to max valid value.");
