...
public class Condition {
  ...
  /**
   * To evaluate 'Path' and 'Operator' to be available for filtering 'Values' not evaluated for
   * availability, to support searching empty values
   *
   * @param condition
   * @return Boolean
   */
  ...
  /**
   * To generate condition list based on selected condition Mandatory inputs validated are path and
   * operator Value is optional and considered as a null input
   *
  ...
  public static List<Condition> generateFromConfiguration(List<Object> configurationList) {
    ...
    for (Object config : configurationList) {
      ...
      if (condition.entrySet().isEmpty()) {
        ...
        if (isColumnOrOperatorEmpty(condition)) {
      ...
      conditionList.add(
          new Condition(condition.get("path"), condition.get("operator"), condition.get("value")));
    }
