	...
	ReflectionUtils.doWithFields(
		...
		field -> {
			...
			if (fieldValue == null) {
				...
				// If this field is null, but the cache has a non-empty list of candidates
				// already,
				// then this is an appsmith field with known annotations
				candidateField =
					new CandidateField(field, CandidateField.Type.APPSMITH_FIELD_KNOWN);
				} else {
				// If it is null and the cache is not aware of the field, this is still a
				// prospect,
			...
			if (subFieldType != null) {
				...
				if (!existingSubTypeCandidates.isEmpty()) {
					finalCandidateFields.add(
						new CandidateField(field, CandidateField.Type.APPSMITH_COLLECTION_KNOWN));
				}
				} else if (AppsmithDomain.class.isAssignableFrom(subFieldType)) {
				// If the type is not known, then this is either not parsed yet, or has
				...
				if (collection == null || collection.isEmpty()) {
					finalCandidateFields.add(
						new CandidateField(field, CandidateField.Type.APPSMITH_COLLECTION_UNKNOWN));
				} else {
			...
			if (this.encryptedFieldsMap.containsKey(subFieldType)) {
				...
				// If the type is not known, then this is either not parsed yet, or has polymorphic
				// implementations
				...
				for (Map.Entry<?, ?> entry : map.entrySet()) {
					...
					finalCandidateFields.add(
						new CandidateField(field, CandidateField.Type.APPSMITH_MAP_POLYMORPHIC));
					}
					break;
				}
				}
...
synchronized boolean convertEncryption(Object source, UnaryOperator<String> transformer) {
	...
	// if it is an unknown type, go to specific subtype for convert and update the current candidate
	...
	if (fieldValue != null) {
		...
		if (Set.of(
			...
			// the collection itself was not empty (if it was empty then we never really scanned
			// anything), and
			// the declared type of the collection was the same as the first element (not
			// polymorphic)
			if (!subTypeHasEncrypted
				&& element != null
			...
			isPolymorphic =
				isPolymorphic || !typeName.equals(entry.getValue().getClass().getCanonicalName());
			}
			// The following condition will be true for unknown types when:
			// none of the elements ended up being encrypted, and
			// the map was not empty (if it was empty then we never really scanned anything), and
			// the declared type of the values in the map was the same as the values in the map (not
			// polymorphic)
