                        ...
                        if (field.getAnnotation(Encrypted.class) != null) {
                            ...
                            if (fieldValue == null) {
                                if (this.encryptedFieldsMap.containsKey(field.getType())) {
                                    // If this field is null, but the cache has a non-empty list of
                                    // candidates already,
                                    ...
                                    // If it is null and the cache is not aware of the field, this
                                    // is still a prospect,
                                ...
                                // If an object exists, check if the object type is the same as the
                                // field type
                            ...
                            if (subFieldType != null) {
                                if (this.encryptedFieldsMap.containsKey(subFieldType)) {
                                    ...
                                    // If the type is not known, then this is either not parsed yet,
                                    // or has polymorphic implementations
                                    ...
                                    if (collection == null || collection.isEmpty()) {
                                        ...
                                        for (final Object o : collection) {
                                            ...
                                            if (o.getClass()
                                                ...
                                                final List<CandidateField>
                                                        candidateFieldsForListMember =
                            ...
                            if (this.encryptedFieldsMap.containsKey(subFieldType)) {
                                ...
                                // If the type is not known, then this is either not parsed yet, or
                                // has polymorphic implementations
            ...
            if (fieldValue != null) {
                if (CandidateField.Type.ANNOTATED_FIELD.equals(candidateField.getType())) {
                    ...
                    if (Set.of(
                        ...
                        // the collection itself was not empty (if it was empty then we never really
                        ...
                        if (!subTypeHasEncrypted
                                ...
                                && typeNames[0]
                                        .getTypeName()
                                        .equals(element.getClass().getCanonicalName())) {
                            candidateFieldIterator.remove();
                        }
                    } else if (Set.of(
                            ...
                            .contains(candidateField.getType())) {
                        // This is a map that will necessarily have element values of AppsmithDomain
                        // type
                        boolean subTypeHasEncrypted = false;
                        ...
                        for (Map.Entry<?, ?> entry : ((Map<?, ?>) fieldValue).entrySet()) {
                            subTypeHasEncrypted =
                                    subTypeHasEncrypted
                                            || convertEncryption(entry.getValue(), transformer);
                            isPolymorphic =
                                    isPolymorphic
                                            || !typeName.equals(
                        ...
                        // The following condition will be true for unknown types when:
                        // none of the elements ended up being encrypted, and
                        // the map was not empty (if it was empty then we never really scanned
                        // anything), and
                        // the declared type of the values in the map was the same as the values in
                        // the map (not polymorphic)
                        if (!subTypeHasEncrypted
