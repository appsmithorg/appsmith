...
public class EncryptionHandler {
  ...
  /**
   * This method finds all the candidate fields for a given type Candidate fields will ony ever be
   * fields that have either been directly annotated, or are custom Appsmith types (and can hence
   * have fields annotated for encryption within them), or are parameterized collections of custom
   * Appsmith types, or are parameterized maps with custom Appsmith type values (keys are not
   * scanned for encrypted fields)
   *
   * @param source document that needs to be checked for encrypted annotations
   * @return list of candidate fields for the given type and null if this list can not be found at
   *     this time
   */
  List<CandidateField> findCandidateFieldsForType(@NonNull Object source) {
    ...
    synchronized (sourceClass) {
      ReflectionUtils.doWithFields(
          ...
          field -> {
            if (field.getAnnotation(Encrypted.class) != null) {
              CandidateField candidateField =
                  new CandidateField(field, CandidateField.Type.ANNOTATED_FIELD);
              finalCandidateFields.add(candidateField);
            } else {
              if (AppsmithDomain.class.isAssignableFrom(field.getType())) {
                ...
                if (fieldValue == null) {
                  if (this.encryptedFieldsMap.containsKey(field.getType())) {
                    // If this field is null, but the cache has a non-empty list
                    // of candidates already,
                    // then this is an appsmith field with known annotations
                    candidateField =
                        new CandidateField(field, CandidateField.Type.APPSMITH_FIELD_KNOWN);
                  } else {
                    // If it is null and the cache is not aware of the field,
                    // this is still a prospect,
                    // but with an unknown type (could also be polymorphic)
                    candidateField =
                        new CandidateField(field, CandidateField.Type.APPSMITH_FIELD_UNKNOWN);
                  }
                } else {
                  // If an object exists, check if the object type is the same as
                  // the field type
                  ...
                  if (field
                    ...
                    // If they match, then this is going to be an appsmith known
                    // field
                  ...
                  List<CandidateField> candidateFieldsForType =
                      findCandidateFieldsForType(fieldValue);

                  if (appsmithFieldType.equals(CandidateField.Type.APPSMITH_FIELD_POLYMORPHIC)
                      || !candidateFieldsForType.isEmpty()) {
                    // This type only qualifies as a candidate if it is
                    // polymorphic,
                ...
                if (Collection.class.isAssignableFrom(field.getType())
                  ...
                  // If this is a collection, check if the Type parameter is an
                  // AppsmithDomain
                  Type[] typeArguments;
                  ...
                  if (subFieldType != null) {
                    if (this.encryptedFieldsMap.containsKey(subFieldType)) {
                      // This is a known type, it should necessarily be of
                      // AppsmithDomain type
                      assert AppsmithDomain.class.isAssignableFrom(subFieldType);
                      final List<CandidateField> existingSubTypeCandidates =
                          this.encryptedFieldsMap.get(subFieldType);
                      if (!existingSubTypeCandidates.isEmpty()) {
                        finalCandidateFields.add(
                            new CandidateField(
                                field, CandidateField.Type.APPSMITH_COLLECTION_KNOWN));
                      }
                    } else {
                      if (AppsmithDomain.class.isAssignableFrom(subFieldType)) {
                        // If the type is not known, then this is either not
                        // parsed yet, or has polymorphic implementations
                        ...
                        if (collection == null || collection.isEmpty()) {
                          finalCandidateFields.add(
                              new CandidateField(
                                  field, CandidateField.Type.APPSMITH_COLLECTION_UNKNOWN));
                        } else {
                          for (final Object o : collection) {
                            ...
                            if (o.getClass()
                                .getCanonicalName()
                                .equals(subFieldType.getTypeName())) {
                              final List<CandidateField> candidateFieldsForListMember =
                                  findCandidateFieldsForType(o);
                              if (candidateFieldsForListMember != null
                                  && !candidateFieldsForListMember.isEmpty()) {
                                finalCandidateFields.add(
                                    new CandidateField(
                                        field, CandidateField.Type.APPSMITH_COLLECTION_KNOWN));
                              }
                              ...
                              finalCandidateFields.add(
                                  new CandidateField(
                                      field, CandidateField.Type.APPSMITH_COLLECTION_POLYMORPHIC));
                            }
                            break;
                          }
                        }
                  ...
                  if (Map.class.isAssignableFrom(field.getType())
                    ...
                    if (this.encryptedFieldsMap.containsKey(subFieldType)) {
                      ...
                      if (!existingSubTypeCandidates.isEmpty()) {
                        finalCandidateFields.add(
                            new CandidateField(field, CandidateField.Type.APPSMITH_MAP_KNOWN));
                      }
                    } else {
                      if (AppsmithDomain.class.isAssignableFrom(subFieldType)) {
                        // If the type is not known, then this is either not
                        ...
                        if (map == null || map.isEmpty()) {
                          finalCandidateFields.add(
                              new CandidateField(field, CandidateField.Type.APPSMITH_MAP_UNKNOWN));
                        } else {
                          for (Map.Entry<?, ?> entry : map.entrySet()) {
                            ...
                            if (value
                                ...
                                .equals(subFieldType.getTypeName())) {
                              final List<CandidateField> candidateFieldsForListMember =
                                  findCandidateFieldsForType(value);
                              if (candidateFieldsForListMember != null
                                  && !candidateFieldsForListMember.isEmpty()) {
                                finalCandidateFields.add(
                                    new CandidateField(
                                        field, CandidateField.Type.APPSMITH_MAP_KNOWN));
                              }
                            } else {
                              finalCandidateFields.add(
                                  new CandidateField(
                                      field, CandidateField.Type.APPSMITH_MAP_POLYMORPHIC));
                            }
                            break;
                          }
                        }
          ...
          field ->
              field.getAnnotation(Encrypted.class) != null
  ...
  synchronized boolean convertEncryption(Object source, UnaryOperator<String> transformer) {
    ...
    // if it is an unknown type, go to specific subtype for convert and update the current
    // candidate field with the verdict
        ...
        if (CandidateField.Type.ANNOTATED_FIELD.equals(candidateField.getType())) {
          ...
          if (Set.of(
            ...
            if (!subTypeHasEncrypted
              ...
              // convert to polymorphic type if it has encrypted and is not the same
              // type
              // haven't done this yet because I'm considering whether we can just
              // consider
              // unknown types as polymorphic candidates over time
              // If that is the case then do we need to store the candidate field type
              // by
            ...
            if (Set.of(
                ...
                .contains(candidateField.getType())) {
              // This is a collection which will necessarily have elements of
              // AppsmithDomain type
              ...
              // the collection itself was not empty (if it was empty then we never
              // really scanned anything), and
              // the declared type of the collection was the same as the first element
              // (not polymorphic)
              if (!subTypeHasEncrypted
                  && element != null
              ...
              if (Set.of(
                  ...
                  .contains(candidateField.getType())) {
                // This is a map that will necessarily have element values of
                // AppsmithDomain type
                boolean subTypeHasEncrypted = false;
                ...
                for (Map.Entry<?, ?> entry : ((Map<?, ?>) fieldValue).entrySet()) {
                  subTypeHasEncrypted =
                      subTypeHasEncrypted || convertEncryption(entry.getValue(), transformer);
                  isPolymorphic =
                      isPolymorphic
                ...
                // The following condition will be true for unknown types when:
                // none of the elements ended up being encrypted, and
                // the map was not empty (if it was empty then we never really
                // scanned anything), and
                // the declared type of the values in the map was the same as the
                // values in the map (not polymorphic)
                if (!subTypeHasEncrypted && !((Map<?, ?>) fieldValue).isEmpty() && !isPolymorphic) {
                  candidateFieldIterator.remove();
                }
              }
            }
