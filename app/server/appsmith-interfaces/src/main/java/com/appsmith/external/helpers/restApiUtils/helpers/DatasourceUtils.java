...
public class DatasourceUtils {
  ...
  public Set<String> validateDatasource(DatasourceConfiguration datasourceConfiguration) {
    /**
     * We don't verify whether the URL is in valid format because it can contain mustache template
     * keys, and so look invalid at this point, but become valid after mustache rendering. So we
     * just check if URL field has a non-empty value.
     */
    ...
    if (!CollectionUtils.isEmpty(datasourceConfiguration.getProperties())) {
      ...
      for (Property property : datasourceConfiguration.getProperties()) {
        if ("isSendSessionEnabled".equals(property.getKey())) {
          ...
          if ("sessionSignatureKey".equals(property.getKey())) {
    ...
    if (datasourceConfiguration.getAuthentication() != null) {
      invalids.addAll(
          DatasourceValidator.validateAuthentication(datasourceConfiguration.getAuthentication()));
    }
