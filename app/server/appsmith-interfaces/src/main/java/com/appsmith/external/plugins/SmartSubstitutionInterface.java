...
/**
* !Warning! - This function changes the values of arraylist insertedParams which can then be
* returned back to the caller with all the values that were finally put during substitution
*
* @param input
...
default Object smartSubstitutionOfBindings(
	...
	for (int i = 0; i < mustacheValuesInOrder.size(); i++) {
		...
		Optional<Param> matchingParam =
			evaluatedParams.stream().filter(param -> param.getKey().trim().equals(key)).findFirst();

		// If the evaluated value of the mustache binding is present, set it in the prepared
		// statement
		...
		throw new AppsmithPluginException(
			AppsmithPluginError.SMART_SUBSTITUTION_VALUE_MISSING, key);
		}
	}
	}
	return input;
...
// Default implementation does not do any substitution. The plugin doing intelligent substitution
// is responsible
...
/**
* This method is part of the pre-processing of the replacement value before the final
* substitution that happens as part of smart substitution process. This is the default
* implementation. Each plugin that implements `SmartSubstitutionInterface` is supposed to
