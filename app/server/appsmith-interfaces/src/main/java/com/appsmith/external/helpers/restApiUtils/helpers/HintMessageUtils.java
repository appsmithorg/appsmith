...
public class HintMessageUtils {
    ...
    public enum DUPLICATE_ATTRIBUTE_LOCATION {
        ...
        DATASOURCE_AND_ACTION_CONFIG // Duplicates with instance in both datasource and action
        // config
    }
    ...
    protected static String HINT_MESSAGE_FOR_DUPLICATE_ATTRIBUTE_IN_DATASOURCE_CONFIG =
            "API queries linked to this datasource may not run as expected because this datasource"
                    + " has duplicate definition(s) for {0}(s): {1}. Please remove the duplicate"
                    + " definition(s) to resolve this warning. Please note that some of the"
    ...
    protected static String
            HINT_MESSAGE_FOR_DUPLICATE_ATTRIBUTE_IN_ACTION_DEFINED_IN_DATASOURCE_CONFIG =
                    "Your API query may not run as expected because its datasource has duplicate"
                            + " definition(s) for {0}(s): {1}. Please remove the duplicate"
                            + " definition(s) from the datasource to resolve this warning.";

    protected static String HINT_MESSAGE_FOR_DUPLICATE_ATTRIBUTE_IN_ACTION_CONFIG =
            "Your API query may not run as expected because it has duplicate definition(s) for"
                + " {0}(s): {1}. Please remove the duplicate definition(s) from the ''{2}'' tab to"
    ...
    protected static String
            HINT_MESSAGE_FOR_DUPLICATE_ATTRIBUTE_WITH_INSTANCE_ACROSS_ACTION_AND_DATASOURCE_CONFIG =
                    "Your API query may not run as expected because it has duplicate definition(s)"
                        + " for {0}(s): {1}. Please remove the duplicate definition(s) from the"
                        + " ''{2}'' section of either the API query or the datasource. Please note"
    ...
    public Set<String> getDatasourceHintMessages(DatasourceConfiguration datasourceConfiguration) {
        ...
        /**
         * Get datasource specific hint message for duplicate headers. ActionConfiguration parameter
         * is passed as `null` so that the hint message that gets generated is only relevant for the
         * datasource.
         */
        ...
        /**
         * Get datasource specific hint message for duplicate query params. ActionConfiguration
         * parameter is passed as `null` so that the hint message that gets generated is only
         * relevant for the datasource.
         */
    ...
    public Set<String> getActionHintMessages(
        ...
        /**
         * Get hint message for localhost URL. For the case of REST API action, the URL is also
         * displayed on the query editor page, hence, this hint message is also added to the action
         * related hint messages - so that it can be displayed on the query editor page too. Same
         * won't apply to other datasources - i.e. datasource attributes generally remain confined
         * to the datasource.
         */
        actionHintMessages.addAll(getHintMessageForLocalhostUrl(datasourceConfiguration));

        /**
         * Get API query page specific hint messages for duplicate headers. It also considers
         * datasource configuration apart from the action configuration since an API inherits all
         * the headers defined in its datasource.
         */
        ...
        /**
         * Get API query page specific hint messages for duplicate query params. It also considers
         * datasource configuration apart from the action configuration since an API inherits all
         * the params defined in its datasource.
         */
    ...
    public Map<DUPLICATE_ATTRIBUTE_LOCATION, Set<String>> getAllDuplicateHeaders(
        ...
        Set duplicatesInDsConfigOnly =
                findDuplicates(getDatasourceHeaders(datasourceConfiguration));
    ...
    protected List getActionHeaders(ActionConfiguration actionConfiguration) {
        ...
        if (actionConfiguration != null
                && !CollectionUtils.isEmpty(actionConfiguration.getHeaders())) {
    ...
    protected List getDatasourceHeaders(DatasourceConfiguration datasourceConfiguration) {
        ...
        if (datasourceConfiguration != null
                && !CollectionUtils.isEmpty(datasourceConfiguration.getHeaders())) {
    ...
    protected Set<String> getAuthenticationHeaders(
            DatasourceConfiguration datasourceConfiguration) {
        ...
        if (API_KEY.equals(datasourceConfiguration.getAuthentication().getAuthenticationType())
                && HEADER.equals(
                        ((ApiKeyAuth) datasourceConfiguration.getAuthentication()).getAddTo())) {
    ...
    public Map<DUPLICATE_ATTRIBUTE_LOCATION, Set<String>> getAllDuplicateParams(
        ...
        Set duplicatesInDsConfigOnly =
                findDuplicates(getDatasourceQueryParams(datasourceConfiguration));
        duplicateMap.put(DATASOURCE_CONFIG_ONLY, duplicatesInDsConfigOnly);
    ...
    protected List getActionParams(ActionConfiguration actionConfiguration) {
        ...
        if (actionConfiguration != null
                && !CollectionUtils.isEmpty(actionConfiguration.getQueryParameters())) {
