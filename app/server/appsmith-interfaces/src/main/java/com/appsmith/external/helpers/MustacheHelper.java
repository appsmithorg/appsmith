...
public static List<MustacheBindingToken> tokenize(String template) {
	...
	// This is set to the quote character of a string in JS. When `null`, it means we're not inside
	// any Javascript
	...
	// The parser is implemented as a pointer (marked by `i`) that loops over each character in the
	// template string.
	// There's majorly two states for the parser, plain-text-mode and mustache-mode, with the
	// current state
	// indicated by `isInsideMustache`. This is set to `true` when the pointer encounters a `{{` in
	// plain-text-mode.
	// It is set back to `false` when the pointer encounters a `}}` in mustache-mode, but not inside
	// a quoted
	// string. Since the contents inside mustache double-braces is supposed to be valid Javascript
	// expression,
...
public static Set<MustacheBindingToken> extractMustacheKeys(String template) {
	...
	if (token.getValue().startsWith("{{") && token.getValue().endsWith("}}")) {
		...
		// Calling `.trim()` before adding because Mustache compiler strips keys in the template
		// before looking
...
// For prepared statements we should extract the bindings in order in a list and include duplicate
// bindings as well.
...
public static Set<MustacheBindingToken> extractMustacheKeysFromFields(Object object) {
	...
	// Linearized recursive search. Instead of calling this function recursively for nested values,
	// we add them to
	// the end of the queue and process them in a linear fashion. This strategy doesn't suffer from
	// a stack overflow
...
public static String render(String template, Map<String, String> keyValueMap) {
	...
	for (MustacheBindingToken token : tokenize(template)) {
	if (token.getValue().startsWith("{{") && token.getValue().endsWith("}}")) {
		// If there is no entry found for the current token in keyValueMap that means the binding is
		// part of the text
		// and hence reflecting the value in the rendered string as is.
		// Example: {{Input.text}} = "This whole string is the value of Input1.text. Even this
		// {{one}}."
		String bindingValue =
...
public static Set<String> getPossibleParents(String mustacheKey) {
	...
	if (subStrings.length >= 2) {
		// For JS actions, the first two words are the action name since action name consists of the
		// collection name
