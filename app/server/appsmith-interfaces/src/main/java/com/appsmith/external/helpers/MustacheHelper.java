...
public class MustacheHelper {
   ...
   *     strings in this list should give the original template back. The tokens are split such that
   *     alternative strings in the list are plain text and the others are mustache interpolations.
   */
  ...
  /**
   * Tokenize-s the given Mustache template string, extracts the Mustache interpolations out, strips
   * the leading and trailing double braces, trims and then returns a set of these replacement keys.
   *
  ...
  private static void clearAndPushToken(
    ...
    if (tokenBuilder.length() > 0) {
      tokenList.add(
          new MustacheBindingToken(tokenBuilder.toString(), tokenStartIndex, includesHandleBars));
      tokenBuilder.setLength(0);
    }
  }

  /**
   * - If object is null, then return object. - If object is an Appsmith domain object then iterate
   * over all fields of the object and render field values for each of them. - If object is list
   * type then iterate over each item in the list and render field value for them. - If object is
   * map type, then iterate over each value in the map and render field value for them. - If the
   * object is string type (base case), then do the binding substitution if applicable. - If the
   * object falls under none of the above conditions then return the object without doing anything.
   */
   ...
   * @return It finally returns the string in which all the keys in template have been replaced with
   *     values.
   */
  public static String render(String template, Map<String, String> keyValueMap) {
    ...
    for (MustacheBindingToken token : tokenize(template)) {
      if (token.getValue().startsWith("{{") && token.getValue().endsWith("}}")) {
        ...
        // this {{one}}."
        String bindingValue =
            keyValueMap.get(token.getValue().substring(2, token.getValue().length() - 2).trim());
   ...
   * references found in each binding and creates entity dependency nodes out of the references that
   * would qualify as a reference of a specific entity type
   *
   ...
   * whether the reference string has a dot notation reference, as long as it is not empty, pick the
   * first word in the string as a possible widget reference. Eg: Text1 which could be derived from
   * {{ Text1 }} Eg: Text1 which could be derived from {{ Text1.text }}
   *
   ...
   *
   * @param reference The string reference computed by AST logic
   * @return A set of all possible action or JS function dependencies that could exist in the given
   *     reference
   */
  ...
  /**
   * This method is used as a fallback for setups where the RTS server is not accessible. It counts
   * all words as possible entity references. This is obviously going to continue to give inaccurate
   * results, but is required to maintain backward compatibility
   *
