...
public class MustacheHelper {
    ...
    /**
     * Appsmith smart replacement : The regex pattern below looks for '?' or "?". This pattern is
     * later replaced with ? to fit the requirements of prepared statements.
     */
    ...
    /**
     * Appsmith smart replacement with placeholder : The regex pattern below looks for
     * `APPSMITH_SUBSTITUTION_PLACEHOLDER` surrounded by quotes. This pattern is later replaced with
     * just APPSMITH_SUBSTITUTION_PLACEHOLDER to fit the requirements of JSON smart replacement aka
     * trim the quotes present.
     */
    private static final String regexPlaceholderTrimming =
            "([\"']" + APPSMITH_SUBSTITUTION_PLACEHOLDER + "[\"'])";

    private static final Pattern placeholderTrimmingPattern =
            Pattern.compile(regexPlaceholderTrimming);

    private static final String laxMustacheBindingRegex = "\\{\\{([\\s\\S]*?)}}";
    private static final Pattern laxMustacheBindingPattern =
            Pattern.compile(laxMustacheBindingRegex);
     ...
     *
     * @param template String Mustache template string from which to extract plain text and
     *     interpolation tokens.
     * @return A list of String tokens, which form parts of the given template String. Joining the
     *     strings in this list should give the original template back. The tokens are split such
     *     that alternative strings in the list are plain text and the others are mustache
     *     interpolations.
     */
    public static List<MustacheBindingToken> tokenize(String template) {
        ...
        // This indicates the state of the pointer. It is `true` when inside mustache double braces,
        // otherwise `false`.
        boolean isInsideMustache = false;

        // This is set to the quote character of a string in JS. When `null`, it means we're not
        // inside any Javascript
        ...
        // The parser is implemented as a pointer (marked by `i`) that loops over each character in
        // the template string.
        // There's majorly two states for the parser, plain-text-mode and mustache-mode, with the
        // current state
        // indicated by `isInsideMustache`. This is set to `true` when the pointer encounters a `{{`
        // in plain-text-mode.
        // It is set back to `false` when the pointer encounters a `}}` in mustache-mode, but not
        // inside a quoted
        // string. Since the contents inside mustache double-braces is supposed to be valid
        // Javascript expression,
        // any braces inside quoted strings (using single, double or back quotes) should not affect
        // the
        ...
        if (currentToken.length() > 0) {
            tokens.add(
                    new MustacheBindingToken(
                            currentToken.toString(), currentTokenStartIndex, false));
        }
    ...
    /**
     * Tokenize-s the given Mustache template string, extracts the Mustache interpolations out,
     * strips the leading and trailing double braces, trims and then returns a set of these
     * replacement keys.
     *
     * @param template The Mustache input template string.
     * @return A Set of strings that serve as replacement keys, with the surrounding double braces
     *     stripped and then trimmed.
     */
    public static Set<MustacheBindingToken> extractMustacheKeys(String template) {
        ...
        for (MustacheBindingToken token : tokenize(template)) {
            if (token.getValue().startsWith("{{") && token.getValue().endsWith("}}")) {
                // Allowing empty tokens to be added, to be compatible with the previous
                // `extractMustacheKeys` method.
                // Calling `.trim()` before adding because Mustache compiler strips keys in the
                // template before looking
    ...
    // For prepared statements we should extract the bindings in order in a list and include
    // duplicate bindings as well.
    public static List<MustacheBindingToken> extractMustacheKeysInOrder(String template) {
        ...
        for (MustacheBindingToken token : tokenize(template)) {
            if (token.getValue().startsWith("{{") && token.getValue().endsWith("}}")) {
                ...
                keys.add(
                        new MustacheBindingToken(
                                token.getValue().substring(2, token.getValue().length() - 2).trim(),
                                (token.getStartIndex() + 2),
                                false));
            }
        }
    ...
    public static Set<MustacheBindingToken> extractMustacheKeysFromFields(Object object) {
        ...
        // Linearized recursive search. Instead of calling this function recursively for nested
        // values, we add them to
        // the end of the queue and process them in a linear fashion. This strategy doesn't suffer
        // from a stack overflow
        // exception, since it doesn't rely on the call-stack. Hence, ideal for processing large
        // DSLs.
    ...
    private static void clearAndPushToken(
        ...
        if (tokenBuilder.length() > 0) {
            tokenList.add(
                    new MustacheBindingToken(
                            tokenBuilder.toString(), tokenStartIndex, includesHandleBars));
            tokenBuilder.setLength(0);
        }
    }

    /**
     * - If object is null, then return object. - If object is an Appsmith domain object then
     * iterate over all fields of the object and render field values for each of them. - If object
     * is list type then iterate over each item in the list and render field value for them. - If
     * object is map type, then iterate over each value in the map and render field value for them.
     * - If the object is string type (base case), then do the binding substitution if applicable. -
     * If the object falls under none of the above conditions then return the object without doing
     * anything.
     */
    public static <T> T renderFieldValues(T object, Map<String, String> context) {
        ...
        if (isDomainModel(object.getClass())) {
            try {
                final BeanWrapper sourceBeanWrapper =
    ...
    /**
     * @param template : This is the string which contains {{key}} which would be replaced with
     *     value
     * @param keyValueMap : This is the map of keys with values.
     * @return It finally returns the string in which all the keys in template have been replaced
     *     with values.
     */
    public static String render(String template, Map<String, String> keyValueMap) {
        ...
        for (MustacheBindingToken token : tokenize(template)) {
            if (token.getValue().startsWith("{{") && token.getValue().endsWith("}}")) {
                // If there is no entry found for the current token in keyValueMap that means the
                // binding is part of the text
                // and hence reflecting the value in the rendered string as is.
                // Example: {{Input.text}} = "This whole string is the value of Input1.text. Even
                // this {{one}}."
                String bindingValue =
                        keyValueMap.get(
                                token.getValue()
    ...
    /**
     * Depending on the entity types that the caller has asked for, this method analyzed the global
     * references found in each binding and creates entity dependency nodes out of the references
     * that would qualify as a reference of a specific entity type
     *
    ...
    public static Mono<Map<String, Set<EntityDependencyNode>>> getPossibleEntityParentsMap(
        ...
        return bindingAndPossibleReferencesFlux.collect(
                HashMap::new,
                (map, tuple) -> {
                    ...
                    tuple.getT2()
                            .forEach(
                                    reference -> {
                                        if ((types & ACTION_ENTITY_REFERENCES)
                                                == ACTION_ENTITY_REFERENCES) {
                                            totalParents.addAll(
                                                    MustacheHelper.getPossibleActions(reference));
                                        }
                                        if ((types & WIDGET_ENTITY_REFERENCES)
                                                == WIDGET_ENTITY_REFERENCES) {
                                            totalParents.addAll(
                                                    MustacheHelper.getPossibleWidgets(reference));
                                        }
                                    });
    ...
    /**
     * Given a global reference, this method returns any possible combinations of widgets that could
     * be derived out of the reference string. The rules for this are as follows: 1) Irrespective of
     * whether the reference string has a dot notation reference, as long as it is not empty, pick
     * the first word in the string as a possible widget reference. Eg: Text1 which could be derived
     * from {{ Text1 }} Eg: Text1 which could be derived from {{ Text1.text }}
     *
     * <p>Please note that we do not filter out any invalid references at this point, because we do
     * not have context of the global namespace here.
     *
        ...
        if (subStrings.length < 1) {
            ...
            EntityDependencyNode entityDependencyNode =
                    new EntityDependencyNode(
                            EntityReferenceType.WIDGET, subStrings[0], reference, null, null, null);
    ...
    /**
     * Given a global reference, this method returns any possible combinations of actions or JS
     * actions that could be derived out of the reference string. The rules for this are as follows:
     * 1) If the reference string has exactly one dot notation reference in its path, it could be a
     * sync JS function call Eg: JsObject1.syncJsFunc which could be derived from {{
     * JsObject1.syncJsFunc() }} If the reference string has more than one dot notation reference in
     * its path, it could either be - 2) An action reference where the data from the actions is
     * consumed using the .data reference Eg: Action1.data.users which could be derived from {{
     * Action1.data.users }} 3) An asynchronous JS function that is being made to run on page load
     * using its fully qualified name followed by the .data reference Eg: JsObject2.asyncJsFunc.data
     * which could be derived from {{ JsObject2.asyncFunc.data }}
     *
     * <p>Please note that we do not filter out any invalid references at this point, because we do
     * not have context of the global namespace here.
     *
     * @param reference The string reference computed by AST logic
     * @return A set of all possible action or JS function dependencies that could exist in the
     *     given reference
     */
    public static Set<EntityDependencyNode> getPossibleActions(String reference) {
        ...
        if (subStrings.length == 2) {
            // This could qualify if it is a sync JS function call, even if it is called
            // `JsObject1.data()`
            // For sync JS actions, the entire reference could be a function call
            EntityDependencyNode entityDependencyNode =
                    new EntityDependencyNode(
                            EntityReferenceType.JSACTION, key, reference, false, true, null);
            dependencyNodes.add(entityDependencyNode);
            ...
            if ("data".equals(subStrings[2])) {
                // For JS actions, the first two words are the action name since action name
                // consists of
    ...
    /**
     * This method is used as a fallback for setups where the RTS server is not accessible. It
     * counts all words as possible entity references. This is obviously going to continue to give
     * inaccurate results, but is required to maintain backward compatibility
     *
    ...
    public static Set<String> getPossibleParents(String mustacheKey) {
        ...
        while (matcher.find()) {
            ...
            if (subStrings.length >= 2) {
                // For JS actions, the first two words are the action name since action name
                // consists of the collection name
    ...
    public static String replaceMustacheWithQuestionMark(
            String query, List<MustacheBindingToken> mustacheBindings) {
    ...
    private static String replaceMustacheUsingPatterns(
        ...
        ActionConfiguration updatedActionConfiguration =
                renderFieldValues(actionConfiguration, replaceParamsMap);
