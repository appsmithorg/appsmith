...
public static Set<String> validateDatasource(DatasourceConfiguration datasourceConfiguration) {
	...
	for (final Endpoint endpoint : datasourceConfiguration.getEndpoints()) {
		...
		invalids.add(
			String.format(MySQLErrorMessages.DS_INVALID_HOSTNAME_ERROR_MSG, endpoint.getHost()));
		}
	}
	}
...
public static ConnectionPool getNewConnectionPool(DatasourceConfiguration datasourceConfiguration)
	...
	/**
	 * The pool configuration object does not seem to have any option to set the minimum pool size,
	 * hence could not configure the minimum pool size.
	 */
