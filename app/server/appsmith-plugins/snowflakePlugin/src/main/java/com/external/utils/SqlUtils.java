...
public class SqlUtils {
   ...
   * +--------------+------------+-----------+-------------+-------------+-------------+----------------+-------------+
   * | TABLE_SCHEMA | TABLE_NAME | COLUMN_ID | COLUMN_NAME | COLUMN_TYPE | IS_NULLABLE |
   * COLUMN_DEFAULT | IS_IDENTITY |
   * +--------------+------------+-----------+-------------+-------------+-------------+----------------+-------------+
   * | test_schema | test | 1 | id | int | 0 | | YES | | test_schema | test | 2 | firstname |
   * varchar | 1 | Foo | NO | | test_schema | test | 3 | middlename | varchar | 1 | | NO | |
   ...
   * +------------+---------------+-------------+------------+-------------+--------------+--------------------+---------+
   * | created_on | database_name | schema_name | table_name | column_name | key_sequence |
   * constraint_name | comment |
   * +------------+---------------+-------------+------------+-------------+--------------+--------------------+---------+
   ...
   * fk_database_name | fk_schema_name | fk_table_name | pk_column_name | key_sequence | update_rule
   * | delete_rule | fk_name | pk_name | deferrability | comment |
   * +------------+------------------+----------------+---------------+----------------+------------------+----------------+---------------+----------------+--------------+----------------+-------------+--------------------+--------------------+-----------------+------------+
   * | test | test_db | test_schema | test | id | test_db | test_schema | test2 | f_id | 1 | NO
   * ACTION | NO ACTION | SYS_CONSTRAINT_hex | SYS_CONSTRAINT_hex | | |
   * +------------+------------------+----------------+---------------+----------------+------------------+----------------+---------------+----------------+--------------+----------------+-------------+--------------------+--------------------+-----------------+------------+
   */
  ...
  public static String getDefaultValueByDataType(String datatype) {
    ...
    switch (datatype) {
      ...
      case "VARIANT":
        return "parse_json(' { \"key1\": \"value1\", \"key2\": \"value2\" } ')";
      case "OBJECT":
        return "parse_json(' { \"outer_key1\": { \"inner_key1A\": \"1a\", \"inner_key1B\":"
            + " NULL }, '\n"
            + "              ||\n"
              ...
              List.of(
                  ...
                  new DatasourceStructure.Template(
                      ...
                      "UPDATE "
                          ...
                          + "\n"
                          + "  WHERE 1 = 0; -- Specify a valid condition"
                          + " here. Removing the condition may update"
  ...
  public static void getForeignKeyInfo(
      ResultSet row,
    ...
    final String prefix =
        (foreignSchema.equalsIgnoreCase(selfSchema) ? "" : foreignSchema + ".") + fkTableName + ".";
  ...
  public static void getPrimaryKeyInfo(
      ResultSet row,
      Map<String, DatasourceStructure.Table> tablesByName,
      Map<String, DatasourceStructure.Key> keyRegistry)
      throws SQLException {
  ...
  public static void getTableInfo(
      ResultSet row, Map<String, DatasourceStructure.Table> tablesByName) throws SQLException {
    ...
    // AUTOINCREMENT and IDENTITY are synonymous. If either is specified for a column, Snowflake
    // utilizes a sequence
    // to generate the values for the column. For more information about sequences, see Using
    // Sequences
    boolean isAutogenerated =
        "YES".equalsIgnoreCase(row.getString("IS_IDENTITY"))
            || (!StringUtils.isEmpty(defaultValue)
                && defaultValue.toLowerCase().contains("nextval"));
