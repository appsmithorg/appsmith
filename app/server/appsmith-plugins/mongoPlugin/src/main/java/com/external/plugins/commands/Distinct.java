...
public class Distinct extends MongoCommand {
        ...
        if (validConfigurationPresentInFormData(formData, DISTINCT_QUERY)) {
            this.query =
                    PluginUtils.getDataValueSafelyFromFormData(
                            formData, DISTINCT_QUERY, STRING_TYPE);
        }

        if (validConfigurationPresentInFormData(formData, DISTINCT_KEY)) {
            this.key =
                    PluginUtils.getDataValueSafelyFromFormData(formData, DISTINCT_KEY, STRING_TYPE);
        }
    }
    ...
    public List<DatasourceStructure.Template> generateTemplate(
        ...
        return Collections.singletonList(
                new DatasourceStructure.Template("Distinct", null, configMap));
    }
}
