...
public class Delete extends MongoCommand {
    String query;
    Integer limit = 1; // Can be only 0 or 1. 0 indicates all matching documents, 1 indicates single
    ...
    public Delete(ActionConfiguration actionConfiguration) {
        ...
        if (validConfigurationPresentInFormData(formData, DELETE_QUERY)) {
            this.query =
                    PluginUtils.getDataValueSafelyFromFormData(formData, DELETE_QUERY, STRING_TYPE);
        }

        if (validConfigurationPresentInFormData(formData, DELETE_LIMIT)) {
            String limitOption =
                    PluginUtils.getDataValueSafelyFromFormData(formData, DELETE_LIMIT, STRING_TYPE);
    ...
    public List<DatasourceStructure.Template> generateTemplate(
        ...
        setDataValueSafelyInFormData(
                configMap, DELETE_QUERY, "{ \"_id\": ObjectId(\"id_of_document_to_delete\") }");
        ...
        return Collections.singletonList(
                new DatasourceStructure.Template("Delete", null, configMap));
    }
