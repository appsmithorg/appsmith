...
public class MongoErrorUtils extends AppsmithPluginErrorUtils {
    ...
    /**
     * Extract small readable portion of error message from a larger less comprehensible error
     * message.
     *
    ...
    public String getReadableError(Throwable error) {
        ...
        // If the external error is wrapped inside Appsmith error, then extract the external error
        // first.
        ...
        if (externalError instanceof MongoCommandException) {
            ...
            switch (errorCode) {
                case 9: // FailedToParse error
                    ...
                    /**
                     * Sample external error message: Failed to parse: { find: "newAction", limit: [
                     * 10 ], $db: "mobtools", ... }. 'limit' field must be numeric.
                     *
                     * <p>Return string: 'limit' field must be numeric.
                     */
                    return getLast(mongoCommandError.getErrorMessage().split("\\.")).trim() + ".";
                default:

                    /**
                     * Sample external error message: Error getting filter : Expected 'filter' to be
                     * BSON (or equivalent), but got string instead. Doc = [{find newAction} {filter
                     * filterx} {limit 10} {$db mobtools} ...]
                     *
                     * <p>Return string: Error getting filter : Expected 'filter' to be BSON (or
                     * equivalent), but got string instead.
                     */
            ...
            switch (errorCode) {
                default:
                    /**
                     * Sample external error message: Exception authenticating
                     * MongoCredential{mechanism=SCRAM-SHA-1, userName='username', source='admin',
                     * password=<hidden>, mechanismProperties=<hidden>}
                     *
                     * <p>Return string: Exception authenticating MongoCredential.
                     */
        ...
        /**
         * Sample external error message: Error getting filter : Expected 'filter' to be BSON (or
         * equivalent), but got string instead. Doc = [{find newAction} {filter filterx} {limit 10}
         * {$db mobtools} ...]
         *
         * <p>Return string: Error getting filter : Expected 'filter' to be BSON (or equivalent),
