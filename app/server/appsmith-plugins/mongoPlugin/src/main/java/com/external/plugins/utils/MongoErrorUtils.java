...
public class MongoErrorUtils extends AppsmithPluginErrorUtils {
  ...
  public String getReadableError(Throwable error) {
    ...
    if (externalError instanceof MongoCommandException) {
      ...
      switch (errorCode) {
        case 9: // FailedToParse error
          ...
          /**
           * Sample external error message: Failed to parse: { find: "newAction", limit: [ 10 ],
           * $db: "mobtools", ... }. 'limit' field must be numeric.
           *
        ...
        default:
           ...
           * equivalent), but got string instead. Doc = [{find newAction} {filter filterx} {limit
           * 10} {$db mobtools} ...]
           *
      ...
      if (externalError instanceof MongoSecurityException) {
    ...
    /**
     * Sample external error message: Error getting filter : Expected 'filter' to be BSON (or
     * equivalent), but got string instead. Doc = [{find newAction} {filter filterx} {limit 10} {$db
     * mobtools} ...]
     *
     * <p>Return string: Error getting filter : Expected 'filter' to be BSON (or equivalent), but
     * got string instead.
     */
