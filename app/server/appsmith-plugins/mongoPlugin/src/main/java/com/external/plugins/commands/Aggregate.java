...
public Aggregate(ActionConfiguration actionConfiguration) {
	...
	this.pipeline =
		PluginUtils.getDataValueSafelyFromFormData(formData, AGGREGATE_PIPELINES, STRING_TYPE);
	}

	if (validConfigurationPresentInFormData(formData, AGGREGATE_LIMIT)) {
	this.limit =
		PluginUtils.getDataValueSafelyFromFormData(formData, AGGREGATE_LIMIT, STRING_TYPE);
	}
}
...
public Document parseCommand() {
	...
	// The command expects the pipelines to be sent in an array. Parse and create a single element
	// array
...
public List<DatasourceStructure.Template> generateTemplate(
	...
	String rawQuery =
		"{\n"
			...
			+ "  \"limit\": 10,\n"
			+ "  \"explain\": \"true\"\n"
			+ // Specifies to return the information on the processing of the pipeline. (This also
			// avoids the use of the 'cursor' aggregate key according to Mongo doc)
			"}\n";
