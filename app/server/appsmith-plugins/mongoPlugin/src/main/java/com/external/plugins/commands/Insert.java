...
public Insert(ActionConfiguration actionConfiguration) {
	...
	this.documents =
		PluginUtils.getDataValueSafelyFromFormData(formData, INSERT_DOCUMENT, STRING_TYPE);
	}
}
...
public Document parseCommand() {
	...
	// The command expects the documents to be sent in an array. Parse and create a single element
	// array
...
public List<DatasourceStructure.Template> generateTemplate(
	...
	Map<String, String> sampleInsertValues =
		(Map<String, String>) templateConfiguration.get("sampleInsertValues");

	String sampleInsertDocuments =
...
/**
* This method coverts Mongo plugin's form inputs to Mongo's native query. Currently, it is meant
* to help users switch easily from form based input to raw input mode by providing a readily
* available translation of the form data to raw query. The `parseCommand` method defined in this
* class could not be used since it tries to parse and validate the form data and fails if the
* data is bad or if it contains mustache binding. However, this is not the desired behavior wrt
* the use case this method is intended to solve i.e. we should be able to covert the form data to
* raw query irrespective of whether the data provided by the user is valid or not since we are
* not trying to execute it immediately. When writing this method the following two alternative
* implementations were also considered - using JSONObject and JsonNode. The issue with JSONObject
* is that it does not maintain the keys in order, which causes the final query to fail since
* order of keys is essential - e.g. `find` must be the first key in the native query for Mongo to
* recognize it as a valid command. JsonNode could not be used because it would enclose all values
* inside double quotes - which is not a true translation of what the user might have fed into the
* form.
*
