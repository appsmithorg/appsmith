...
public class UpdateMany extends MongoCommand {
    ...
    public UpdateMany(ActionConfiguration actionConfiguration) {
        ...
        if (validConfigurationPresentInFormData(formData, UPDATE_QUERY)) {
            this.query =
                    PluginUtils.getDataValueSafelyFromFormData(formData, UPDATE_QUERY, STRING_TYPE);
        }

        if (validConfigurationPresentInFormData(formData, UPDATE_OPERATION)) {
            this.update =
                    PluginUtils.getDataValueSafelyFromFormData(
                            formData, UPDATE_OPERATION, STRING_TYPE);
        }
        ...
        if (validConfigurationPresentInFormData(formData, UPDATE_LIMIT)) {
            String limitOption =
                    PluginUtils.getDataValueSafelyFromFormData(formData, UPDATE_LIMIT, STRING_TYPE);
    ...
    public List<DatasourceStructure.Template> generateTemplate(
        ...
        setDataValueSafelyInFormData(
                configMap, UPDATE_QUERY, "{ \"_id\": ObjectId(\"id_of_document_to_update\") }");
        setDataValueSafelyInFormData(
                configMap,
                UPDATE_OPERATION,
                "{ \"$set\": { \"" + filterFieldName + "\": \"new value\" } }");
        ...
        return Collections.singletonList(
                new DatasourceStructure.Template("Update", null, configMap));
    }
