...
public class FileListMethod implements ExecutionMethod, TriggerMethod {
  ...
  public JsonNode transformExecutionResponse(
    ...
    List<Map<String, String>> filesList =
        StreamSupport.stream(response.get("files").spliterator(), false)
            .map(
                file ->
                    getSpreadsheetData(
                        (JsonNode) file, userAuthorizedSheetIds, GoogleSheetMethodEnum.EXECUTE))
    ...
    List<Map<String, String>> filesList =
        StreamSupport.stream(response.get("files").spliterator(), false)
            .map(
                file ->
                    getSpreadsheetData(
                        (JsonNode) file, userAuthorizedSheetIds, GoogleSheetMethodEnum.TRIGGER))
            .filter(Objects::nonNull)
            .collect(Collectors.toList());
