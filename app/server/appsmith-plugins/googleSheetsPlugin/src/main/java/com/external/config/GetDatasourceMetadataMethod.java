...
public class GetDatasourceMetadataMethod {
    ...
    public static Mono<DatasourceConfiguration> getDatasourceMetadata(
            DatasourceConfiguration datasourceConfiguration) {

        // verifying the presence of Oauth Token for now making call to fetch email address
        if (datasourceConfiguration.getAuthentication() == null
                || datasourceConfiguration.getAuthentication().getAuthenticationResponse()
                        == null) {
            return Mono.just(datasourceConfiguration);
        }

        String accessToken =
                datasourceConfiguration.getAuthentication().getAuthenticationResponse().getToken();
                ...
                .map(
                        emailAddress -> {
                            ...
                            datasourceConfiguration.setProperties(
                                    setPropertiesWithEmailAddress(properties, emailAddress));
                            return datasourceConfiguration;
                        });
    }

    public static List<Property> setPropertiesWithEmailAddress(
            List<Property> properties, String emailAddress) {
