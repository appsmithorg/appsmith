...
/**
 * This interface defines the behaviour required to execute an action template It is separated from
 * `ExecutionMethod` so that we are able to reuse as well as differentiate trigger behaviours
 */
public interface TriggerMethod {

    /**
     * Checks for whether the configuration that has been parsed from the TriggerRequestDTO is valid
     * or not
     */
    ...
    WebClient.RequestHeadersSpec<?> getTriggerClient(
    ...
    JsonNode transformTriggerResponse(
