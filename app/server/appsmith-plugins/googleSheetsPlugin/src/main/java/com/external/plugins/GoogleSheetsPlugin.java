...
public class GoogleSheetsPlugin extends BasePlugin {
    ...
    public static final ExchangeStrategies EXCHANGE_STRATEGIES =
    ...
    @Extension
    public static class GoogleSheetsPluginExecutor
            ...
            if (!validConfigurationPresentInFormData(formData, FieldName.SMART_SUBSTITUTION)) {
                ...
                Object ssubValue =
                        getDataValueSafelyFromFormData(
                                formData, FieldName.SMART_SUBSTITUTION, OBJECT_TYPE, true);
                ...
                if (TRUE.equals(smartJsonSubstitution)) {
                    jsonFields.forEach(
                            jsonField -> {
                                String property =
                                        getDataValueSafelyFromFormData(
                                                formData, jsonField, STRING_TYPE, null);
                                if (property != null) {
        ...
        public Mono<ActionExecutionResult> executeCommon(
            ...
            final ExecutionMethod executionMethod =
                    CollectionUtils.isEmpty(formData)
                            ? null
            ...
            // This will get list of authorised sheet ids from datasource config, and transform
            // execution response to contain only authorised files
            final Set<String> userAuthorizedSheetIds =
                    getUserAuthorizedSheetIds(datasourceConfiguration);

            // Triggering the actual REST API call
            return executionMethod
                    .executePrerequisites(methodConfig, oauth2)
                    // This method call will populate the request with all the configurations it
                    // needs for a particular method
                    .flatMap(
                            res -> {
                                return executionMethod
                                        ...
                                        .map(
                                                response -> {
                                                    ...
                                                    // Convert the headers into json tree to store
                                                    // in the results
                                        ...
                                        .onErrorResume(
                                                e -> {
                                                    errorResult.setBody(
                                                            Exceptions.unwrap(e).getMessage());
                                                    log.debug(
                                                            "Received error on Google Sheets action"
                                                                    + " execution",
                                                            e);
        ...
        private Mono<ActionExecutionResult> handleEmptyMono() {
            ...
            result.setBody(
                    objectMapper.valueToTree(Map.of("message", "No operation was performed")));
        ...
        public Mono<ActionExecutionResult> execute(
            ...
            return Mono.error(
                    new AppsmithPluginException(
                            GSheetsPluginError.QUERY_EXECUTION_FAILED, "Unsupported Operation"));
        }
        ...
        public Mono<TriggerResultDTO> trigger(
            ...
            final TriggerMethod triggerMethod =
                    GoogleSheetsMethodStrategy.getTriggerMethod(request, objectMapper);
            ...
            // This will get list of authorised sheet ids from datasource config, and transform
            // trigger response to contain only authorised files
                    ...
                    .map(
                            response -> {
                                ...
                                if (response.getStatusCode().is2xxSuccessful()) {
                                    final JsonNode triggerResponse =
                                            triggerMethod.transformTriggerResponse(
                                                    jsonNodeBody,
                                                    methodConfig,
                                                    userAuthorizedSheetIds);
                                    final TriggerResultDTO triggerResultDTO =
                                            new TriggerResultDTO();
        ...
        /**
         * This overridden implementation conforms to the form/JS mode data structure that was
         * introduced in UQI
         *
        ...
        public void updateCrudTemplateFormData(
            ...
            pluginSpecificTemplateParams.forEach(
                    (k, v) -> {
            ...
            final TemplateMethod templateMethod =
                    GoogleSheetsMethodStrategy.getTemplateMethod(formData);

            templateMethod.replaceMethodConfigTemplate(formData, mappedColumns);
        }

        @Override
        public Mono<DatasourceConfiguration> getDatasourceMetadata(
                DatasourceConfiguration datasourceConfiguration) {
