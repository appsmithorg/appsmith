      ...
      try {
        ...
        if (TRUE.equals(smartJsonSubstitution)) {
          jsonFields.forEach(
              jsonField -> {
                String property =
                    getDataValueSafelyFromFormData(formData, jsonField, STRING_TYPE, null);
                if (property != null) {
                        ...
                        response -> {
                          ...
                          try {
                            ...
                            if (response.getStatusCode().is2xxSuccessful()) {
                              result.setBody(
                                  executionMethod.transformExecutionResponse(
                                      jsonNodeBody, methodConfig, userAuthorizedSheetIds));
                            } else {
                              result.setBody(jsonNodeBody.get("error").get("message").asText());
                            }
                          } catch (IOException e) {
                            throw Exceptions.propagate(
                                new AppsmithPluginException(
