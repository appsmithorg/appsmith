...
public class PostgresDataTypeUtils {
  ...
  /**
   * questionWithCast will match the following sample strings in a query - "?" - "?::text"
   *
   * <p>Capturing only the words post "::" so that the explict data type to which the parameter must
   * be cast can be read and ignoring the group "::" from getting captured by using regex "?:" which
   * ignores the subsequent string
   */
  ...
  private static Map getDataTypeMapper() {
    if (dataTypeMapper == null) {
      ...
      // Must ensure that all the declared postgres data types have a mapping to appsmith data
      // types
  ...
  public static List<com.appsmith.external.constants.DataType> extractExplicitCasting(
      String query) {
    ...
    while (matcher.find()) {
      ...
      // Either no external casting exists or unsupported data type is being used. Do not use
      // external casting for this
      // and instead default to implicit type casting (default behaviour) by setting the entry
      // to null.
  ...
  public static class DataType {
    ...
    /**
     * Declare all the explicitly castable postgresql types below. These would be automatically
     * added to the dataTypes set automatically.
     *
     * <p>!!! WARNING !!! When adding a new data type to support for explicit casting, please ensure
     * to add an entry in the Map dataTypeMapper which maps the postgres data types to Appsmith data
     * types.
     */
    ...
    public Set getDataTypes() {
      // if data types hasn't been initialized, read and set all the supported data types for
      // postgres
        ...
        for (Field field : fields) {
          if (field.getType().equals(String.class)) { // if it is a String field
            try {
              ...
              // We weren't able to read the value of the field. Ignore this field and
              // continue
