...
public class MssqlDatasourceUtils {
     ...
     * </pre>
     */
    private static final String QUERY_TO_GET_ALL_TABLE_COLUMN_TYPE =
            "SELECT \n"
     ...
     * </pre>
     */
    private static final String QUERY_TO_GET_ALL_TABLE_COLUMN_KEY_CONSTRAINTS =
            "SELECT \n"
                    + "    cols.table_name,\n"
                    + "    cols.column_name,\n"
    ...
    public static Connection getConnectionFromConnectionPool(
            HikariDataSource hikariDSConnectionPool) throws SQLException {

        if (hikariDSConnectionPool == null
                || hikariDSConnectionPool.isClosed()
                || !hikariDSConnectionPool.isRunning()) {
    ...
    private static void setTableNamesAndColumnNamesAndColumnTypes(
        ...
        try (ResultSet columnsResultSet =
                statement.executeQuery(QUERY_TO_GET_ALL_TABLE_COLUMN_TYPE)) {
            while (columnsResultSet.next()) {
                ...
                table.getColumns()
                        .add(new DatasourceStructure.Column(columnName, columnType, null, false));
            }
        }
    }
     ...
     * column is a primary or foreign key. Please check the SQL query macro definition to find a
     * sample response as comment.
     */
    private static void setPrimaryAndForeignKeyInfoInTables(
        ...
        try (ResultSet constraintsResultSet =
                statement.executeQuery(QUERY_TO_GET_ALL_TABLE_COLUMN_KEY_CONSTRAINTS)) {
                ...
                .forEach(
                        table -> {
                            ...
                            String selectQueryTemplate =
                                    MessageFormat.format(
                                            "SELECT TOP 10 * FROM {0}", table.getName());
                            ...
                            table.getTemplates()
                                    .add(
                                            new DatasourceStructure.Template(
                                                    "SELECT", selectQueryTemplate));
                            table.getTemplates()
                                    .add(
                                            new DatasourceStructure.Template(
                                                    "INSERT", insertQueryTemplate));
                            table.getTemplates()
                                    .add(
                                            new DatasourceStructure.Template(
                                                    "UPDATE", updateQueryTemplate));
                            table.getTemplates()
                                    .add(
                                            new DatasourceStructure.Template(
