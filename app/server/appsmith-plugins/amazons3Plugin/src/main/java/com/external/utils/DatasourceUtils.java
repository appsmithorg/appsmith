...
/**
* This method builds an `AmazonS3ClientBuilder` object from the datasourceConfiguration provided
* by user. The `AmazonS3ClientBuilder` object can then be used to get a connection object to
...
public static AmazonS3ClientBuilder getS3ClientBuilder(
	 ...
	 *
	 * <p>Ideally, properties.get(S3_SERVICE_PROVIDER_PROPERTY_INDEX) must always exist, because the
	...
	/**
	 * AmazonS3 provides an attribute `forceGlobalBucketAccessEnabled` that automatically routes the
	 * request to a region such that request should succeed. Ref:
	 ...
	 * Objects, or Digital Ocean Spaces. Also, some these services failed on usage of this attribute
	 * - hence this attribute could not be reliably used for these S3 service providers. For these
	 * service providers, the region information is chained in the endpoint URL. Hence, the endpoint
	 * URL is used to extract the exact object storage region.
	 *
	 * <p>Apart from the listed S3 services - AWS, Upcloud, Wasabi, Dream Objects and Digital Ocean
	 * spaces, any other service provider falls in the category `other` and there is no special
	 * handling defined for it since we cannot assume any information about them beforehand. For
	 * this S3 service provider type region must be explicitly provided.
	 */
	...
	switch (s3ServiceProvider) {
		...
		if (StringUtils.isBlank(region)) {
			/**
			 * Set a default region in case user has not provided a region. Minio server can be
			 * configured to work both ways - with or without region attribute. Hence, it is upto
			 * the user to know whether the Minio server they want to connect to has been configured
			 * with a region or not. As per my experimentation, in case the Minio server has not
			 * been configured with a region attribute, then any placeholder value will work.
			 * However, I am going with US_EAST_1 here since this the value that Minio documentation
