...
import static com.external.plugins.constants.DeepseekAIConstants.BODY;
...
public class DeepseekAiPlugin extends BasePlugin {
    ...
    public DeepseekAiPlugin(PluginWrapper wrapper) {
        super(wrapper);
    }

    public static class DeepseekAiPluginExecutor extends BaseRestApiPluginExecutor {

        private static final Gson gson = new Gson();
        private static final Cache<String, JSONObject> modelResponseCache =
                CacheBuilder.newBuilder().expireAfterWrite(1, TimeUnit.DAYS).build();

        public DeepseekAiPluginExecutor(SharedConfig config) {
            super(config);
        }
        ...
        public Mono<ActionExecutionResult> executeParameterized(
            ...
            log.debug(Thread.currentThread().getName() + ": executeParameterized() called for DeepseekAI plugin.");
        ...
        public Mono<ActionExecutionResult> executeCommon(
            ...
            log.debug(Thread.currentThread().getName() + ": executeCommon() called for DeepseekAI plugin.");
            // Initializing object for error condition
            ActionExecutionResult errorResult = new ActionExecutionResult();
            initUtils.initializeResponseWithError(errorResult);

            // Find the right execution command
            AICommand aICommand = AIMethodStrategy.selectExecutionMethod(actionConfiguration, gson);

            // morph the essentials
            AIRequestDTO aIRequestDTO = aICommand.makeRequestBody(actionConfiguration);
            URI uri = aICommand.createExecutionUri(datasourceConfiguration);
            HttpMethod httpMethod = aICommand.getExecutionMethod();
            ...
            return RequestUtils.makeRequest(httpMethod, uri, bearerTokenAuth, BodyInserters.fromValue(aIRequestDTO))
                    ...
                    .onErrorResume(error -> {
                        ...
                        log.debug(
                                "An error has occurred while trying to run the Deepseek API query command with error {}",
        ...
        public Set<String> validateDatasource(DatasourceConfiguration datasourceConfiguration) {
            log.debug(Thread.currentThread().getName() + ": validateDatasource() called for DeepseekAI plugin.");
        ...
        public Mono<TriggerResultDTO> trigger(
            ...
            log.debug(Thread.currentThread().getName() + ": trigger() called for DeepseekAI plugin.");

            // Authentication will already be valid at this point
            final BearerTokenAuth bearerTokenAuth = (BearerTokenAuth) datasourceConfiguration.getAuthentication();
            assert (bearerTokenAuth.getBearerToken() != null);

            // If not, then it must be for chat models
            AICommand aICommand = AIMethodStrategy.selectTriggerMethod(request, gson);
            HttpMethod httpMethod = aICommand.getTriggerHTTPMethod();
            URI uri = aICommand.createTriggerUri(datasourceConfiguration);
            ...
            return responseMono
                    .map(jsonObject -> {
                        ...
                        while (modelListIndex < modelList.length()) {
                            ...
                            if (aICommand.isModelCompatible(model)) {
                        ...
                        return compatibleModels.stream()
                                .sorted()
                                .map(model -> aICommand.getModelMap(modelsMap.get(model)))
        ...
        public Mono<DatasourceTestResult> testDatasource(DatasourceConfiguration datasourceConfiguration) {
            log.debug(Thread.currentThread().getName() + ": testDatasource() called for DeepseekAI plugin.");
            final BearerTokenAuth bearerTokenAuth = (BearerTokenAuth) datasourceConfiguration.getAuthentication();

            HttpMethod httpMethod = HttpMethod.GET;
            URI uri = RequestUtils.createUriFromCommand(MODEL,datasourceConfiguration);
