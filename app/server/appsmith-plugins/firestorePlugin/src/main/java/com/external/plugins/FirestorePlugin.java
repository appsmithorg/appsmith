  ...
  public static class FirestorePluginExecutor
    ...
    public Mono<ActionExecutionResult> executeParameterized(
      ...
      if (smartSubstitutionObject instanceof Boolean) {
        ...
        if (smartSubstitutionObject instanceof String) {
              ...
              mapBody -> {
                if (method.isDocumentLevel()) {
                  return handleDocumentLevelMethod(
                      connection, path, method, mapBody, query, requestParams);
                } else {
    ...
    private boolean isTimestampAndDeleteFieldValuePathEmpty(Map<String, Object> formData) {
      if (isBlank(
          ...
          && isBlank(
              PluginUtils.getDataValueSafelyFromFormData(formData, DELETE_KEY_PATH, STRING_TYPE))) {
        return true;
      }
    ...
    private void insertFieldValues(
      ...
      if (!isBlank(
        ...
        try {
          deletePathsList =
              objectMapper.readValue(deletePaths, new TypeReference<List<String>>() {});
        } catch (IOException e) {
          throw new AppsmithPluginException(
              AppsmithPluginError.PLUGIN_EXECUTE_ARGUMENT_ERROR,
      ...
      if (!isBlank(
        ...
        requestParams.add(
            new RequestParamDTO(TIMESTAMP_VALUE_PATH, timestampValuePaths, null, null, null));
                ...
                try {
                  switch (method1) {
                    ...
                    case UPDATE_DOCUMENT:
                      ...
                      return Mono.justOrEmpty(
                          DocumentReference.class.getMethod(methodName, Map.class));
                    default:
                      return Mono.error(
                          new AppsmithPluginException(
                              FirestorePluginError.QUERY_EXECUTION_FAILED,
                  ...
                  return Mono.error(
                      new AppsmithPluginException(
                          FirestorePluginError.QUERY_EXECUTION_FAILED,
                          String.format(
                              FirestoreErrorMessages.ACTUAL_METHOD_GETTING_ERROR_MSG, method1),
                          e.getMessage()));
                }
              })
    ...
    public Mono<ActionExecutionResult> handleCollectionLevelMethod(
      ...
      if (method == Method.GET_COLLECTION) {
        ...
        if (method == Method.ADD_TO_COLLECTION) {
          requestParams.add(
              new RequestParamDTO(ACTION_CONFIGURATION_BODY, query, null, null, null));
      ...
      return Mono.error(
          new AppsmithPluginException(
              FirestorePluginError.QUERY_EXECUTION_FAILED,
              String.format(
                  FirestoreErrorMessages.UNSUPPORTED_COLLECTION_METHOD_ERROR_MSG, method)));
    }
    ...
    private Mono<ActionExecutionResult> methodGetCollection(
      ...
      requestParams.add(
          new RequestParamDTO(
              LIMIT_DOCUMENTS, limitString == null ? "" : limitString, null, null, null));
    ...
    private Object resultToMap(Object objResult, boolean isRoot) throws AppsmithPluginException {
      if (objResult instanceof WriteResult) {
        ...
        if (objResult instanceof DocumentSnapshot) {
          // Individual document.
    ...
    public Mono<Firestore> datasourceCreate(DatasourceConfiguration datasourceConfiguration) {
      ...
      if (!CollectionUtils.isEmpty(errors)) {
        return Mono.error(
            new AppsmithPluginException(
                AppsmithPluginError.PLUGIN_DATASOURCE_ARGUMENT_ERROR, errors.iterator().next()));
      }
          ...
          .flatMap(
              connection -> {
                try {
                  ...
                  if (e.getMessage()
                      .contains("Metadata operations require admin" + " authentication")) {
      ...
      return Mono.fromSupplier(
              () -> {
                ...
                List<DatasourceStructure.Table> tables =
                    StreamSupport.stream(collectionReferences.spliterator(), false)
                        .map(
                            collectionReference -> {
                              String id = collectionReference.getId();
                              final ArrayList<DatasourceStructure.Template> templates =
                                  new ArrayList<>();
