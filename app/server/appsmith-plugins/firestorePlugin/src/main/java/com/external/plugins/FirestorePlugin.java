...
public static class FirestorePluginExecutor
	...
	if (smartSubstitutionObject instanceof Boolean) {
		...
		// Older UI configuration used to set this value as a string which may/may not be castable
		// to a boolean
	...
	*/
	...
	return Mono.just(query)
			...
			query1 -> {
				...
				if (StringUtils.isEmpty(path)
					...
					String emptyConditionMessage =
						"At least one of the conditions in the 'where' clause has missing operator"
							+ " or operand(s). These conditions were ignored during the execution"
							+ " of the query.";
		...
		// Apply limit, always provided, since without it, we can inadvertently end up processing
		// too much data.
		.map(
			query1 -> {
				if (PaginationField.PREV.equals(paginationField)
