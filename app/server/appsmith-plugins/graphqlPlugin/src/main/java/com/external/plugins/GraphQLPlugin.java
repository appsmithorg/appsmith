...
public static class GraphQLPluginExecutor extends BaseRestApiPluginExecutor {
	 ...
	 * implementation affords an opportunity also update the datasource and action configuration for
	 * pagination and some minor cleanup of the configuration before execution
	 *
	...
	if (TRUE.equals(smartSubstitution)) {
		...
		// Replace all the bindings with a ? as expected in a prepared statement.
		String updatedQuery =
			MustacheHelper.replaceMustacheWithPlaceholder(query, mustacheKeysInOrder);
	...
	if (HttpMethod.POST.equals(httpMethod)) {
		 ...
		 *
		 * <p>Graphql reference doc also mentions that content-type=application/graphql does not
		 * require any re-formatting. Ref: https://graphql.org/learn/serving-over-http/#post-request
		 *
		 * <p>On searching over the web I also found that there are some custom content-type in use
		 * like `application/graphql+json` or `application/graphql-json` that expect the data in the
		 * same format as is for `application/json`. Hence, the current check assumes that any
		 * content type that differs from `application/graphql` would expect the data in the same
		 * format as for `application/json`
		 */
		if (!ApiContentType.GRAPHQL.getValue().equalsIgnoreCase(reqContentType)) {
		/**
		* When a GraphQL request is sent using HTTP POST method, then the request body needs to
		* be in the following format: { "query": "... graphql query body ...", "variables":
		* {"var1": val1, "var2": val2 ...}, "operationName": "name of operation" // only required
		* if multiple operations are defined in a single query body } Ref:
		...
		*/
		...
		/**
		 * When a GraphQL request is sent using GET method, the GraphQL body and variables are sent
		 * as part of query parameters in the URL. Ref: https://graphql.org/learn/serving-over-http/
		 */
		...
		.onErrorResume(
			error -> {
				errorResult.setRequest(
					requestCaptureFilter.populateRequestFields(actionExecutionRequest));
				errorResult.setIsExecutionSuccess(false);
	 ...
	 * same action object i.e. a cyclic reference. e.g. A GraphQL API response can contain
	 * pagination cursors that are required to be configured in the pagination tab of the same API.
	 * We don't want to treat these cyclic references as cyclic dependency errors.
	 */
	@Override
