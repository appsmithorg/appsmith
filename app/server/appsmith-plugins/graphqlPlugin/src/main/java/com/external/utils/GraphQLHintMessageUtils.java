...
public class GraphQLHintMessageUtils extends HintMessageUtils {
  ...
  /**
   * This method checks if the user has defined a query variable in both the places - pagination tab
   * and the query variables editor section. If so, then a warning message is returned informing
   * user of the same.
   */
      ...
      if (PaginationType.PAGE_NO.equals(actionConfiguration.getPaginationType())) {
        ...
        if (PaginationType.CURSOR.equals(actionConfiguration.getPaginationType())) {
          ...
          throw new AppsmithPluginException(
              ...
              "Appsmith server encountered an unexpected error: unrecognized"
                  + " pagination type: "
                  + actionConfiguration.getPaginationType()
                  + ". Please reach out to our customer support to resolve"
                  + " this.");
        }
      }
      ...
      if (!isEmpty(duplicateVariables)) {
        String message =
            MessageFormat.format(
                HINT_MESSAGE_FOR_DUPLICATE_VARIABLE_DEFINITION, duplicateVariables.toString());
