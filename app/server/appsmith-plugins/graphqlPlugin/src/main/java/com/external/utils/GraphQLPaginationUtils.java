...
public static void updateVariablesWithPaginationValues(
	...
	throw new AppsmithPluginException(
		AppsmithPluginError.PLUGIN_EXECUTE_ARGUMENT_ERROR,
		"Appsmith server could not find any GraphQL pagination data even though pagination is"
			+ " toggled on. Please provide pagination data by editing relevant fields in the"
			+ " pagination tab.");
	}
	...
	try {
		...
		throw new AppsmithPluginException(
			AppsmithPluginError.PLUGIN_EXECUTE_ARGUMENT_ERROR,
			"Please provide a valid integer value for the limit variable in the pagination tab."
				+ " Current value: "
				+ limitValueString);
	}
	...
	try {
		...
		throw new AppsmithPluginException(
			AppsmithPluginError.PLUGIN_EXECUTE_ARGUMENT_ERROR,
			"Please provide a valid integer value for the offset variable in the pagination tab."
				+ " Current value: "
				+ offsetValueString);
	}
	...
	if (PaginationField.PREV.equals(executeActionDTO.getPaginationField())) {
		...
		throw new AppsmithPluginException(
			AppsmithPluginError.PLUGIN_EXECUTE_ARGUMENT_ERROR,
			"Please provide a valid integer value for the previous page limit variable in the"
		...
		throw new AppsmithPluginException(
			AppsmithPluginError.PLUGIN_EXECUTE_ARGUMENT_ERROR,
			"Please provide a non empty value for the previous page cursor variable in the"
				+ " pagination tab.");
		}

		/**
		 * This check ensures that during the very first run when the query does not have any cursor
		 * data the cursor related variable does not get added. In this case since the cursor data
		 * is not available the dynamic binding value returned by the client is "null" string. Some
		 * GraphQL severs are able to handle the "null" string as cursor value but some are not e.g.
		 * GitHub's GraphQL endpoints are not able to handle "null" as value. Hence, it is better to
		 * skip passing the variable and allow the GraphQL servers to run the query without cursor
		 * value. The GraphQL servers against which we are testing seem to handle the case well
		 * where the value is skipped.
		 */
		...
		throw new AppsmithPluginException(
			AppsmithPluginError.PLUGIN_EXECUTE_ARGUMENT_ERROR,
			"Please provide a non empty value for the next page cursor variable in the pagination"
				+ " tab.");
		}
		 ...
		 * check ensures that when a query is run by clicking the run button as opposed to clicking
		 * the next button on any paginated widget, then it only takes the limit value and not the
		 * cursor value. This will make sure that the just clicking on the run button will not fetch
