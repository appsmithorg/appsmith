...
public class GraphQLPaginationUtils {
  ...
  public static Map getPaginationData(ActionConfiguration actionConfiguration)
    ...
    if (PaginationType.PAGE_NO.equals(actionConfiguration.getPaginationType())) {
      paginationData =
          getValueSafelyFromFormData(
              (Map) properties.get(PAGINATION_DATA_INDEX).getValue(),
              "limitBased",
              Map.class,
              null);
    } else {
      if (PaginationType.CURSOR.equals(actionConfiguration.getPaginationType())) {
        paginationData =
            getValueSafelyFromFormData(
                (Map) properties.get(PAGINATION_DATA_INDEX).getValue(),
                "cursorBased",
    ...
    if (PaginationType.PAGE_NO.equals(actionConfiguration.getPaginationType())) {
      String limitVarName =
          getValueSafelyFromFormData(paginationData, "limit.name", String.class, "");
      String limitValString =
          getValueSafelyFromFormData(paginationData, "limit.value", String.class, "");
      String offsetVarName =
          getValueSafelyFromFormData(paginationData, "offset.name", String.class, "");
      String offsetValString =
          getValueSafelyFromFormData(paginationData, "offset.value", String.class, "");
      ...
      if (PaginationType.CURSOR.equals(actionConfiguration.getPaginationType())) {
        String prevLimitVarName =
            getValueSafelyFromFormData(paginationData, "previous.limit.name", String.class, "");
        String prevLimitValString =
            getValueSafelyFromFormData(paginationData, "previous.limit.value", String.class, "");
        String prevCursorVarName =
            getValueSafelyFromFormData(paginationData, "previous.cursor.name", String.class, "");
        String prevCursorValString =
            getValueSafelyFromFormData(paginationData, "previous.cursor.value", String.class, "");

        String nextLimitVarName =
            getValueSafelyFromFormData(paginationData, "next.limit.name", String.class, "");
        String nextLimitValString =
            getValueSafelyFromFormData(paginationData, "next.limit.value", String.class, "");
        String nextCursorVarName =
            getValueSafelyFromFormData(paginationData, "next.cursor.name", String.class, "");
        String nextCursorValString =
            getValueSafelyFromFormData(paginationData, "next.cursor.value", String.class, "");
  ...
  public static void updateVariablesWithPaginationValues(
      ActionConfiguration actionConfiguration, ExecuteActionDTO executeActionDTO)
      throws AppsmithPluginException {
    ...
    if (isEmpty(paginationDataMap)) {
      throw new AppsmithPluginException(
          AppsmithPluginError.PLUGIN_EXECUTE_ARGUMENT_ERROR,
          "Appsmith server could not find any GraphQL pagination data even though"
              + " pagination is toggled on. Please provide pagination data by editing"
              + " relevant fields in the pagination tab.");
    }
    ...
    if (PaginationType.PAGE_NO.equals(actionConfiguration.getPaginationType())) {
      ...
      try {
        ...
        throw new AppsmithPluginException(
            AppsmithPluginError.PLUGIN_EXECUTE_ARGUMENT_ERROR,
            "Please provide a valid integer value for the limit variable in the"
                + " pagination tab. Current value: "
                + limitValueString);
      }
      ...
      try {
        ...
        throw new AppsmithPluginException(
            AppsmithPluginError.PLUGIN_EXECUTE_ARGUMENT_ERROR,
            "Please provide a valid integer value for the offset variable in the"
                + " pagination tab. Current value: "
                + offsetValueString);
      }
      ...
      if (PaginationType.CURSOR.equals(actionConfiguration.getPaginationType())) {
        if (PaginationField.PREV.equals(executeActionDTO.getPaginationField())) {
          ...
          try {
            ...
            throw new AppsmithPluginException(
                AppsmithPluginError.PLUGIN_EXECUTE_ARGUMENT_ERROR,
                "Please provide a valid integer value for the previous page limit"
                    + " variable in the pagination tab. Current value: "
                    + prevLimitValueString);
          }
          ...
          if (isBlank(prevCursorValue)) {
            throw new AppsmithPluginException(
                AppsmithPluginError.PLUGIN_EXECUTE_ARGUMENT_ERROR,
                "Please provide a non empty value for the previous page cursor"
                    + " variable in the pagination tab.");
          }

          /**
           * This check ensures that during the very first run when the query does not have any
           * cursor data the cursor related variable does not get added. In this case since the
           * cursor data is not available the dynamic binding value returned by the client is "null"
           * string. Some GraphQL severs are able to handle the "null" string as cursor value but
           * some are not e.g. GitHub's GraphQL endpoints are not able to handle "null" as value.
           * Hence, it is better to skip passing the variable and allow the GraphQL servers to run
           * the query without cursor value. The GraphQL servers against which we are testing seem
           * to handle the case well where the value is skipped.
           */
          ...
          try {
            ...
            throw new AppsmithPluginException(
                AppsmithPluginError.PLUGIN_EXECUTE_ARGUMENT_ERROR,
                "Please provide a valid integer value for the next page limit"
                    + " variable in the pagination tab. Current value: "
                    + nextLimitValueString);
          }
          ...
          if (isBlank(nextCursorValue)) {
            throw new AppsmithPluginException(
                AppsmithPluginError.PLUGIN_EXECUTE_ARGUMENT_ERROR,
                "Please provide a non empty value for the next page cursor variable"
                    + " in the pagination tab.");
          }

          /**
           * The cursor being non-empty check ensures that during the very first run when the query
           * does not have any cursor data the cursor related variable does not get added. In this
           * case since the cursor data is not available the dynamic binding value returned by the
           * client is "null" string. Some GraphQL severs are able to handle the "null" string as
           * cursor value but some are not e.g. GitHub's GraphQL endpoints are not able to handle
           * "null" as value. Hence, it is better to skip passing the variable and allow the GraphQL
           * servers to run the query without cursor value. The GraphQL servers against which we are
           * testing seem to handle the case well where the value is skipped. The pagination field
           * check ensures that when a query is run by clicking the run button as opposed to
           * clicking the next button on any paginated widget, then it only takes the limit value
           * and not the cursor value. This will make sure that the just clicking on the run button
           * will not fetch paginated results based on the cursor value.
           */
          if (!NULL_STRING.equals(nextCursorValue)
              && PaginationField.NEXT.equals(executeActionDTO.getPaginationField())) {
