...
public class AiAssistantPlugin extends BasePlugin {
    public AiAssistantPlugin(PluginWrapper wrapper) {
        super(wrapper);
    }

    public static class AiAssistantPluginExecutor extends BaseRestApiPluginExecutor {
        private static final Gson gson = new Gson();
        private static final Cache<String, TriggerResultDTO> triggerResponseCache =
                CacheBuilder.newBuilder().expireAfterWrite(1, TimeUnit.DAYS).build();

        protected AiAssistantPluginExecutor(SharedConfig sharedConfig) {
            super(sharedConfig);
        }
