...
#  Placeholder to use access points more effectively
  ...
  cat ./scripts/efs_guardian_job.json | jq ".spec.template.spec.containers[0].env += [{ \"name\": \"HOSTNAME\" , value: \"$NAMESPACE\" }]" | jq ".spec.template.spec.containers[0].args += [\"-c\", \"rm -rf \/appsmith-stacks\/ce\/$NAMESPACE/*\"]" | kubectl apply -f -
  kubectl delete job appsmith-efs-guardian
