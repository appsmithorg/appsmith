...
RUN apt-get update && \
    ...
    apt-get update && \
    apt-get install -y git tar zstd && \
    apt-get clean && \
...
# Create the data-alloy directory with group writeable permissions so non-root users can write to it.
RUN mkdir --mode 775 /opt/appsmith/data-alloy
