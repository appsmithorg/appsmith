...
#### Note:
- Make sure to disable the build step within Intellij, otherwise the run configuration will fail to start:
...
- Make sure you are disabling the build step within the Intellij run configuration.

#### Using WSL2 and Run/Debug immediately fails with error `/bin/sh: 1: export: xxx: bad variable name`

- Ensure that your Windows environmental variables are not being used by unticking the box below:

  `Run Config > Environment Variables dialog > Untick "Include System Environment Variables"`
...
- If the "Environment Variables" box isn't available, enable it with Run Config > Modify Options > Operating System > Environment Variables.
  ...
  `Run Config > Modify Options > Operating System > Environment Variables`
...
#### Intellij says classes are missing
  ...
  ```
- This can happen if the build step was not executed.
- If you've successfully ran the build script, yet Intellij still fails to see the classes, this may be due to indexing issues. Invalidate the caches to restart the indexing process:

  `File > Invalidate Caches... > Invalidate and restart`
- If you are still having problems or the indexing a known performance issue with Intellij and WSL2. Please see the recommendations immediately below.

#### Using WSL2 and Intellij indexing fails, takes a long time or runs indefinitely

- There is a known issue with Intellij + WSL2 and Windows real-time threat protection. Unfortunately, the only reliable fix at this time is to disable it temporarily.
- You can do this in the Windows Security settings:
